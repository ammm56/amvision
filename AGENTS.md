...
- 各类协议集成、硬件桥接、模块连接和场景化能力优先通过自定义节点扩展
...
- 如确有直连硬件或特殊模块连接需求，应通过独立自定义节点在受控边界中实现，而不是写入核心平台主链路
...
- 前端构建结果默认本地分发，不把外网 CDN 或额外系统级 Node 运行时作为部署前提
...
- 节点包必须具备 manifest、version、capabilities、config schema、timeout 和禁用机制
- 模型输出文件、流程模板、自定义节点包版本都必须可追溯、可回滚
- API、协议集成接口、模型输入输出 schema 一旦公开，变更必须显式版本化
- backend、worker、Broker、独立运行时和仓库内 .NET SDK 只支持 64-bit 进程；不得为 32-bit 系统增加数据面兼容分支、容量协商或降级路径
...
- AI 工程师：模型训练、验证、推理优化、模型转换、前后处理、视觉流程与可扩展模型链路
...
- 技术文档工程师：README、架构说明、API 文档、部署文档、节点扩展文档、教程和迁移指南
...
- 行为变更必须附带最小可验证结果
- 每次任务完成并确认没有仍在使用临时文件的本任务进程后，必须清理仓库根目录 `.tmp/` 中的测试缓存、审计中间文件和其他临时产物；不得把 `.tmp/` 内容作为长期资产或交付物保留
