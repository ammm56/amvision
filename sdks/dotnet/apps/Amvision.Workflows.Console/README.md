...
- 后续 `ModelDeployment`：封装已存在模型 DeploymentInstance 的 sync/async 启动、预热、重置、停止、状态、health、同步推理和异步 inference task 调用，不封装创建或删除部署实例。
...
模型 DeploymentInstance 没有 WorkflowAppRuntime + TriggerSource 这种一组一文件的清晰边界，因此后续允许每个 `config_*.json` 都包含 `model_deployments` 列表。程序启动时从所有配置文件中合并 `model_deployments`，建立统一的 `ModelDeployments` 字典。

模型部署调用仍按 key 使用：

```csharp
var health = await runner.GetModelDeploymentRuntimeHealthAsync("barcode_detector", cancellationToken).ConfigureAwait(false);
...
启动加载所有 `config_*.json` 时，`runtime.name`、`trigger_sources[].name` 和 `model_deployments[].name` 都必须在各自 catalog 中唯一。重复 key 不允许静默覆盖，也不随机保留某一项；重复项应从有效 catalog 中排除，并在启动阶段抛出配置错误，列出重复 key 和来源文件。

完整规划见 [docs/api/model-deployment-sdks.md](../../../../docs/api/model-deployment-sdks.md)。
