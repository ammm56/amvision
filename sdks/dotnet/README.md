...
`examples/Amvision.Workflows.Net461Console` 提供 .NET Framework 4.6.1 console 示例，按方法封装 WorkflowAppRuntime create/list/get/start/stop/restart/health/instances、sync invoke、async run、run/event 查询等 SDK 调用。真实联调也可以继续使用 SDK 测试工程的 smoke 测试，或按下面的最小调用代码嵌入现场上位机、MES、采集程序和调试工具。
