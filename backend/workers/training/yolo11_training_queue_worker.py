...
from backend.service.application.models.training.yolo11_training_service import YOLO11_TRAINING_QUEUE_NAME
...
from backend.workers.training.yolo11_trainer_runner import SqlAlchemyYolo11TrainerRunner


class Yolo11TrainingQueueWorker:
    """消费 yolo11-trainings 队列任务的最小 worker。"""
    ...
    def __init__(
        ...
        *,
        ...
        worker_id: str = "yolo11-training-worker",
    ) -> None:
        """初始化 YOLO11 训练队列 worker。"""
    ...
    def run_once(self) -> bool:
        """消费并执行一条 YOLO11 训练队列任务。"""

        queue_task = claim_next_training_queue(
            self.queue_backend,
            queue_names=(YOLO11_TRAINING_QUEUE_NAME,),
        ...
        try:
            ...
            training_backend = self.training_backend or SqlAlchemyYolo11TrainerRunner(
