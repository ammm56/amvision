...
YoloModelConversionRunner = ConversionBackend


class LocalYoloModelConversionRunner:
    """执行 YOLO 主线 ONNX/OpenVINO/TensorRT 转换流程。"""
    ...
    def run_conversion(
        ...
        openvino_ir_build_precision = resolve_openvino_ir_build_precision(
            request.metadata
        )
        ...
        onnx_output: YoloModelConversionOutput | None = None
        optimized_output: YoloModelConversionOutput | None = None
        openvino_output: YoloModelConversionOutput | None = None
        tensorrt_output: YoloModelConversionOutput | None = None
            ...
            if step.kind == "optimize-onnx":
                if onnx_output is None:
                    raise ServiceConfigurationError(
                        "optimize-onnx 缺少 export-onnx 输出"
                    )
        ...
        outputs: list[YoloModelConversionOutput] = []
        if onnx_output is not None:
    ...
    def _resolve_task_runtime_session_cls(self, task_type: str) -> type:
        """按 task_type 返回当前模型要使用的 PyTorch export session。"""
