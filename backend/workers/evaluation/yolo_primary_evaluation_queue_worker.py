...
class ClassificationEvaluationQueueWorker:
    ...
    def run_once(self) -> bool:
        ...
        try:
            ...
            service = SqlAlchemyClassificationEvaluationTaskService(
...
class SegmentationEvaluationQueueWorker:
    ...
    def run_once(self) -> bool:
        ...
        try:
            task_id = queue_task.payload["task_id"]
            service = SqlAlchemySegmentationEvaluationTaskService(
                session_factory=self.session_factory,
                dataset_storage=self.dataset_storage,
                queue_backend=self.queue_backend,
            )
...
class DetectionEvaluationQueueWorker:
    ...
    def __init__(self, *, session_factory: SessionFactory, dataset_storage: LocalDatasetStorage, queue_backend: QueueBackend, worker_id: str = "detection-evaluation") -> None:
    ...
    def run_once(self) -> bool:
        queue_task = self.queue_backend.claim_next(queue_name=DETECTION_EVALUATION_QUEUE_NAME, worker_id=self.worker_id)
        if queue_task is None: return False
        try:
            task_id = queue_task.payload["task_id"]
            service = SqlAlchemyDetectionEvaluationTaskService(session_factory=self.session_factory, dataset_storage=self.dataset_storage, queue_backend=self.queue_backend)
            result = service.process_evaluation_task(task_id)
            self.queue_backend.complete(queue_task, metadata={"task_id": task_id, "status": "succeeded", "map50": result.map50, "map50_95": result.map50_95, "sample_count": result.sample_count, "report_object_key": result.report_object_key})
        except Exception as error:
            self.queue_backend.fail(queue_task, error_message=str(error), metadata={"task_id": queue_task.payload.get("task_id"), "error_type": error.__class__.__name__})
        return True
...
class PoseEvaluationQueueWorker:
    ...
    def __init__(self, *, session_factory: SessionFactory, dataset_storage: LocalDatasetStorage, queue_backend: QueueBackend, worker_id: str = "pose-evaluation") -> None:
        self.session_factory = session_factory; self.dataset_storage = dataset_storage; self.queue_backend = queue_backend; self.worker_id = worker_id

    def run_once(self) -> bool:
        queue_task = self.queue_backend.claim_next(queue_name=POSE_EVALUATION_QUEUE_NAME, worker_id=self.worker_id)
        if queue_task is None: return False
        try:
            task_id = queue_task.payload["task_id"]
            service = SqlAlchemyPoseEvaluationTaskService(session_factory=self.session_factory, dataset_storage=self.dataset_storage, queue_backend=self.queue_backend)
            result = service.process_evaluation_task(task_id)
            self.queue_backend.complete(queue_task, metadata={"task_id": task_id, "status": "succeeded", "oks_ap50": result.oks_ap50, "oks_ap50_95": result.oks_ap50_95, "sample_count": result.sample_count, "report_object_key": result.report_object_key})
        except Exception as error:
            self.queue_backend.fail(queue_task, error_message=str(error), metadata={"task_id": queue_task.payload.get("task_id"), "error_type": error.__class__.__name__})
        return True
...
class ObbEvaluationQueueWorker:
    ...
    def __init__(self, *, session_factory: SessionFactory, dataset_storage: LocalDatasetStorage, queue_backend: QueueBackend, worker_id: str = "obb-evaluation") -> None:
        self.session_factory = session_factory; self.dataset_storage = dataset_storage; self.queue_backend = queue_backend; self.worker_id = worker_id

    def run_once(self) -> bool:
        queue_task = self.queue_backend.claim_next(queue_name=OBB_EVALUATION_QUEUE_NAME, worker_id=self.worker_id)
        if queue_task is None: return False
        try:
            task_id = queue_task.payload["task_id"]
            service = SqlAlchemyObbEvaluationTaskService(session_factory=self.session_factory, dataset_storage=self.dataset_storage, queue_backend=self.queue_backend)
            result = service.process_evaluation_task(task_id)
            self.queue_backend.complete(queue_task, metadata={"task_id": task_id, "status": "succeeded", "map50": result.map50, "map50_95": result.map50_95, "sample_count": result.sample_count, "report_object_key": result.report_object_key})
        except Exception as error:
            self.queue_backend.fail(queue_task, error_message=str(error), metadata={"task_id": queue_task.payload.get("task_id"), "error_type": error.__class__.__name__})
