"""deployment classification Batch 节点。"""

from backend.nodes.core_nodes.support.deployment_batch import (
    build_deployment_batch_node_spec,
)
from backend.service.domain.models.model_task_types import CLASSIFICATION_TASK_TYPE


CORE_NODE_SPEC = build_deployment_batch_node_spec(
    node_type_id="core.model.classification-batch",
    display_name="Classification Batch",
    task_type=CLASSIFICATION_TASK_TYPE,
    output_name="categories_batch",
    output_display_name="Categories Batch",
    output_payload_type_id="categories-batch.v1",
    format_id="amvision.categories-batch.v1",
)
