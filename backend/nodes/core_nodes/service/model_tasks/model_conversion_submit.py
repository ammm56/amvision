...
from backend.nodes.core_nodes.support.platform.schemas import (
...
def _model_conversion_submit_handler(request: WorkflowNodeExecutionRequest) -> dict[str, object]:
    ...
    request_cls = _resolve_conversion_request_class(task_type=task_type, model_type=model_type)
    request_kwargs = {
    ...
    if task_type == DETECTION_TASK_TYPE and model_type == "rfdetr":
...
def _resolve_conversion_request_class(*, task_type: str, model_type: str) -> type:
    """按 task_type/model_type 返回转换任务请求类型。"""
    ...
    return YoloConversionTaskRequest
