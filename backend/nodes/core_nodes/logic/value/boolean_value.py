...
def _handler(request: WorkflowNodeExecutionRequest) -> dict[str, object]:
    """把显式布尔参数包装为标准 value.v1。"""

    value = request.parameters.get("value", False)
    if not isinstance(value, bool):
        raise InvalidRequestError("Boolean Value 的 value 必须是布尔值")
...
CORE_NODE_SPEC = CoreNodeSpec(
    node_definition=NodeDefinition(
        ...
        description="输出一个明确配置的布尔 value.v1，供通用参数端口复用。",
