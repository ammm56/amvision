...
def _value_to_regions_handler(
    request: WorkflowNodeExecutionRequest,
) -> dict[str, object]:
    """把 value.v1 内的 regions 结构恢复为正式 regions.v1。"""

    raw_value = require_value_payload(request.input_values.get("value"), field_name="value")["value"]
    normalized_regions = require_regions_payload(
        raw_value,
        node_id=request.node_id,
        field_name="value",
    )
    source_image = _resolve_source_image(request, normalized_regions.get("source_image"))
    region_items = [dict(item) for item in normalized_regions["items"]]
    return {
        "regions": {
...
CORE_NODE_SPEC = CoreNodeSpec(
    node_definition=NodeDefinition(
        node_type_id="core.logic.value-to-regions",
        display_name="Value To Regions",
        category="logic.transform",
        description="把 value.v1 中的 regions 结构恢复为正式 regions.v1，适合批处理循环体里把逐项 value 输入重新接回工业规则链。",
        ...
        capability_tags=("logic.transform", "payload.bridge", "regions.bridge"),
    ),
    handler=_value_to_regions_handler,
)
