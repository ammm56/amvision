...
class PyTorchYoloPrimaryPoseRuntimeSession:
        ...
        *,
        ...
        if runtime_target.runtime_backend != "pytorch":
            raise InvalidRequestError(
                f"当前 {cls.model_label} pose predictor 仅支持 pytorch runtime_backend",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
        if runtime_target.task_type != cls.task_type:
            raise InvalidRequestError(
                f"当前 {cls.model_label} pose predictor 收到了错误的 task_type",
    ...
    def predict(self, request: PosePredictionRequest) -> PosePredictionExecutionResult:
        ...
        instances, kpt_shape = _build_pose_instances(
            ...
            input_size=self.runtime_target.input_size,
            default_kpt_shape=_infer_kpt_shape(self.runtime_target),
        )
    ...
    def load(
        ...
        *,
        ...
        if runtime_target.runtime_backend != "onnxruntime":
            raise InvalidRequestError(
                f"当前 {cls.model_label} pose predictor 仅支持 onnxruntime runtime_backend",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
        if runtime_target.task_type != cls.task_type:
            raise InvalidRequestError(
        ...
        if runtime_target.runtime_precision != "fp32":
            raise InvalidRequestError(
                "当前 pose onnxruntime session 仅支持 fp32 precision",
    ...
    def load(
        ...
        *,
        ...
        if runtime_target.runtime_backend != "openvino":
            raise InvalidRequestError(
                f"当前 {cls.model_label} pose predictor 仅支持 openvino runtime_backend",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
        if runtime_target.task_type != cls.task_type:
            raise InvalidRequestError(
...
class TensorRTYoloPrimaryPoseRuntimeSession:
    ...
    def load(
        ...
        *,
        ...
        if runtime_target.runtime_backend != "tensorrt":
            raise InvalidRequestError(
                f"当前 {cls.model_label} pose predictor 仅支持 tensorrt runtime_backend",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
        if runtime_target.task_type != cls.task_type:
            raise InvalidRequestError(
    ...
    def predict(self, request: PosePredictionRequest) -> PosePredictionExecutionResult:
        ...
        if not resolved_output_shape or any(d <= 0 for d in resolved_output_shape):
            raise ServiceConfigurationError(
                ...
                details={"output_shape": list(resolved_output_shape)},
            )
        ...
        if (
            ...
            raise ServiceConfigurationError(
                "TensorRT pose execution context 绑定输入张量失败",
        ...
        if (
            ...
            raise ServiceConfigurationError(
                "TensorRT pose execution context 绑定输出张量失败",
        ...
        ensure_cuda_success(
            ...
            details={"byte_size": int(output_array.nbytes)},
        )
        ensure_cuda_success(
    ...
    def close(self) -> None:
        try:
            ensure_cuda_success(
                ...
                operation_name="TensorRT pose runtime 清理前绑定 CUDA device",
                details={"device_name": self.device_name},
            )
        except Exception:
            return
    ...
    def _ensure_io_buffers(
        self, *, input_array: Any, resolved_output_shape: tuple[int, ...]
    ) -> Any:
        ...
        if self.input_device_ptr is None or input_nbytes > self.input_capacity_bytes:
            ...
            self.input_device_ptr = ensure_cuda_success(
                ...
                operation_name="TensorRT pose runtime 分配复用输入显存",
        ...
        if self.output_device_ptr is None or output_nbytes > self.output_capacity_bytes:
            ...
            self.output_device_ptr = ensure_cuda_success(
                ...
                operation_name="TensorRT pose runtime 分配复用输出显存",
...
def _normalize_pytorch_prediction(
    outputs: object, *, np_module: Any, model_type: str
) -> Any:
    ...
    if normalized_model_type == "yolo11":
        return normalize_yolo11_pose_inference_outputs(
            outputs=outputs,
            np_module=np_module,
        )
    ...
    if prediction.ndim < 3:
        raise InvalidRequestError(
            "pose 推理输出维度不合法", details={"shape": list(prediction.shape)}
        )
    return prediction


def _normalize_onnx_prediction(
    outputs: object, *, np_module: Any, model_type: str
) -> Any:
    normalized_model_type = normalize_optional_platform_model_type(model_type)
    if normalized_model_type == "yolov8":
    ...
    *,
    ...
    if normalized_model_type in {"yolov8", "yolo11"}:
        build_instances = (
            build_yolo11_pose_inference_instances
            if normalized_model_type == "yolo11"
            else build_yolov8_pose_inference_instances
        )
        core_instances, core_kpt_shape = build_instances(
            np_module=np_module,
            prediction_array=prediction_array,
            labels=labels,
    ...
    for image_prediction in normalized_prediction:
        ...
        if int(keep_indices.size) <= 0:
            continue
        for box, score, cls_id, kpt_row in zip(
            boxes[keep_indices],
            ...
            class_name = (
                labels[resolved_class_id]
                if 0 <= resolved_class_id < len(labels)
                else None
            )
            ...
            for j in range(default_kpt_shape[0]):
                kx = float(
                    kpt_row[j * default_kpt_shape[1] + 0] / max(resize_ratio, 1e-8)
                )
                ky = float(
                    kpt_row[j * default_kpt_shape[1] + 1] / max(resize_ratio, 1e-8)
                )
                kc = (
                    float(kpt_row[j * default_kpt_shape[1] + 2])
                    if has_confidence
                    and j * default_kpt_shape[1] + 2 < int(kpt_row.shape[0])
                    else None
                )
                ...
                kpts.append(
                    PosePredictionKeypoint(
                        x=round(kx, 3),
                        y=round(ky, 3),
                        confidence=round(kc, 6) if kc is not None else None,
                    )
                )
            results.append(
                PosePredictionInstance(
...
def _infer_kpt_shape(runtime_target: RuntimeTargetSnapshot) -> tuple[int, int]:
    ...
    config = get_yolo_primary_model_config(
        model_type=runtime_target.model_type, task_type="pose"
    )
...
def _require_primary_model_type(model_type: str, model_label: str) -> str:
    ...
    if not normalized_model_type or normalized_model_type == "yolo-primary":
        raise ServiceConfigurationError(
            f"当前 {model_label} pose predictor 缺少正式 model_type 配置",
