    ...
    def load(cls, *, dataset_storage, runtime_target):
        if runtime_target.runtime_backend != "pytorch":
            raise InvalidRequestError(
                f"当前 {cls.model_label} obb predictor 仅支持 pytorch",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
        if runtime_target.task_type != cls.task_type:
            raise InvalidRequestError(
                f"当前 {cls.model_label} obb predictor 收到了错误的 task_type",
        ...
        device_name = resolve_execution_device_name(
            torch_module=imports.torch, requested_device_name=runtime_target.device_name
        )
        enable_pytorch_cuda_inference_fast_path(
...
class OnnxRuntimeYoloPrimaryObbRuntimeSession:
    ...
    def load(cls, *, dataset_storage, runtime_target):
        if runtime_target.runtime_backend != "onnxruntime":
            raise InvalidRequestError(
                f"当前 {cls.model_label} obb predictor 仅支持 onnxruntime",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
        if runtime_target.task_type != cls.task_type:
            raise InvalidRequestError(
        ...
        if runtime_target.runtime_precision != "fp32":
            raise InvalidRequestError(
                "当前 obb onnxruntime session 仅支持 fp32",
        ...
        session = onnxruntime_module.InferenceSession(
            str(runtime_target.runtime_artifact_path), providers=providers
        )
        return cls(
            dataset_storage=dataset_storage,
            runtime_target=runtime_target,
...
class OpenVINOYoloPrimaryObbRuntimeSession:
    ...
    def load(cls, *, dataset_storage, runtime_target):
        if runtime_target.runtime_backend != "openvino":
            raise InvalidRequestError(
                f"当前 {cls.model_label} obb predictor 仅支持 openvino",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
        if runtime_target.task_type != cls.task_type:
            raise InvalidRequestError(
        ...
        return cls(
            ...
            compiled_runtime_precision=resolve_openvino_compiled_runtime_precision(
                session=session, fallback_precision=runtime_target.runtime_precision
            ),
        )
...
class TensorRTYoloPrimaryObbRuntimeSession:
    ...
    def load(
        ...
        *,
        ...
        if runtime_target.runtime_backend != "tensorrt":
            raise InvalidRequestError(
                f"当前 {cls.model_label} obb predictor 仅支持 tensorrt",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
        if runtime_target.task_type != cls.task_type:
            raise InvalidRequestError(
        ...
        logger = get_tensorrt_logger(
            tensorrt_module=tensorrt_module, severity=tensorrt_module.Logger.WARNING
        )
        ...
        stream = ensure_cuda_success(
            ...
            operation_name="TensorRT obb runtime 创建 CUDA stream",
            details={"device_name": device_name},
        )[0]
    ...
    def close(self):
        try:
            ensure_cuda_success(
                ...
                operation_name="TensorRT obb runtime 清理前绑定 CUDA device",
    ...
    def describe_memory_usage(self):
        ...
        return {
            ...
            "output_host_buffer_bytes": b,
            "output_host_pinned_bytes": p,
    ...
    def _ensure_io_buffers(self, *, input_array, resolved_output_shape):
        output_dtype = resolve_numpy_dtype(
            np_module=self.imports.np, dtype_name=self.output_dtype_name
        )
        input_nbytes = int(input_array.nbytes)
        output_nbytes = int(
            self.imports.np.empty(resolved_output_shape, dtype=output_dtype).nbytes
        )
        if self.input_device_ptr is None or input_nbytes > self.input_capacity_bytes:
            ...
            self.input_device_ptr = ensure_cuda_success(
                ...
                operation_name="TensorRT obb runtime 分配输入显存",
        ...
        if self.output_device_ptr is None or output_nbytes > self.output_capacity_bytes:
            ...
            self.output_device_ptr = ensure_cuda_success(
                ...
                operation_name="TensorRT obb runtime 分配输出显存",
        ...
        if self._should_use_pinned_output_buffer(output_nbytes):
            ...
            if (
                ...
                or tuple(int(d) for d in self.output_host_array.shape)
                != resolved_output_shape
                or self.output_host_array.dtype != output_dtype
            ):
                self.output_host_array = self.imports.np.ctypeslib.as_array(
                    (
                        self.imports.np.ctypeslib.as_ctypes_type(output_dtype)
                        * int(self.imports.np.prod(resolved_output_shape))
                    ).from_address(int(self.output_host_ptr))
                ).reshape(resolved_output_shape)
            self.output_host_memory_kind = "pinned"
...
def _predict_pytorch(session, request):
    ...
    image = load_prediction_image(
        ...
        dataset_storage=session.dataset_storage,
        request=request,
    )
    ...
    input_tensor, resize_ratio = preprocess_image(
        ...
        np_module=session.imports.np,
        image=image,
        input_size=session.runtime_target.input_size,
    )
    input_tensor = (
        session.imports.torch.from_numpy(input_tensor)
        .unsqueeze(0)
        .to(session.device_name)
        .float()
    )
    ...
    instances = _build_obb_instances(
        ...
        model_type=session.runtime_target.model_type,
        labels=session.runtime_target.labels,
    ...
    return ObbPredictionExecutionResult(
        ...
        runtime_session_info=ObbRuntimeSessionInfo(
            backend_name=session.runtime_target.runtime_backend,
            model_uri=session.runtime_target.runtime_artifact_storage_uri,
            ...
            output_specs=(
                ObbRuntimeTensorSpec(
                    name="predictions",
                    shape=tuple(int(d) for d in prediction_array.shape),
                    dtype=output_dtype,
                ),
            ),
            metadata={
                "model_version_id": session.runtime_target.model_version_id,
...
def _predict_onnx(session, request):
    ...
    input_tensor = session.imports.np.expand_dims(input_tensor, axis=0).astype(
        session.imports.np.float32, copy=False
    )
    preprocess_ms = round((perf_counter() - preprocess_started_at) * 1000, 3)
    infer_started_at = perf_counter()
    outputs = session.session.run(
        list(session.output_names), {session.input_name: input_tensor}
    )
    infer_ms = round((perf_counter() - infer_started_at) * 1000, 3)
    ...
    return ObbPredictionExecutionResult(
        ...
        runtime_session_info=ObbRuntimeSessionInfo(
            ...
            metadata={
                "model_version_id": session.runtime_target.model_version_id,
                "runtime_precision": session.runtime_target.runtime_precision,
                "score_threshold": request.score_threshold,
                "class_count": len(session.runtime_target.labels),
                "decode_ms": decode_ms,
                "preprocess_ms": preprocess_ms,
                "infer_ms": infer_ms,
                "provider_names": list(session.session.get_providers()),
        ...
        runtime_session_info=ObbRuntimeSessionInfo(
            ...
            input_spec=ObbRuntimeTensorSpec(
                ...
                dtype=resolve_openvino_port_dtype(
                    session.input_port, fallback="float32"
                ),
            ),
            output_specs=(
                ObbRuntimeTensorSpec(
                    ...
                    shape=tuple(int(d) for d in prediction_array.shape),
                    dtype=resolve_openvino_port_dtype(
                        session.output_port, fallback="float32"
                    ),
                ),
            ),
            metadata={
                ...
                "preprocess_ms": preprocess_ms,
                "infer_ms": infer_ms,
                "compiled_device_name": session.compiled_device_name,
                "compiled_runtime_precision": session.compiled_runtime_precision,
            },
        ),
    )
...
def _predict_tensorrt(session, request):
    ...
    input_array = session.imports.np.expand_dims(input_tensor, axis=0).astype(
        resolve_numpy_dtype(
            np_module=session.imports.np, dtype_name=session.input_dtype_name
        ),
        copy=False,
    )
    ...
    infer_started_at = perf_counter()
    ensure_cuda_success(
        session.imports.cudart.cudaSetDevice(
            resolve_cuda_device_index(session.device_name)
        ),
    ...
    engine_input_shape = normalize_tensor_shape(
        session.engine.get_tensor_shape(session.input_name)
    )
    if any(d < 0 for d in engine_input_shape):
        if (
            session.context.set_input_shape(session.input_name, requested_input_shape)
            is not True
        ):
            raise ServiceConfigurationError(
    ...
    resolved_output_shape = normalize_tensor_shape(
        session.context.get_tensor_shape(session.output_name)
    )
    ...
    output_array = session._ensure_io_buffers(
        input_array=input_array, resolved_output_shape=resolved_output_shape
    )
    ensure_cuda_success(
        session.imports.cudart.cudaMemcpyAsync(
            session.input_device_ptr,
            int(input_array.ctypes.data),
            int(input_array.nbytes),
            session.imports.cudart.cudaMemcpyKind.cudaMemcpyHostToDevice,
            session.stream,
        ),
        operation_name="TensorRT obb runtime 拷贝输入到显存",
    )
    if (
        session.context.set_tensor_address(
            session.input_name, int(session.input_device_ptr)
        )
        is not True
    ):
    ...
    if (
        session.context.set_tensor_address(
            session.output_name, int(session.output_device_ptr)
        )
        is not True
    ):
        raise ServiceConfigurationError("TensorRT obb 绑定输出张量失败")
    ensure_cuda_success(
        session.imports.cudart.cudaEventRecord(
            session.execute_start_event, session.stream
        ),
    ...
    ensure_cuda_success(
        session.imports.cudart.cudaEventRecord(
            session.execute_end_event, session.stream
        ),
    ...
    ensure_cuda_success(
        session.imports.cudart.cudaMemcpyAsync(
            ...
            session.output_device_ptr,
            int(output_array.nbytes),
            session.imports.cudart.cudaMemcpyKind.cudaMemcpyDeviceToHost,
            session.stream,
        ),
        operation_name="TensorRT obb runtime 拷贝输出到主存",
    )
    ensure_cuda_success(
        session.imports.cudart.cudaStreamSynchronize(session.stream),
        operation_name="TensorRT obb runtime 同步 CUDA stream",
    )
    infer_execute_gpu_ms = measure_cuda_event_elapsed_ms(
        cudart_module=session.imports.cudart,
        start_event=session.execute_start_event,
        end_event=session.execute_end_event,
        device_name=session.device_name,
    )
    infer_ms = round((perf_counter() - infer_started_at) * 1000, 3)
    prediction_array = _normalize_onnx_prediction(
    ...
    return ObbPredictionExecutionResult(
        ...
        runtime_session_info=ObbRuntimeSessionInfo(
            ...
            input_spec=ObbRuntimeTensorSpec(
                ...
                dtype=session.input_dtype_name,
            ),
            output_specs=(
                ObbRuntimeTensorSpec(
                    name=session.output_name,
                    shape=resolved_output_shape,
                    dtype=session.output_dtype_name,
                ),
            ),
            metadata={
                "model_version_id": session.runtime_target.model_version_id,
...
def _normalize_pytorch_prediction(outputs, *, np_module, model_type):
    ...
    if normalized_model_type == "yolov8":
        return normalize_yolov8_obb_inference_outputs(
            outputs=outputs,
            np_module=np_module,
        )
    if normalized_model_type == "yolo11":
        return normalize_yolo11_obb_inference_outputs(
            outputs=outputs,
            np_module=np_module,
        )
...
def _build_obb_instances(
    *,
    ...
    normalized_model_type = normalize_optional_platform_model_type(model_type)
    if normalized_model_type in {"yolov8", "yolo11"}:
        build_instances = (
            build_yolo11_obb_inference_instances
            if normalized_model_type == "yolo11"
            else build_yolov8_obb_inference_instances
        )
        core_instances = build_instances(
            np_module=np_module,
            prediction_array=prediction_array,
            labels=labels,
    ...
    for image_prediction in p:
        ...
        if int(keep_indices.size) <= 0:
            continue
        for box, score, cls_id, angle in zip(
            boxes[keep_indices],
...
def _render_obb_preview_image(*, cv2_module, image, instances, model_type):
    ...
    if normalize_optional_platform_model_type(model_type) in {"yolov8", "yolo11"}:
...
def _require_primary_model_type(model_type, model_label):
    ...
    if not n or n == "yolo-primary":
        raise ServiceConfigurationError(
            f"当前 {model_label} obb predictor 缺少正式 model_type 配置",
