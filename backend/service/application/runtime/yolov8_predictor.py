...
class OpenVINOYoloV8RuntimeSession(OpenVINOYoloXRuntimeSession):
    ...
    def load(
        ...
        *,
        ...
        """加载一套 OpenVINO YOLOv8 会话。"""

        if runtime_target.runtime_backend != "openvino":
            raise InvalidRequestError(
                "当前 YOLOv8 predictor 仅支持 openvino runtime_backend",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
...
class TensorRTYoloV8RuntimeSession(TensorRTYoloXRuntimeSession):
    ...
    def load(
        ...
        *,
        ...
        """加载一套 TensorRT YOLOv8 会话。"""

        if runtime_target.runtime_backend != "tensorrt":
            raise InvalidRequestError(
                "当前 YOLOv8 predictor 仅支持 tensorrt runtime_backend",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
