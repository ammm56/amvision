"""YOLO26 detection PyTorch runtime session。"""
...
class Yolo26DetectionPytorchInferenceImports:
    """描述 YOLO26 detection PyTorch 推理依赖。"""
...
class PyTorchYolo26RuntimeSession:
    ...
    def __init__(
        ...
        *,
        ...
        imports: Yolo26DetectionPytorchInferenceImports,
        model: Any,
        device_name: str,
        runtime_precision: str,
    ) -> None:
        """初始化 PyTorch YOLO26 detection 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 PyTorch YOLO26 detection 会话。"""

        if runtime_target.runtime_backend != "pytorch":
            raise InvalidRequestError(
                "当前 YOLO26 detection predictor 仅支持 pytorch runtime_backend",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
        if runtime_target.model_type != "yolo26":
            raise InvalidRequestError(
                "YOLO26 detection predictor 只支持 yolo26 model_type",
                details={"model_type": runtime_target.model_type},
            )

        imports = require_yolo26_detection_pytorch_imports()
    ...
    def predict(
        ...
        """执行一次 PyTorch YOLO26 detection 预测。"""
        ...
        return DetectionPredictionExecutionResult(
            ...
            runtime_session_info=DetectionRuntimeSessionInfo(
                ...
                output_spec=DetectionRuntimeTensorSpec(
                    name="predictions",
                    shape=tuple(int(dim) for dim in prediction_array.shape),
...
def require_yolo26_detection_pytorch_imports() -> (
    Yolo26DetectionPytorchInferenceImports
):
    """按需导入 YOLO26 detection PyTorch 推理依赖。"""

    inference_imports = require_inference_imports()
    try:
        import torch
    except ImportError as error:  # pragma: no cover - 依赖存在时不会进入该分支
        raise ServiceConfigurationError("当前运行环境缺少 torch 依赖") from error
    return Yolo26DetectionPytorchInferenceImports(
...
__all__ = [
    "PyTorchYolo26RuntimeSession",
    "Yolo26DetectionPytorchInferenceImports",
    "require_yolo26_detection_pytorch_imports",
]
