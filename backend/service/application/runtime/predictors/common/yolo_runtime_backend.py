...
class YoloRuntimeInferenceImports:
    """描述 YOLO runtime 基础推理依赖。"""
...
@dataclass(frozen=True)
class YoloRuntimePytorchInferenceImports(YoloRuntimeInferenceImports):
    """描述 YOLO runtime PyTorch 推理依赖。"""

    torch: Any


@dataclass(frozen=True)
class YoloRuntimeCudaInferenceImports(YoloRuntimeInferenceImports):
    """描述 YOLO runtime TensorRT/CUDA 推理依赖。"""

    cudart: Any


def require_yolo_runtime_inference_imports() -> YoloRuntimeInferenceImports:
    """按需导入 YOLO runtime 基础推理依赖。"""
    ...
    return YoloRuntimeInferenceImports(cv2=cv2, np=np)


def require_yolo_runtime_pytorch_imports() -> YoloRuntimePytorchInferenceImports:
    """按需导入 YOLO runtime PyTorch 推理依赖。"""
    ...
    return YoloRuntimePytorchInferenceImports(cv2=imports.cv2, np=imports.np, torch=torch)


def require_yolo_runtime_cuda_imports() -> YoloRuntimeCudaInferenceImports:
    """按需导入 YOLO runtime TensorRT/CUDA 推理依赖。"""

    imports = require_yolo_runtime_inference_imports()
    return YoloRuntimeCudaInferenceImports(
        cv2=imports.cv2,
        np=imports.np,
        cudart=import_yolo_runtime_cuda_runtime_module(),
    )


def import_yolo_runtime_cuda_runtime_module() -> Any:
...
def import_yolo_runtime_onnxruntime_module() -> Any:
    """导入 ONNXRuntime 模块。"""
...
def import_yolo_runtime_openvino_module() -> Any:
    """导入 OpenVINO 模块。"""
...
def import_yolo_runtime_tensorrt_module() -> Any:
    """导入 TensorRT 模块。"""
...
def resolve_yolo_runtime_onnxruntime_providers(
    *,
    onnxruntime_module: Any,
    requested_device_name: str,
) -> list[object]:
    """按 device_name 解析 ONNXRuntime provider。"""

    if requested_device_name != "cpu":
        raise InvalidRequestError(
            "当前 onnxruntime session 仅支持 cpu device_name",
...
def resolve_yolo_runtime_openvino_device_name(*, requested_device_name: str) -> str:
    ...
    if resolved is None:
        raise InvalidRequestError(
            "当前 openvino session 仅支持 auto、cpu、gpu 或 npu device_name",
            details={"device_name": requested_device_name},
        )
    return resolved


def build_yolo_runtime_openvino_compile_properties(
    *,
    openvino_module: Any,
...
def normalize_yolo_runtime_openvino_type_name(value: object) -> str:
...
def resolve_yolo_runtime_openvino_compiled_runtime_precision(
    *,
    session: Any,
    ...
    normalized = normalize_yolo_runtime_openvino_type_name(resolved)
    precision_map = {
        "float16": "fp16",
        "float32": "fp32",
    }
...
def resolve_yolo_runtime_openvino_port_dtype(port: Any, *, fallback: str) -> str:
    ...
    if not callable(element_type_getter):
        return fallback
    try:
        return normalize_yolo_runtime_openvino_type_name(element_type_getter()) or fallback
    except Exception:
        return fallback


def resolve_yolo_runtime_openvino_port_name(port: Any, *, fallback: str) -> str:
...
def get_yolo_runtime_tensorrt_logger(*, tensorrt_module: Any, severity: Any) -> Any:
    """返回进程级复用的 TensorRT logger。"""
...
def resolve_yolo_runtime_tensorrt_io_tensor_name(
    *,
    engine: Any,
    tensorrt_module: Any,
    io_mode: Any,
...
def normalize_yolo_runtime_tensor_shape(shape: object) -> tuple[int, ...]:
...
def resolve_yolo_runtime_tensorrt_dtype_name(
    *,
    tensorrt_module: Any,
    tensor_dtype: Any,
    fallback: str,
...
def resolve_yolo_runtime_cuda_device_index(device_name: str) -> int:
...
def resolve_yolo_runtime_cuda_runtime_device_name(
    *,
    cudart_module: Any,
    requested_device_name: str,
) -> str:
    ...
    device_index = resolve_yolo_runtime_cuda_device_index(device_name)
...
def ensure_yolo_runtime_cuda_success(
    result: object,
    *,
    operation_name: str,
...
def release_yolo_runtime_cuda_resource(result: object) -> None:
...
def resolve_yolo_runtime_torch_device_name(
    *,
    torch_module: Any,
    requested_device_name: str,
) -> str:
    """解析 YOLO PyTorch 推理 device。"""
...
def enable_yolo_runtime_cuda_fast_path(*, torch_module: Any, device_name: str) -> None:
    """按需打开 YOLO CUDA 推理常用加速设置。"""
...
__all__ = [
    "YoloRuntimeCudaInferenceImports",
    "YoloRuntimeInferenceImports",
    "YoloRuntimePytorchInferenceImports",
