"""YOLO11 OBB TensorRT runtime session。"""
...
from backend.service.application.runtime.predictors.yolo11_obb_backend import (
...
from backend.service.application.runtime.predictors.yolo11_obb_buffer import (
    build_yolo11_obb_numpy_array_from_host_pointer,
    resolve_yolo11_obb_numpy_dtype,
    resolve_yolo11_obb_tensor_byte_size,
)
from backend.service.application.runtime.predictors.yolo11_obb_contracts import (
    Yolo11ObbPredictionExecutionResult,
    Yolo11ObbPredictionRequest,
    Yolo11ObbRuntimeSessionInfo,
    Yolo11ObbRuntimeTensorSpec,
)
from backend.service.application.runtime.predictors.yolo11_obb_io import (
    load_yolo11_obb_prediction_image,
    preprocess_yolo11_obb_image,
)
from backend.service.application.runtime.predictors.yolo11_obb_postprocess import (
    build_yolo11_obb_runtime_instances,
)
from backend.service.application.runtime.predictors.yolo11_obb_preview import (
    render_yolo11_obb_preview_image_if_requested,
)
from backend.service.application.runtime.predictors.yolo11_obb_timing import (
...
class TensorRTYolo11ObbRuntimeSession:
    """已经加载完成并可重复推理的 TensorRT YOLO11 OBB 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 TensorRT YOLO11 OBB 会话。"""

        if runtime_target.runtime_backend != "tensorrt":
            raise InvalidRequestError(
                "当前 YOLO11 OBB predictor 仅支持 tensorrt runtime_backend",
        ...
        imports = require_yolo11_obb_cuda_imports()
        tensorrt_module = import_yolo11_obb_tensorrt_module()
        device_name = resolve_yolo11_obb_cuda_runtime_device_name(
            cudart_module=imports.cudart,
            requested_device_name=runtime_target.device_name,
        )
        ensure_yolo11_obb_cuda_success(
            imports.cudart.cudaSetDevice(
                resolve_yolo11_obb_cuda_device_index(device_name)
            ),
            operation_name="TensorRT OBB runtime 切换 CUDA device",
            details={"device_name": device_name},
        )
        logger = get_yolo11_obb_tensorrt_logger(
        ...
        return cls(
            ...
            input_dtype_name=resolve_yolo11_obb_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
                tensor_dtype=engine.get_tensor_dtype(input_name),
                fallback="float32",
            ),
            output_dtype_name=resolve_yolo11_obb_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
    ...
    def predict(
        ...
        ensure_yolo11_obb_cuda_success(
            self.imports.cudart.cudaSetDevice(
                resolve_yolo11_obb_cuda_device_index(self.device_name)
            ),
        ...
        infer_execute_gpu_ms = measure_yolo11_obb_cuda_event_elapsed_ms(
        ...
        return Yolo11ObbPredictionExecutionResult(
            ...
            runtime_session_info=Yolo11ObbRuntimeSessionInfo(
                ...
                metadata={
                    ...
                    "debugger_attached": is_yolo11_obb_debugger_attached(),
    ...
    def _set_or_validate_input_shape(
        ...
        engine_input_shape = normalize_yolo11_obb_tensor_shape(
    ...
    def _resolve_output_shape(self) -> tuple[int, ...]:
        ...
        resolved_output_shape = normalize_yolo11_obb_tensor_shape(
    ...
    def _ensure_io_buffers(
        ...
        output_dtype = resolve_yolo11_obb_numpy_dtype(
            np_module=self.imports.np,
            dtype_name=self.output_dtype_name,
        )
        input_nbytes = int(input_array.nbytes)
        output_nbytes = resolve_yolo11_obb_tensor_byte_size(
            np_module=self.imports.np,
            shape=resolved_output_shape,
            dtype=output_dtype,
        )
    ...
    def _ensure_device_buffer(
        ...
        *,
        ...
        if current_ptr is not None:
            release_yolo11_obb_cuda_resource(self.imports.cudart.cudaFree(current_ptr))
        device_ptr = ensure_yolo11_obb_cuda_success(
    ...
    def _ensure_pinned_output_array(
        ...
        *,
        ...
        if (
            ...
            self.output_host_ptr = ensure_yolo11_obb_cuda_success(
        ...
        if (
            ...
            self.output_host_array = build_yolo11_obb_numpy_array_from_host_pointer(
                np_module=self.imports.np,
...
__all__ = ["TensorRTYolo11ObbRuntimeSession"]
