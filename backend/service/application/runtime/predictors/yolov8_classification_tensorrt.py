"""YOLOv8 classification TensorRT runtime session。"""
...
from backend.service.application.runtime.predictors.yolov8_classification_backend import (
...
from backend.service.application.runtime.predictors.yolov8_classification_buffer import (
...
from backend.service.application.runtime.predictors.yolov8_classification_contracts import (
...
from backend.service.application.runtime.predictors.yolov8_classification_io import (
    load_yolov8_classification_prediction_image,
    preprocess_yolov8_classification_image,
)
from backend.service.application.runtime.predictors.yolov8_classification_postprocess import (
    build_yolov8_classification_runtime_categories,
)
from backend.service.application.runtime.predictors.yolov8_classification_preview import (
    render_yolov8_classification_preview_image_if_requested,
)
from backend.service.application.runtime.predictors.yolov8_classification_timing import (
...
class TensorRTYoloV8ClassificationRuntimeSession:
    ...
    def __init__(
        ...
        *,
        ...
        """初始化 TensorRT YOLOv8 classification 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 TensorRT YOLOv8 classification 会话。"""

        if runtime_target.runtime_backend != "tensorrt":
            raise InvalidRequestError(
                "当前 YOLOv8 classification predictor 仅支持 tensorrt runtime_backend",
                details={"runtime_backend": runtime_target.runtime_backend},
            )
        if runtime_target.task_type != cls.task_type:
            raise InvalidRequestError(
                "当前 YOLOv8 classification predictor 收到了错误的 task_type",
                details={"task_type": runtime_target.task_type},
            )

        imports = require_yolov8_classification_cuda_imports()
        tensorrt_module = import_yolov8_classification_tensorrt_module()
        device_name = resolve_yolov8_classification_cuda_runtime_device_name(
            cudart_module=imports.cudart,
            requested_device_name=runtime_target.device_name,
        )
        ensure_yolov8_classification_cuda_success(
            imports.cudart.cudaSetDevice(resolve_yolov8_classification_cuda_device_index(device_name)),
            operation_name="TensorRT classification runtime 切换 CUDA device",
            details={"device_name": device_name},
        )
        logger = get_yolov8_classification_tensorrt_logger(
        ...
        if engine is None:
            raise ServiceConfigurationError(
                "TensorRT classification engine 反序列化失败",
                details={"model_build_id": runtime_target.model_build_id},
            )
        context = engine.create_execution_context()
        if context is None:
            raise ServiceConfigurationError(
                "TensorRT classification engine 无法创建 execution context",
                details={"model_build_id": runtime_target.model_build_id},
            )
        stream = ensure_yolov8_classification_cuda_success(
            imports.cudart.cudaStreamCreate(),
            operation_name="TensorRT classification runtime 创建复用 CUDA stream",
            details={"device_name": device_name},
        )[0]
        execute_start_event = ensure_yolov8_classification_cuda_success(
            imports.cudart.cudaEventCreate(),
            operation_name="TensorRT classification runtime 创建执行起点 event",
            details={"device_name": device_name},
        )[0]
        execute_end_event = ensure_yolov8_classification_cuda_success(
            imports.cudart.cudaEventCreate(),
            operation_name="TensorRT classification runtime 创建执行终点 event",
            details={"device_name": device_name},
        )[0]
        input_name = resolve_yolov8_classification_tensorrt_io_tensor_name(
            engine=engine,
            tensorrt_module=tensorrt_module,
            io_mode=tensorrt_module.TensorIOMode.INPUT,
            fallback="images",
        )
        output_name = resolve_yolov8_classification_tensorrt_io_tensor_name(
            engine=engine,
            tensorrt_module=tensorrt_module,
        ...
        return cls(
            ...
            input_dtype_name=resolve_yolov8_classification_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
                tensor_dtype=engine.get_tensor_dtype(input_name),
                fallback="float32",
            ),
            output_dtype_name=resolve_yolov8_classification_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
    ...
    def predict(
        ...
        """执行一次 TensorRT YOLOv8 classification 预测。"""

        top_k = resolve_yolov8_classification_top_k(
            request=request,
            class_count=len(self.runtime_target.labels),
        )
        decode_started_at = perf_counter()
        image = load_yolov8_classification_prediction_image(
            cv2_module=self.imports.cv2,
            np_module=self.imports.np,
            dataset_storage=self.dataset_storage,
            request=request,
        )
        decode_ms = measure_yolov8_classification_stage_elapsed_ms(
        ...
        input_tensor = preprocess_yolov8_classification_image(
            cv2_module=self.imports.cv2,
            np_module=self.imports.np,
            image=image,
        ...
        preprocess_ms = measure_yolov8_classification_stage_elapsed_ms(
            imports=self.imports,
            device_name=self.device_name,
        ...
        device_index = resolve_yolov8_classification_cuda_device_index(self.device_name)
        set_device_started_at = perf_counter()
        ensure_yolov8_classification_cuda_success(
            self.imports.cudart.cudaSetDevice(device_index),
            operation_name="TensorRT classification runtime 绑定 CUDA device",
            details={"device_name": self.device_name},
        )
        infer_set_device_ms = measure_yolov8_classification_elapsed_ms(set_device_started_at)
        ...
        infer_prepare_io_ms = measure_yolov8_classification_elapsed_ms(prepare_io_started_at)
        ...
        ensure_yolov8_classification_cuda_success(
            ...
            operation_name="TensorRT classification runtime 拷贝输入到显存",
            details={"input_name": self.input_name, "byte_size": int(input_array.nbytes)},
        )
        infer_enqueue_h2d_ms = measure_yolov8_classification_elapsed_ms(enqueue_h2d_started_at)

        bind_tensor_started_at = perf_counter()
        self._bind_io_tensor_addresses()
        infer_bind_tensor_ms = measure_yolov8_classification_elapsed_ms(bind_tensor_started_at)

        execute_enqueue_started_at = perf_counter()
        ensure_yolov8_classification_cuda_success(
            self.imports.cudart.cudaEventRecord(self.execute_start_event, self.stream),
            operation_name="TensorRT classification runtime 记录执行起点 event",
            details={"device_name": self.device_name},
        )
        if self.context.execute_async_v3(stream_handle=self.stream) is not True:
            raise ServiceConfigurationError(
                "TensorRT classification execution context 执行推理失败",
        ...
        ensure_yolov8_classification_cuda_success(
            ...
            operation_name="TensorRT classification runtime 记录执行终点 event",
            details={"device_name": self.device_name},
        )
        infer_execute_enqueue_ms = measure_yolov8_classification_elapsed_ms(execute_enqueue_started_at)
        ...
        ensure_yolov8_classification_cuda_success(
            ...
            operation_name="TensorRT classification runtime 拷贝输出到主存",
            details={"output_name": self.output_name, "byte_size": int(output_array.nbytes)},
        )
        infer_enqueue_d2h_host_ms = measure_yolov8_classification_elapsed_ms(enqueue_d2h_started_at)

        output_ready_wait_started_at = perf_counter()
        ensure_yolov8_classification_cuda_success(
            self.imports.cudart.cudaStreamSynchronize(self.stream),
            operation_name="TensorRT classification runtime 同步 CUDA stream",
            details={"device_name": self.device_name},
        )
        infer_output_ready_wait_ms = measure_yolov8_classification_elapsed_ms(output_ready_wait_started_at)
        infer_execute_gpu_ms = measure_yolov8_classification_cuda_event_elapsed_ms(
        ...
        probabilities, logits = normalize_yolov8_classification_outputs_for_backend(
            outputs=[output_array],
            np_module=self.imports.np,
        )
        categories = build_yolov8_classification_runtime_categories(
            np_module=self.imports.np,
        ...
        preview_image_bytes = render_yolov8_classification_preview_image_if_requested(
            cv2_module=self.imports.cv2,
            image=image,
            categories=categories,
            save_result_image=request.save_result_image,
        )
        return YoloV8ClassificationPredictionExecutionResult(
            categories=categories,
            ...
            runtime_session_info=YoloV8ClassificationRuntimeSessionInfo(
                ...
                output_spec=YoloV8ClassificationRuntimeTensorSpec(
                ...
                metadata={
                    ...
                    "debugger_attached": is_yolov8_classification_debugger_attached(),
    ...
    def close(self) -> None:
        """释放 TensorRT classification session 持有的 CUDA 资源。"""

        try:
            release_yolov8_classification_cuda_resource(
                self.imports.cudart.cudaSetDevice(
                    resolve_yolov8_classification_cuda_device_index(self.device_name)
                )
            )
        except Exception:
            return
    ...
    def describe_memory_usage(self) -> dict[str, object]:
        """返回当前 TensorRT classification session 的输出 host buffer 占用快照。"""
    ...
    def _set_or_validate_input_shape(self, requested_input_shape: tuple[int, ...]) -> None:
        ...
        engine_input_shape = normalize_yolov8_classification_tensor_shape(
        ...
        if any(dim < 0 for dim in engine_input_shape):
            ...
            if shape_set_result is not True:
                raise ServiceConfigurationError(
                    "TensorRT classification execution context 设置输入 shape 失败",
                    details={
            ...
            raise InvalidRequestError(
                "TensorRT classification engine 输入尺寸与 runtime input_size 不一致",
                details={
    ...
    def _resolve_output_shape(self) -> tuple[int, ...]:
        ...
        resolved_output_shape = normalize_yolov8_classification_tensor_shape(
            self.context.get_tensor_shape(self.output_name)
        )
        if not resolved_output_shape or any(dim <= 0 for dim in resolved_output_shape):
            raise ServiceConfigurationError(
                "TensorRT classification execution context 返回了无效输出 shape",
                details={
    ...
    def _bind_io_tensor_addresses(self) -> None:
        ...
        if self.context.set_tensor_address(self.input_name, int(self.input_device_ptr)) is not True:
            raise ServiceConfigurationError(
                "TensorRT classification execution context 绑定输入张量失败",
        ...
        if self.context.set_tensor_address(self.output_name, int(self.output_device_ptr)) is not True:
            raise ServiceConfigurationError(
                "TensorRT classification execution context 绑定输出张量失败",
    ...
    def _ensure_io_buffers(
        ...
        *,
        ...
        output_dtype = resolve_yolov8_classification_numpy_dtype(
            np_module=self.imports.np,
            dtype_name=self.output_dtype_name,
        )
        input_nbytes = int(input_array.nbytes)
        output_nbytes = resolve_yolov8_classification_tensor_byte_size(
            np_module=self.imports.np,
            shape=resolved_output_shape,
            dtype=output_dtype,
        )
    ...
    def _ensure_device_buffer(
        ...
        *,
        ...
        if current_ptr is not None:
            release_yolov8_classification_cuda_resource(self.imports.cudart.cudaFree(current_ptr))
        device_ptr = ensure_yolov8_classification_cuda_success(
            self.imports.cudart.cudaMalloc(required_bytes),
            operation_name=f"TensorRT classification runtime 分配复用{label}显存",
    ...
    def _ensure_pinned_output_array(
        ...
        *,
        ...
        if self.output_host_ptr is None or output_nbytes > self.output_host_capacity_bytes:
            ...
            self.output_host_ptr = ensure_yolov8_classification_cuda_success(
                self.imports.cudart.cudaMallocHost(output_nbytes),
                operation_name="TensorRT classification runtime 分配 pinned 输出主存",
        ...
        if (
            ...
            self.output_host_array = build_yolov8_classification_numpy_array_from_host_pointer(
                np_module=self.imports.np,
...
__all__ = ["TensorRTYoloV8ClassificationRuntimeSession"]
