"""YOLOv8 detection OpenVINO runtime session。"""
...
from backend.service.application.runtime.predictors.yolov8_detection_backend import (
...
from backend.service.application.runtime.predictors.yolov8_detection_contracts import (
...
from backend.service.application.runtime.predictors.yolov8_detection_preview import (
...
class OpenVINOYoloV8RuntimeSession:
    """已经加载完成并可重复推理的 OpenVINO YOLOv8 detection 会话。"""
    ...
    def __init__(
        ...
        *,
        ...
        """初始化 OpenVINO YOLOv8 detection 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 OpenVINO YOLOv8 detection 会话。"""

        if runtime_target.runtime_backend != "openvino":
            raise InvalidRequestError(
                "当前 YOLOv8 detection predictor 仅支持 openvino runtime_backend",
                details={"runtime_backend": runtime_target.runtime_backend},
            )

        imports = require_yolov8_detection_inference_imports()
        openvino_module = import_yolov8_detection_openvino_module()
        compiled_device_name = resolve_yolov8_detection_openvino_device_name(
            requested_device_name=runtime_target.device_name,
        )
        compile_properties = build_yolov8_detection_openvino_compile_properties(
        ...
        return cls(
            ...
            input_name=resolve_yolov8_detection_openvino_port_name(input_port, fallback="images"),
            output_name=resolve_yolov8_detection_openvino_port_name(output_port, fallback="predictions"),
            input_port=input_port,
            output_port=output_port,
            compiled_device_name=compiled_device_name,
            compiled_runtime_precision=resolve_yolov8_detection_openvino_compiled_runtime_precision(
                session=session,
    ...
    def predict(
        ...
        """执行一次 OpenVINO YOLOv8 detection 预测。"""
        ...
        input_tensor, resize_ratio = preprocess_yolov8_detection_image(
            cv2_module=self.imports.cv2,
            np_module=self.imports.np,
            image=image,
        ...
        prediction_array = normalize_yolov8_detection_openvino_outputs(
            outputs=outputs,
        ...
        return YoloV8DetectionPredictionExecutionResult(
            ...
            runtime_session_info=YoloV8DetectionRuntimeSessionInfo(
                ...
                input_spec=YoloV8DetectionRuntimeTensorSpec(
                    ...
                    dtype=resolve_yolov8_detection_openvino_port_dtype(
                        self.input_port,
                        fallback="float32",
                    ),
                ),
...
__all__ = ["OpenVINOYoloV8RuntimeSession"]
