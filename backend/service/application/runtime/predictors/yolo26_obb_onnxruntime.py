"""YOLO26 OBB ONNXRuntime runtime session。"""
...
class OnnxRuntimeYolo26ObbRuntimeSession:
    """已经加载完成并可重复推理的 ONNXRuntime YOLO26 OBB 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 ONNXRuntime YOLO26 OBB 会话。"""

        if runtime_target.runtime_backend != "onnxruntime":
            raise InvalidRequestError(
                "当前 YOLO26 OBB predictor 仅支持 onnxruntime runtime_backend",
        ...
        if runtime_target.runtime_precision != "fp32":
            raise InvalidRequestError(
                "当前 YOLO26 OBB onnxruntime session 仅支持 fp32 precision",
                details={"runtime_precision": runtime_target.runtime_precision},
            )
        imports = require_yolo26_obb_inference_imports()
        onnxruntime_module = import_yolo26_obb_onnxruntime_module()
        providers = resolve_yolo26_obb_onnxruntime_providers(
    ...
    def predict(
        ...
        """执行一次 ONNXRuntime YOLO26 OBB 预测。"""
...
__all__ = ["OnnxRuntimeYolo26ObbRuntimeSession"]
