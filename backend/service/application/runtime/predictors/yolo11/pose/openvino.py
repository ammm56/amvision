"""YOLO11 pose OpenVINO runtime session。"""
...
class OpenVINOYolo11PoseRuntimeSession:
    """已经加载完成并可重复推理的 OpenVINO YOLO11 pose 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 OpenVINO YOLO11 pose 会话。"""

        if runtime_target.runtime_backend != "openvino":
            raise InvalidRequestError(
                "当前 YOLO11 pose predictor 仅支持 openvino runtime_backend",
        ...
        imports = require_yolo11_pose_inference_imports()
        openvino_module = import_yolo11_pose_openvino_module()
        compiled_device_name = resolve_yolo11_pose_openvino_device_name(
            requested_device_name=runtime_target.device_name,
        )
        compile_properties = build_yolo11_pose_openvino_compile_properties(
        ...
        return cls(
            ...
            input_name=resolve_yolo11_pose_openvino_port_name(
                input_port, fallback="images"
            ),
            output_name=resolve_yolo11_pose_openvino_port_name(
                output_port, fallback="predictions"
            ),
            input_port=input_port,
            output_port=output_port,
            compiled_device_name=compiled_device_name,
            compiled_runtime_precision=resolve_yolo11_pose_openvino_compiled_runtime_precision(
                session=session,
        ...
        return Yolo11PosePredictionExecutionResult(
            ...
            runtime_session_info=Yolo11PoseRuntimeSessionInfo(
                ...
                input_spec=Yolo11PoseRuntimeTensorSpec(
                    ...
                    dtype=resolve_yolo11_pose_openvino_port_dtype(
...
__all__ = ["OpenVINOYolo11PoseRuntimeSession"]
