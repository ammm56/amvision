"""YOLO11 classification TensorRT runtime session。"""
...
from backend.service.application.runtime.predictors.yolo11.classification.backend import (
    ensure_yolo11_classification_cuda_success,
    get_yolo11_classification_tensorrt_logger,
    import_yolo11_classification_tensorrt_module,
    normalize_yolo11_classification_outputs_for_backend,
    normalize_yolo11_classification_tensor_shape,
    release_yolo11_classification_cuda_resource,
    require_yolo11_classification_cuda_imports,
    resolve_yolo11_classification_cuda_device_index,
    resolve_yolo11_classification_cuda_runtime_device_name,
    resolve_yolo11_classification_tensorrt_dtype_name,
    resolve_yolo11_classification_tensorrt_io_tensor_name,
)
from backend.service.application.runtime.predictors.yolo11.classification.buffer import (
...
from backend.service.application.runtime.predictors.yolo11.classification.timing import (
...
class TensorRTYolo11ClassificationRuntimeSession:
    ...
    def __init__(
        ...
        *,
        ...
        """初始化 TensorRT YOLO11 classification 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 TensorRT YOLO11 classification 会话。"""

        if runtime_target.runtime_backend != "tensorrt":
            raise InvalidRequestError(
                "当前 YOLO11 classification predictor 仅支持 tensorrt runtime_backend",
        ...
        imports = require_yolo11_classification_cuda_imports()
        tensorrt_module = import_yolo11_classification_tensorrt_module()
        device_name = resolve_yolo11_classification_cuda_runtime_device_name(
            cudart_module=imports.cudart,
            requested_device_name=runtime_target.device_name,
        )
        ensure_yolo11_classification_cuda_success(
            imports.cudart.cudaSetDevice(
                resolve_yolo11_classification_cuda_device_index(device_name)
            ),
            operation_name="TensorRT classification runtime 切换 CUDA device",
            details={"device_name": device_name},
        )
        logger = get_yolo11_classification_tensorrt_logger(
        ...
        stream = ensure_yolo11_classification_cuda_success(
        ...
        execute_start_event = ensure_yolo11_classification_cuda_success(
            imports.cudart.cudaEventCreate(),
            operation_name="TensorRT classification runtime 创建执行起点 event",
            details={"device_name": device_name},
        )[0]
        execute_end_event = ensure_yolo11_classification_cuda_success(
            imports.cudart.cudaEventCreate(),
            operation_name="TensorRT classification runtime 创建执行终点 event",
            details={"device_name": device_name},
        )[0]
        input_name = resolve_yolo11_classification_tensorrt_io_tensor_name(
            engine=engine,
            tensorrt_module=tensorrt_module,
            io_mode=tensorrt_module.TensorIOMode.INPUT,
            fallback="images",
        )
        output_name = resolve_yolo11_classification_tensorrt_io_tensor_name(
            engine=engine,
            tensorrt_module=tensorrt_module,
        ...
        return cls(
            ...
            input_dtype_name=resolve_yolo11_classification_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
                tensor_dtype=engine.get_tensor_dtype(input_name),
                fallback="float32",
            ),
            output_dtype_name=resolve_yolo11_classification_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
    ...
    def predict(
        ...
        """执行一次 TensorRT YOLO11 classification 预测。"""
        ...
        decode_ms = measure_yolo11_classification_stage_elapsed_ms(
        ...
        preprocess_ms = measure_yolo11_classification_stage_elapsed_ms(
            imports=self.imports,
            device_name=self.device_name,
        ...
        device_index = resolve_yolo11_classification_cuda_device_index(self.device_name)
        ...
        infer_set_device_ms = measure_yolo11_classification_elapsed_ms(
        ...
        infer_prepare_io_ms = measure_yolo11_classification_elapsed_ms(
        ...
        infer_enqueue_h2d_ms = measure_yolo11_classification_elapsed_ms(
            enqueue_h2d_started_at
        )

        bind_tensor_started_at = perf_counter()
        self._bind_io_tensor_addresses()
        infer_bind_tensor_ms = measure_yolo11_classification_elapsed_ms(
        ...
        infer_execute_enqueue_ms = measure_yolo11_classification_elapsed_ms(
        ...
        infer_enqueue_d2h_host_ms = measure_yolo11_classification_elapsed_ms(
        ...
        infer_output_ready_wait_ms = measure_yolo11_classification_elapsed_ms(
            output_ready_wait_started_at
        )
        infer_execute_gpu_ms = measure_yolo11_classification_cuda_event_elapsed_ms(
        ...
        return Yolo11ClassificationPredictionExecutionResult(
            ...
            runtime_session_info=Yolo11ClassificationRuntimeSessionInfo(
                ...
                metadata={
                    ...
                    "debugger_attached": is_yolo11_classification_debugger_attached(),
        ...
        try:
            release_yolo11_classification_cuda_resource(
                self.imports.cudart.cudaSetDevice(
                    resolve_yolo11_classification_cuda_device_index(self.device_name)
                )
            )
        except Exception:
            return
    ...
    def _set_or_validate_input_shape(
        ...
        engine_input_shape = normalize_yolo11_classification_tensor_shape(
    ...
    def _resolve_output_shape(self) -> tuple[int, ...]:
        ...
        resolved_output_shape = normalize_yolo11_classification_tensor_shape(
    ...
    def _ensure_io_buffers(
        ...
        *,
        ...
        output_dtype = resolve_yolo11_classification_numpy_dtype(
            np_module=self.imports.np,
            dtype_name=self.output_dtype_name,
        )
        input_nbytes = int(input_array.nbytes)
        output_nbytes = resolve_yolo11_classification_tensor_byte_size(
            np_module=self.imports.np,
            shape=resolved_output_shape,
            dtype=output_dtype,
        )
    ...
    def _ensure_device_buffer(
        ...
        *,
        ...
        device_ptr = ensure_yolo11_classification_cuda_success(
    ...
    def _ensure_pinned_output_array(
        ...
        *,
        ...
        if (
            ...
            self.output_host_ptr = ensure_yolo11_classification_cuda_success(
...
__all__ = ["TensorRTYolo11ClassificationRuntimeSession"]
