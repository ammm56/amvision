"""YOLO11 segmentation TensorRT runtime session。"""
...
from backend.service.application.runtime.predictors.yolo11.segmentation.backend import (
...
from backend.service.application.runtime.predictors.yolo11.segmentation.buffer import (
...
from backend.service.application.runtime.predictors.yolo11.segmentation.contracts import (
...
from backend.service.application.runtime.predictors.yolo11.segmentation.postprocess import (
    build_yolo11_segmentation_runtime_instances,
)
from backend.service.application.runtime.predictors.yolo11.segmentation.preview import (
    render_yolo11_segmentation_preview_image_if_requested,
)
from backend.service.application.runtime.predictors.yolo11.segmentation.timing import (
...
class TensorRTYolo11SegmentationRuntimeSession:
    ...
    def __init__(
        ...
        *,
        ...
        """初始化 TensorRT YOLO11 segmentation 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 TensorRT YOLO11 segmentation 会话。"""

        if runtime_target.runtime_backend != "tensorrt":
            raise InvalidRequestError(
                "当前 YOLO11 segmentation predictor 仅支持 tensorrt runtime_backend",
        ...
        imports = require_yolo11_segmentation_cuda_imports()
        tensorrt_module = import_yolo11_segmentation_tensorrt_module()
        device_name = resolve_yolo11_segmentation_cuda_runtime_device_name(
            cudart_module=imports.cudart,
            requested_device_name=runtime_target.device_name,
        )
        ensure_yolo11_segmentation_cuda_success(
            imports.cudart.cudaSetDevice(
                resolve_yolo11_segmentation_cuda_device_index(device_name)
            ),
            operation_name="TensorRT segmentation runtime 切换 CUDA device",
            details={"device_name": device_name},
        )
        logger = get_yolo11_segmentation_tensorrt_logger(
        ...
        stream = ensure_yolo11_segmentation_cuda_success(
        ...
        execute_start_event = ensure_yolo11_segmentation_cuda_success(
            imports.cudart.cudaEventCreate(),
            operation_name="TensorRT segmentation runtime 创建执行起点 event",
            details={"device_name": device_name},
        )[0]
        execute_end_event = ensure_yolo11_segmentation_cuda_success(
            imports.cudart.cudaEventCreate(),
            operation_name="TensorRT segmentation runtime 创建执行终点 event",
            details={"device_name": device_name},
        )[0]
        input_name = resolve_yolo11_segmentation_tensorrt_io_tensor_name(
            engine=engine,
            tensorrt_module=tensorrt_module,
            io_mode=tensorrt_module.TensorIOMode.INPUT,
            fallback="images",
        )
        prediction_name, proto_name = _resolve_yolo11_segmentation_output_names(
            engine=engine,
            tensorrt_module=tensorrt_module,
        )
        return cls(
            ...
            input_dtype_name=resolve_yolo11_segmentation_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
                tensor_dtype=engine.get_tensor_dtype(input_name),
                fallback="float32",
            ),
            prediction_dtype_name=resolve_yolo11_segmentation_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
                tensor_dtype=engine.get_tensor_dtype(prediction_name),
                fallback="float32",
            ),
            proto_dtype_name=resolve_yolo11_segmentation_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
    ...
    def predict(
        ...
        """执行一次 TensorRT YOLO11 segmentation 预测。"""
        ...
        device_index = resolve_yolo11_segmentation_cuda_device_index(self.device_name)
        ...
        infer_set_device_ms = measure_yolo11_segmentation_elapsed_ms(
        ...
        infer_prepare_io_ms = measure_yolo11_segmentation_elapsed_ms(
        ...
        infer_enqueue_h2d_ms = measure_yolo11_segmentation_elapsed_ms(
            enqueue_h2d_started_at
        )

        bind_tensor_started_at = perf_counter()
        self._bind_io_tensor_addresses()
        infer_bind_tensor_ms = measure_yolo11_segmentation_elapsed_ms(
        ...
        infer_execute_enqueue_ms = measure_yolo11_segmentation_elapsed_ms(
        ...
        infer_enqueue_d2h_host_ms = measure_yolo11_segmentation_elapsed_ms(
        ...
        infer_output_ready_wait_ms = measure_yolo11_segmentation_elapsed_ms(
            output_ready_wait_started_at
        )
        infer_execute_gpu_ms = measure_yolo11_segmentation_cuda_event_elapsed_ms(
        ...
        return Yolo11SegmentationPredictionExecutionResult(
            ...
            runtime_session_info=Yolo11SegmentationRuntimeSessionInfo(
                ...
                metadata={
                    ...
                    "debugger_attached": is_yolo11_segmentation_debugger_attached(),
        ...
        try:
            release_yolo11_segmentation_cuda_resource(
                self.imports.cudart.cudaSetDevice(
                    resolve_yolo11_segmentation_cuda_device_index(self.device_name)
                )
            )
        except Exception:
            return
    ...
    def _set_or_validate_input_shape(
        ...
        engine_input_shape = normalize_yolo11_segmentation_tensor_shape(
    ...
    def _resolve_output_shape(self, *, tensor_name: str, label: str) -> tuple[int, ...]:
        ...
        resolved_shape = normalize_yolo11_segmentation_tensor_shape(
    ...
    def _ensure_io_buffers(
        ...
        *,
        ...
        prediction_dtype = resolve_yolo11_segmentation_numpy_dtype(
            np_module=self.imports.np,
            dtype_name=self.prediction_dtype_name,
        )
        proto_dtype = resolve_yolo11_segmentation_numpy_dtype(
            np_module=self.imports.np,
            dtype_name=self.proto_dtype_name,
        )
        input_nbytes = int(input_array.nbytes)
        prediction_nbytes = resolve_yolo11_segmentation_tensor_byte_size(
            np_module=self.imports.np,
            shape=resolved_prediction_shape,
            dtype=prediction_dtype,
        )
        proto_nbytes = resolve_yolo11_segmentation_tensor_byte_size(
            np_module=self.imports.np,
            shape=resolved_proto_shape,
            dtype=proto_dtype,
        )
    ...
    def _ensure_device_buffer(
        ...
        *,
        ...
        device_ptr = ensure_yolo11_segmentation_cuda_success(
    ...
    def _ensure_pinned_host_buffer(
        ...
        *,
        ...
        if host_ptr is None or nbytes > host_capacity:
            ...
            host_ptr = ensure_yolo11_segmentation_cuda_success(
        ...
        if _host_array_needs_rebuild(
            ...
            host_array = build_yolo11_segmentation_numpy_array_from_host_pointer(
                np_module=self.imports.np,
...
def _resolve_yolo11_segmentation_output_names(
    *,
    engine: Any,
    tensorrt_module: Any,
) -> tuple[str, str]:
    ...
    output_names = list_yolo11_segmentation_tensorrt_output_names(
        engine=engine,
        tensorrt_module=tensorrt_module,
    )
...
__all__ = ["TensorRTYolo11SegmentationRuntimeSession"]
