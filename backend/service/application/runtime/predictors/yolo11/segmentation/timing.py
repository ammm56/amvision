"""YOLO11 segmentation runtime 耗时统计工具。"""
...
def measure_yolo11_segmentation_stage_elapsed_ms(
    *,
    imports: Any,
    device_name: str,
    started_at: float,
...
def synchronize_yolo11_segmentation_device_for_timing(
...
def measure_yolo11_segmentation_elapsed_ms(started_at: float) -> float:
    """返回从 started_at 到当前时刻的毫秒耗时。"""

    return round((perf_counter() - started_at) * 1000, 3)


def measure_yolo11_segmentation_cuda_event_elapsed_ms(
    *,
    cudart_module: Any,
    start_event: Any,
    end_event: Any,
    ...
    try:
        elapsed_ms = ensure_yolo11_segmentation_cuda_success(
...
def is_yolo11_segmentation_debugger_attached() -> bool:
