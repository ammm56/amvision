"""YOLOv8 detection runtime 耗时统计工具。"""
...
def measure_yolov8_detection_stage_elapsed_ms(
    *,
    imports: Any,
    device_name: str,
    started_at: float,
    ...
    synchronize_yolov8_detection_device_for_timing(imports=imports, device_name=device_name)
    return round((perf_counter() - started_at) * 1000, 3)


def synchronize_yolov8_detection_device_for_timing(*, imports: Any, device_name: str) -> None:
...
def measure_yolov8_detection_elapsed_ms(started_at: float) -> float:
    """返回从 started_at 到当前时刻的毫秒耗时。"""

    return round((perf_counter() - started_at) * 1000, 3)


def measure_yolov8_detection_cuda_event_elapsed_ms(
    *,
    cudart_module: Any,
    start_event: Any,
    end_event: Any,
    ...
    try:
        elapsed_ms = ensure_yolov8_detection_cuda_success(
...
def is_yolov8_detection_debugger_attached() -> bool:
