"""YOLOv8 pose TensorRT runtime session。"""
...
from backend.service.application.runtime.predictors.yolov8_pose_backend import (
...
from backend.service.application.runtime.predictors.yolov8_pose_preview import (
    render_yolov8_pose_preview_image_if_requested,
)
from backend.service.application.runtime.predictors.yolov8_pose_timing import (
...
class TensorRTYoloV8PoseRuntimeSession:
    """已经加载完成并可重复推理的 TensorRT YOLOv8 pose 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 TensorRT YOLOv8 pose 会话。"""

        if runtime_target.runtime_backend != "tensorrt":
            raise InvalidRequestError(
                "当前 YOLOv8 pose predictor 仅支持 tensorrt runtime_backend",
        ...
        imports = require_yolov8_pose_cuda_imports()
        tensorrt_module = import_yolov8_pose_tensorrt_module()
        device_name = resolve_yolov8_pose_cuda_runtime_device_name(
            cudart_module=imports.cudart,
            requested_device_name=runtime_target.device_name,
        )
        ensure_yolov8_pose_cuda_success(
            imports.cudart.cudaSetDevice(resolve_yolov8_pose_cuda_device_index(device_name)),
            operation_name="TensorRT pose runtime 切换 CUDA device",
            details={"device_name": device_name},
        )
        logger = get_yolov8_pose_tensorrt_logger(
        ...
        if engine is None:
            raise ServiceConfigurationError(
                "TensorRT pose engine 反序列化失败",
                details={"model_build_id": runtime_target.model_build_id},
            )
        context = engine.create_execution_context()
        if context is None:
            raise ServiceConfigurationError(
                "TensorRT pose engine 无法创建 execution context",
                details={"model_build_id": runtime_target.model_build_id},
            )
        stream = ensure_yolov8_pose_cuda_success(
            imports.cudart.cudaStreamCreate(),
            operation_name="TensorRT pose runtime 创建复用 CUDA stream",
            details={"device_name": device_name},
        )[0]
        execute_start_event = ensure_yolov8_pose_cuda_success(
            imports.cudart.cudaEventCreate(),
            operation_name="TensorRT pose runtime 创建执行起点 event",
            details={"device_name": device_name},
        )[0]
        execute_end_event = ensure_yolov8_pose_cuda_success(
            imports.cudart.cudaEventCreate(),
            operation_name="TensorRT pose runtime 创建执行终点 event",
            details={"device_name": device_name},
        )[0]
        input_name = resolve_yolov8_pose_tensorrt_io_tensor_name(
            engine=engine,
            tensorrt_module=tensorrt_module,
            io_mode=tensorrt_module.TensorIOMode.INPUT,
            fallback="images",
        )
        output_name = resolve_yolov8_pose_tensorrt_io_tensor_name(
            engine=engine,
            tensorrt_module=tensorrt_module,
        ...
        return cls(
            ...
            input_dtype_name=resolve_yolov8_pose_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
                tensor_dtype=engine.get_tensor_dtype(input_name),
                fallback="float32",
            ),
            output_dtype_name=resolve_yolov8_pose_tensorrt_dtype_name(
                tensorrt_module=tensorrt_module,
    ...
    def predict(self, request: YoloV8PosePredictionRequest) -> YoloV8PosePredictionExecutionResult:
        ...
        ensure_yolov8_pose_cuda_success(
            self.imports.cudart.cudaSetDevice(resolve_yolov8_pose_cuda_device_index(self.device_name)),
            operation_name="TensorRT pose runtime 绑定 CUDA device",
        ...
        ensure_yolov8_pose_cuda_success(
            ...
            operation_name="TensorRT pose runtime 拷贝输入到显存",
        ...
        ensure_yolov8_pose_cuda_success(
            ...
            operation_name="TensorRT pose runtime 记录执行起点 event",
            details={"device_name": self.device_name},
        )
        if self.context.execute_async_v3(stream_handle=self.stream) is not True:
            raise ServiceConfigurationError(
                "TensorRT pose execution context 执行推理失败",
        ...
        ensure_yolov8_pose_cuda_success(
            ...
            operation_name="TensorRT pose runtime 记录执行终点 event",
            details={"device_name": self.device_name},
        )
        ensure_yolov8_pose_cuda_success(
            self.imports.cudart.cudaMemcpyAsync(
            ...
            operation_name="TensorRT pose runtime 拷贝输出到主存",
        ...
        ensure_yolov8_pose_cuda_success(
            ...
            operation_name="TensorRT pose runtime 同步 CUDA stream",
            details={"device_name": self.device_name},
        )
        infer_execute_gpu_ms = measure_yolov8_pose_cuda_event_elapsed_ms(
        ...
        return YoloV8PosePredictionExecutionResult(
            ...
            runtime_session_info=YoloV8PoseRuntimeSessionInfo(
                ...
                metadata={
                    ...
                    "debugger_attached": is_yolov8_pose_debugger_attached(),
    ...
    def close(self) -> None:
        """释放 TensorRT pose session 持有的 CUDA 资源。"""

        try:
            release_yolov8_pose_cuda_resource(
                self.imports.cudart.cudaSetDevice(
                    resolve_yolov8_pose_cuda_device_index(self.device_name)
                )
            )
        except Exception:
            return
        if self.input_device_ptr is not None:
            release_yolov8_pose_cuda_resource(self.imports.cudart.cudaFree(self.input_device_ptr))
            self.input_device_ptr = None
            self.input_capacity_bytes = 0
        if self.output_device_ptr is not None:
            release_yolov8_pose_cuda_resource(self.imports.cudart.cudaFree(self.output_device_ptr))
            self.output_device_ptr = None
            self.output_capacity_bytes = 0
        if self.output_host_ptr is not None:
            release_yolov8_pose_cuda_resource(self.imports.cudart.cudaFreeHost(self.output_host_ptr))
            self.output_host_ptr = None
            self.output_host_capacity_bytes = 0
            self.output_host_array = None
        self.output_host_memory_kind = "pageable"
        if self.stream is not None:
            release_yolov8_pose_cuda_resource(self.imports.cudart.cudaStreamDestroy(self.stream))
            self.stream = None
        if self.execute_start_event is not None:
            release_yolov8_pose_cuda_resource(self.imports.cudart.cudaEventDestroy(self.execute_start_event))
            self.execute_start_event = None
        if self.execute_end_event is not None:
            release_yolov8_pose_cuda_resource(self.imports.cudart.cudaEventDestroy(self.execute_end_event))
            self.execute_end_event = None
        self.output_host_array = None

    def describe_memory_usage(self) -> dict[str, object]:
        """返回当前 TensorRT pose session 的输出 host buffer 占用快照。"""
    ...
    def _set_or_validate_input_shape(self, requested_input_shape: tuple[int, ...]) -> None:
        ...
        engine_input_shape = normalize_yolov8_pose_tensor_shape(
    ...
    def _ensure_device_buffer(
        ...
        *,
        ...
        if current_ptr is not None:
            release_yolov8_pose_cuda_resource(self.imports.cudart.cudaFree(current_ptr))
        device_ptr = ensure_yolov8_pose_cuda_success(
            self.imports.cudart.cudaMalloc(required_bytes),
            operation_name=f"TensorRT pose runtime 分配复用{label}显存",
    ...
    def _ensure_pinned_output_array(
        ...
        *,
        ...
        if self.output_host_ptr is None or output_nbytes > self.output_host_capacity_bytes:
            if self.output_host_ptr is not None:
                release_yolov8_pose_cuda_resource(self.imports.cudart.cudaFreeHost(self.output_host_ptr))
            self.output_host_ptr = ensure_yolov8_pose_cuda_success(
                self.imports.cudart.cudaMallocHost(output_nbytes),
                operation_name="TensorRT pose runtime 分配 pinned 输出主存",
        ...
        if (
            ...
            self.output_host_array = build_yolov8_pose_numpy_array_from_host_pointer(
                np_module=self.imports.np,
...
__all__ = ["TensorRTYoloV8PoseRuntimeSession"]
