"""YOLO26 pose PyTorch runtime session。"""
...
class PyTorchYolo26PoseRuntimeSession:
    """已经加载完成并可重复推理的 PyTorch YOLO26 pose 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 PyTorch YOLO26 pose 会话。"""

        if runtime_target.runtime_backend != "pytorch":
            raise InvalidRequestError(
                "当前 YOLO26 pose predictor 仅支持 pytorch runtime_backend",
        ...
        device_name = resolve_yolo26_pose_torch_device_name(
            torch_module=imports.torch,
    ...
    def predict(
        ...
        if callable(inference_mode):
            ...
            with self.imports.torch.no_grad():
                outputs = self.model(input_tensor)
        infer_ms = measure_yolo26_pose_stage_elapsed_ms(
            imports=self.imports,
            device_name=self.device_name,
...
__all__ = ["PyTorchYolo26PoseRuntimeSession"]
