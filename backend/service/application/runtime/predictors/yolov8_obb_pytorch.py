"""YOLOv8 OBB PyTorch runtime session。"""
...
from backend.service.application.runtime.predictors.yolov8_obb_backend import (
...
from backend.service.application.runtime.predictors.yolov8_obb_io import (
...
from backend.service.application.runtime.predictors.yolov8_obb_preview import (
    render_yolov8_obb_preview_image_if_requested,
)
from backend.service.application.runtime.predictors.yolov8_obb_timing import (
...
class PyTorchYoloV8ObbRuntimeSession:
    """已经加载完成并可重复推理的 PyTorch YOLOv8 OBB 会话。"""
    ...
    def load(
        ...
        *,
        ...
        """加载一套 PyTorch YOLOv8 OBB 会话。"""

        if runtime_target.runtime_backend != "pytorch":
            raise InvalidRequestError(
                "当前 YOLOv8 OBB predictor 仅支持 pytorch runtime_backend",
        ...
        imports = require_yolov8_obb_pytorch_imports()
        ...
        device_name = resolve_yolov8_obb_torch_device_name(
            torch_module=imports.torch,
            requested_device_name=runtime_target.device_name,
        )
        enable_yolov8_obb_cuda_fast_path(torch_module=imports.torch, device_name=device_name)
    ...
    def predict(self, request: YoloV8ObbPredictionRequest) -> YoloV8ObbPredictionExecutionResult:
        ...
        if callable(inference_mode):
            ...
            with self.imports.torch.no_grad():
                outputs = self.model(input_tensor)
        infer_ms = measure_yolov8_obb_stage_elapsed_ms(
            imports=self.imports,
            device_name=self.device_name,
...
__all__ = ["PyTorchYoloV8ObbRuntimeSession"]
