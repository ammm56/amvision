"""YOLOv8 segmentation TensorRT runtime buffer 工具。"""
...
def resolve_yolov8_segmentation_numpy_dtype(*, np_module: Any, dtype_name: str) -> Any:
    """把稳定字符串 dtype 映射为 NumPy dtype。"""

    return resolve_yolov8_detection_numpy_dtype(np_module=np_module, dtype_name=dtype_name)


def resolve_yolov8_segmentation_tensor_byte_size(
    *,
    np_module: Any,
    shape: tuple[int, ...],
    dtype: Any,
    ...
    return resolve_yolov8_detection_tensor_byte_size(
        np_module=np_module,
        shape=shape,
        dtype=dtype,
    )
...
def build_yolov8_segmentation_numpy_array_from_host_pointer(
    *,
    np_module: Any,
    host_ptr: int,
    byte_size: int,
    ...
    return build_yolov8_detection_numpy_array_from_host_pointer(
        np_module=np_module,
        host_ptr=host_ptr,
        byte_size=byte_size,
        dtype=dtype,
