"""YOLO11 segmentation TensorRT runtime buffer 工具。"""
...
def resolve_yolo11_segmentation_numpy_dtype(*, np_module: Any, dtype_name: str) -> Any:
    ...
    if resolved_dtype is None:
        raise ServiceConfigurationError(
            "当前 YOLO11 segmentation TensorRT session 发现了不支持的张量 dtype",
            details={"dtype_name": dtype_name},
        )
    return resolved_dtype


def resolve_yolo11_segmentation_tensor_byte_size(
    *,
    np_module: Any,
    shape: tuple[int, ...],
    dtype: Any,
...
def build_yolo11_segmentation_numpy_array_from_host_pointer(
    *,
    np_module: Any,
    host_ptr: int,
    byte_size: int,
