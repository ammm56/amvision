"""segmentation DeploymentInstance 公共服务。"""
...
class SegmentationDeploymentInstanceCreateRequest:
    project_id: str
    model_type: str
...
class SqlAlchemySegmentationDeploymentService(SqlAlchemyDeploymentInstanceService):
    ...
    def create_deployment_instance(
        self,
        request: SegmentationDeploymentInstanceCreateRequest,
        *,
        created_by: str | None,
    ...
    def list_deployment_instances(
        ...
        *,
        ...
        limit: int = 100,
    ) -> tuple[SegmentationDeploymentInstanceView, ...]:
        ...
        if normalized_model_type is None:
            return views
        result: list[SegmentationDeploymentInstanceView] = []
        for item in views:
...
def _require_model_type(model_type: str | None) -> str:
    return require_supported_platform_model_type(
        ...
        unsupported_message="当前 segmentation deployment 不支持指定模型分类",
    )
