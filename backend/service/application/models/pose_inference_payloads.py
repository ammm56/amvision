...
class PoseInferencePayload:
    """描述 pose 同步直返与异步结果共用的推理结果载荷。"""
...
def build_pose_prediction_request(
    *,
    normalized_input: PoseNormalizedInferenceInput,
    score_threshold: float,
    keypoint_confidence_threshold: float,
    save_result_image: bool,
    return_preview_image_base64: bool,
    ...
    """把统一输入规则转换为 pose runtime prediction request。"""
    ...
    return PosePredictionRequest(
        ...
        input_image_bytes=(
            ...
            if normalized_input.input_transport_mode == POSE_INFERENCE_INPUT_TRANSPORT_MEMORY
            else None
        ),
...
def build_pose_inference_payload(
    *,
    request_id: str,
    inference_task_id: str | None,
    deployment_instance_id: str,
    ...
    return PoseInferencePayload(
        ...
        instances=tuple(serialize_pose_instance(item) for item in execution_result.instances),
...
def serialize_pose_inference_payload(payload: PoseInferencePayload) -> dict[str, object]:
    """把 pose 推理结果载荷序列化为 JSON 字典。"""
    ...
    return {
        ...
        "keypoint_confidence_threshold": payload.keypoint_confidence_threshold,
...
def attach_pose_inference_serialize_timing(
    *,
    payload: dict[str, object],
    serialize_ms: float,
) -> dict[str, object]:
    """把 pose 响应序列化阶段耗时写回统一推理载荷。"""
