...
OBB_INFERENCE_TASK_KIND = "obb-inference"
...
ObbInferenceTaskSubmission = TaskNativeInferenceTaskSubmission
ObbInferenceTaskResult = TaskNativeInferenceTaskResult


class SqlAlchemyObbInferenceTaskService(TaskNativeInferenceTaskServiceBase):
    task_kind = OBB_INFERENCE_TASK_KIND
    queue_name = OBB_INFERENCE_QUEUE_NAME
    task_label = "obb"
    task_spec_cls = ObbInferenceTaskSpec

    def _validate_request(self, request: ObbInferenceTaskRequest) -> None:
    ...
    def _build_request_from_task_record(self, task_record) -> ObbInferenceTaskRequest:
    ...
    def _execute_task_inference(
        ...
        *,
        ...
        if self.async_inference_executor is not None:
            ...
            parsed = deserialize_obb_async_inference_execution_result_payload(payload)
    ...
    def _serialize_normalized_input(self, normalized_input) -> dict[str, object]:
        from backend.service.application.models.inference.obb_inference_payloads import (  # noqa: PLC0415
            serialize_obb_normalized_inference_input,
        )

        return serialize_obb_normalized_inference_input(normalized_input)
    ...
    def _deserialize_normalized_input(self, task_spec: dict[str, object]):
        ...
        if not isinstance(normalized_input_payload, dict):
            raise InvalidRequestError("obb inference task 缺少 normalized_input")
        return deserialize_obb_normalized_inference_input(normalized_input_payload)
