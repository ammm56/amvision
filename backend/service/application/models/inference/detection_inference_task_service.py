...
DETECTION_INFERENCE_QUEUE_NAME = "detection-inferences"
...
@dataclass(frozen=True)
class DetectionInferenceTaskSubmission:
    """描述一次 detection 推理任务提交结果。"""
...
@dataclass(frozen=True)
class DetectionInferenceExecutionResult:
    """描述一次底层 detection 推理执行结果。"""
...
@dataclass(frozen=True)
class DetectionInferenceTaskResult:
    """描述一次 detection 推理任务处理结果。"""
...
class SqlAlchemyDetectionInferenceTaskService:
    """基于 SQLAlchemy、本地队列和本地文件存储实现 detection 推理任务。"""
        ...
        *,
        ...
        created_task = self.task_service.create_task(
            CreateTaskRequest(
                ...
                or f"detection inference {request.deployment_instance_id}",
    ...
    def process_inference_task(self, task_id: str) -> DetectionInferenceTaskResult:
        ...
        try:
            ...
            raw_payload = attach_detection_inference_serialize_timing(
                payload=raw_payload,
        ...
        task_result = DetectionInferenceTaskResult(
            task_id=task_id,
            status="succeeded",
            deployment_instance_id=request.deployment_instance_id,
            instance_id=execution_result.instance_id,
        ...
        self.task_service.append_task_event(
            AppendTaskEventRequest(
                task_id=task_id,
                event_type="result",
                message="detection inference completed",
                payload={
                    "state": "succeeded",
                    "finished_at": _now_isoformat(),
                    "attempt_no": attempt_no,
    ...
    def _validate_request(self, request: DetectionInferenceTaskRequest) -> None:
        """校验推理任务请求。"""
    ...
    def _build_deployment_service(self) -> SqlAlchemyDetectionDeploymentService:
        """构建 detection DeploymentInstance 服务。"""
    ...
    def _resolve_input_uri(self, request: DetectionInferenceTaskRequest) -> str:
    ...
    def _require_inference_task(self, task_id: str) -> TaskRecord:
        ...
        if task_record.task_kind != DETECTION_INFERENCE_TASK_KIND:
            raise InvalidRequestError(
                "当前任务不是 detection 推理任务",
    ...
    def _build_existing_result(self, task_record: TaskRecord) -> DetectionInferenceTaskResult | None:
        ...
        return DetectionInferenceTaskResult(
    ...
    def _resolve_score_threshold(self, request: DetectionInferenceTaskRequest) -> float:
        """解析推理阈值。"""
    ...
    def _execute_inference(
        ...
        *,
        process_config: DeploymentProcessConfig,
        prediction_request: DetectionPredictionRequest,
    ...
    def _build_prediction_request(
        *,
        ...
        return build_detection_prediction_request(
            normalized_input=normalized_input,
    ...
    def _normalize_input_transport_mode(value: object) -> str:
        ...
        if isinstance(value, str) and value.strip():
            ...
            normalized_value = DETECTION_INFERENCE_INPUT_TRANSPORT_STORAGE
...
def run_detection_inference_task(
    *,
    ...
    返回：
    - DetectionInferenceExecutionResult：标准化后的推理结果。
    """
