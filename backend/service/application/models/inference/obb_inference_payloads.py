...
class ObbInferencePayload:
    """描述 obb 同步直返与异步结果共用的推理结果载荷。"""
...
def build_obb_prediction_request(
    *,
    normalized_input: ObbNormalizedInferenceInput,
    score_threshold: float,
    save_result_image: bool,
    return_preview_image_base64: bool,
    extra_options: dict[str, object],
) -> ObbPredictionRequest:
    """把统一输入规则转换为 obb runtime prediction request。"""
    ...
    return ObbPredictionRequest(
        ...
        input_image_bytes=(
            ...
            if normalized_input.input_transport_mode == OBB_INFERENCE_INPUT_TRANSPORT_MEMORY
            else None
        ),
...
def build_obb_inference_payload(
    *,
    request_id: str,
    inference_task_id: str | None,
    deployment_instance_id: str,
    ...
    return ObbInferencePayload(
        ...
        instances=tuple(serialize_obb_instance(item) for item in execution_result.instances),
...
def serialize_obb_inference_payload(payload: ObbInferencePayload) -> dict[str, object]:
    """把 obb 推理结果载荷序列化为 JSON 字典。"""
...
def attach_obb_inference_serialize_timing(
    *,
    payload: dict[str, object],
    serialize_ms: float,
) -> dict[str, object]:
    """把 obb 响应序列化阶段耗时写回统一推理载荷。"""
