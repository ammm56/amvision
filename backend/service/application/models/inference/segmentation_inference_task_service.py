...
from backend.service.domain.tasks.inference_task_specs import (
    SegmentationInferenceTaskSpec,
)


SEGMENTATION_INFERENCE_TASK_KIND = "segmentation-inference"
...
SegmentationInferenceTaskSubmission = TaskNativeInferenceTaskSubmission
SegmentationInferenceTaskResult = TaskNativeInferenceTaskResult


class SqlAlchemySegmentationInferenceTaskService(TaskNativeInferenceTaskServiceBase):
    """segmentation task-native 推理任务服务。"""

    task_kind = SEGMENTATION_INFERENCE_TASK_KIND
    queue_name = SEGMENTATION_INFERENCE_QUEUE_NAME
    task_label = "segmentation"
    task_spec_cls = SegmentationInferenceTaskSpec

    def _validate_request(self, request: SegmentationInferenceTaskRequest) -> None:
        ...
        if float(request.mask_threshold) < 0 or float(request.mask_threshold) > 1:
            raise InvalidRequestError(
                "mask_threshold 必须位于 0 到 1 之间",
                details={"mask_threshold": request.mask_threshold},
            )
    ...
    def _validate_resolved_model_type(self, *, deployment_instance_id: str) -> None:
        ...
        registration = get_segmentation_backend_registration(
            process_config.runtime_target.model_type
        )
    ...
    def _build_task_spec(
        ...
        *,
        ...
        return SegmentationInferenceTaskSpec(
    ...
    def _serialize_task_spec(
        ...
        return {
            ...
            "mask_threshold": task_spec.mask_threshold,
    ...
    def _execute_task_inference(
        ...
        *,
        ...
        if self.async_inference_executor is not None:
            ...
            parsed = deserialize_segmentation_async_inference_execution_result_payload(
                payload
            )
    ...
    def _serialize_normalized_input(self, normalized_input) -> dict[str, object]:
        from backend.service.application.models.inference.segmentation_inference_payloads import (  # noqa: PLC0415
            serialize_segmentation_normalized_inference_input,
        )

        return serialize_segmentation_normalized_inference_input(normalized_input)
    ...
    def _deserialize_normalized_input(self, task_spec: dict[str, object]):
        ...
        if not isinstance(normalized_input_payload, dict):
            raise InvalidRequestError(
                "segmentation inference task 缺少 normalized_input"
            )
        return deserialize_segmentation_normalized_inference_input(
