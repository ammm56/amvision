...
def _normalize_model_type(model_type: str | None) -> str:
    ...
    if normalized not in _SUPPORTED_OBB_MODEL_TYPES:
        raise InvalidRequestError(
            "当前 obb validation session 不支持指定模型分类",
            details={"model_type": normalized, "supported_model_types": list(_SUPPORTED_OBB_MODEL_TYPES)},
        )
    return normalized
...
def _normalize_runtime_backend(runtime_backend: str | None) -> str:
    ...
    if normalized != _VALIDATION_RUNTIME_BACKEND:
        raise InvalidRequestError(
            "当前 obb validation session 仅支持 pytorch runtime_backend",
