...
_YOLO_EVALUATION_RUNTIME_TARGET_RESOLVER_BY_MODEL_TYPE: dict[str, type] = {
...
_SEGMENTATION_EVALUATION_RUNTIME_TARGET_RESOLVER_BY_MODEL_TYPE: dict[str, type] = {
    **_YOLO_EVALUATION_RUNTIME_TARGET_RESOLVER_BY_MODEL_TYPE,
...
def get_yolo_evaluation_runtime_target_resolver(model_type: str) -> type:
    """按 model_type 返回 YOLO 主线评估使用的 resolver 类。"""

    normalized_model_type = require_platform_model_type(model_type)
    resolver_cls = _YOLO_EVALUATION_RUNTIME_TARGET_RESOLVER_BY_MODEL_TYPE.get(
    ...
    if resolver_cls is None:
        raise InvalidRequestError(
            ...
            details={
                ...
                "supported": sorted(
                    _YOLO_EVALUATION_RUNTIME_TARGET_RESOLVER_BY_MODEL_TYPE.keys()
                ),
            },
        )
    return resolver_cls
