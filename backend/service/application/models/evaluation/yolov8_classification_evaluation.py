...
def run_yolov8_classification_evaluation(
    ...
    split_name, samples, label_names = _parse_classification_manifest(
    ...
    try:
        return _run_classification_evaluation_with_session(
            dataset_storage=dataset_storage,
            runtime_target=runtime_target,
            split_name=split_name,
            samples=samples,
...
def _run_classification_evaluation_with_session(
    *,
    dataset_storage: LocalDatasetStorage,
    runtime_target: RuntimeTargetSnapshot,
    split_name: str,
    ...
    """在受控 runtime session 内计算 classification 指标。"""
        ...
        predictions.append(
            {
                ...
                "ground_truth_class_name": label_names[gt_class_id]
                if gt_class_id < len(label_names)
                else None,
                "predicted_top_k": [
                    {
                        "class_id": c.class_id,
    ...
    all_class_ids = sorted(
        set(list(per_class_total.keys()) + list(range(len(label_names))))
    )
...
def _parse_classification_manifest(
    ...
    for ann in annotations:
        ...
        class_id = ann["class_id"] if "class_id" in ann else ann.get("category_id")
        if class_id is None:
            continue
        full_path = (
            f"{image_root}/{file_name}" if image_root and file_name else str(file_name)
        )
