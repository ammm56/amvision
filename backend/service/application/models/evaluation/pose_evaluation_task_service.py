...
class SqlAlchemyPoseEvaluationTaskService:
        ...
        *,
        ...
        created_task = self.task_service.create_task(
            CreateTaskRequest(
                ...
                or f"pose evaluation {dataset_export.dataset_export_id}",
    ...
    def process_evaluation_task(
        ...
        *,
        ...
        started_event = AppendTaskEventRequest(
                ...
                message="pose evaluation started",
                payload={
        ...
        self.task_service.execute_task_state_event_command(
            AppendTaskEventRequest(
                task_id=task_id,
                event_type="result",
                message="pose evaluation completed",
                payload={
                    "state": "succeeded",
                    "finished_at": datetime.now(timezone.utc).isoformat(),
                    "attempt_no": attempt_no,
    ...
    def _resolve_runtime_target(
        self, request: PoseEvaluationTaskRequest
    ) -> RuntimeTargetSnapshot:
        uow = SqlAlchemyUnitOfWork(self.session_factory.create_session())
        try:
...
def _run_pose_evaluation_for_runtime_target(
    *,
    ...
    if runtime_target.model_type == "yolo11":
        return run_yolo11_pose_evaluation(
            Yolo11PoseEvaluationRequest(
                dataset_storage=dataset_storage,
                runtime_target=runtime_target,
                manifest_payload=manifest,
