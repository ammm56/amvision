"""YOLO segmentation 共用 target 编码。"""
...
def select_object_segmentation_polygons(
    ...
    *,
    ...
    参数：
    - segmentations：COCO segmentation 数据。
    - object_index：当前目标序号。
    - object_count：当前样本内目标数量。

    返回：
    - 单个目标的 polygon 列表。没有合法 polygon 时返回 ``None``。
    """

    if not isinstance(segmentations, list) or len(segmentations) == 0:
        return None
    if object_count == 1:
        return _normalize_polygon_group(segmentations)
    if object_index >= len(segmentations):
        return None
    return _normalize_polygon_group(segmentations[object_index])
...
def rasterize_segmentation_polygons(
    *,
    ...
    - np_module：NumPy 模块。
    - polygons：单个目标的 polygon 列表，坐标为原图像素。
    - output_size：输出 mask 尺寸，格式为 ``(width, height)``。
    - resize_scale：letterbox 缩放比例。
    - pad_xy：letterbox 左上角 padding，格式为 ``(pad_x, pad_y)``。

    返回：
    - ``(mask, valid)``。mask 为 ``uint8``，valid 表示是否有合法 polygon。
    """
