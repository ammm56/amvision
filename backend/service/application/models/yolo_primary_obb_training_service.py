...
from backend.service.application.models.training.yolo_primary_obb_task_control import (
    YoloPrimaryObbTrainingControlState,
    build_yolo_primary_obb_training_control_metadata,
    clear_yolo_primary_obb_manual_save_request,
    read_yolo_primary_obb_training_control_state,
)
from backend.service.application.models.training.yolo_primary_obb_task_dataset import (
    resolve_yolo_primary_obb_training_dataset_export,
)
from backend.service.application.models.training.yolo_primary_obb_task_events import (
    build_yolo_primary_obb_training_cancelled_event,
    build_yolo_primary_obb_training_failed_event,
    build_yolo_primary_obb_training_paused_event,
    build_yolo_primary_obb_training_started_event,
    build_yolo_primary_obb_training_succeeded_event,
)
from backend.service.application.models.training.yolo_primary_obb_task_payload import (
    build_yolo_primary_obb_create_task_metadata,
    build_yolo_primary_obb_queue_payload,
    build_yolo_primary_obb_task_spec,
    read_yolo_primary_obb_task_payload,
)
...
class SqlAlchemyYoloPrimaryObbTrainingTaskService:
    ...
    model_label = "YOLO primary obb"
    training_task_kind = OBB_TRAINING_TASK_KIND
    training_queue_name = OBB_TRAINING_QUEUE_NAME
    ...
    def process_training_task(
        ...
        *,
        ...
        def on_epoch(
            progress: YoloPrimaryObbTrainingEpochProgress,
        ) -> YoloPrimaryObbTrainingControlCommand | None:
            control_state = self._read_control_state(task_record.task_id)
            if control_state.terminate_requested:
                return YoloPrimaryObbTrainingControlCommand(
            ...
            if control_state.save_requested:
                ...
                return YoloPrimaryObbTrainingControlCommand(save_checkpoint=True)
            return None

        def on_savepoint(savepoint: YoloPrimaryObbTrainingSavePoint) -> None:
        ...
        request = YoloPrimaryObbTrainingExecutionRequest(
            ...
            evaluation_interval=int(
                ...
                or YOLO_PRIMARY_OBB_DEFAULT_EVALUATION_INTERVAL
            ),
        ...
        try:
            ...
            self.task_service.append_task_event(
                build_yolo_primary_obb_training_cancelled_event(
                    ...
                    control_metadata_key=OBB_TRAINING_CONTROL_METADATA_KEY,
                )
            )
    ...
    def _resolve_dataset_export(
        ...
        *,
        ...
        return resolve_yolo_primary_obb_training_dataset_export(
    ...
    def _read_control_state(self, task_id: str) -> YoloPrimaryObbTrainingControlState:
        """从任务 metadata 中读取最新控制状态。"""

        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        return read_yolo_primary_obb_training_control_state(
            metadata=metadata,
            control_metadata_key=OBB_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _clear_manual_save_request(self, task_id: str) -> None:
        ...
        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        updated_metadata = clear_yolo_primary_obb_manual_save_request(
            metadata=metadata,
            control_metadata_key=OBB_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _set_control_flag(
        ...
        updated_metadata = build_yolo_primary_obb_training_control_metadata(
            metadata=metadata,
            control_metadata_key=OBB_TRAINING_CONTROL_METADATA_KEY,
            flag=flag,
            value=value,
