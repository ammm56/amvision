...
_OBB_MODEL_SERVICE_MAP: dict[str, tuple[type, type]] = {
...
@dataclass(frozen=True)
class _ObbTrainingControlState:
    """描述 OBB 训练控制状态快照。"""
...
class SqlAlchemyYoloPrimaryObbTrainingTaskService:
            ...
            self.task_service.append_task_event(
                AppendTaskEventRequest(
                    ...
                    payload={
                        ...
                        "metadata": {OBB_TRAINING_CONTROL_METADATA_KEY: {}},
    ...
    def _normalize_model_type(self, model_type: object) -> str:
        ...
        if normalized not in _OBB_MODEL_SERVICE_MAP:
            raise InvalidRequestError(
                "当前 obb 训练不支持指定模型分类",
                details={
                    "model_type": normalized,
                    "supported": tuple(_OBB_MODEL_SERVICE_MAP.keys()),
                },
            )
        return normalized
    ...
    def _resolve_dataset_export(
        ...
        *,
        ...
        if dataset_export is None:
            raise ResourceNotFoundError("找不到可用于 obb 训练的 DatasetExport")
        if (
        ...
        if dataset_export.task_type != OBB_TASK_TYPE:
            raise InvalidRequestError(
                "当前 DatasetExport 不是 obb 导出",
                details={
                    "dataset_export_id": dataset_export.dataset_export_id,
        ...
        require_supported_dataset_export_format(
            ...
            format_id=dataset_export.format_id,
            dataset_export_id=dataset_export.dataset_export_id,
            unsupported_message="当前 obb 训练只接受当前模型支持的 obb 导出格式",
        )
    ...
    def _register_training_output_model_version(
        ...
        *,
        ...
        service_cls, registration_cls = _OBB_MODEL_SERVICE_MAP[model_type]
    ...
    def _read_control_state(self, task_id: str) -> _ObbTrainingControlState:
        """从任务 metadata 中读取最新控制状态。"""

        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        raw_control = metadata.get(OBB_TRAINING_CONTROL_METADATA_KEY)
        if not isinstance(raw_control, dict):
            return _ObbTrainingControlState()
        return _ObbTrainingControlState(
    ...
    def _clear_manual_save_request(self, task_id: str) -> None:
        ...
        metadata[OBB_TRAINING_CONTROL_METADATA_KEY] = updated_control
