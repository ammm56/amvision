...
def run_yolox_detection_evaluation(
    ...
    """执行一次 YOLOX PyTorch 数据集级评估。

    该函数负责 DatasetExport split 选择、PyTorch 模型加载、DataLoader 构建和
    COCO evaluator 执行。应用层负责创建任务、传入 manifest 和登记结果文件。
    """
    ...
    resolved_splits = resolve_yolox_detection_splits(request.dataset_storage, manifest_payload)
    evaluation_split = resolve_yolox_evaluation_split(
    ...
    device_name = resolve_yolox_evaluation_device_name(
    ...
    precision = resolve_yolox_evaluation_precision(
        imports=imports,
    ...
    detections = collect_yolox_coco_detections(
        ...
        autocast_context_factory=build_yolox_autocast_context,
        model=model,
        loader=data_loader,
        device=device_name,
        precision=precision,
    ...
    report_payload = {
        ...
        "map50": evaluation_metrics.map50,
        "map50_95": evaluation_metrics.map50_95,
        "per_class_metrics": [dict(item) for item in evaluation_metrics.per_class_metrics],
    }
    detections_payload = {
        "model_version_id": request.runtime_target.model_version_id,
        "dataset_export_id": request.dataset_export_id,
    ...
    return YoloXDetectionEvaluationResult(
        ...
        map50=evaluation_metrics.map50,
        map50_95=evaluation_metrics.map50_95,
        per_class_metrics=tuple(dict(item) for item in evaluation_metrics.per_class_metrics),
...
def resolve_yolox_evaluation_split(
    *,
    resolved_splits: tuple[YoloXDetectionSplit, ...],
...
def resolve_yolox_evaluation_precision(
    *,
    imports: Any,
