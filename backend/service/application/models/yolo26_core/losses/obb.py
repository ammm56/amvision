...
def compute_yolo26_obb_loss(
    *,
    ...
    """计算 YOLO26 OBB 的分类、旋转框、DFL 和角度损失。"""
    ...
    pred_angles = _normalize_yolo26_obb_angle_tensor(
        torch_module=torch,
    ...
    pred_rboxes_grid = yolo26_decode_distances_to_rboxes(
        torch_module=torch,
    ...
    for batch_index in range(batch_size):
        loss_state = _compute_yolo26_obb_image_loss(
            torch_module=torch,
...
def _compute_yolo26_obb_image_loss(
    *,
    torch_module: Any,
    ...
    foreground_gt_rboxes_grid = yolo26_scale_rbox_to_grid(
        rboxes=foreground_gt_rboxes_pixel,
        stride_tensor=foreground_stride,
    )
    iou_values = yolo26_probiou_aligned(
        torch_module=torch_module,
...
def _compute_yolo26_obb_dfl_loss(
    *,
    torch_module: Any,
    distance_logits: Any,
    foreground_mask: Any,
    ...
    """计算 YOLO26 OBB DFL 分量。"""

    target_distances = yolo26_rbox_to_distances(
        torch_module=torch_module,
        rboxes=foreground_gt_rboxes_pixel,
...
def _normalize_yolo26_obb_angle_tensor(
    *,
    ...
    """把 YOLO26 OBB angle 输出规整为 (bs, anchors, 1)。"""
    ...
    raise ValueError(f"YOLO26 OBB angle 输出 shape 不合法: {tuple(pred_angles.shape)}")
