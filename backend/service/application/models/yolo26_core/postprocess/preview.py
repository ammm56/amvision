...
def render_yolo26_detection_preview_image(
    *,
    cv2_module: Any,
    image: Any,
    instances: tuple[Any, ...],
) -> bytes:
    """把 YOLO26 实例结果叠加到原图并编码为 JPEG。"""

    preview = image.copy()
    for instance in instances:
        x1, y1, x2, y2 = (int(round(value)) for value in instance.bbox_xyxy)
        color = _select_yolo26_preview_color(int(instance.class_id))
    ...
    if success is not True:
        raise InvalidRequestError("YOLO26 预测预览图编码失败")
    return bytes(encoded.tobytes())


def _select_yolo26_preview_color(class_id: int) -> tuple[int, int, int]:
    """根据类别 id 返回稳定的 YOLO26 预览颜色。"""
...
__all__ = ["render_yolo26_detection_preview_image"]
