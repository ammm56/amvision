...
class Yolo26PoseTrainingBatch:
    ...
    def pin_memory(self) -> "Yolo26PoseTrainingBatch":
...
def build_yolo26_pose_training_batch(
    *,
    ...
    if not training and augmentation_options is not None:
        raise ValueError("YOLO26 pose validation 不允许随机增强")
    if not samples:
        return None
...
def _build_yolo26_pose_mosaic_sample(
    *,
    ...
    for placement in placements:
        ...
        placed_target = _build_yolo26_pose_sample_targets(
            sample=sample,
        ...
        placed_target = _filter_yolo26_pose_target(placed_target)
        merged_target = (
            placed_target
            if merged_target is None
            else _merge_yolo26_pose_targets(primary=merged_target, other=placed_target)
        )
    if merged_target is None:
        return None
...
def _build_yolo26_pose_sample_targets(
    *,
    ...
    for object_index, (bbox, class_id) in enumerate(
        ...
        if x2 <= x1 or y2 <= y1:
            continue
        if not preserve_evaluation_geometry and (x2 - x1 < 2.0 or y2 - y1 < 2.0):
            continue
