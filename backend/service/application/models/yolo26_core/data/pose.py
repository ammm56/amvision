...
class Yolo26PosePreparedTarget:
    """描述单张图的 YOLO26 pose 训练目标。"""
...
@dataclass(frozen=True)
class Yolo26PoseTrainingBatch:
    """描述 YOLO26 pose 训练或评估使用的 batch。"""

    images: Any
    targets: tuple[Yolo26PosePreparedTarget, ...]


def build_yolo26_pose_training_batch(
    *,
...
) -> Yolo26PoseTrainingBatch | None:
    """把样本列表编码为 YOLO26 pose 训练 batch。"""
    ...
    images: list[Any] = []
    targets: list[Yolo26PosePreparedTarget] = []
    resolved_available_samples = tuple(available_samples or samples)
    for sample in samples:
        prepared = _prepare_yolo26_pose_sample_with_mix(
            imports=imports,
        ...
        canvas, target = _apply_yolo26_pose_augmentation(
            image=canvas,
            target=target,
            target_width=target_width,
            target_height=target_height,
    ...
    if not images:
        return None
    return Yolo26PoseTrainingBatch(
        images=imports.torch.stack(images, dim=0),
        targets=tuple(targets),
    )


def _prepare_yolo26_pose_sample_with_mix(
    *,
    imports: Any,
    primary_sample: Any,
...
) -> tuple[Any, Yolo26PosePreparedTarget] | None:
    """构造可能包含 mosaic / mixup 的 YOLO26 pose 样本。"""
    ...
    mixup_prepared = _prepare_yolo26_pose_mixup_sample(
        imports=imports,
        sample=mixup_sample,
        available_samples=available_samples,
        target_width=target_width,
    ...
    return (
        ...
        _merge_yolo26_pose_targets(primary=target, other=mixup_target),
    )


def _prepare_yolo26_pose_mixup_sample(
    *,
    imports: Any,
    sample: Any,
    available_samples: Sequence[Any],
...
def _build_yolo26_pose_mosaic_sample(
    *,
    imports: Any,
    primary_sample: Any,
    available_samples: Sequence[Any],
    ...
    merged_target: Yolo26PosePreparedTarget | None = None
    ...
    for sample, (left, top, cell_width, cell_height) in zip(
        ...
        shifted_target = _offset_yolo26_pose_target(
            target=cell_target, offset_xy=(left, top)
        )
        merged_target = (
            shifted_target
            if merged_target is None
            else _merge_yolo26_pose_targets(primary=merged_target, other=shifted_target)
        )
    if merged_target is None:
        return None
...
def _prepare_yolo26_pose_single_sample(
    *,
    imports: Any,
    sample: Any,
    ...
    return canvas, _build_yolo26_pose_sample_targets(
        sample=sample,
...
def _build_yolo26_pose_sample_targets(
    *,
    sample: Any,
    resize_ratio: float,
    pad_xy: tuple[int, int],
) -> Yolo26PosePreparedTarget:
    """构造单张图的 YOLO26 pose target。"""
    ...
    for object_index, (bbox, class_id) in enumerate(
        ...
        keypoints.append(
            _transform_yolo26_pose_keypoints(
                sample=sample,
    ...
    return Yolo26PosePreparedTarget(
...
def _offset_yolo26_pose_target(
    *,
    target: Yolo26PosePreparedTarget,
    offset_xy: tuple[int, int],
) -> Yolo26PosePreparedTarget:
    """把 cell 内 YOLO26 pose target 平移到 mosaic 画布。"""
    ...
    keypoints = (
        [
            _offset_yolo26_pose_keypoints(
                keypoints=keypoint_row, offset_xy=(offset_x, offset_y)
            )
            for keypoint_row in target.keypoints
        ]
        if target.keypoints
        else None
    )
    return _filter_yolo26_pose_target(
        Yolo26PosePreparedTarget(
            boxes_xyxy=boxes,
...
def _merge_yolo26_pose_targets(
    *,
    primary: Yolo26PosePreparedTarget,
    other: Yolo26PosePreparedTarget,
) -> Yolo26PosePreparedTarget:
    """合并两组 YOLO26 pose targets。"""
...
def _filter_yolo26_pose_target(
    target: Yolo26PosePreparedTarget,
) -> Yolo26PosePreparedTarget:
    """过滤退化的 YOLO26 pose boxes，并同步 keypoints。"""
    ...
    return Yolo26PosePreparedTarget(
        ...
        keypoints=(
            select_yolo26_items_by_indices(list(target.keypoints), kept_indices)
...
def _transform_yolo26_pose_keypoints(
    *,
    sample: Any,
    object_index: int,
    resize_ratio: float,
...
def _offset_yolo26_pose_keypoints(
    *, keypoints: list[float], offset_xy: tuple[float, float]
) -> list[float]:
    """平移一组 YOLO26 pose keypoints。"""
...
def _apply_yolo26_pose_augmentation(
    *,
    image: Any,
    target: Yolo26PosePreparedTarget,
    target_width: int,
    ...
    imports: Any,
    augmentation_options: Yolo26TaskAugmentationOptions | None,
) -> tuple[Any, Yolo26PosePreparedTarget]:
    """对 YOLO26 pose 图像和 keypoint target 执行受控增强。"""
...
def _apply_yolo26_pose_random_affine(
    *,
    imports: Any,
    image: Any,
    target: Yolo26PosePreparedTarget,
    ...
    """对 YOLO26 pose bbox 和 keypoints 同步执行 random affine。"""
...
def _flip_yolo26_pose_keypoints(
    *,
    keypoints: list[float],
    target_width: int,
    flip_indices: tuple[int, ...],
) -> list[float]:
    """水平翻转并按 flip index 交换 YOLO26 pose keypoints。"""
