"""YOLO26 detection DatasetExport split 解析。"""
...
def resolve_yolo26_detection_splits(
    *,
    dataset_storage: Any,
    cv2_module: Any,
    manifest_payload: dict[str, object],
) -> tuple[Yolo26DetectionResolvedSplit, ...]:
    """从 DatasetExport manifest 解析 YOLO26 detection 可用 split。"""
...
def resolve_yolo26_detection_train_split(
    resolved_splits: tuple[Yolo26DetectionResolvedSplit, ...],
) -> Yolo26DetectionResolvedSplit:
    """优先选择 YOLO26 detection train split。"""
...
def resolve_yolo26_detection_validation_split(
    resolved_splits: tuple[Yolo26DetectionResolvedSplit, ...],
...
def _resolve_coco_detection_splits(
    *,
    ...
    if not isinstance(splits_payload, list):
        raise InvalidRequestError("YOLO26 detection 训练输入 manifest 缺少 splits 定义")
    resolved_splits: list[Yolo26DetectionResolvedSplit] = []
    for split_item in splits_payload:
        ...
        if not annotation_path.is_file():
            raise InvalidRequestError(
                "YOLO26 detection 训练输入 split 缺少 annotation 文件",
                details={
        ...
        if not image_root_path.is_dir():
            raise InvalidRequestError(
                "YOLO26 detection 训练输入 split 缺少图片目录",
    ...
    if not resolved_splits:
        raise InvalidRequestError("YOLO26 detection 训练输入 manifest 没有可用的 split")
    ...
    *,
    ...
    for split_item in splits_payload:
        ...
        if not category_names:
            raise InvalidRequestError(
                "YOLO26 detection 训练输入缺少有效的 category_names"
            )
        ...
        if not label_root_path.is_dir():
            raise InvalidRequestError(
                "YOLO26 detection 训练输入 split 缺少标签目录",
