...
class Yolo26TaskAugmentationOptions:
    """描述 YOLO26 非 detection 任务使用的受控增强参数。"""
...
def build_yolo26_task_augmentation_options(
    extra_options: dict[str, object] | None,
) -> Yolo26TaskAugmentationOptions:
    """从训练 extra_options 构造 YOLO26 task 增强参数。"""
    ...
    if augmentation_disabled:
        return Yolo26TaskAugmentationOptions(
            ...
            multi_scale=False,
            keypoint_flip_indices=_parse_keypoint_flip_indices(
                extra.get("keypoint_flip_indices")
            ),
        )
    ...
    return Yolo26TaskAugmentationOptions(
        ...
        flip_prob=_clamp_probability(
            _read_float_option(
                extra,
                "flip_prob",
                default=_read_float_option(extra, "fliplr", default=0.5),
            )
        ),
        mosaic_prob=_clamp_probability(
            _read_float_option(
                extra,
                "mosaic_prob",
                default=_read_float_option(extra, "mosaic", default=1.0),
            )
        ),
        ...
        multi_scale_stride=max(
            1, int(_read_float_option(extra, "multi_scale_stride", default=32.0))
        ),
        keypoint_flip_indices=flip_indices,
    )


def resolve_yolo26_task_augmentation_for_epoch(
    *,
    augmentation_options: Yolo26TaskAugmentationOptions | None,
    epoch_index: int,
    max_epochs: int,
) -> Yolo26TaskAugmentationOptions | None:
    """按当前 epoch 解析实际生效的 YOLO26 task 增强参数。"""
...
def resolve_yolo26_task_batch_input_size(
    *,
...
def resize_yolo26_image_to_canvas(
    *,
    imports: Any,
    image: Any,
...
def blend_yolo26_mixup_images(*, imports: Any, image: Any, other_image: Any) -> Any:
    """按 YOLO26 MixUp 权重混合两张同尺寸图片。"""
...
def apply_yolo26_random_hsv(*, imports: Any, image: Any, hsv_prob: float) -> Any:
    """按概率执行 YOLO26 task HSV 抖动。"""
    ...
    return imports.cv2.cvtColor(
        hsv_image.astype(imports.np.uint8), imports.cv2.COLOR_HSV2BGR
    )


def should_apply_yolo26_horizontal_flip(flip_prob: float) -> bool:
    """判断当前样本是否执行水平翻转。"""

    return flip_prob > 0.0 and random.random() < flip_prob


def flip_yolo26_image_horizontally(image: Any) -> Any:
    """水平翻转训练图像。"""

    return image[:, ::-1].copy()


def apply_yolo26_random_affine(
    *,
    imports: Any,
    image: Any,
    output_size: tuple[int, int],
    ...
    """按 YOLO26 affine 参数变换图像，并返回同步标注用的矩阵。
    ...
    apply_random_transform = should_apply_yolo26_random_affine(augmentation_options)
    if not apply_random_transform and not needs_canvas_transform:
        return image, None, False
    matrix = (
        build_yolo26_random_affine_matrix(
            imports=imports,
...
def should_apply_yolo26_random_affine(
    augmentation_options: Yolo26TaskAugmentationOptions,
) -> bool:
    """判断当前样本是否执行 YOLO26 random affine。"""
...
def build_yolo26_random_affine_matrix(
    *,
    imports: Any,
    ...
    """构造 YOLO26 random affine / perspective 使用的 3x3 矩阵。"""
    ...
    angle = (
        ...
        if augmentation_options.degrees > 0.0
        else 0.0
    )
    scale = (
        random.uniform(
        ...
        if augmentation_options.scale > 0.0
        else 1.0
    )
    ...
    translate_matrix[0, 2] = random.uniform(0.5 - translate, 0.5 + translate) * float(
        output_width
    )
    translate_matrix[1, 2] = random.uniform(0.5 - translate, 0.5 + translate) * float(
        output_height
    )
    return (
        translate_matrix
...
def transform_yolo26_boxes_xyxy(
    *,
    imports: Any,
    ...
    for box_index, box in enumerate(boxes_xyxy):
        ...
        transformed = _transform_yolo26_points(
            imports=imports,
            points=corners,
            matrix=matrix,
            perspective=perspective,
        )
        transformed_box = _clip_yolo26_box_xyxy(
            box_xyxy=[
        ...
        if transformed_box is None or not _is_yolo26_box_candidate(
            original_box=box,
...
def build_yolo26_boxes_from_masks(
    *,
    imports: Any,
    masks: Any,
...
def transform_yolo26_keypoints(
    *,
    imports: Any,
    ...
    for object_keypoints in keypoints:
        ...
        for keypoint_index in range(keypoint_count):
            ...
            if (
                x_value < 0.0
                or y_value < 0.0
                or x_value > output_width
                or y_value > output_height
            ):
                visibility = 0.0
...
def transform_yolo26_obb_boxes(
    *,
    imports: Any,
    ...
    for box_index, box in enumerate(boxes_xywhr):
        corners = build_yolo26_obb_corners(imports=imports, box_xywhr=box)
        original_box = _points_to_yolo26_xyxy_bbox(
            imports=imports,
            points=corners[:, :2],
            output_size=None,
        )
        if original_box is None:
            continue
        transformed_corners = _transform_yolo26_points(
            imports=imports,
            points=corners,
            matrix=matrix,
            perspective=perspective,
        )
        transformed_box = _points_to_yolo26_xyxy_bbox(
            imports=imports,
        ...
        if transformed_box is None or not _is_yolo26_box_candidate(
            ...
            continue
        transformed_rbox = _min_area_rect_to_yolo26_xywhr(
            imports=imports,
            points=transformed_corners[:, :2],
            output_size=(output_width, output_height),
        )
...
def build_yolo26_obb_corners(*, imports: Any, box_xywhr: list[float]) -> Any:
    ...
    height_vector = imports.np.array(
        [-sin_value * half_height, cos_value * half_height]
    )
...
def resolve_yolo26_pose_flip_indices(
    *,
    keypoint_count: int,
    keypoint_flip_indices: tuple[int, ...] | None,
) -> tuple[int, ...] | None:
    """解析 YOLO26 pose 水平翻转使用的 keypoint 左右交换索引。"""
...
def select_yolo26_items_by_indices(items: list[Any], indices: list[int]) -> list[Any]:
    """按保留索引同步筛选标注列表。"""

    return [items[index] for index in indices if index < len(items)]


def normalize_yolo26_obb_angle(angle: float) -> float:
...
def _points_to_yolo26_xyxy_bbox(
    *,
    imports: Any,
    points: Any,
...
def _min_area_rect_to_yolo26_xywhr(
    *,
    imports: Any,
    points: Any,
    output_size: tuple[int, int],
) -> list[float] | None:
    """把变换后的四角点转换为 YOLO26 OBB ``xywhr``。"""
    ...
    return [
        ...
        float(width),
        float(height),
        normalize_yolo26_obb_angle(math.radians(float(angle_degrees))),
    ]
...
__all__ = [
    ...
    "should_apply_yolo26_random_affine",
    "transform_yolo26_boxes_xyxy",
    "transform_yolo26_keypoints",
