...
def load_yolo26_detection_training_samples(
    *,
    split: Yolo26DetectionResolvedSplit,
) -> tuple[
    tuple[Yolo26DetectionTrainingSample, ...],
    tuple[str, ...],
    tuple[int, ...],
]:
    """把 YOLO26 detection split 转成训练阶段可直接消费的样本列表。"""
    ...
    if not isinstance(categories_payload, list) or not isinstance(images_payload, list):
        raise InvalidRequestError(
            "YOLO26 detection annotation 结构不合法",
            details={
    ...
    if not category_names:
        raise InvalidRequestError("YOLO26 detection 训练输入缺少有效的 categories")
    ...
    resolved_samples: list[Yolo26DetectionTrainingSample] = []
    for image_id, image_meta in image_meta_by_id.items():
        ...
        annotations = tuple(
            ...
            if isinstance(item, Yolo26DetectionTrainingAnnotation)
        )
...
__all__ = ["load_yolo26_detection_training_samples"]
