...
def decode_yolo26_detection_boxes_xywh(
    *,
    raw_outputs: dict[str, torch.Tensor],
    strides: tuple[int, ...],
    dfl_decoder: nn.Module,
) -> torch.Tensor:
    """把 YOLO26 detection head 原始 box 输出解码成 xywh 边界框。"""
...
def decode_yolo26_detection_boxes_xyxy(
    *,
    raw_outputs: dict[str, torch.Tensor],
    strides: tuple[int, ...],
    dfl_decoder: nn.Module,
) -> torch.Tensor:
    """把 YOLO26 detection head 原始 box 输出解码成 xyxy 边界框。"""
...
def build_yolo26_detection_prediction(
    *,
    raw_outputs: dict[str, torch.Tensor],
    strides: tuple[int, ...],
    dfl_decoder: nn.Module,
...
def decode_yolo26_detection_training_predictions(
    *,
    torch_module: Any,
    detect_head: Any,
    raw_outputs: dict[str, Any],
) -> dict[str, Any]:
    """把 YOLO26 detection 训练态 raw outputs 解码为 loss 输入。"""
