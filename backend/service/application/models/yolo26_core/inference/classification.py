"""YOLO26 classification inference 输出适配。"""
...
def normalize_yolo26_classification_inference_outputs(
    *,
    outputs: object,
    np_module: Any,
) -> tuple[Any, Any | None]:
    """归一化 YOLO26 classification inference 输出。"""

    if isinstance(outputs, list | tuple):
        if len(outputs) >= 2:
            probabilities = ensure_yolo26_probability_array(
        ...
        if len(outputs) == 1:
            ...
            raise InvalidRequestError("YOLO26 classification inference 输出为空")
...
def build_yolo26_classification_inference_categories(
    *,
    np_module: Any,
    probabilities: Any,
    logits: Any | None,
    ...
    """把 YOLO26 classification inference 输出转换为平台 category 结果。"""

    return build_yolo26_classification_categories(
        np_module=np_module,
...
def _to_numpy_2d_array(value: object, *, np_module: Any) -> Any:
    ...
    if array.ndim != 2:
        raise InvalidRequestError(
            "YOLO26 classification inference 输出维度不合法",
