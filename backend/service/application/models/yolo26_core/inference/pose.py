...
def normalize_yolo26_pose_inference_outputs(*, outputs: object, np_module: Any) -> Any:
    """归一化 YOLO26 pose inference 输出。"""

    if isinstance(outputs, list | tuple):
        if not outputs:
            raise InvalidRequestError("YOLO26 pose inference 输出为空")
        outputs = outputs[0]
    normalized = outputs
    ...
    if prediction.ndim < 3:
        raise InvalidRequestError(
            "YOLO26 pose inference 输出维度不合法",
            details={"shape": list(prediction.shape)},
        )
    return prediction


def build_yolo26_pose_inference_instances(
    *,
    np_module: Any,
    ...
    return build_yolo26_pose_postprocess_instances(
        np_module=np_module,
