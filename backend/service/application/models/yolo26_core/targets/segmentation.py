"""YOLO26 segmentation mask target 编码。"""
...
def select_yolo26_object_segmentation_polygons(
    segmentations: Any,
    *,
    object_index: int,
    object_count: int,
    ...
    if not isinstance(segmentations, list) or len(segmentations) == 0:
        return None
    if object_count == 1:
        return _normalize_yolo26_polygon_group(segmentations)
    if object_index >= len(segmentations):
        return None
    return _normalize_yolo26_polygon_group(segmentations[object_index])


def rasterize_yolo26_segmentation_polygons(
    *,
    cv2_module: Any,
    np_module: Any,
    ...
    mask = np_module.zeros(
        (int(output_height), int(output_width)), dtype=np_module.uint8
    )
    if not polygons:
        return mask, False
...
def _normalize_yolo26_polygon_group(value: Any) -> list[list[float]] | None:
