"""YOLO26 detection resume checkpoint 校验规则。"""
...
class Yolo26DetectionResumeValidationRequest:
    """描述 YOLO26 detection 恢复训练 checkpoint 的期望配置。"""
...
def validate_yolo26_detection_resume_checkpoint(
    *,
    checkpoint_payload: dict[str, object],
    request: Yolo26DetectionResumeValidationRequest,
) -> None:
    """校验 YOLO26 detection resume checkpoint 是否匹配当前训练请求。"""

    _validate_yolo26_resume_model_identity(
        checkpoint_payload=checkpoint_payload,
        request=request,
    )
    _validate_yolo26_resume_input_config(
        checkpoint_payload=checkpoint_payload,
        request=request,
    )
    _validate_yolo26_resume_validation_config(
        checkpoint_payload=checkpoint_payload,
        request=request,
    )
    _validate_yolo26_resume_optimization_config(
        checkpoint_payload=checkpoint_payload,
        request=request,
    )


def _validate_yolo26_resume_model_identity(
    *,
    checkpoint_payload: dict[str, object],
    request: Yolo26DetectionResumeValidationRequest,
) -> None:
    ...
    if checkpoint_model_type != request.model_type:
        raise InvalidRequestError(
            "YOLO26 resume checkpoint 的 model_type 与当前训练请求不一致",
            details={
    ...
    if checkpoint_model_scale != request.model_scale:
        raise InvalidRequestError(
            "YOLO26 resume checkpoint 的 model_scale 与当前训练请求不一致",
            details={
    ...
    if (
        ...
        raise InvalidRequestError(
            "YOLO26 resume checkpoint 的类别数量与当前训练请求不一致",
            details={
...
def _validate_yolo26_resume_input_config(
    *,
    checkpoint_payload: dict[str, object],
    request: Yolo26DetectionResumeValidationRequest,
) -> None:
    ...
    if checkpoint_precision != request.precision:
        raise InvalidRequestError(
            "YOLO26 resume checkpoint 的 precision 与当前训练请求不一致",
            details={
...
def _validate_yolo26_resume_validation_config(
    *,
    checkpoint_payload: dict[str, object],
    request: Yolo26DetectionResumeValidationRequest,
) -> None:
    ...
    if checkpoint_validation_split_name != request.validation_split_name:
        raise InvalidRequestError(
            "YOLO26 resume checkpoint 的 validation_split_name 与当前训练请求不一致",
            details={
...
def _validate_yolo26_resume_optimization_config(
    *,
    checkpoint_payload: dict[str, object],
    request: Yolo26DetectionResumeValidationRequest,
) -> None:
    ...
    _assert_yolo26_resume_float_matches(
...
def _assert_yolo26_resume_float_matches(
    *,
    ...
    if not isinstance(checkpoint_value, int | float) or not math.isclose(
        ...
        raise InvalidRequestError(
            f"YOLO26 resume checkpoint 的 {field_name} 与当前训练请求不一致"
        )


def _assert_yolo26_resume_optional_float_matches(
    *,
    checkpoint_value: object,
...
def _assert_yolo26_resume_int_matches(
    *,
    checkpoint_value: object,
    expected_value: int,
    field_name: str,
