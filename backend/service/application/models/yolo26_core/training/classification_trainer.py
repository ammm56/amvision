...
class Yolo26ClassificationTrainingEpochProgress:
    """描述 YOLO26 classification 单轮训练进度。"""
...
@dataclass(frozen=True)
class Yolo26ClassificationTrainingControlCommand:
    """描述 YOLO26 classification 训练控制命令。"""
...
@dataclass(frozen=True)
class Yolo26ClassificationTrainingSavePoint:
    """描述 YOLO26 classification 训练过程中的保存点。"""
...
@dataclass(frozen=True)
class Yolo26ClassificationTrainingLoopResult:
    """描述 YOLO26 classification 完整训练循环结果。"""
...
class Yolo26ClassificationTrainingPausedError(Exception):
    """表示 YOLO26 classification 训练在 epoch 边界暂停。"""


class Yolo26ClassificationTrainingTerminatedError(Exception):
    """表示 YOLO26 classification 训练在 epoch 边界终止。"""


def run_yolo26_classification_training_loop(
    *,
    imports: Any,
    model: Any,
    optimizer: Any,
    ...
    | None = None,
    savepoint_callback: Callable[[Yolo26ClassificationTrainingSavePoint], None]
    | None = None,
    ...
    checkpoint_bytes = build_yolo26_classification_checkpoint_bytes(
        epoch=start_epoch - 1,
    ...
    last_completed_savepoint = Yolo26ClassificationTrainingSavePoint(
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        train_dataloader = training_loader_lifecycle.resolve(
            ...
            build_loader=lambda: build_yolo_classification_training_dataloader(
                ...
                shuffle=True,
                build_batch=build_yolo26_classification_training_batch,
        ...
        for iteration, cpu_batch in enumerate(train_dataloader, start=1):
            ...
            with autocast_context():
                ...
                loss, probabilities = compute_yolo26_classification_loss(
                    torch_module=imports.torch,
                    outputs=outputs,
        ...
        if should_evaluate:
            val_metrics = evaluate_yolo26_classification_samples(
                model=ema.model,
