...
def is_yolo26_detection_core_model(model: Any) -> bool:
    """判断当前模型是否是 YOLO26 detection core 构建结果。"""

    model_name = str(getattr(model, "model_name", ""))
    model_module = str(model.__class__.__module__)
    return model_name.startswith("yolo26-") and "yolo26_core.nn.model" in model_module


def compute_yolo26_detection_training_loss(
    *,
    torch_module: Any,
    model: Any,
