...
class Yolo26DetectionTrainingExecutionPlan:
    """描述 YOLO26 detection 训练循环的执行边界。"""
...
def plan_yolo26_detection_training_execution(
    *,
    ...
    """根据样本数量和 resume 状态返回 YOLO26 detection 训练执行计划。"""
    ...
    best_metric_name = (
        ...
        else resolve_yolo26_detection_best_metric_name(has_validation=has_validation)
    )
    best_metric_value = (
        float(resume_best_metric_value)
        if resume_best_metric_value is not None
        else resolve_yolo26_detection_initial_best_metric_value(
            has_validation=has_validation
        )
    )
    return Yolo26DetectionTrainingExecutionPlan(
        has_validation=has_validation,
