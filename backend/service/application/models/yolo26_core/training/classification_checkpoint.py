"""YOLO26 classification 训练 checkpoint 与 resume 规则。"""
...
class Yolo26ClassificationResumeState:
    """描述 YOLO26 classification resume checkpoint 内容。"""
...
def build_yolo26_classification_checkpoint_bytes(
    *,
    epoch: int,
    global_iteration: int,
    model: Any,
    ...
    """把 YOLO26 classification 训练状态编码为 checkpoint bytes。"""
...
def load_yolo26_classification_resume_state(
    *,
    checkpoint_path: Path,
    torch_module: Any,
) -> Yolo26ClassificationResumeState:
    """读取 YOLO26 classification resume checkpoint。"""
    ...
    if not isinstance(checkpoint, dict):
        raise InvalidRequestError("YOLO26 classification checkpoint 内容不合法")
    return Yolo26ClassificationResumeState(
...
def validate_yolo26_classification_resume_parameters(
    state: Yolo26ClassificationResumeState,
    *,
    batch_size: int,
    max_epochs: int,
    learning_rate: float,
    ...
    if mismatches:
        raise InvalidRequestError(
            "YOLO26 classification resume 参数与 checkpoint 记录不一致",
            details={"mismatches": mismatches},
        )


def load_yolo26_classification_model_state(
    *,
    model: Any,
    state_dict: dict[str, object],
    device_name: str,
) -> None:
    """把 checkpoint 里的可匹配权重加载进 YOLO26 classification 模型。"""
