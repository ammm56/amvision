"""YOLO26 OBB checkpoint 与 resume 规则。"""
...
class Yolo26ObbResumeState:
    """描述 YOLO26 OBB resume checkpoint 内容。"""
...
def load_yolo26_obb_resume_state(
    *,
    ...
    """读取 YOLO26 OBB resume checkpoint。"""
    ...
    if not isinstance(checkpoint, dict):
        raise InvalidRequestError("YOLO26 OBB checkpoint 内容不合法")
    return Yolo26ObbResumeState(
...
def validate_yolo26_obb_resume_parameters(
    state: Yolo26ObbResumeState,
    *,
    batch_size: int,
    max_epochs: int,
    learning_rate: float,
    ...
    if mismatches:
        raise InvalidRequestError(
            "YOLO26 OBB resume 参数与 checkpoint 记录不一致",
            details={"mismatches": mismatches},
        )


def restore_yolo26_obb_training_state(
    *,
    model: Any,
    optimizer: Any,
    scheduler: Any,
    ...
    """恢复 YOLO26 OBB 模型、optimizer、scheduler 和 scaler 状态。"""

    _load_yolo26_obb_model_state(model=model, state_dict=state.model_state_dict)
...
def build_yolo26_obb_checkpoint_bytes(
    *,
    epoch: int,
    global_iteration: int,
    model: Any,
...
def _load_yolo26_obb_model_state(*, model: Any, state_dict: dict[str, object]) -> None:
    """加载 checkpoint 中 shape 匹配的 YOLO26 OBB 权重。"""
