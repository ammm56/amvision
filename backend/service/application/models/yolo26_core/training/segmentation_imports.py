...
class Yolo26SegmentationTrainingImports:
    """描述 YOLO26 segmentation 训练需要的本地模块。"""
...
def require_yolo26_segmentation_training_imports() -> Yolo26SegmentationTrainingImports:
    """导入 YOLO26 segmentation 训练依赖。"""
    ...
    try:
        ...
        raise ServiceConfigurationError(
            "YOLO26 segmentation 训练缺少必要依赖",
            details={"missing": str(exc)},
        ) from exc
    return Yolo26SegmentationTrainingImports(cv2=cv2, np=np, torch=torch)


def resolve_yolo26_segmentation_training_device(
    *,
    torch_module: Any,
