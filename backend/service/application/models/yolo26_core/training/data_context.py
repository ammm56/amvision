...
def prepare_yolo26_detection_training_data_context(
    *,
    ...
    test_split = resolve_yolo26_detection_test_split(resolved_splits)
    ...
    if validation_split is None or not validation_samples:
        raise InvalidRequestError(
            "YOLO26 detection 训练 manifest 缺少独立 validation 样本"
        )

    test_samples: tuple[Yolo26DetectionTrainingSample, ...] = ()
    if test_split is not None:
        test_samples, test_category_names, test_category_ids = (
            load_yolo26_detection_training_samples(split=test_split)
        )
        if test_category_names != category_names or test_category_ids != category_ids:
            raise InvalidRequestError(
                "YOLO26 detection test split 的 categories 与训练 split 不一致"
            )
