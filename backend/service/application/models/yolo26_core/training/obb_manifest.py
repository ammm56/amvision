"""YOLO26 OBB DatasetExport manifest 解析。"""
...
class Yolo26ObbTrainingAnnotation:
    """描述一张 YOLO26 OBB 训练图片及其标注。"""
...
@dataclass(frozen=True)
class Yolo26ObbTrainingManifest:
    """描述 YOLO26 OBB 训练 manifest 解析结果。"""

    labels: tuple[str, ...]
    train_annotations: list[Yolo26ObbTrainingAnnotation]
    val_annotations: list[Yolo26ObbTrainingAnnotation]
    test_annotations: list[Yolo26ObbTrainingAnnotation]


def load_yolo26_obb_training_manifest(
    *,
    ...
    if not labels:
        raise InvalidRequestError("YOLO26 OBB 训练集没有合法类别")
    if not train_annotations:
        raise InvalidRequestError("YOLO26 OBB 训练集没有合法样本")
    if not val_annotations:
        raise InvalidRequestError(
            "YOLO26 OBB 训练需要独立 validation split，禁止使用 train 或 test 回退"
        )
    return Yolo26ObbTrainingManifest(
        labels=labels,
...
def _build_obb_split_records(
    *,
    ...
    image_root: str,
) -> list[Yolo26ObbTrainingAnnotation]:
    """把 OBB payload 转成 YOLO26 OBB 训练样本。"""
    ...
    records: list[Yolo26ObbTrainingAnnotation] = []
    for image_id, file_name in image_map.items():
        boxes_xywhr: list[list[float]] = []
        class_ids: list[int] = []
        for annotation in annotations_by_image.get(image_id, []):
            box_xywhr = parse_yolo26_obb_annotation(annotation)
...
def parse_yolo26_obb_annotation(annotation: dict[str, Any]) -> list[float] | None:
    ...
    if isinstance(polygon, list) and len(polygon) == 8:
        return polygon_to_yolo26_xywhr([float(value) for value in polygon])
...
def polygon_to_yolo26_xywhr(polygon: list[float]) -> list[float] | None:
...
def _remap_obb_categories(
    annotations: list[Yolo26ObbTrainingAnnotation],
    category_id_map: dict[int, int],
) -> list[Yolo26ObbTrainingAnnotation]:
    """把原始 category id 映射为连续训练类别索引。"""

    remapped: list[Yolo26ObbTrainingAnnotation] = []
...
__all__ = [
    ...
    "parse_yolo26_obb_annotation",
    "polygon_to_yolo26_xywhr",
]
