"""YOLO26 detection 训练 checkpoint 构建。"""
...
class Yolo26DetectionEpochCheckpointUpdate:
    """描述 YOLO26 detection 一个 epoch 后的 checkpoint 更新结果。"""
...
def encode_yolo26_detection_checkpoint_state(
    *,
    ...
    """把 YOLO26 detection checkpoint state 编码为 bytes。"""
...
def decode_yolo26_detection_checkpoint_state(
    *,
    torch_module: Any,
    checkpoint_bytes: bytes,
) -> dict[str, object]:
    """从 bytes 解码 YOLO26 detection checkpoint state。"""
...
def build_yolo26_detection_epoch_checkpoint_update(
    *,
    torch_module: Any,
    model: Any,
    optimizer: Any,
    ...
    """构建 YOLO26 detection 一个 epoch 后的 latest / best checkpoint bytes。"""
    ...
    if improved_best:
        ...
        best_checkpoint_bytes = encode_yolo26_detection_checkpoint_state(
            torch_module=torch_module,
            checkpoint_state=current_best_checkpoint_state,
        )

    latest_checkpoint_bytes = encode_yolo26_detection_checkpoint_state(
        torch_module=torch_module,
        checkpoint_state=current_checkpoint_state,
    )
    return Yolo26DetectionEpochCheckpointUpdate(
