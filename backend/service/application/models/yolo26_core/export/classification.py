...
YOLO26_CLASSIFICATION_EXPORT_OUTPUT_NAMES = ("probabilities",)


def resolve_yolo26_classification_export_output_names() -> tuple[str]:
    """返回 YOLO26 classification 导出输出名。"""

    return YOLO26_CLASSIFICATION_EXPORT_OUTPUT_NAMES
...
def normalize_yolo26_classification_export_outputs(
    *,
    ...
    if not outputs:
        raise InvalidRequestError("YOLO26 classification 导出输出为空")
    return (outputs[0],)
