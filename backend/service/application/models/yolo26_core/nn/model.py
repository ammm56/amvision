...
from backend.service.application.models.yolo26_core.nn.modules import (
    C2PSA,
    C3k2,
    Concat,
    SPPF,
...
class Yolo26ScaleProfile:
    """描述 YOLO26 单个 scale 的复合缩放参数。"""
...
class Yolo26Model(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.model, self.save = parse_yolo26_model(
    ...
    def forward(self, x: torch.Tensor) -> Any:
        ...
        for layer in self.model:
            ...
            if isinstance(from_index, tuple):
                layer_input = [
                    current if index == -1 else outputs[index] for index in from_index
                ]
            elif from_index == -1:
                layer_input = current
            else:
            ...
            layer_index = int(getattr(layer, "layer_index"))
            outputs.append(current if layer_index in self.save else None)
        return current


def build_yolo26_graph_model(
    *,
    model_name: str,
    model_scale: str,
    num_classes: int,
...
def _build_yolo26_layer(
    *,
    layer_index: int,
    raw_layer_def: object,
    module_map: dict[str, type[nn.Module]],
    channels: list[int],
    scale_profile: Yolo26ScaleProfile,
    model_name: str,
    model_scale: str,
    num_classes: int,
    model_config: dict[str, object],
    ...
    if module_type in {Conv, C3k2, C2PSA, SPPF}:
...
def _build_scaled_module(
    *,
    ...
    """构建会受 depth / width scale 影响的 YOLO26 模块。"""
    ...
    if module_type in {Conv, SPPF}:
        ...
        if module_type is C3k2:
            # Ultralytics YOLO26 在 m/l/x scale 会启用 C3k 路径，权重结构依赖这个规则。
            built_module_args[3] = bool(built_module_args[3]) or model_scale in {
                "m",
                "l",
                "x",
            }
...
def _build_classify_layer(
    *,
    ...
    """构建 YOLO26 classification head。"""

    source_channels = channels[_resolve_single_from_index(from_index)]
    resolved_module_args = [
        _resolve_model_config_argument(
            item, num_classes=num_classes, model_config=model_config
        )
        for item in module_args
    ]
    output_channels = (
        int(resolved_module_args[0]) if resolved_module_args else num_classes
    )
    return module_type(
        source_channels, output_channels, *resolved_module_args[1:]
    ), output_channels
...
def _build_prediction_head(
    *,
    ...
    """构建 YOLO26 detection / segmentation / pose / obb head。"""
    ...
    module_args = _scale_yolo26_prediction_head_args(
        module_type=module_type,
        module_args=module_args,
        scale_profile=scale_profile,
    )
    ...
    module = module_type(
        *resolved_module_args,
        ch=detect_channels,
        reg_max=int(model_config.get("reg_max", 1)),
        strides=tuple(int(item) for item in model_config.get("strides", (8, 16, 32))),
        end2end=bool(model_config.get("end2end", True)),
        legacy_class_head=bool(model_config.get("legacy_class_head", False)),
    )
    return module, output_channels


def _scale_yolo26_prediction_head_args(
    *,
    module_type: type[nn.Module],
    module_args: list[object],
    scale_profile: Yolo26ScaleProfile,
) -> list[object]:
    """按 YOLO26 scale 规则缩放 task head 的结构参数。"""

    scaled_args = list(module_args)
    if module_type.__name__ == "Segment26" and len(scaled_args) >= 3:
        scaled_args[2] = make_divisible(
            min(float(scaled_args[2]), float(scale_profile.max_channels))
            * scale_profile.width,
            8,
        )
    return scaled_args
    ...
    *,
    ...
    if value == "kpt_shape":
        ...
        if not isinstance(kpt_shape, list | tuple) or len(kpt_shape) != 2:
            raise ServiceConfigurationError("当前 YOLO26 pose 配置缺少合法的 kpt_shape")
...
def _normalize_from_index(raw_from: object) -> int | tuple[int, ...]:
    ...
    if isinstance(raw_from, list | tuple):
        ...
        return normalized
    raise InvalidRequestError(
        "模型配置里的 from 字段不合法", details={"raw_from": raw_from}
    )
