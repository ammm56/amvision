...
class Yolo26PoseEvaluationRequest:
    """描述一次 YOLO26 pose 数据集级评估请求。"""
...
Yolo26PoseEvaluationResult = PoseEvaluationResult


def run_yolo26_pose_evaluation(
    request: Yolo26PoseEvaluationRequest,
) -> Yolo26PoseEvaluationResult:
    """执行 YOLO26 pose 数据集级评估。"""
...
def evaluate_yolo26_pose_samples(
    *,
    model: Any,
    samples: list[Any],
    labels: tuple[str, ...],
    ...
    with imports.torch.no_grad():
        for image_index, sample in enumerate(samples[:8]):
            batch = build_yolo26_pose_training_batch(
                samples=[sample],
            ...
            if batch is None:
                continue
            with _yolo26_evaluation_autocast(imports, precision, device):
                outputs = model(batch.images)
            prediction_array = _yolo26_tensor_to_np(outputs, imports)
            instances, _ = build_yolo26_pose_postprocess_instances(
            ...
            pred_items.extend(
                _build_yolo26_pose_prediction_items(
                    image_index=image_index,
...
def _append_yolo26_pose_gt_items(
    *,
    image_index: int,
    target: Any,
    gt_items: list[dict[str, object]],
) -> None:
    """把 YOLO26 pose target 转成 OKS AP 使用的 GT 项。"""
        ...
        gt_items.append(
            {
                ...
                "area": _yolo26_pose_box_area(box),
            }
        )


def _build_yolo26_pose_prediction_items(
    *,
    image_index: int,
    predictions: tuple[Any, ...],
) -> list[dict[str, object]]:
    """把 YOLO26 pose 后处理实例转成 OKS AP 预测项。"""
        ...
        items.append(
            {
                ...
                "keypoints": _flatten_yolo26_pose_prediction_keypoints(
...
def _flatten_yolo26_pose_prediction_keypoints(
    keypoints: tuple[Any, ...],
) -> list[float]:
    """把 YOLO26 pose 后处理 keypoints 展平成 COCO 格式。"""
...
def _yolo26_pose_box_area(box: list[float] | tuple[float, ...]) -> float:
    """用 bbox 面积作为 OKS area。"""

    return max((float(box[2]) - float(box[0])) * (float(box[3]) - float(box[1])), 1.0)


def _yolo26_evaluation_autocast(imports: Any, precision: str, device: str):
    """返回 YOLO26 训练期评估使用的 autocast 上下文。"""
...
def _yolo26_tensor_to_np(outputs: Any, imports: Any) -> Any:
    """把 YOLO26 输出转换为 NumPy 数组。"""
