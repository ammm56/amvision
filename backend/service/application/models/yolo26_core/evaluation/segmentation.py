...
class Yolo26SegmentationEvaluationRequest:
    """描述一次 YOLO26 segmentation 数据集级评估请求。"""
...
@dataclass(frozen=True)
class Yolo26SegmentationEvaluationResult:
    """描述一次 YOLO26 segmentation 数据集级评估结果。"""
...
def run_yolo26_segmentation_evaluation(
    request: Yolo26SegmentationEvaluationRequest,
) -> Yolo26SegmentationEvaluationResult:
    """执行 YOLO26 segmentation 数据集级评估。"""

    dataset_storage = request.dataset_storage
    runtime_target = request.runtime_target
    split_name, samples, label_names = _parse_yolo26_segmentation_manifest(
        request.manifest_payload,
        dataset_storage,
    )
    if not samples:
        return Yolo26SegmentationEvaluationResult(
    ...
    for image_index, sample in enumerate(samples):
        ...
        for annotation in gt_annotations:
            ...
            mask = _build_yolo26_segmentation_annotation_mask(
    ...
    per_class_metrics = _merge_yolo26_segmentation_per_class_metrics(
...
def _parse_yolo26_segmentation_manifest(
    manifest: dict[str, object],
    dataset_storage: LocalDatasetStorage,
) -> tuple[str, list[dict[str, object]], tuple[str, ...]]:
    """解析 YOLO26 segmentation DatasetExport manifest。"""

    chosen_split = select_independent_evaluation_split(manifest.get("splits"))
    if chosen_split is None:
        raise InvalidRequestError("YOLO26 segmentation manifest 不包含可用 split")
    ...
    if annotation_file:
        ...
        if not isinstance(annotation_payload, dict):
            raise InvalidRequestError(
                "YOLO26 segmentation annotation 文件格式无效",
        ...
        return (
            split_name,
            _build_yolo26_segmentation_samples(
...
def _build_yolo26_segmentation_samples(
    *,
    image_root: str,
    payload: dict[str, object],
) -> list[dict[str, object]]:
    """把 COCO 风格 images / annotations 组装为 YOLO26 segmentation 评估样本。"""
...
def _build_yolo26_segmentation_annotation_mask(
    *,
    annotation: dict[str, object],
    width: int,
    height: int,
) -> Any:
    """把 YOLO26 segmentation 标注 polygon 转换为二值 mask。"""

    polygons = _normalize_yolo26_segmentation_polygons(annotation.get("segmentation"))
    if not polygons:
        return None
    return _rasterize_yolo26_polygons(polygons=polygons, width=width, height=height)


def _build_yolo26_segmentation_instance_mask(
    *,
    segments: object,
    width: int,
    height: int,
) -> Any:
    """把 YOLO26 segmentation 预测 polygon 转换为二值 mask。"""
...
def _normalize_yolo26_segmentation_polygons(
    segmentation: object,
...
def _rasterize_yolo26_polygons(
    *,
    polygons: list[list[tuple[float, float]]],
    width: int,
    height: int,
...
def _merge_yolo26_segmentation_per_class_metrics(
    *,
...
def evaluate_yolo26_segmentation_samples(
    *,
    model: Any,
    samples: list[Any],
    labels: tuple[str, ...],
    ...
    with (
        ...
        for batch in evaluation_loader:
            ...
            with _yolo26_segmentation_autocast(imports, precision, device):
            ...
            for image_index, target, output_slices in iter_yolo_task_evaluation_items(
                ...
                _append_yolo26_segmentation_gt_items(
                    image_index=image_index,
                    target=target,
...
def _append_yolo26_segmentation_gt_items(
    *,
    image_index: int,
    target: dict[str, Any],
    gt_bbox_items: list[dict[str, object]],
    ...
    for object_index, (box, class_id) in enumerate(zip(boxes, class_ids, strict=True)):
        ...
        if (
            ...
            and object_index < int(mask_valid.shape[0])
            and not bool(mask_valid[object_index])
        ):
            continue
        gt_mask_items.append(
            {
...
def _build_yolo26_segmentation_prediction_items(
    *,
    ...
    """把 YOLO26 segmentation 输出转成 bbox / mask AP 预测项。"""
...
def _yolo26_segmentation_autocast(imports: Any, precision: str, device: str):
    """返回 YOLO26 segmentation 训练期评估使用的 autocast 上下文。"""
...
def _yolo26_segmentation_tensor_to_np(tensor: Any, imports: Any) -> Any:
    """把 YOLO26 segmentation tensor 输出转换为 NumPy 数组。"""
