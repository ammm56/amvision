...
def normalize_yolov8_obb_inference_outputs(
    *,
    outputs: object,
    np_module: Any,
) -> Any:
    """归一化 YOLOv8 OBB inference 输出。"""

    if isinstance(outputs, list | tuple):
        if not outputs:
            raise InvalidRequestError("YOLOv8 OBB inference 输出为空")
        outputs = outputs[0]
    ...
    if prediction.ndim < 3:
        raise InvalidRequestError(
            "YOLOv8 OBB inference 输出维度不合法",
            details={"shape": list(prediction.shape)},
        )
    return prediction


def build_yolov8_obb_inference_instances(
    *,
    np_module: Any,
    ...
    """把 YOLOv8 OBB inference 输出转换为 core 实例记录。"""

    return build_yolov8_obb_postprocess_instances(
        np_module=np_module,
