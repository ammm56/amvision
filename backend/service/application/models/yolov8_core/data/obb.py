...
class YoloV8ObbTrainingBatch:
    ...
    def pin_memory(self) -> "YoloV8ObbTrainingBatch":
...
def build_yolov8_obb_training_batch(
    *,
    ...
    if not training and augmentation_options is not None:
        raise ValueError("YOLOv8 OBB validation 不允许随机增强")
    if not samples:
        return None
...
def _prepare_yolov8_obb_mixup_sample(
    *,
    ...
    if prepared is None:
        return None
    image, target = prepared
    return _apply_yolov8_obb_random_affine(
        imports=imports,
        image=image,
        target=target,
        target_width=target_width,
    ...
    *,
    ...
    for box_xywhr, class_id in zip(sample.boxes_xywhr, sample.class_ids, strict=True):
        ...
        mapped_box = scale_yolo_xywhr_to_letterbox(
            box_xywhr=(cx, cy, width, height, angle),
