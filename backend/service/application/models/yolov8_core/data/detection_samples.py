...
def load_yolov8_detection_training_samples(
    *,
    split: YoloV8DetectionResolvedSplit,
) -> tuple[
    tuple[YoloV8DetectionTrainingSample, ...],
    tuple[str, ...],
    tuple[int, ...],
]:
    """把 YOLOv8 detection split 转成训练阶段可直接消费的样本列表。"""
    ...
    if not isinstance(categories_payload, list) or not isinstance(images_payload, list):
        raise InvalidRequestError(
            "YOLOv8 detection annotation 结构不合法",
            details={
    ...
    resolved_samples: list[YoloV8DetectionTrainingSample] = []
    for image_id, image_meta in image_meta_by_id.items():
        ...
        annotations = tuple(
            ...
            if isinstance(item, YoloV8DetectionTrainingAnnotation)
        )
...
__all__ = ["load_yolov8_detection_training_samples"]
