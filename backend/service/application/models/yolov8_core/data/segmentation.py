"""YOLOv8 segmentation 训练和评估 batch 编码。"""
...
class YoloV8SegmentationTrainingBatch:
    """描述 YOLOv8 segmentation 训练或评估使用的 batch。"""

    images: Any
    targets: list[dict[str, Any]]


def build_yolov8_segmentation_training_batch(
    *,
    ...
    for sample in samples:
        ...
        canvas, target = _apply_yolov8_segmentation_augmentation(
            imports=imports,
            image=canvas,
            target=target,
            target_width=target_width,
        ...
        target = _finalize_yolov8_segmentation_target(
            imports=imports,
            target=target,
            device=device,
        )
    ...
    if not images:
        return None
    return YoloV8SegmentationTrainingBatch(
...
def _prepare_yolov8_segmentation_sample_with_mix(
    *,
    ...
    mixup_prepared = _prepare_yolov8_segmentation_mixup_sample(
        imports=imports,
        sample=mixup_sample,
        available_samples=available_samples,
        target_width=target_width,
    ...
    return (
        ...
        _merge_yolov8_segmentation_targets(
            imports=imports,
            primary=target,
            other=mixup_target,
            target_width=target_width,
...
def _prepare_yolov8_segmentation_mixup_sample(
    *,
    imports: Any,
    sample: Any,
    available_samples: Sequence[Any],
...
def _build_yolov8_segmentation_mosaic_sample(
    *,
    imports: Any,
    primary_sample: Any,
    available_samples: Sequence[Any],
    ...
    """构造 2x2 YOLOv8 segmentation mosaic 样本。"""
    ...
    for sample, (left, top, cell_width, cell_height) in zip(selected_samples, placements, strict=True):
        ...
        shifted_target = _offset_yolov8_segmentation_target(
            imports=imports,
            target=cell_target,
            offset_xy=(left, top),
            output_size=(target_width, target_height),
        ...
        merged_target = (
            ...
            else _merge_yolov8_segmentation_targets(
                imports=imports,
...
def _prepare_yolov8_segmentation_single_sample(
    *,
    imports: Any,
    sample: Any,
    ...
    return canvas, _build_yolov8_segmentation_sample_targets(
        sample=sample,
        target_width=target_width,
        target_height=target_height,
...
def _build_yolov8_segmentation_sample_targets(
    *,
    sample: Any,
    target_width: int,
    target_height: int,
    ...
    """构造单张图的 YOLOv8 segmentation target。"""
    ...
    for object_index, (bbox, class_id) in enumerate(
        ...
        mask, valid = rasterize_yolov8_segmentation_polygons(
...
def _offset_yolov8_segmentation_target(
    *,
    imports: Any,
    target: dict[str, Any],
    ...
    if masks is not None:
        ...
        shifted_masks[:, offset_y:offset_y + source_height, offset_x:offset_x + source_width] = masks[
            :,
            : min(source_height, output_height - offset_y),
            : min(source_width, output_width - offset_x),
        ]
        shifted["masks_array"] = shifted_masks
    return _filter_yolov8_segmentation_target_by_boxes(
        imports=imports,
        target=shifted,
    )


def _merge_yolov8_segmentation_targets(
    *,
    imports: Any,
    ...
    if primary_masks is not None or other_masks is not None:
        merged["masks_array"] = imports.np.concatenate(
            [
                _ensure_yolov8_segmentation_masks(
                    imports=imports,
                    masks=primary_masks,
...
def _ensure_yolov8_segmentation_masks(
    *,
    imports: Any,
    masks: Any,
    count: int,
...
def _filter_yolov8_segmentation_target_by_boxes(
    *,
    imports: Any,
    target: dict[str, Any],
) -> dict[str, Any]:
    ...
    filtered["class_ids"] = select_yolov8_items_by_indices(
...
def _apply_yolov8_segmentation_augmentation(
    *,
    imports: Any,
    image: Any,
    target: dict[str, Any],
    ...
    """对 YOLOv8 segmentation 图像和 mask target 执行受控增强。"""
    ...
    return flip_yolov8_image_horizontally(image), flipped_target


def _apply_yolov8_segmentation_random_affine(
    *,
    imports: Any,
    image: Any,
    target: dict[str, Any],
    ...
    transformed_boxes, transformed_box_indices = transform_yolov8_boxes_xyxy(
        imports=imports,
        boxes_xyxy=boxes,
        matrix=matrix,
        output_size=(target_width, target_height),
    ...
    if masks is not None:
        warped_masks = warp_yolov8_masks(
            imports=imports,
            masks=masks,
            matrix=matrix,
            output_size=(target_width, target_height),
        ...
        kept_pairs = _filter_yolov8_segmentation_affine_indices(
            imports=imports,
    ...
    transformed_target["class_ids"] = select_yolov8_items_by_indices(
        list(target.get("class_ids", [])),
        kept_indices,
    )
...
def _filter_yolov8_segmentation_affine_indices(
    *,
    imports: Any,
...
def _finalize_yolov8_segmentation_target(
    *,
    imports: Any,
    target: dict[str, Any],
    device: str,
