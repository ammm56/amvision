...
def _yolov8_box_ciou_matrix(
    *,
    ...
    center_distance = ((box1_center[:, None, :] - box2_center[None, :, :]) ** 2).sum(dim=-1)

    enclosing_top_left = torch_module.minimum(boxes1[:, None, 0:2], boxes2[None, :, 0:2])
    enclosing_bottom_right = torch_module.maximum(boxes1[:, None, 2:4], boxes2[None, :, 2:4])
