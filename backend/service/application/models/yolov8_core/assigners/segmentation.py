"""YOLOv8 segmentation target assigner。"""
...
class YoloV8SegmentationAssignment:
    """描述单张图 YOLOv8 segmentation loss 使用的 anchor 分配结果。"""
...
def assign_yolov8_segmentation_targets(
    *,
    torch_module: Any,
    ...
    beta: float,
    num_classes: int,
) -> YoloV8SegmentationAssignment | None:
    """根据当前图片 GT 和 anchor 生成 YOLOv8 segmentation 正样本。"""
    ...
    pred_boxes = _decode_yolov8_segmentation_prediction_boxes(
    ...
    anchor_centers_xy = anchor_points.to(device=device, dtype=prediction.dtype) * stride_tensor.to(
        device=device,
    ...
    return YoloV8SegmentationAssignment(
        batch_idx=batch_idx,
...
def _decode_yolov8_segmentation_prediction_boxes(
    *,
    torch_module: Any,
    prediction: Any,
    anchor_points: Any,
    ...
    """把 YOLOv8 segmentation 训练预测距离解码为像素级 xyxy bbox。"""
