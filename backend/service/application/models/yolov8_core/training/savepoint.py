"""YOLOv8 detection 训练 savepoint 组装规则。"""
...
class YoloV8DetectionTrainingSavepointPayload:
    """描述 YOLOv8 detection 在 epoch 边界需要交给应用层保存的数据。"""
...
def build_yolov8_detection_training_savepoint_payload(
    *,
    epoch: int,
    latest_checkpoint_bytes: bytes,
    ...
    """把 YOLOv8 detection 内部 checkpoint 状态转换成 savepoint payload。"""

    resolved_best_checkpoint_bytes = (
        best_checkpoint_bytes
        if best_checkpoint_bytes
        else latest_checkpoint_bytes
    )
    return YoloV8DetectionTrainingSavepointPayload(
        epoch=int(epoch),
