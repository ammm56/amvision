...
class YoloV8ClassificationTrainingExecutionRequest:
    ...
    batch_callback: (
        Callable[[YoloV8ClassificationTrainingBatchProgress], None] | None
    ) = None
...
def run_yolov8_classification_training(
    ...
    batch_size = resolve_training_batch_size(
        ...
        default_batch_size=YOLOV8_CLASSIFICATION_DEFAULT_BATCH_SIZE,
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        for iteration, cpu_batch in enumerate(train_dataloader, start=1):
            ...
            if request.batch_callback is not None:
                batch_count = max(1, int(batch_targets.size(0)))
                request.batch_callback(
                    YoloV8ClassificationTrainingBatchProgress(
                        ...
                        train_metrics={
                            ...
                            "accuracy": round(
                                float((predicted == batch_targets).sum().item())
                                / batch_count,
                                6,
                            ),
                        },
        ...
        if is_best and checkpoint_decision.should_serialize:
            best_checkpoint_bytes = checkpoint_bytes
        if checkpoint_decision.should_serialize and request.savepoint_callback is not None:
...
def _autocast_context(imports: Any, precision: str, device_name: str):
    if precision in {"fp16", "bf16"} and "cuda" in device_name:
        return imports.torch.amp.autocast(
            resolve_torch_amp_device_type(device_name),
            dtype=(
                imports.torch.float16
                if precision == "fp16"
                else imports.torch.bfloat16
            ),
        )
    return nullcontext()
