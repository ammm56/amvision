...
def is_yolov8_detection_core_model(model: Any) -> bool:
    """判断当前模型是否是 YOLOv8 detection core 构建结果。"""

    model_name = str(getattr(model, "model_name", ""))
    model_module = str(model.__class__.__module__)
    return model_name.startswith("yolov8-") and "yolov8_core.nn.model" in model_module


def compute_yolov8_detection_training_loss(
    *,
    torch_module: Any,
    model: Any,
    ...
    """按 YOLOv8 detection core 规则计算训练损失。"""

    detect_head = model.model[-1]
    return compute_yolov8_detection_loss(
