...
class YoloV8ObbTrainingExecutionRequest:
    ...
    batch_callback: Callable[[YoloV8ObbTrainingBatchProgress], None] | None = None
...
def run_yolov8_obb_training(
    ...
    labels, train_annotations, val_annotations, test_annotations = _load_obb_manifest(
    ...
    bs = resolve_training_batch_size(
        ...
        default_batch_size=_OBB_DEF_BS,
    ...
    last_completed_savepoint = YoloV8ObbTrainingSavePoint(
    ...
    optimizer_step.require_successful_optimizer_step(
        task_name=f"{request.model_type.upper()} OBB",
    )
    ...
    if test_annotations:
        ...
        if not isinstance(best_state_dict, dict):
            raise InvalidRequestError("YOLOv8 OBB best checkpoint 缺少模型权重")
        ema.model.load_state_dict(best_state_dict, strict=False)
        ema.model.to(device)
        test_metrics = evaluate_yolov8_obb_samples(
            model=ema.model,
            ...
            device=device,
            precision=precision,
            score_threshold=float(extra.get("evaluation_confidence_threshold", 0.01)),
            nms_threshold=float(extra.get("evaluation_nms_threshold", 0.7)),
            imports=_build_yolov8_training_imports(cv2, np, torch),
            batch_size=bs,
            control_callback=observe_control,
...
def _load_obb_manifest(
    ...
    if not labels:
        raise InvalidRequestError("YOLOv8 OBB 训练集没有合法类别")
    if not train_anns:
        raise InvalidRequestError("YOLOv8 OBB 训练集没有合法样本")
    if not val_anns:
        raise InvalidRequestError(
            "YOLOv8 OBB 训练需要独立 validation split，禁止使用 train 或 test 回退"
        )

    def remap(anns: list[_ObbAnnotation]) -> list[_ObbAnnotation]:
        remapped: list[_ObbAnnotation] = []
