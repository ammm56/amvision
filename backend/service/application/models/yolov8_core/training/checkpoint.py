"""YOLOv8 detection 训练 checkpoint 构建。"""
...
class YoloV8DetectionEpochCheckpointUpdate:
    """描述 YOLOv8 detection 一个 epoch 后的 checkpoint 更新结果。"""
...
def build_yolov8_detection_checkpoint_state(
    *,
    model: Any,
    optimizer: Any,
    scheduler: Any,
    ...
    """构建 YOLOv8 detection 可恢复 checkpoint state。"""
...
def encode_yolov8_detection_checkpoint_state(
    *,
    torch_module: Any,
    checkpoint_state: dict[str, object] | None,
) -> bytes:
    """把 YOLOv8 detection checkpoint state 编码为 bytes。"""
...
def decode_yolov8_detection_checkpoint_state(
    *,
    torch_module: Any,
    checkpoint_bytes: bytes,
) -> dict[str, object]:
    """从 bytes 解码 YOLOv8 detection checkpoint state。"""
...
def build_yolov8_detection_epoch_checkpoint_update(
    *,
    torch_module: Any,
    model: Any,
    optimizer: Any,
    ...
    current_checkpoint_state = build_yolov8_detection_checkpoint_state(
        model=model,
        optimizer=optimizer,
        scheduler=scheduler,
        scaler=scaler,
    ...
    if improved_best:
        ...
        best_checkpoint_bytes = encode_yolov8_detection_checkpoint_state(
            torch_module=torch_module,
            checkpoint_state=current_best_checkpoint_state,
        )

    latest_checkpoint_bytes = encode_yolov8_detection_checkpoint_state(
        torch_module=torch_module,
