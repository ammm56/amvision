...
class Proto(nn.Module):
    """YOLOv8 实例分割使用的原型 mask 生成头。"""

    def __init__(self, c1: int, c_: int = 256, c2: int = 32) -> None:
        """初始化 YOLOv8 segmentation proto 头。"""
...
class Segment(Detect):
    """YOLOv8 segmentation head 的项目内 full core 实现。"""
    ...
    def forward(
        ...
        """执行 YOLOv8 segmentation head 前向。"""

        if not isinstance(x, list | tuple) or len(x) != self.nl:
            raise InvalidRequestError(
                "YOLOv8 Segment 头收到的特征层数量不合法",
