...
def _require_yolov8_model_input_spec_payload(
    *,
    session: object,
) -> dict[str, object]:
    """返回 YOLOv8 导出源模型的必填输入契约。"""

    input_spec = session.runtime_target.model_input_spec
    if input_spec is None:
        raise ServiceConfigurationError("YOLOv8 导出缺少 model_input_spec")
    return input_spec.to_payload()


def _build_yolov8_input_tensor_payload(
    *,
    session: object,
    input_name: str,
) -> dict[str, object]:
    """构建 YOLOv8 ONNX 输入张量摘要。"""
