...
YOLOV8_CLASSIFICATION_EXPORT_OUTPUT_NAMES = ("probabilities",)


def resolve_yolov8_classification_export_output_names() -> tuple[str]:
    """返回 YOLOv8 classification 导出输出名。"""

    return YOLOV8_CLASSIFICATION_EXPORT_OUTPUT_NAMES
...
def normalize_yolov8_classification_export_outputs(
    *,
    ...
    if not outputs:
        raise InvalidRequestError("YOLOv8 classification 导出输出为空")
    return (outputs[0],)
