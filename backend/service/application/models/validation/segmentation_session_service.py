...
class LocalSegmentationValidationSessionService:
    ...
    def __init__(
        ...
        *,
        ...
        self.segmentation_runtime = (
            segmentation_runtime or DefaultSegmentationModelRuntime()
        )
    ...
    def create_session(
        ...
        *,
        ...
        mask_threshold = _resolve_probability(
            request.mask_threshold, default=_DEFAULT_MASK_THRESHOLD
        )
    ...
    def predict(
        self, session_id: str, request: SegmentationValidationSessionPredictRequest
        ...
        if input_file_id is not None:
            ref = resolve_public_project_file_reference(
                dataset_storage=self.dataset_storage,
        ...
        mask_threshold = _resolve_probability(
            request.mask_threshold, default=session.mask_threshold
        )
        save_result_image = (
        ...
        runtime_session = self.segmentation_runtime.load_session(
            dataset_storage=self.dataset_storage, runtime_target=runtime_target
        )
    ...
    def _session_path(session_id: str) -> str:
        return str(
            ...
            / "validation-sessions-segmentation"
            / session_id
            / "session.json"
        )
...
def _build_runtime_target_from_session(
    *, session: SegmentationValidationSessionView, dataset_storage: LocalDatasetStorage
) -> RuntimeTargetSnapshot:
...
def _serialize_session(session: SegmentationValidationSessionView) -> dict[str, object]:
    return {
        ...
        "mask_threshold": session.mask_threshold,
...
def _serialize_summary(
    s: SegmentationValidationPredictionSummary | None,
...
def _build_session_from_payload(
    ...
    mt = _require_payload_str(payload, "model_type")
    ...
    return SegmentationValidationSessionView(
        ...
        model_type=_normalize_model_type(mt),
...
def _build_summary_from_payload(
    p: object,
) -> SegmentationValidationPredictionSummary | None:
    ...
    return SegmentationValidationPredictionSummary(
        prediction_id=_require_payload_str(p, "prediction_id"),
        created_at=_require_payload_str(p, "created_at"),
        input_uri=_read_payload_optional_str(p, "input_uri"),
        input_file_id=_read_payload_optional_str(p, "input_file_id"),
        instance_count=ic,
        preview_image_uri=_read_payload_optional_str(p, "preview_image_uri"),
        raw_result_uri=_read_payload_optional_str(p, "raw_result_uri"),
        latency_ms=lm,
    )
...
def _normalize_runtime_backend(rb: str | None) -> str:
    n = normalize_runtime_target_backend(
        _normalize_optional_str(rb) or _VALIDATION_RUNTIME_BACKEND
    )
    if n not in _SUPPORTED_VALIDATION_RUNTIME_BACKENDS:
        raise InvalidRequestError(
            "当前 segmentation validation session 不支持指定 runtime_backend",
            details={
                "runtime_backend": n,
...
def _normalize_device_name(device_name: str | None, *, runtime_backend: str) -> str:
    return normalize_runtime_target_device_name(
        _normalize_optional_str(device_name), runtime_backend=runtime_backend
    )
