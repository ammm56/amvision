"""RF-DETR core 模型结构模块：`models.transformer`。"""
...
from backend.service.application.models.rfdetr_core.models.ops.modules import MSDeformAttn
...
def gen_sineembed_for_position(pos_tensor, dim=128):
    """执行 `gen_sineembed_for_position`。
    
    参数：
    - `pos_tensor`：传入的 `pos_tensor` 参数。
    - `dim`：传入的 `dim` 参数。
    """

    scale = 2 * math.pi
    dim_index = torch.arange(dim, dtype=torch.int64, device=pos_tensor.device)
    dim_t = 10000 ** (2 * torch.div(dim_index, 2, rounding_mode="floor").to(pos_tensor.dtype) / dim)
...
def gen_encoder_output_proposals(memory, memory_padding_mask=None, spatial_shapes=None, unsigmoid=True):
    """执行 `gen_encoder_output_proposals`。
    
    参数：
    - `memory`：传入的 `memory` 参数。
    - `memory_padding_mask`：传入的 `memory_padding_mask` 参数。
    - `spatial_shapes`：传入的 `spatial_shapes` 参数。
    - `unsigmoid`：传入的 `unsigmoid` 参数。
    """
    proposals = []
    _cur = 0
    for lvl, (height, width) in enumerate(spatial_shapes):
        ...
        grid = (grid.unsqueeze(0) + 0.5) / scale.float()

        wh = torch.ones_like(grid) * 0.05 * (2.0**lvl)

        proposal = torch.cat((grid, wh), -1).reshape(-1, height * width, 4)
...
def _get_activation_fn(activation):
    """执行 `_get_activation_fn`。
    
    参数：
    - `activation`：传入的 `activation` 参数。
    """
    if activation == "relu":
        return F.relu
    if activation == "gelu":
