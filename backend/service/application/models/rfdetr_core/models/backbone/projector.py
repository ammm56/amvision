"""RF-DETR core 模型结构模块：`models.backbone.projector`。"""
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5, act="silu", layer_norm=False, rms_norm=False):
        ...
        - `c1`：传入的 `c1` 参数。
        - `c2`：传入的 `c2` 参数。
        - `shortcut`：传入的 `shortcut` 参数。
        - `g`：传入的 `g` 参数。
        - `k`：传入的 `k` 参数。
        - `e`：传入的 `e` 参数。
        - `act`：传入的 `act` 参数。
        - `layer_norm`：传入的 `layer_norm` 参数。
        - `rms_norm`：传入的 `rms_norm` 参数。
        """
        super().__init__()
        c_ = int(c2 * e)
...
class MultiScaleProjector(nn.Module):
    """RF-DETR core 类：`MultiScaleProjector`。"""
    ...
    def __init__(
        ...
        参数：
        - `in_channels`：传入的 `in_channels` 参数。
        - `out_channels`：传入的 `out_channels` 参数。
        - `scale_factors`：传入的 `scale_factors` 参数。
        - `num_blocks`：传入的 `num_blocks` 参数。
        - `layer_norm`：传入的 `layer_norm` 参数。
        - `rms_norm`：传入的 `rms_norm` 参数。
        - `survival_prob`：传入的 `survival_prob` 参数。
        - `force_drop_last_n_features`：传入的 `force_drop_last_n_features` 参数。
