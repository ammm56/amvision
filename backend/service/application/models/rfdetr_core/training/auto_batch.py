"""RF-DETR core 训练处理模块：`training.auto_batch`。"""
...
from backend.service.application.models.rfdetr_core.models import build_criterion_from_config
...
class AutoBatchResult:
    """RF-DETR core 类：`AutoBatchResult`。"""
...
def _make_synthetic_batch(
    ...
    """执行 `_make_synthetic_batch`。
    
    参数：
    - `micro_batch_size`：传入的 `micro_batch_size` 参数。
    - `resolution`：传入的 `resolution` 参数。
    - `device`：传入的 `device` 参数。
    - `num_classes`：传入的 `num_classes` 参数。
    - `segmentation_head`：传入的 `segmentation_head` 参数。
    - `max_targets_per_image`：传入的 `max_targets_per_image` 参数。
    - `num_channels`：传入的 `num_channels` 参数。
...
def probe_max_micro_batch(
    ...
    - `segmentation_head`：传入的 `segmentation_head` 参数。
    - `max_targets_per_image`：传入的 `max_targets_per_image` 参数。
    - `safety_margin`：传入的 `safety_margin` 参数。
    - `max_micro_batch`：传入的 `max_micro_batch` 参数。
    - `num_channels`：传入的 `num_channels` 参数。
...
def recommend_grad_accum_steps(safe_micro_batch: int, target_effective_batch: int) -> int:
    """执行 `recommend_grad_accum_steps`。
    
    参数：
    - `safe_micro_batch`：传入的 `safe_micro_batch` 参数。
    - `target_effective_batch`：传入的 `target_effective_batch` 参数。
...
def resolve_auto_batch_config(
    ...
    """执行 `resolve_auto_batch_config`。
    
    参数：
    - `model_context`：传入的 `model_context` 参数。
