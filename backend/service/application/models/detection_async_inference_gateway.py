"""detection async inference task 的 service-worker queue IPC 网关。"""
...
DETECTION_ASYNC_INFERENCE_GATEWAY_QUEUE_PREFIX = "detection-ai-gw"
DETECTION_ASYNC_INFERENCE_GATEWAY_RESPONSE_QUEUE_PREFIX = (
    "detection-ai-rsp"
)


class DetectionAsyncInferenceExecutor(Protocol):
    """定义异步推理任务执行客户端的稳定边界。"""
    ...
    def execute_inference(
        ...
        *,
        ...
        - request：统一的 detection prediction 请求。
...
@dataclass
class QueueBackedDetectionAsyncInferenceClient:
    ...
    client_id: str = "detection-async-inference-client"
    ...
    def execute_inference(
        ...
        *,
        ...
        response_queue_name = (
            f"{DETECTION_ASYNC_INFERENCE_GATEWAY_RESPONSE_QUEUE_PREFIX}-{uuid4().hex[:12]}"
        )
        normalized_owner_id = normalize_detection_async_inference_owner_id(owner_id)
        normalized_deployment_instance_id = normalize_detection_async_inference_deployment_id(
            process_config.deployment_instance_id
        )
        request_queue_name = build_detection_async_inference_gateway_queue_name(
            owner_id=owner_id,
...
@dataclass
class DetectionAsyncInferenceGatewayDispatcher:
    ...
    worker_id: str = "backend-service-detection-async-inference-gateway"
    ...
    def __post_init__(self) -> None:
        ...
        self.service_id = normalize_detection_async_inference_owner_id(self.service_id)
        self.deployment_instance_id = normalize_detection_async_inference_deployment_id(
    ...
    def request_queue_name(self) -> str:
        ...
        return build_detection_async_inference_gateway_queue_name(
    ...
    def start(self) -> None:
        ...
        with self._lock:
            ...
            self._thread = Thread(
                ...
                name="detection-async-inference-gateway-dispatcher",
                daemon=True,
            )
            self._thread.start()
    ...
    def _cleanup_response_queues_if_needed(self) -> None:
        ...
        if callable(cleanup):
            cleanup(
                queue_name_prefix=f"{DETECTION_ASYNC_INFERENCE_GATEWAY_RESPONSE_QUEUE_PREFIX}-",
                retention_seconds=self.response_queue_retention_seconds,
            )


@dataclass
class DetectionAsyncInferenceGatewayDispatcherRegistry:
    ...
    worker_id_prefix: str = "backend-service-detection-async-inference-gateway"
    poll_interval_seconds: float = 0.05
    request_queue_lease_timeout_seconds: float = 120.0
    response_queue_retention_seconds: float = 300.0
    response_queue_cleanup_interval_seconds: float = 60.0
    ...
    def __post_init__(self) -> None:
        ...
        self._lock = Lock()
        self._running = False
        self._dispatchers: dict[str, DetectionAsyncInferenceGatewayDispatcher] = {}
    ...
    def get_dispatcher_for_deployment(
        self,
        deployment_instance_id: str,
    ) -> DetectionAsyncInferenceGatewayDispatcher | None:
...
def build_detection_async_inference_gateway_queue_name(
    *,
    ...
    return (
        f"{DETECTION_ASYNC_INFERENCE_GATEWAY_QUEUE_PREFIX}-"
...
def serialize_detection_async_inference_execution_result(result: object) -> dict[str, object]:
    ...
    参数：
    - result：执行结果对象；通常来自 run_detection_inference_task。
...
def _deserialize_gateway_request(
    ...
    *,
...
) -> tuple[str, str, DeploymentProcessConfig, DetectionPredictionRequest]:
    """把请求队列消息载荷恢复为执行所需对象。"""

    if not isinstance(payload, dict):
        raise InvalidRequestError("async inference gateway 请求载荷格式不合法")
    request_id = _require_str(payload, "request_id")
    owner_id = normalize_detection_async_inference_owner_id(payload.get("owner_id"))
    ...
    deployment_instance_id = normalize_detection_async_inference_deployment_id(
    ...
    process_config_deployment_id = normalize_detection_async_inference_deployment_id(
        process_config.deployment_instance_id
    )
...
def _serialize_prediction_request(request: DetectionPredictionRequest) -> dict[str, object]:
...
def _deserialize_prediction_request(payload: object) -> DetectionPredictionRequest:
