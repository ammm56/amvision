...
from backend.service.application.models.yolo11_core.nn.tasks import Classify


@dataclass(frozen=True)
class Yolo11ScaleProfile:
    """描述 YOLO11 单个 scale 的复合缩放参数。"""
...
class Yolo11Model(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.model, self.save = parse_yolo11_model(
...
def _build_yolo11_layer(
    *,
    layer_index: int,
    raw_layer_def: object,
    module_map: dict[str, type[nn.Module]],
    channels: list[int],
    scale_profile: Yolo11ScaleProfile,
    model_name: str,
    model_scale: str,
    num_classes: int,
    model_config: dict[str, object],
...
def _build_scaled_module(
    *,
    ...
    """构建会受 depth / width scale 影响的 YOLO11 模块。"""
    ...
    if module_type in {Conv, SPPF}:
        ...
        if module_type is C3k2:
            # Ultralytics YOLO11 在 m/l/x scale 会启用 C3k 路径，权重结构依赖这个规则。
...
def _build_prediction_head(
    *,
    ...
    """构建 YOLO11 detection / segmentation / pose / obb head。"""
    ...
    module_args = _scale_yolo11_prediction_head_args(
        module_type=module_type,
        module_args=module_args,
        scale_profile=scale_profile,
    )
...
def _scale_yolo11_prediction_head_args(
    *,
    module_type: type[nn.Module],
    module_args: list[object],
    scale_profile: Yolo11ScaleProfile,
) -> list[object]:
    """按 YOLO11 scale 规则缩放 task head 的结构参数。"""
    ...
    *,
    ...
    if value == "kpt_shape":
        ...
        if not isinstance(kpt_shape, list | tuple) or len(kpt_shape) != 2:
            raise ServiceConfigurationError("当前 YOLO11 pose 配置缺少合法的 kpt_shape")
