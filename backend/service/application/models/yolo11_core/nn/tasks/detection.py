...
class Detect(nn.Module):
    """YOLO11 detection head 的项目内 full core 实现。"""
        ...
        *,
        ...
        if len(self.strides) != self.nl:
            raise ServiceConfigurationError(
                "YOLO11 Detect 头的 stride 数量与特征层数量不一致",
    ...
    def forward(
        ...
        """执行 YOLO11 detection head 前向。"""

        if not isinstance(x, list | tuple) or len(x) != self.nl:
            raise InvalidRequestError(
                "YOLO11 Detect 头收到的特征层数量不合法",
    ...
    def _inference(self, raw_outputs: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        return build_yolo11_detection_prediction(
    ...
    def forward_head(
        ...
        *,
        ...
        """按 Ultralytics Detect.forward_head 组装 box / class 原始输出。"""
