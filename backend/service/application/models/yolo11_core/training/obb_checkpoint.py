"""YOLO11 OBB checkpoint 与 resume 规则。"""
...
class Yolo11ObbResumeState:
    """描述 YOLO11 OBB resume checkpoint 内容。"""
...
def load_yolo11_obb_resume_state(
    *,
    checkpoint_path: Path,
    torch_module: Any,
) -> Yolo11ObbResumeState:
    """读取 YOLO11 OBB resume checkpoint。"""
    ...
    if not isinstance(checkpoint, dict):
        raise InvalidRequestError("YOLO11 OBB checkpoint 内容不合法")
    return Yolo11ObbResumeState(
...
def validate_yolo11_obb_resume_parameters(
    state: Yolo11ObbResumeState,
    *,
    batch_size: int,
    max_epochs: int,
    learning_rate: float,
    ...
    """校验 YOLO11 OBB resume 参数是否匹配当前请求。"""
    ...
    if mismatches:
        raise InvalidRequestError(
            "YOLO11 OBB resume 参数与 checkpoint 记录不一致",
            details={"mismatches": mismatches},
        )


def restore_yolo11_obb_training_state(
    *,
    model: Any,
    optimizer: Any,
    scheduler: Any,
    ...
    """恢复 YOLO11 OBB 模型、optimizer、scheduler 和 scaler 状态。"""

    _load_yolo11_obb_model_state(model=model, state_dict=state.model_state_dict)
...
def build_yolo11_obb_checkpoint_bytes(
    *,
    epoch: int,
    global_iteration: int,
    model: Any,
    ...
    """把 YOLO11 OBB 训练状态编码为 checkpoint bytes。"""
...
def _load_yolo11_obb_model_state(*, model: Any, state_dict: dict[str, object]) -> None:
    """加载 checkpoint 中 shape 匹配的 YOLO11 OBB 权重。"""
