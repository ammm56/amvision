...
class Yolo11DetectionTrainingRuntime:
    """描述 YOLO11 detection 训练需要复用的运行时对象。"""
...
def build_yolo11_detection_training_runtime(
    *,
    torch_module: Any,
    model: Any,
    learning_rate: float,
    ...
    """构建 YOLO11 detection optimizer、scheduler 和 GradScaler。"""
    ...
    return Yolo11DetectionTrainingRuntime(
        optimizer=optimizer,
        scheduler=scheduler,
        scaler=scaler,
    )
...
def move_yolo11_optimizer_state_to_device(*, optimizer: Any, device: str) -> None:
    """把 YOLO11 optimizer 状态里的 tensor 移到训练设备。"""
...
def build_yolo11_autocast_context(
    *,
    torch_module: Any,
    device: str,
    runtime_precision: str,
) -> Any:
    """构建 YOLO11 detection 训练使用的 autocast context。"""
