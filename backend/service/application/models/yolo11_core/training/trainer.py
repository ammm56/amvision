...
def run_yolo11_detection_training_loop(
    *,
    ...
    training_dataloader_factory: Callable[
        [int],
        Iterable[Yolo11DetectionDataLoaderBatch],
    ]
    | None = None,
    ...
    | None = None,
    control_callback: Callable[[], Yolo11DetectionEpochControlDecision | None]
    | None = None,
    ...
    def build_completed_epoch_checkpoint(
        *,
        ...
        return build_yolo11_detection_epoch_checkpoint_update(
    ...
    last_completed_savepoint = build_yolo11_detection_training_savepoint_payload(
        epoch=resume_epoch,
    ...
    def observe_control() -> None:
        ...
        if decision.pause_training:
            raise Yolo11DetectionTrainingPausedError(last_completed_savepoint)
        if decision.terminate_training:
            raise Yolo11DetectionTrainingTerminatedError()

    def on_batch(progress: Yolo11DetectionTrainingBatchProgress) -> None:
        """发布 batch 进度后检查训练控制。"""
    ...
    for epoch in range(int(resume_epoch) + 1, int(max_epochs) + 1):
        ...
        control_decision = _resolve_yolo11_epoch_control_decision(
