"""YOLO11 segmentation 训练 manifest 解析。"""
...
class Yolo11SegmentationTrainingAnnotation:
    """描述 YOLO11 segmentation 训练样本标注。"""
...
@dataclass(frozen=True)
class Yolo11SegmentationTrainingManifest:
    """描述 YOLO11 segmentation 训练 manifest 解析结果。"""

    labels: tuple[str, ...]
    train_annotations: list[Yolo11SegmentationTrainingAnnotation]
    val_annotations: list[Yolo11SegmentationTrainingAnnotation]
    test_annotations: list[Yolo11SegmentationTrainingAnnotation]


def load_yolo11_segmentation_training_manifest(
    *,
    dataset_storage: LocalDatasetStorage,
    manifest_payload: dict[str, object],
) -> Yolo11SegmentationTrainingManifest:
    """读取 DatasetExport manifest 并转换成 YOLO11 segmentation 样本。"""

    splits = manifest_payload.get("splits")
    if not isinstance(splits, list):
        raise InvalidRequestError("YOLO11 segmentation 训练 manifest 缺少合法 splits")
    ...
    for split_payload in splits:
        ...
        payload = _load_yolo11_segmentation_split_payload(
        ...
        _collect_yolo11_segmentation_categories(payload, all_categories)
        split_annotations = _build_yolo11_segmentation_split_annotations(
            dataset_storage=dataset_storage,
            payload=payload,
            image_root=image_root,
        )
    ...
    labels, category_id_to_index = _build_yolo11_segmentation_label_mapping(
        all_categories
    )
    if not labels:
        raise InvalidRequestError("YOLO11 segmentation 训练集没有合法类别")
    if not train_annotations:
        raise InvalidRequestError("YOLO11 segmentation 训练集没有合法样本")
    if not val_annotations:
        raise InvalidRequestError(
            "YOLO11 segmentation 训练需要独立 validation split，禁止使用 train 或 test 回退"
        )
    return Yolo11SegmentationTrainingManifest(
        labels=labels,
        train_annotations=_remap_yolo11_segmentation_annotation_classes(
            train_annotations,
            category_id_to_index,
        ),
        val_annotations=_remap_yolo11_segmentation_annotation_classes(
            val_annotations,
            category_id_to_index,
        ),
        test_annotations=_remap_yolo11_segmentation_annotation_classes(
            test_annotations,
            category_id_to_index,
        ),
    )
...
def _load_yolo11_segmentation_split_payload(
    *,
    dataset_storage: LocalDatasetStorage,
    ...
    if not annotation_path.is_file():
        raise InvalidRequestError(
            f"YOLO11 segmentation 标注文件不存在: {annotation_file}"
        )
    payload = dataset_storage.read_json(annotation_file)
    if not isinstance(payload, dict):
        raise InvalidRequestError(
            f"YOLO11 segmentation 标注格式无效: {annotation_file}"
        )
    return payload


def _collect_yolo11_segmentation_categories(
...
def _build_yolo11_segmentation_split_annotations(
    *,
    dataset_storage: LocalDatasetStorage,
    payload: dict[str, object],
    image_root: str,
) -> list[Yolo11SegmentationTrainingAnnotation]:
    """把 COCO annotations 转成 YOLO11 segmentation 训练样本。"""
    ...
    annotations_by_image: dict[int, Yolo11SegmentationTrainingAnnotation] = {
        image_id: Yolo11SegmentationTrainingAnnotation(
    ...
    for annotation_payload in payload.get("annotations") or []:
        ...
        polygons = _extract_yolo11_segmentation_polygons(annotation_payload)
...
def _extract_yolo11_segmentation_polygons(
...
def _build_yolo11_segmentation_label_mapping(
    category_map: dict[int, str],
...
def _remap_yolo11_segmentation_annotation_classes(
    annotations: list[Yolo11SegmentationTrainingAnnotation],
    category_id_to_index: dict[int, int],
) -> list[Yolo11SegmentationTrainingAnnotation]:
    """把原始 category_id 重映射为连续 class index。"""

    remapped: list[Yolo11SegmentationTrainingAnnotation] = []
    for annotation in annotations:
        ...
        if unknown_ids:
            raise InvalidRequestError(
                "YOLO11 segmentation 标注引用了未声明类别",
