...
class Yolo11PoseTrainingManifest:
    ...
    test_annotations: list[Yolo11PoseTrainingAnnotation]
...
def resolve_yolo11_pose_keypoint_shape(
    *,
    ...
    train_annotations: list[Yolo11PoseTrainingAnnotation],
    val_annotations: list[Yolo11PoseTrainingAnnotation],
    test_annotations: list[Yolo11PoseTrainingAnnotation] | None = None,
) -> tuple[int, int]:
...
def _load_pose_annotations(
    *,
    ...
    if not labels:
        raise InvalidRequestError("YOLO11 pose 训练集没有合法类别")
    if not train_annotations:
        raise InvalidRequestError("YOLO11 pose 训练集没有合法样本")
    if not val_annotations:
        raise InvalidRequestError(
            "YOLO11 pose 训练需要独立 validation split，禁止使用 train 或 test 回退"
        )
    return (
        labels,
...
def _remap_pose_categories(
    ...
    remapped: list[Yolo11PoseTrainingAnnotation] = []
