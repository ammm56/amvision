...
class Yolo11PoseTrainingEpochProgress:
    """描述 YOLO11 pose 单轮训练进度。"""
...
@dataclass(frozen=True)
class Yolo11PoseTrainingControlCommand:
    """描述 YOLO11 pose 训练控制命令。"""
...
@dataclass(frozen=True)
class Yolo11PoseTrainingSavePoint:
    """描述 YOLO11 pose 训练保存点。"""
...
@dataclass(frozen=True)
class Yolo11PoseTrainingLoopResult:
    """描述 YOLO11 pose 完整训练循环结果。"""
...
class Yolo11PoseTrainingPausedError(Exception):
    """表示 YOLO11 pose 训练在 epoch 边界暂停。"""


class Yolo11PoseTrainingTerminatedError(Exception):
    """表示 YOLO11 pose 训练在 epoch 边界终止。"""


def run_yolo11_pose_training_loop(
    *,
    imports: Any,
    model: Any,
    optimizer: Any,
    ...
    *,
    ...
    for batch_start in range(0, len(shuffled), batch_size):
        ...
        with autocast_context():
            ...
            loss_payload = compute_yolo11_pose_loss(
                torch=imports.torch,
                model=model,
...
def _run_yolo11_pose_validation(
    *,
    imports: Any,
    model: Any,
    val_annotations: list[Any],
