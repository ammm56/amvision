...
def evaluate_yolo11_detection_validation_losses(
    *,
    torch_module: Any,
    model: Any,
    samples: tuple[Any, ...],
    ...
    assign_alpha: float,
    assign_beta: float,
    dataloader_batches: Iterable[Yolo11DetectionDataLoaderBatch] | None = None,
    device: str | None = None,
    runtime_precision: str = "fp32",
) -> dict[str, float]:
    """在验证集上统计 YOLO11 detection loss。"""

    if not samples:
        return _empty_yolo11_detection_validation_losses()
    ...
    try:
        with torch_module.no_grad():
            if dataloader_batches is None:
                batch_iterator: Iterable[Any] = _iter_yolo11_validation_batches(
                    samples,
                    batch_size,
                )
            else:
            ...
            for batch_samples in batch_iterator:
                if isinstance(batch_samples, Yolo11DetectionDataLoaderBatch):
...
def _iter_yolo11_validation_batches(
    samples: tuple[Any, ...],
    batch_size: int,
) -> Iterable[list[Any]]:
    """按 batch size 迭代 YOLO11 validation 样本。"""
...
def _empty_yolo11_detection_validation_losses() -> dict[str, float]:
    """返回空验证集的 YOLO11 detection loss 摘要。"""

    return {"loss": 0.0, "class_loss": 0.0, "box_loss": 0.0, "dfl_loss": 0.0}


__all__ = ["evaluate_yolo11_detection_validation_losses"]
