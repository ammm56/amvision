...
class Yolo11PoseTrainingImports:
    """描述 YOLO11 pose 训练需要的本地模块。"""
...
def require_yolo11_pose_training_imports() -> Yolo11PoseTrainingImports:
    """导入 YOLO11 pose 训练依赖。"""
    ...
    try:
        ...
        raise ServiceConfigurationError(
            "YOLO11 pose 训练缺少必要依赖",
            details={"missing": str(exc)},
        ) from exc
    return Yolo11PoseTrainingImports(cv2=cv2, np=np, torch=torch)


def resolve_yolo11_pose_training_device(
    *,
    torch_module: Any,
    extra_options: dict[str, object] | None,
) -> str:
    """根据训练参数解析 YOLO11 pose 训练设备。"""
...
def build_yolo11_pose_autocast_context(
    *,
    torch_module: Any,
    precision: str,
    device_name: str,
) -> Any:
    """构建 YOLO11 pose 训练使用的 autocast context。"""
