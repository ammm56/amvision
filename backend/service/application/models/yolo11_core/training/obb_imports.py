...
class Yolo11ObbTrainingImports:
    """描述 YOLO11 OBB 训练需要的本地模块。"""
...
def require_yolo11_obb_training_imports() -> Yolo11ObbTrainingImports:
    """导入 YOLO11 OBB 训练依赖。"""
    ...
    return Yolo11ObbTrainingImports(cv2=cv2, np=np, torch=torch)


def resolve_yolo11_obb_training_device(
    *,
    torch_module: Any,
    extra_options: dict[str, object] | None,
) -> str:
    """根据训练参数解析 YOLO11 OBB 训练设备。"""
...
def build_yolo11_obb_autocast_context(
    *,
    torch_module: Any,
    precision: str,
    device_name: str,
) -> Any:
    """构建 YOLO11 OBB 训练使用的 autocast context。"""
