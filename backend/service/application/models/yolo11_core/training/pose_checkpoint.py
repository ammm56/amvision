"""YOLO11 pose checkpoint 与 resume 规则。"""
...
class Yolo11PoseResumeState:
    """描述 YOLO11 pose resume checkpoint 内容。"""
...
def load_yolo11_pose_resume_state(
    *,
    ...
    """读取 YOLO11 pose resume checkpoint。"""
    ...
    if not isinstance(checkpoint, dict):
        raise InvalidRequestError("YOLO11 pose checkpoint 内容不合法")
    return Yolo11PoseResumeState(
...
def validate_yolo11_pose_resume_parameters(
    state: Yolo11PoseResumeState,
    *,
    batch_size: int,
    max_epochs: int,
    learning_rate: float,
    ...
    if mismatches:
        raise InvalidRequestError(
            "YOLO11 pose resume 参数与 checkpoint 记录不一致",
            details={"mismatches": mismatches},
        )


def restore_yolo11_pose_training_state(
    *,
    model: Any,
    optimizer: Any,
    scheduler: Any,
    ...
    """恢复 YOLO11 pose 模型、optimizer、scheduler 和 scaler 状态。"""

    _load_yolo11_pose_model_state(model=model, state_dict=state.model_state_dict)
...
def build_yolo11_pose_checkpoint_bytes(
    *,
    epoch: int,
    global_iteration: int,
    model: Any,
...
def _load_yolo11_pose_model_state(*, model: Any, state_dict: dict[str, object]) -> None:
    """加载 checkpoint 中 shape 匹配的 YOLO11 pose 权重。"""
