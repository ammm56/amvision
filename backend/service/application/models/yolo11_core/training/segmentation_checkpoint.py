"""YOLO11 segmentation checkpoint 与 resume 规则。"""
...
class Yolo11SegmentationResumeState:
    """描述 YOLO11 segmentation resume checkpoint 内容。"""
...
def load_yolo11_segmentation_resume_state(
    *,
    checkpoint_path: Path,
    torch_module: Any,
) -> Yolo11SegmentationResumeState:
    """读取 YOLO11 segmentation resume checkpoint。"""
    ...
    if not isinstance(checkpoint, dict):
        raise InvalidRequestError("YOLO11 segmentation checkpoint 内容不合法")
    return Yolo11SegmentationResumeState(
...
def validate_yolo11_segmentation_resume_parameters(
    state: Yolo11SegmentationResumeState,
    *,
    batch_size: int,
    max_epochs: int,
    learning_rate: float,
    ...
    if mismatches:
        raise InvalidRequestError(
            "YOLO11 segmentation resume 参数与 checkpoint 记录不一致",
            details={"mismatches": mismatches},
        )


def load_yolo11_segmentation_model_state(
    *,
    model: Any,
    state_dict: dict[str, object],
) -> None:
    """把 checkpoint 里的可匹配权重加载进 YOLO11 segmentation 模型。"""
...
def restore_yolo11_segmentation_training_state(
    *,
    model: Any,
    optimizer: Any,
    scheduler: Any,
    ...
    """恢复 YOLO11 segmentation 模型、optimizer、scheduler 和 scaler 状态。"""
    ...
    _move_yolo11_segmentation_optimizer_state_to_device(
        optimizer=optimizer,
...
def _move_yolo11_segmentation_optimizer_state_to_device(
    *,
    optimizer: Any,
    device_name: str,
) -> None:
    """把 YOLO11 segmentation optimizer state 移到训练设备。"""
...
def build_yolo11_segmentation_checkpoint_bytes(
    *,
    epoch: int,
    global_iteration: int,
    model: Any,
    ...
    """把 YOLO11 segmentation 训练状态编码为 checkpoint bytes。"""
