...
def compute_yolo11_detection_loss(
    *,
    torch_module: Any,
    detect_head: Any,
    ...
    """按 YOLO11 detection 规则计算分类、框回归和 DFL 损失。"""
    ...
    for batch_index, target in enumerate(batch_targets):
        ...
        if target.boxes_xyxy:
            loss_state = _compute_yolo11_image_detection_loss(
                torch_module=torch_module,
...
def yolo11_distribution_focal_loss(
    *,
    torch_module: Any,
    logits: Any,
    target: Any,
...
def _compute_yolo11_image_detection_loss(
    *,
    ...
    """计算单张图片的 YOLO11 detection 正样本损失。"""
    ...
    iou_values = yolo11_box_iou_aligned(
        torch_module=torch_module,
