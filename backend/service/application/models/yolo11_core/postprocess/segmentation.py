...
class Yolo11SegmentationNmsInputArrays:
    """描述单张图片进入 NMS 前的 YOLO11 segmentation 候选结果。"""
...
@dataclass(frozen=True)
class Yolo11SegmentationPostprocessInstance:
    """描述 YOLO11 segmentation 后处理后的单个实例。"""
...
SegmentationPostprocessInstance = Yolo11SegmentationPostprocessInstance


def normalize_yolo11_segmentation_outputs(
    *,
    outputs: object,
    np_module: Any,
    num_classes: int,
) -> tuple[Any, Any]:
    """归一化 YOLO11 segmentation 的 prediction / proto 输出。"""
    ...
    if _is_yolo11_channel_first_prediction(
...
def build_yolo11_segmentation_postprocess_instances(
    *,
    cv2_module: Any,
    np_module: Any,
    ...
    instances: list[Yolo11SegmentationPostprocessInstance] = []
...
def prepare_yolo11_segmentation_nms_inputs_array(
    *,
    image_prediction: Any,
    np_module: Any,
    num_classes: int,
    score_threshold: float,
) -> Yolo11SegmentationNmsInputArrays | None:
    """筛选 YOLO11 segmentation NMS 候选。"""

    _validate_yolo11_segmentation_prediction_channel_count(
        channel_count=int(image_prediction.shape[1]),
        num_classes=num_classes,
    )
    boxes = _convert_yolo11_xywh_to_xyxy(
    ...
    if int(boxes.shape[0]) <= 0:
        return None
    return Yolo11SegmentationNmsInputArrays(
        boxes_xyxy=boxes,
        scores=best_scores,
...
def _convert_yolo11_xywh_to_xyxy(
    *,
    boxes_xywh: Any,
    np_module: Any,
) -> Any:
    """把 YOLO11 export 默认 xywh 转换为 NMS 使用的 xyxy。"""
...
def postprocess_yolo11_segmentation_prediction_array(
    *,
    prediction_array: Any,
    np_module: Any,
    num_classes: int,
    ...
    results: list[Yolo11SegmentationNmsInputArrays | None] = []
...
def decode_yolo11_segmentation_masks(
    *,
    cv2_module: Any,
    np_module: Any,
    proto: Any,
...
def _validate_yolo11_segmentation_prediction_channel_count(
    *,
    channel_count: int,
    num_classes: int,
) -> None:
    """校验 YOLO11 segmentation 预测通道数是否包含 box、类别分数和 mask coeff。"""

    required_min_channels = 5 + int(num_classes)
    if int(channel_count) < required_min_channels:
        raise InvalidRequestError(
            "YOLO11 segmentation 推理输出通道数不足",
            details={
...
def _is_yolo11_channel_first_prediction(
    *,
    prediction_array: Any,
    num_classes: int,
) -> bool:
