...
class Yolo11ExportImports:
    """YOLO11 导出源模型需要的 Python 依赖。"""

    np: Any
    torch: Any


class Yolo11ExportSourceSession:
    """供 ONNX/OpenVINO/TensorRT 转换使用的 YOLO11 PyTorch 源模型会话。"""
    ...
    def load(
        ...
        *,
        ...
        """从 RuntimeTargetSnapshot 加载 YOLO11 导出源模型。"""

        _require_yolo11_export_runtime_target(runtime_target)
        imports = require_yolo11_export_imports()
        model = build_yolo11_model(
            ...
            model_config_overrides=_resolve_yolo11_export_model_config(runtime_target),
        )
        load_yolo11_checkpoint_file(
        ...
        device_name = resolve_yolo11_export_torch_device_name(
            torch_module=imports.torch,
...
def require_yolo11_export_imports() -> Yolo11ExportImports:
    """加载 YOLO11 导出源模型所需依赖。"""
    ...
    try:
        ...
        raise ServiceConfigurationError(
            "当前运行环境缺少 YOLO11 导出所需的 numpy 或 torch 依赖"
        ) from error
    return Yolo11ExportImports(np=np, torch=torch)


def resolve_yolo11_export_torch_device_name(
    *,
    torch_module: Any,
    requested_device_name: str,
) -> str:
    """解析 YOLO11 导出源模型使用的 torch device。"""
    ...
    if requested.startswith("cuda:"):
        ...
        raise InvalidRequestError(
            "当前环境没有可用 CUDA，无法使用 YOLO11 导出 CUDA device",
            details={"device_name": requested_device_name},
        )
    raise InvalidRequestError(
        "YOLO11 导出 device_name 必须是 auto、cpu、cuda、cuda:<index> 或 gpu",
        details={"device_name": requested_device_name},
    )


def enable_yolo11_export_cuda_fast_path(*, torch_module: Any, device_name: str) -> None:
    """为 YOLO11 导出源模型开启 CUDA 快路径。"""
...
def _require_yolo11_export_runtime_target(
    runtime_target: RuntimeTargetSnapshot,
) -> None:
    """校验 YOLO11 导出源模型的 RuntimeTargetSnapshot。"""

    if runtime_target.model_type != "yolo11":
        raise InvalidRequestError(
            "YOLO11 导出源模型只支持 yolo11 model_type",
            details={"model_type": runtime_target.model_type},
        )
    if runtime_target.runtime_backend != "pytorch":
        raise InvalidRequestError(
            "YOLO11 导出源模型只支持 pytorch runtime_backend",
            details={"runtime_backend": runtime_target.runtime_backend},
        )


def _resolve_yolo11_export_model_config(
    runtime_target: RuntimeTargetSnapshot,
) -> dict[str, object] | None:
    """解析 YOLO11 导出源模型需要的模型配置。"""
