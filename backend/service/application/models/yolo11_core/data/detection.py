...
def _prepare_yolo11_detection_sample_without_augmentation(
    *,
    ...
    """按当前输入尺寸直接缩放单张 YOLO11 detection 样本。"""

    image = _load_yolo11_detection_image(imports=imports, sample=sample)
    return _resize_yolo11_detection_sample_to_size(
        imports=imports,
        image=image,
...
def _build_yolo11_detection_mosaic_sample(
    *,
    ...
    """构造一张 2x2 YOLO11 detection Mosaic 样本。"""
    ...
    for sample, (top, left, cell_height, cell_width) in zip(
        ...
        cell_image, cell_boxes, cell_categories = _build_yolo11_detection_scaled_cell(
            imports=imports,
            sample=sample,
...
def _resize_yolo11_detection_sample_to_size(
    *,
    imports: Any,
    image: Any,
    ...
    """把单张 YOLO11 detection 样本直接缩放到目标尺寸。"""
