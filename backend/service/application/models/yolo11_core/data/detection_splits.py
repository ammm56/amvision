"""YOLO11 detection DatasetExport split 解析。"""
...
def resolve_yolo11_detection_splits(
    *,
    dataset_storage: Any,
    cv2_module: Any,
    manifest_payload: dict[str, object],
) -> tuple[Yolo11DetectionResolvedSplit, ...]:
    """从 DatasetExport manifest 解析 YOLO11 detection 可用 split。"""
...
def resolve_yolo11_detection_train_split(
    resolved_splits: tuple[Yolo11DetectionResolvedSplit, ...],
) -> Yolo11DetectionResolvedSplit:
    """优先选择 YOLO11 detection train split。"""
...
def resolve_yolo11_detection_validation_split(
    resolved_splits: tuple[Yolo11DetectionResolvedSplit, ...],
) -> Yolo11DetectionResolvedSplit | None:
    """选择 YOLO11 detection validation split。"""
...
def resolve_yolo11_detection_test_split(
    resolved_splits: tuple[Yolo11DetectionResolvedSplit, ...],
) -> Yolo11DetectionResolvedSplit | None:
    """选择独立的 YOLO11 detection test split。"""
...
def _resolve_coco_detection_splits(
    *,
    ...
    if not isinstance(splits_payload, list):
        raise InvalidRequestError("YOLO11 detection 训练输入 manifest 缺少 splits 定义")
    resolved_splits: list[Yolo11DetectionResolvedSplit] = []
    ...
    if not resolved_splits:
        raise InvalidRequestError("YOLO11 detection 训练输入 manifest 没有可用的 split")
    ...
    *,
    ...
    for split_item in splits_payload:
        ...
        if not label_root_path.is_dir():
            raise InvalidRequestError(
                "YOLO11 detection 训练输入 split 缺少标签目录",
...
def _parse_yolo_detection_label_file(
    *,
    ...
    for line_number, line in enumerate(
        ...
        if class_index < 0 or class_index >= category_count:
            continue
        cx, cy, width, height = (float(value) for value in parts[1:])
        box_width = max(0.0, width * float(image_width))
        box_height = max(0.0, height * float(image_height))
        if box_width <= 0.0 or box_height <= 0.0:
            continue
        x = max(0.0, (cx * float(image_width)) - box_width / 2.0)
        y = max(0.0, (cy * float(image_height)) - box_height / 2.0)
        x2 = min(float(image_width), x + box_width)
        y2 = min(float(image_height), y + box_height)
        clipped_width = max(0.0, x2 - x)
        clipped_height = max(0.0, y2 - y)
        ...
        annotation_rows.append(
            {
                ...
                "bbox": [x, y, clipped_width, clipped_height],
