...
class Yolo11SegmentationTrainingBatch:
    ...
    def pin_memory(self) -> "Yolo11SegmentationTrainingBatch":
...
def build_yolo11_segmentation_training_batch(
    *,
    ...
    if not training and augmentation_options is not None:
        raise ValueError("YOLO11 segmentation validation 不允许随机增强")
    if not samples:
        return None
...
def _prepare_yolo11_segmentation_mixup_sample(
    *,
    ...
    if prepared is None:
        return None
    image, target = prepared
    return _apply_yolo11_segmentation_random_affine(
        imports=imports,
        image=image,
        target=target,
        target_width=target_width,
