...
def build_yolo11_task_augmentation_options(
    extra_options: dict[str, object] | None,
) -> Yolo11TaskAugmentationOptions:
    """从训练 extra_options 构造 YOLO11 task 增强参数。"""
...
def resolve_yolo11_task_augmentation_for_epoch(
    *,
    augmentation_options: Yolo11TaskAugmentationOptions | None,
    epoch_index: int,
    max_epochs: int,
) -> Yolo11TaskAugmentationOptions | None:
    """按当前 epoch 解析实际生效的 YOLO11 task 增强参数。"""
...
def resolve_yolo11_task_batch_input_size(
    *,
...
def should_apply_yolo11_horizontal_flip(flip_prob: float) -> bool:
    """判断当前样本是否执行水平翻转。"""

    return flip_prob > 0.0 and random.random() < flip_prob


def flip_yolo11_image_horizontally(image: Any) -> Any:
    """水平翻转训练图像。"""

    return image[:, ::-1].copy()


def apply_yolo11_random_affine(
    *,
    imports: Any,
    image: Any,
    output_size: tuple[int, int],
    ...
    """按 YOLO11 affine 参数变换图像，并返回同步标注用的矩阵。

    参数中的 ``output_size`` 使用 ``(width, height)``，和本文件内的
    几何矩阵计算保持一致。调用方负责把项目上层的输入尺寸转换清楚。
    """

    if not should_apply_yolo11_random_affine(augmentation_options):
        return image, None, False
    matrix = build_yolo11_random_affine_matrix(
        imports=imports,
...
def should_apply_yolo11_random_affine(
    augmentation_options: Yolo11TaskAugmentationOptions,
) -> bool:
    """判断当前样本是否执行 YOLO11 random affine。"""
...
def build_yolo11_random_affine_matrix(
    *,
    imports: Any,
    ...
    """构造 YOLO11 random affine / perspective 使用的 3x3 矩阵。"""
...
def transform_yolo11_boxes_xyxy(
    *,
    imports: Any,
    ...
    for box_index, box in enumerate(boxes_xyxy):
        ...
        transformed = _transform_yolo11_points(
            imports=imports,
            points=corners,
            matrix=matrix,
            perspective=perspective,
        )
        transformed_box = _clip_yolo11_box_xyxy(
            box_xyxy=[
...
def build_yolo11_boxes_from_masks(
    *,
    imports: Any,
    masks: Any,
...
def transform_yolo11_keypoints(
    *,
    imports: Any,
...
def transform_yolo11_obb_boxes(
    *,
    imports: Any,
...
def build_yolo11_obb_corners(*, imports: Any, box_xywhr: list[float]) -> Any:
...
def resolve_yolo11_pose_flip_indices(
    *,
    keypoint_count: int,
    keypoint_flip_indices: tuple[int, ...] | None,
) -> tuple[int, ...] | None:
    """解析 YOLO11 pose 水平翻转使用的 keypoint 左右交换索引。"""
...
def select_yolo11_items_by_indices(items: list[Any], indices: list[int]) -> list[Any]:
    """按保留索引同步筛选标注列表。"""

    return [items[index] for index in indices if index < len(items)]


def normalize_yolo11_obb_angle(angle: float) -> float:
...
def _min_area_rect_to_yolo11_xywhr(
    *,
    imports: Any,
    points: Any,
    output_size: tuple[int, int],
) -> list[float] | None:
    """把变换后的四角点转换为 YOLO11 OBB ``xywhr``。"""
    ...
    return [
        ...
        float(width),
        float(height),
        normalize_yolo11_obb_angle(math.radians(float(angle_degrees))),
    ]
...
__all__ = [
    ...
    "build_yolo11_boxes_from_masks",
    "build_yolo11_obb_corners",
    "build_yolo11_random_affine_matrix",
