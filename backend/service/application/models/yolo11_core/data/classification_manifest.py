"""YOLO11 classification DatasetExport manifest 解析。"""
...
class Yolo11ClassificationTrainingAnnotation:
    """描述 YOLO11 classification 训练样本。"""
...
@dataclass(frozen=True)
class Yolo11ClassificationTrainingManifest:
    """描述 YOLO11 classification 训练 manifest 的已解析内容。"""
...
def load_yolo11_classification_training_manifest(
    *,
    ...
    """解析 DatasetExport manifest，输出 YOLO11 classification 训练样本。"""

    splits = manifest_payload.get("splits")
    if not isinstance(splits, list) or len(splits) < 1:
        raise InvalidRequestError("YOLO11 classification 训练 manifest 缺少合法 splits")
    ...
    for split in splits:
        ...
        resolved = _load_yolo11_classification_split_annotations(
    ...
    return Yolo11ClassificationTrainingManifest(
        labels=labels,
        train_annotations=_remap_yolo11_classification_annotations(
...
def _load_yolo11_classification_split_annotations(
    *,
    dataset_storage: LocalDatasetStorage,
    ...
    if not ann_path.is_file():
        raise InvalidRequestError(
            f"YOLO11 classification 标注文件不存在: {annotation_file}"
        )
    ann_payload = dataset_storage.read_json(annotation_file)
    if not isinstance(ann_payload, dict):
        raise InvalidRequestError(
            f"YOLO11 classification 标注格式无效: {annotation_file}"
        )

    _collect_yolo11_classification_labels(
        categories=ann_payload.get("categories", []),
        all_labels=all_labels,
    )
    return _resolve_yolo11_classification_annotations(
        dataset_storage=dataset_storage,
...
def _collect_yolo11_classification_labels(
    *,
    categories: Any,
    all_labels: dict[int, str],
) -> None:
...
def _resolve_yolo11_classification_annotations(
    *,
    dataset_storage: LocalDatasetStorage,
    image_root: str,
    images: Any,
    ...
    image_map = _build_yolo11_classification_image_map(images)
    resolved: list[Yolo11ClassificationTrainingAnnotation] = []
...
def _build_yolo11_classification_image_map(images: Any) -> dict[int, str]:
    """构建 image id 到文件名的映射。"""
...
def _build_yolo11_classification_label_mapping(
...
def _remap_yolo11_classification_annotations(
    *,
    annotations: list[Yolo11ClassificationTrainingAnnotation],
