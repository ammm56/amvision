...
def _build_yolo11_obb_mosaic_sample(
    *,
    ...
    for sample, (left, top, cell_width, cell_height) in zip(
        ...
        shifted_target = _offset_yolo11_obb_target(
            target=cell_target, offset_xy=(left, top)
        )
        merged_target = (
            shifted_target
            if merged_target is None
            else _merge_yolo11_obb_targets(primary=merged_target, other=shifted_target)
        )
    if merged_target is None:
        return None
...
def _offset_yolo11_obb_target(
    *,
    target: Yolo11ObbPreparedTarget,
    offset_xy: tuple[int, int],
) -> Yolo11ObbPreparedTarget:
    """把 cell 内 YOLO11 OBB target 平移到 mosaic 画布。"""
