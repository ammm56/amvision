...
def normalize_yolo11_obb_inference_outputs(*, outputs: object, np_module: Any) -> Any:
    """归一化 YOLO11 OBB inference 输出。"""

    if isinstance(outputs, list | tuple):
        if not outputs:
            raise InvalidRequestError("YOLO11 OBB inference 输出为空")
        outputs = outputs[0]
    normalized = outputs
    ...
    if prediction.ndim < 3:
        raise InvalidRequestError(
            "YOLO11 OBB inference 输出维度不合法",
            details={"shape": list(prediction.shape)},
        )
    return prediction


def build_yolo11_obb_inference_instances(
    *,
    np_module: Any,
    ...
    """把 YOLO11 OBB inference 输出转换为 core 实例记录。"""

    return build_yolo11_obb_postprocess_instances(
        np_module=np_module,
