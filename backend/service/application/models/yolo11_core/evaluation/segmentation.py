...
class Yolo11SegmentationEvaluationRequest:
    """描述一次 YOLO11 segmentation 数据集级评估请求。"""
...
@dataclass(frozen=True)
class Yolo11SegmentationEvaluationResult:
    """描述一次 YOLO11 segmentation 数据集级评估结果。"""
...
def run_yolo11_segmentation_evaluation(
    request: Yolo11SegmentationEvaluationRequest,
) -> Yolo11SegmentationEvaluationResult:
    """执行 YOLO11 segmentation 数据集级评估。"""

    dataset_storage = request.dataset_storage
    runtime_target = request.runtime_target
    split_name, samples, label_names = _parse_yolo11_segmentation_manifest(
        request.manifest_payload,
        dataset_storage,
    )
    if not samples:
        return Yolo11SegmentationEvaluationResult(
    ...
    for image_index, sample in enumerate(samples):
        ...
        for annotation in gt_annotations:
            ...
            mask = _build_yolo11_segmentation_annotation_mask(
    ...
    per_class_metrics = _merge_yolo11_segmentation_per_class_metrics(
...
def _parse_yolo11_segmentation_manifest(
    ...
    if annotation_file:
        ...
        if not isinstance(annotation_payload, dict):
            raise InvalidRequestError(
                "YOLO11 segmentation annotation 文件格式无效",
        ...
        return (
            split_name,
            _build_yolo11_segmentation_samples(
    ...
    if label_root:
        category_names = normalize_yolo_category_names(
            ...
            format_label="YOLO11 segmentation",
        )
...
def _build_yolo11_segmentation_samples(
    *,
    image_root: str,
    payload: dict[str, object],
) -> list[dict[str, object]]:
    """把 COCO 风格 images / annotations 组装为 YOLO11 segmentation 评估样本。"""
...
def _build_yolo11_segmentation_annotation_mask(
    *,
    annotation: dict[str, object],
    width: int,
    height: int,
) -> Any:
    """把 YOLO11 segmentation 标注 polygon 转换为二值 mask。"""

    polygons = _normalize_yolo11_segmentation_polygons(annotation.get("segmentation"))
    if not polygons:
        return None
    return _rasterize_yolo11_polygons(polygons=polygons, width=width, height=height)


def _build_yolo11_segmentation_instance_mask(
    *,
    segments: object,
    width: int,
    height: int,
) -> Any:
    """把 YOLO11 segmentation 预测 polygon 转换为二值 mask。"""
...
def _normalize_yolo11_segmentation_polygons(
    segmentation: object,
...
def _rasterize_yolo11_polygons(
    *,
    polygons: list[list[tuple[float, float]]],
    width: int,
    height: int,
...
def _merge_yolo11_segmentation_per_class_metrics(
    *,
...
def evaluate_yolo11_segmentation_samples(
    *,
    model: Any,
    samples: list[Any],
    labels: tuple[str, ...],
...
def _append_yolo11_segmentation_gt_items(
    *,
    image_index: int,
    target: dict[str, Any],
    gt_bbox_items: list[dict[str, object]],
    ...
    if masks is None and target.get("masks") is not None:
        masks = _yolo11_segmentation_tensor_to_np(target.get("masks"), imports)
    mask_valid = (
        _yolo11_segmentation_tensor_to_np(target.get("mask_valid"), imports).astype(
            bool
        )
...
def _build_yolo11_segmentation_prediction_items(
    *,
    ...
    """把 YOLO11 segmentation 输出转成 bbox / mask AP 预测项。"""
...
def _yolo11_segmentation_autocast(imports: Any, precision: str, device: str):
    """返回 YOLO11 segmentation 训练期评估使用的 autocast 上下文。"""
...
def _yolo11_segmentation_tensor_to_np(tensor: Any, imports: Any) -> Any:
    """把 YOLO11 segmentation tensor 输出转换为 NumPy 数组。"""
