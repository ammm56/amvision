...
def select_yolo11_object_segmentation_polygons(
    ...
    *,
    ...
    if not isinstance(segmentations, list) or len(segmentations) == 0:
        return None
    if object_count == 1:
        return _normalize_yolo11_polygon_group(segmentations)
    if object_index >= len(segmentations):
        return None
    return _normalize_yolo11_polygon_group(segmentations[object_index])
...
def _normalize_yolo11_polygon_group(value: Any) -> list[list[float]] | None:
