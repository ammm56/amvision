...
def decode_yolo11_detection_boxes_xywh(
    *,
    raw_outputs: dict[str, torch.Tensor],
    strides: tuple[int, ...],
    dfl_decoder: nn.Module,
) -> torch.Tensor:
    """把 YOLO11 detection head 原始 box 分布解码成 Ultralytics 默认 xywh 边界框。"""
...
def decode_yolo11_detection_boxes_xyxy(
    *,
    raw_outputs: dict[str, torch.Tensor],
    strides: tuple[int, ...],
    dfl_decoder: nn.Module,
) -> torch.Tensor:
    """把 YOLO11 detection head 原始 box 分布解码成训练和 NMS 使用的 xyxy 边界框。"""
...
def build_yolo11_detection_prediction(
    *,
    raw_outputs: dict[str, torch.Tensor],
    strides: tuple[int, ...],
    dfl_decoder: nn.Module,
) -> torch.Tensor:
    """把 YOLO11 detection head 原始输出组装成推理预测张量。"""

    decoded_boxes = decode_yolo11_detection_boxes_xywh(
...
def decode_yolo11_detection_training_predictions(
    *,
    torch_module: Any,
    detect_head: Any,
    raw_outputs: dict[str, Any],
) -> dict[str, Any]:
    """把 YOLO11 detection 训练态 raw outputs 解码为 loss 输入。"""
