...
YOLOV8_DEFAULT_INPUT_SIZE = (640, 640)
YOLOV8_DEFAULT_BATCH_SIZE = 1
YOLOV8_DEFAULT_MAX_EPOCHS = 1
YOLOV8_DEFAULT_EVALUATION_INTERVAL = 5


@dataclass(frozen=True)
class YoloV8TrainingBatchProgress:
...
@dataclass(frozen=True)
class YoloV8TrainingEpochProgress:
    """描述单轮训练结束后的进度快照。"""
...
class YoloV8DetectionTrainingExecutionRequest:
    """描述一次 YOLOv8 detection bootstrap 训练执行请求。"""
    ...
    batch_callback: Callable[[YoloV8TrainingBatchProgress], None] | None = None
    epoch_callback: Callable[[YoloV8TrainingEpochProgress], None] | None = None


@dataclass(frozen=True)
class YoloV8DetectionTrainingExecutionResult:
    """描述一次 YOLOv8 detection bootstrap 训练执行结果。"""
...
class _TrainingImports:
    """描述 YOLOv8 bootstrap 训练所需的第三方依赖对象。"""
...
def run_yolov8_detection_training(
    ...
    """执行一轮项目内 YOLOv8 detection bootstrap 训练。"""
    ...
    batch_size = max(1, int(request.batch_size or YOLOV8_DEFAULT_BATCH_SIZE))
    max_epochs = max(1, int(request.max_epochs or YOLOV8_DEFAULT_MAX_EPOCHS))
    evaluation_interval = max(1, int(request.evaluation_interval or YOLOV8_DEFAULT_EVALUATION_INTERVAL))
...
def _resolve_input_size(input_size: tuple[int, int] | None) -> tuple[int, int]:
    ...
    if input_size is None:
        return YOLOV8_DEFAULT_INPUT_SIZE
