...
class Yolo26ObbTaskExecutionRequest(Protocol):
    """描述平台传入 YOLO26 OBB 训练执行层的字段集合。"""
...
def run_yolo26_obb_training_from_task_request(
    request: Yolo26ObbTaskExecutionRequest,
) -> Yolo26ObbTrainingExecutionResult:
    """把平台 OBB task request 转成 YOLO26 专属训练请求。"""

    return run_yolo26_obb_training(
        Yolo26ObbTrainingExecutionRequest(
...
def _build_yolo26_epoch_callback(request: Yolo26ObbTaskExecutionRequest):
...
def _build_yolo26_savepoint_callback(request: Yolo26ObbTaskExecutionRequest):
    """把平台 savepoint callback 适配成 YOLO26 callback。"""

    if request.savepoint_callback is None:
        return None

    def on_yolo26_savepoint(savepoint: Yolo26ObbTrainingSavePoint) -> None:
