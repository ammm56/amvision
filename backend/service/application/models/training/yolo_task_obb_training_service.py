...
from backend.service.application.models.training.yolo_task_obb_training_control import (
    YoloTaskObbTrainingControlState,
    build_yolo_task_obb_training_control_metadata,
    clear_yolo_task_obb_manual_save_request,
    read_yolo_task_obb_training_control_state,
)
from backend.service.application.models.training.yolo_task_obb_training_dataset import (
    resolve_yolo_task_obb_training_dataset_export,
)
from backend.service.application.models.training.yolo_task_obb_training_events import (
    build_yolo_task_obb_training_cancelled_event,
    build_yolo_task_obb_training_failed_event,
    build_yolo_task_obb_training_paused_event,
    build_yolo_task_obb_training_started_event,
    build_yolo_task_obb_training_succeeded_event,
)
from backend.service.application.models.training.yolo_task_obb_training_payload import (
    build_yolo_task_obb_training_create_task_metadata,
    build_yolo_task_obb_training_queue_payload,
    build_yolo_task_obb_training_task_spec,
    read_yolo_task_obb_training_payload,
)
from backend.service.application.models.training.yolo_task_obb_training_registration import (
    YOLO_TASK_OBB_MODEL_SERVICE_MAP,
    resolve_yolo_task_obb_implementation_mode,
    register_yolo_task_obb_training_output_model_version,
)
from backend.service.application.models.training.yolo_task_obb_training import (
    YOLO_TASK_OBB_DEFAULT_EVALUATION_INTERVAL,
    YoloTaskObbTrainingControlCommand,
    YoloTaskObbTrainingEpochProgress,
    YoloTaskObbTrainingPausedError,
    YoloTaskObbTrainingSavePoint,
    YoloTaskObbTrainingTerminatedError,
    YoloTaskObbTrainingExecutionRequest,
...
YOLO_TASK_OBB_TRAINING_TASK_KIND = "obb-training"
YOLO_TASK_OBB_TRAINING_QUEUE_NAME = "obb-trainings"
YOLO_TASK_OBB_TRAINING_CONTROL_METADATA_KEY = "obb_training_control"
YOLO_TASK_OBB_TRAINING_SUPPORTED_MODEL_TYPES = (
    *tuple(YOLO_TASK_OBB_MODEL_SERVICE_MAP.keys()),
)


@dataclass(frozen=True)
class YoloTaskObbTrainingRequest:
    """描述一次 OBB 训练任务创建请求。"""
...
class SqlAlchemyYoloTaskObbTrainingService:
    """管理 YOLO 主线 OBB 训练任务的完整生命周期。"""

    task_type = OBB_TASK_TYPE
    model_label = "YOLO task obb"
    training_task_kind = YOLO_TASK_OBB_TRAINING_TASK_KIND
    training_queue_name = YOLO_TASK_OBB_TRAINING_QUEUE_NAME
    ...
    def submit_training_task(
        self,
        request: YoloTaskObbTrainingRequest,
        *,
    ...
    def process_training_task(
        ...
        *,
        ...
        def on_epoch(
            progress: YoloTaskObbTrainingEpochProgress,
        ) -> YoloTaskObbTrainingControlCommand | None:
            control_state = self._read_control_state(task_record.task_id)
            if control_state.terminate_requested:
                return YoloTaskObbTrainingControlCommand(
            ...
            if control_state.save_requested:
                ...
                return YoloTaskObbTrainingControlCommand(save_checkpoint=True)
            return None

        def on_savepoint(savepoint: YoloTaskObbTrainingSavePoint) -> None:
        ...
        request = YoloTaskObbTrainingExecutionRequest(
            ...
            evaluation_interval=int(
                ...
                or YOLO_TASK_OBB_DEFAULT_EVALUATION_INTERVAL
            ),
        ...
        try:
            ...
            self.task_service.append_task_event(
                build_yolo_task_obb_training_cancelled_event(
                    ...
                    control_metadata_key=YOLO_TASK_OBB_TRAINING_CONTROL_METADATA_KEY,
                )
            )
    ...
    def _normalize_model_type(self, model_type: object) -> str:
        ...
        if normalized not in YOLO_TASK_OBB_TRAINING_SUPPORTED_MODEL_TYPES:
            raise InvalidRequestError(
                "当前 obb 训练不支持指定模型分类",
                details={
                    "model_type": normalized,
                    "supported": YOLO_TASK_OBB_TRAINING_SUPPORTED_MODEL_TYPES,
                },
            )
        return normalized
    ...
    def _build_task_spec(
        ...
        *,
        ...
        return build_yolo_task_obb_training_task_spec(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
        )
    ...
    def _build_create_task_metadata(
        ...
        *,
        ...
        return build_yolo_task_obb_training_create_task_metadata(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
            task_spec=task_spec,
    ...
    def _build_queue_payload(
        ...
        *,
        ...
        return build_yolo_task_obb_training_queue_payload(
            task_id=task_id,
            task_kind=task_kind,
            task_spec=task_spec,
        )
    ...
    def _read_task_payload(self, task_record: TaskRecord) -> dict[str, object]:
        ...
        return read_yolo_task_obb_training_payload(task_record)
    ...
    def _resolve_dataset_export(
        ...
        *,
        ...
        return resolve_yolo_task_obb_training_dataset_export(
    ...
    def _build_training_summary(
        ...
        *,
        task_record: TaskRecord,
        dataset_export: DatasetExport,
        execution_result: YoloTaskObbTrainingExecutionResult,
    ...
    def _register_training_output_model_version(
        ...
        *,
        ...
        return register_yolo_task_obb_training_output_model_version(
            session_factory=self.session_factory,
            task_record=task_record,
            dataset_export=dataset_export,
            payload=payload,
    ...
    def _resolve_implementation_mode(model_type: str) -> str:
        ...
        return resolve_yolo_task_obb_implementation_mode(model_type)
    ...
    def _read_control_state(self, task_id: str) -> YoloTaskObbTrainingControlState:
        """从任务 metadata 中读取最新控制状态。"""

        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        return read_yolo_task_obb_training_control_state(
            metadata=metadata,
            control_metadata_key=YOLO_TASK_OBB_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _clear_manual_save_request(self, task_id: str) -> None:
        ...
        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        updated_metadata = clear_yolo_task_obb_manual_save_request(
            metadata=metadata,
            control_metadata_key=YOLO_TASK_OBB_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _set_control_flag(
        ...
        updated_metadata = build_yolo_task_obb_training_control_metadata(
            metadata=metadata,
            control_metadata_key=YOLO_TASK_OBB_TRAINING_CONTROL_METADATA_KEY,
            flag=flag,
            value=value,
