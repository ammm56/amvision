...
def resolve_yolo11_segmentation_training_dataset_export(
    *,
    ...
    """根据训练请求解析可用于 YOLO11 segmentation 训练的 DatasetExport。"""

    export_by_id = None
    if dataset_export_id is not None:
        export_by_id = _get_yolo11_segmentation_dataset_export(
    ...
    if dataset_export_manifest_key is not None:
        export_by_manifest = _get_yolo11_segmentation_dataset_export_by_manifest(
            session_factory=session_factory,
    ...
    if dataset_export is None:
        raise ResourceNotFoundError(
            "找不到可用于 YOLO11 segmentation 训练的 DatasetExport"
        )
    if (
    ...
    require_supported_dataset_export_format(
        ...
        dataset_export_id=dataset_export.dataset_export_id,
        unsupported_message="YOLO11 segmentation 训练只接受 YOLO11 支持的 segmentation 导出格式",
    )
    return dataset_export


def _get_yolo11_segmentation_dataset_export(
    *,
    session_factory: SessionFactory,
    dataset_export_id: str,
) -> DatasetExport:
    """按 id 读取一个 YOLO11 segmentation DatasetExport。"""
...
def _get_yolo11_segmentation_dataset_export_by_manifest(
    *,
    session_factory: SessionFactory,
    manifest_object_key: str,
) -> DatasetExport:
    """按 manifest object key 读取一个 YOLO11 segmentation DatasetExport。"""
...
__all__ = ["resolve_yolo11_segmentation_training_dataset_export"]
