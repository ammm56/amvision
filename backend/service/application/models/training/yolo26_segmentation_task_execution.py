...
class Yolo26SegmentationTaskExecutionRequest(Protocol):
    """描述平台传入 YOLO26 segmentation 训练执行层的字段集合。"""
...
def run_yolo26_segmentation_training_from_task_request(
    request: Yolo26SegmentationTaskExecutionRequest,
) -> Yolo26SegmentationTrainingExecutionResult:
    """把平台 segmentation task request 转成 YOLO26 专属训练请求。"""

    return run_yolo26_segmentation_training(
        Yolo26SegmentationTrainingExecutionRequest(
...
def _build_yolo26_savepoint_callback(
    ...
    def on_yolo26_savepoint(savepoint: Yolo26SegmentationTrainingSavePoint) -> None:
