...
YOLO_TASK_CLASSIFICATION_IMPLEMENTATION_MODE = "yolo-task-classification"
YOLO_TASK_CLASSIFICATION_DEFAULT_INPUT_SIZE = (224, 224)
YOLO_TASK_CLASSIFICATION_DEFAULT_BATCH_SIZE = 16
YOLO_TASK_CLASSIFICATION_DEFAULT_MAX_EPOCHS = 30
YOLO_TASK_CLASSIFICATION_DEFAULT_EVALUATION_INTERVAL = 1
YOLO_TASK_CLASSIFICATION_DEFAULT_LR = 1e-3
YOLO_TASK_CLASSIFICATION_DEFAULT_WEIGHT_DECAY = 1e-4
YOLO_TASK_CLASSIFICATION_DEFAULT_MIN_LR_RATIO = 0.01


@dataclass(frozen=True)
class YoloTaskClassificationTrainingBatchProgress:
    epoch: int
    max_epochs: int
    iteration: int
    max_iterations: int
...
@dataclass(frozen=True)
class YoloTaskClassificationTrainingEpochProgress:
    epoch: int
    max_epochs: int
    input_size: tuple[int, int]
    learning_rate: float
...
@dataclass(frozen=True)
class YoloTaskClassificationTrainingSavePoint:
...
@dataclass(frozen=True)
class YoloTaskClassificationTrainingControlCommand:
    save_checkpoint: bool = False
    pause_training: bool = False
    terminate_training: bool = False


class YoloTaskClassificationTrainingPausedError(Exception):
    """训练被显式暂停。"""


class YoloTaskClassificationTrainingTerminatedError(Exception):
    """训练被显式终止。"""
...
@dataclass(frozen=True)
class YoloTaskClassificationTrainingExecutionRequest:
    dataset_storage: LocalDatasetStorage
    manifest_payload: dict[str, object]
    model_type: str
    model_scale: str
    batch_size: int = YOLO_TASK_CLASSIFICATION_DEFAULT_BATCH_SIZE
    max_epochs: int = YOLO_TASK_CLASSIFICATION_DEFAULT_MAX_EPOCHS
    evaluation_interval: int = YOLO_TASK_CLASSIFICATION_DEFAULT_EVALUATION_INTERVAL
    ...
    savepoint_callback: (
        Callable[[YoloTaskClassificationTrainingSavePoint], None] | None
    ) = None


@dataclass(frozen=True)
class YoloTaskClassificationTrainingExecutionResult:
    best_metric_value: float
    best_metric_name: str
    latest_checkpoint_bytes: bytes
...
def run_yolo_task_classification_training(
    ...
    input_size = request.input_size or YOLO_TASK_CLASSIFICATION_DEFAULT_INPUT_SIZE
    ...
    learning_rate = float(
        extra.get("learning_rate", YOLO_TASK_CLASSIFICATION_DEFAULT_LR)
    )
    weight_decay = float(
        extra.get(
            "weight_decay",
            YOLO_TASK_CLASSIFICATION_DEFAULT_WEIGHT_DECAY,
        )
    )
    min_lr_ratio = float(
        extra.get(
            "min_lr_ratio",
            YOLO_TASK_CLASSIFICATION_DEFAULT_MIN_LR_RATIO,
        )
    )
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        epoch_progress = YoloTaskClassificationTrainingEpochProgress(
            epoch=epoch,
            max_epochs=max_epochs,
            input_size=input_size,
        ...
        if cmd is not None and cmd.terminate_training:
            raise YoloTaskClassificationTrainingTerminatedError()
        ...
        if cmd is not None and cmd.pause_training:
            raise YoloTaskClassificationTrainingPausedError()
