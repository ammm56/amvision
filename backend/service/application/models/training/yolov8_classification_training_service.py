"""YOLOv8 classification 训练任务服务入口。"""
...
from backend.service.application.models.training.yolov8_classification_training_events import (
    ...
    build_yolov8_classification_training_queued_event,
    build_yolov8_classification_training_started_event,
    build_yolov8_classification_training_succeeded_event,
)
from backend.service.application.models.training.yolov8_classification_training_control import (
    YoloV8ClassificationTrainingControlState,
    build_yolov8_classification_training_control_metadata,
    clear_yolov8_classification_manual_save_request,
    read_yolov8_classification_training_control_state,
)
from backend.service.application.models.training.yolov8_classification_training_payload import (
    build_yolov8_classification_training_create_task_metadata,
    build_yolov8_classification_training_queue_payload,
    build_yolov8_classification_training_task_spec,
    read_yolov8_classification_training_payload,
)
from backend.service.application.models.training.yolov8_classification_training_registration import (
...
class YoloV8ClassificationTrainingRequest:
...
class SqlAlchemyYoloV8ClassificationTrainingService:
    """管理 YOLOv8 classification 训练任务的完整生命周期。"""

    task_type = CLASSIFICATION_TASK_TYPE
    model_label = "YOLOv8 classification"
    training_task_kind = YOLOV8_CLASSIFICATION_TRAINING_TASK_KIND
    training_queue_name = YOLOV8_CLASSIFICATION_TRAINING_QUEUE_NAME
    ...
    def submit_training_task(
        self,
        request: YoloV8ClassificationTrainingRequest,
        *,
    ...
    def _build_task_spec(
        ...
        *,
        ...
        return build_yolov8_classification_training_task_spec(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
        )
    ...
    def _build_create_task_metadata(
        ...
        *,
        ...
        return build_yolov8_classification_training_create_task_metadata(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
            task_spec=task_spec,
    ...
    def _build_queue_payload(
        ...
        *,
        ...
        return build_yolov8_classification_training_queue_payload(
            task_id=task_id,
            task_kind=task_kind,
            task_spec=task_spec,
        )
    ...
    def _read_task_payload(self, task_record: TaskRecord) -> dict[str, object]:
        ...
        return read_yolov8_classification_training_payload(task_record)

    def _run_training_execution(
        self,
        request: YoloV8ClassificationTrainingExecutionRequest,
    ) -> YoloV8ClassificationTrainingExecutionResult:
        """执行 YOLOv8 classification 训练。"""

        return run_yolov8_classification_training(request)

    @staticmethod
    def _terminated_error_types() -> tuple[type[BaseException], ...]:
        """返回应按取消处理的训练异常类型。"""

        return (YoloV8ClassificationTrainingTerminatedError,)

    @staticmethod
    def _paused_error_types() -> tuple[type[BaseException], ...]:
        """返回应按暂停处理的训练异常类型。"""

        return (YoloV8ClassificationTrainingPausedError,)
    ...
    def _resolve_dataset_export(
        ...
        *,
        ...
        return resolve_yolov8_classification_training_dataset_export(
            session_factory=self.session_factory,
    ...
    def _build_training_summary(
        ...
        *,
        task_record: TaskRecord,
        dataset_export: DatasetExport,
        execution_result: YoloV8ClassificationTrainingExecutionResult,
    ...
    def _register_training_output_model_version(
        ...
        *,
        ...
        return register_yolov8_classification_training_output_model_version(
            session_factory=self.session_factory,
            task_record=task_record,
            dataset_export=dataset_export,
            payload=payload,
    ...
    def _resolve_implementation_mode(model_type: str) -> str:
        ...
        return YOLOV8_CLASSIFICATION_IMPLEMENTATION_MODE
    ...
    def _read_control_state(
        ...
        return read_yolov8_classification_training_control_state(
            metadata=metadata,
            control_metadata_key=YOLOV8_CLASSIFICATION_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _clear_manual_save_request(self, task_id: str) -> None:
        ...
        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        updated_metadata = clear_yolov8_classification_manual_save_request(
            metadata=metadata,
            control_metadata_key=YOLOV8_CLASSIFICATION_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _set_control_flag(
        ...
        updated_metadata = build_yolov8_classification_training_control_metadata(
            metadata=metadata,
            control_metadata_key=YOLOV8_CLASSIFICATION_TRAINING_CONTROL_METADATA_KEY,
            flag=flag,
            value=value,
