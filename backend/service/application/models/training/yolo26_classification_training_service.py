"""YOLO26 classification 训练任务服务入口。"""
...
from backend.service.application.models.training.yolo26_classification_task_control import (
    Yolo26ClassificationTrainingControlState,
    build_yolo26_classification_training_control_metadata,
    clear_yolo26_classification_manual_save_request,
    read_yolo26_classification_training_control_state,
)
from backend.service.application.models.training.yolo26_classification_task_dataset import (
    resolve_yolo26_classification_training_dataset_export,
)
from backend.service.application.models.training.yolo26_classification_task_events import (
    build_yolo26_classification_training_cancelled_event,
    build_yolo26_classification_training_failed_event,
    build_yolo26_classification_training_paused_event,
    build_yolo26_classification_training_queue_failed_event,
    build_yolo26_classification_training_queued_event,
    build_yolo26_classification_training_started_event,
    build_yolo26_classification_training_succeeded_event,
)
from backend.service.application.models.training.yolo26_classification_task_service import (
    build_yolo26_classification_service_create_task_metadata,
    build_yolo26_classification_service_queue_payload,
    build_yolo26_classification_service_task_spec,
    get_yolo26_classification_paused_error_types,
    get_yolo26_classification_terminated_error_types,
    read_yolo26_classification_service_task_payload,
    register_yolo26_classification_service_training_output,
    resolve_yolo26_classification_service_implementation_mode,
...
YOLO26_CLASSIFICATION_DEFAULT_EVALUATION_INTERVAL = 5


@dataclass(frozen=True)
class Yolo26ClassificationTrainingTaskRequest:
    """描述一次 YOLO26 classification 训练任务创建请求。"""
...
class SqlAlchemyYolo26ClassificationTrainingTaskService:
    """管理 YOLO26 classification 训练任务的 service 层入口。"""

    task_type = CLASSIFICATION_TASK_TYPE
    model_label = "YOLO26 classification"
    training_task_kind = YOLO26_CLASSIFICATION_TRAINING_TASK_KIND
    training_queue_name = YOLO26_CLASSIFICATION_TRAINING_QUEUE_NAME
    ...
    def __init__(
        ...
        *,
        ...
        """初始化 YOLO26 classification 训练任务服务。"""
    ...
    def submit_training_task(
        self,
        request: Yolo26ClassificationTrainingTaskRequest,
        *,
        created_by: str | None = None,
    ) -> dict[str, object]:
        """创建 YOLO26 classification 训练任务并入队。"""
    ...
    def process_training_task(
        ...
        *,
        ...
        """执行 YOLO26 classification 训练工作负载。"""
        ...
        if manifest_object_key is None or not manifest_object_key.strip():
            raise InvalidRequestError(
                "YOLO26 classification 训练任务缺少 manifest_object_key",
        ...
        if not isinstance(manifest_payload, dict):
            raise InvalidRequestError("YOLO26 classification 训练 manifest 无效")
        ...
        def on_epoch(
            ...
            if control_state.save_requested:
                ...
                return Yolo26ClassificationTrainingControlCommand(save_checkpoint=True)
            return None

        def on_savepoint(savepoint: Yolo26ClassificationTrainingSavePoint) -> None:
        ...
        request = Yolo26ClassificationTrainingExecutionRequest(
            ...
            evaluation_interval=int(
                ...
                or YOLO26_CLASSIFICATION_DEFAULT_EVALUATION_INTERVAL
            ),
    ...
    def _normalize_model_type(self, model_type: object) -> str:
        ...
        return require_yolo26_model_type(model_type, task_type="classification")
    ...
    def _build_task_spec(
        ...
        *,
        ...
        """构建 YOLO26 classification 训练任务规格快照。"""

        return build_yolo26_classification_service_task_spec(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
        )
    ...
    def _build_create_task_metadata(
        ...
        *,
        ...
        """构建 YOLO26 classification TaskRecord metadata。"""

        return build_yolo26_classification_service_create_task_metadata(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
            task_spec=task_spec,
    ...
    def _build_queue_payload(
        ...
        *,
        ...
        """构建 YOLO26 classification 队列负载。"""

        return build_yolo26_classification_service_queue_payload(
            task_id=task_id,
            task_kind=task_kind,
            task_spec=task_spec,
        )
    ...
    def _read_task_payload(self, task_record: TaskRecord) -> dict[str, object]:
        """从任务记录中解析 YOLO26 classification 训练负载。"""

        return read_yolo26_classification_service_task_payload(task_record)

    def _run_training_execution(
        self,
        request: Yolo26ClassificationTrainingExecutionRequest,
    ) -> Yolo26ClassificationTrainingExecutionResult:
        """执行 YOLO26 classification 训练。"""

        return run_yolo26_classification_service_training_execution(request)

    @staticmethod
    def _terminated_error_types() -> tuple[type[BaseException], ...]:
        """返回 YOLO26 classification 应按取消处理的异常类型。"""

        return get_yolo26_classification_terminated_error_types()

    @staticmethod
    def _paused_error_types() -> tuple[type[BaseException], ...]:
        """返回 YOLO26 classification 应按暂停处理的异常类型。"""

        return get_yolo26_classification_paused_error_types()
    ...
    def _resolve_dataset_export(
        ...
        *,
        ...
        """根据 id 或 manifest key 解析 YOLO26 classification 训练输入。"""

        return resolve_yolo26_classification_training_dataset_export(
            session_factory=self.session_factory,
    ...
    def _resolve_resume_checkpoint_path(self, task_record: TaskRecord) -> Path | None:
        """为 paused 的 YOLO26 classification 训练任务解析 resume checkpoint 路径。"""
    ...
    def _build_training_summary(
        ...
        *,
        task_record: TaskRecord,
        dataset_export: DatasetExport,
        execution_result: Yolo26ClassificationTrainingExecutionResult,
    ...
    def _register_training_output_model_version(
        ...
        *,
        ...
        """把 YOLO26 classification 训练输出登记为 ModelVersion。"""

        return register_yolo26_classification_service_training_output(
            session_factory=self.session_factory,
            task_record=task_record,
            dataset_export=dataset_export,
            payload=payload,
    ...
    def _resolve_implementation_mode(model_type: str) -> str:
        """返回 YOLO26 classification 训练实现标记。"""

        return resolve_yolo26_classification_service_implementation_mode(model_type)
    ...
    def _read_control_state(
        ...
        """从任务 metadata 中读取最新 YOLO26 classification 控制状态。"""

        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        return read_yolo26_classification_training_control_state(metadata=metadata)
    ...
    def _clear_manual_save_request(self, task_id: str) -> None:
        ...
        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        updated_metadata = clear_yolo26_classification_manual_save_request(
            metadata=metadata
        )
    ...
    def _set_control_flag(
        ...
        """设置 YOLO26 classification 训练控制标记。"""

        metadata = dict(task_record.metadata) if task_record.metadata else {}
        updated_metadata = build_yolo26_classification_training_control_metadata(
            metadata=metadata,
            flag=flag,
            value=value,
        )
