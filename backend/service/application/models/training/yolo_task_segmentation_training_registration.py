...
from backend.service.application.models.training.yolo_task_segmentation_training import (
    YOLO_TASK_SEGMENTATION_IMPLEMENTATION_MODE,
)
...
YOLO_TASK_SEGMENTATION_MODEL_SERVICE_MAP: dict[str, tuple[type, type]] = {
...
class YoloTaskSegmentationTrainingResultLike(Protocol):
    """描述登记 segmentation 训练输出所需的执行结果字段。"""

    labels: tuple[str, ...]


def resolve_yolo_task_segmentation_implementation_mode(model_type: str) -> str:
    """按模型分类返回 segmentation 训练实现标记。"""

    if model_type == "rfdetr":
        return RFDETR_SEGMENTATION_IMPLEMENTATION_MODE
    return YOLO_TASK_SEGMENTATION_IMPLEMENTATION_MODE


def register_yolo_task_segmentation_training_output_model_version(
    *,
    ...
    model_type: str,
    execution_result: YoloTaskSegmentationTrainingResultLike,
    ...
    service_cls, registration_cls = YOLO_TASK_SEGMENTATION_MODEL_SERVICE_MAP[
        model_type
    ]
    ...
    return model_service.register_training_output(
        registration_cls(
            ...
            metadata={
                ...
                "implementation_mode": resolve_yolo_task_segmentation_implementation_mode(
                    model_type
                ),
            },
        )
