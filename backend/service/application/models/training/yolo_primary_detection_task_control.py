"""YOLO 主线 detection 训练任务控制状态工具。"""

from __future__ import annotations


def read_yolo_primary_detection_training_control(
    *,
...
def read_yolo_primary_detection_training_control_flag(
...
def read_yolo_primary_detection_training_control_counter(
    control: dict[str, object],
    key: str,
) -> int:
    """从训练控制字典中读取非负计数器。
...
def build_requested_yolo_primary_detection_training_control(
    *,
    control: dict[str, object],
    save_requested: bool,
...
def build_requested_yolo_primary_detection_training_terminate_control(
    *,
    control: dict[str, object],
    requested_by: str | None,
    requested_at: str,
    ...
    updated_control = clear_yolo_primary_detection_training_control_requests(control)
...
def build_yolo_primary_detection_training_resume_control(
    *,
    control: dict[str, object],
    ...
    updated_control["resume_count"] = (
        read_yolo_primary_detection_training_control_counter(control, "resume_count") + 1
    )
    return updated_control


def clear_yolo_primary_detection_training_control_requests(
    control: dict[str, object],
) -> dict[str, object]:
    """清理训练控制字典中的一次性请求字段。
...
def mark_yolo_primary_detection_training_control_saved(
    *,
    control: dict[str, object],
    saved_at: str,
    saved_epoch: int,
...
def resolve_yolo_primary_detection_resume_checkpoint_object_key(
    *,
    metadata: dict[str, object],
    ...
    if isinstance(result_checkpoint_object_key, str) and result_checkpoint_object_key.strip():
