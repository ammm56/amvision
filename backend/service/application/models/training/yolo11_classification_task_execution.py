"""YOLO11 classification 训练任务执行适配器。"""
...
class Yolo11ClassificationTaskExecutionRequest(Protocol):
    """描述平台传入 YOLO11 classification 训练执行层的字段集合。"""
...
def run_yolo11_classification_training_from_task_request(
    request: Yolo11ClassificationTaskExecutionRequest,
) -> Yolo11ClassificationTrainingExecutionResult:
    """把平台 classification task request 转成 YOLO11 专属训练请求。"""

    return run_yolo11_classification_training(
        Yolo11ClassificationTrainingExecutionRequest(
...
def _build_yolo11_savepoint_callback(
    ...
    def on_yolo11_savepoint(savepoint: Yolo11ClassificationTrainingSavePoint) -> None:
