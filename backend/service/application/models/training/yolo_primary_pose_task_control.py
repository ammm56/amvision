...
class YoloPrimaryPoseTrainingControlState:
    """描述 pose 训练控制状态快照。"""
...
def read_yolo_primary_pose_training_control_state(
    *,
    metadata: dict[str, object],
    control_metadata_key: str,
) -> YoloPrimaryPoseTrainingControlState:
    """从任务 metadata 中读取 pose 训练控制状态。"""

    raw_control = metadata.get(control_metadata_key)
    if not isinstance(raw_control, dict):
        return YoloPrimaryPoseTrainingControlState()
    return YoloPrimaryPoseTrainingControlState(
...
def clear_yolo_primary_pose_manual_save_request(
    *,
    metadata: dict[str, object],
    control_metadata_key: str,
) -> dict[str, object] | None:
    """清理 pose 训练的一次性手动保存请求。"""
...
def build_yolo_primary_pose_training_control_metadata(
    *,
    metadata: dict[str, object],
    control_metadata_key: str,
    flag: str,
