"""YOLO 主线 segmentation 训练数据集解析工具。"""
...
def resolve_yolo_primary_segmentation_training_dataset_export(
    *,
    ...
    """根据训练请求解析可用于 segmentation 训练的 DatasetExport。
    ...
    - model_type：当前模型分类，例如 yolov8、yolo11、yolo26、rfdetr。
    """

    export_by_id = None
    if dataset_export_id is not None:
        export_by_id = _get_yolo_primary_segmentation_dataset_export(
    ...
    if dataset_export_manifest_key is not None:
        export_by_manifest = _get_yolo_primary_segmentation_dataset_export_by_manifest(
            session_factory=session_factory,
...
def _get_yolo_primary_segmentation_dataset_export(
    *,
    session_factory: SessionFactory,
    dataset_export_id: str,
) -> DatasetExport:
    """按 id 读取一个 segmentation DatasetExport。"""
...
def _get_yolo_primary_segmentation_dataset_export_by_manifest(
    *,
    session_factory: SessionFactory,
    manifest_object_key: str,
) -> DatasetExport:
    """按 manifest object key 读取一个 segmentation DatasetExport。"""
