...
class Yolo11SegmentationTrainingControlState:
    """描述 YOLO11 segmentation 训练控制状态快照。"""
...
def read_yolo11_segmentation_training_control_state(
    *,
    metadata: dict[str, object],
) -> Yolo11SegmentationTrainingControlState:
    """从任务 metadata 中读取 YOLO11 segmentation 训练控制状态。"""

    raw_control = metadata.get(YOLO11_SEGMENTATION_TRAINING_CONTROL_METADATA_KEY)
    if not isinstance(raw_control, dict):
        return Yolo11SegmentationTrainingControlState()
    return Yolo11SegmentationTrainingControlState(
...
def clear_yolo11_segmentation_manual_save_request(
    *,
    metadata: dict[str, object],
) -> dict[str, object] | None:
    """清理 YOLO11 segmentation 训练的一次性手动保存请求。"""
    ...
    updated_metadata[YOLO11_SEGMENTATION_TRAINING_CONTROL_METADATA_KEY] = (
        updated_control
    )
    return updated_metadata


def build_yolo11_segmentation_training_control_metadata(
    *,
    metadata: dict[str, object],
    flag: str,
    value: bool,
) -> dict[str, object]:
    """设置 YOLO11 segmentation 训练控制标记并返回新的 metadata。"""
    ...
    control[flag] = value
    updated_metadata[YOLO11_SEGMENTATION_TRAINING_CONTROL_METADATA_KEY] = control
    return updated_metadata
