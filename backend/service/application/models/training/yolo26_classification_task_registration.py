"""YOLO26 classification 训练输出登记工具。"""
...
class Yolo26ClassificationTrainingResultLike(Protocol):
    """描述登记 YOLO26 classification 训练输出所需的执行结果字段。"""

    labels: tuple[str, ...]


def resolve_yolo26_classification_implementation_mode() -> str:
    """返回 YOLO26 classification 训练实现标记。"""

    return YOLO26_CLASSIFICATION_IMPLEMENTATION_MODE


def register_yolo26_classification_training_output_model_version(
    *,
    session_factory: SessionFactory,
    task_record: TaskRecord,
    dataset_export: DatasetExport,
    payload: dict[str, object],
    execution_result: Yolo26ClassificationTrainingResultLike,
