...
from backend.service.application.models.training.yolo_task_pose_training_control import (
    YoloTaskPoseTrainingControlState,
    build_yolo_task_pose_training_control_metadata,
    clear_yolo_task_pose_manual_save_request,
    read_yolo_task_pose_training_control_state,
)
from backend.service.application.models.training.yolo_task_pose_training_dataset import (
    resolve_yolo_task_pose_training_dataset_export,
)
from backend.service.application.models.training.yolo_task_pose_training_events import (
    build_yolo_task_pose_training_cancelled_event,
    build_yolo_task_pose_training_failed_event,
    build_yolo_task_pose_training_paused_event,
    build_yolo_task_pose_training_started_event,
    build_yolo_task_pose_training_succeeded_event,
)
from backend.service.application.models.training.yolo_task_pose_training_payload import (
    build_yolo_task_pose_training_create_task_metadata,
    build_yolo_task_pose_training_queue_payload,
    build_yolo_task_pose_training_task_spec,
    read_yolo_task_pose_training_payload,
)
from backend.service.application.models.training.yolo_task_pose_training_registration import (
    YOLO_TASK_POSE_MODEL_SERVICE_MAP,
    resolve_yolo_task_pose_implementation_mode,
    register_yolo_task_pose_training_output_model_version,
)
from backend.service.application.models.training.yolo_task_pose_training import (
    YOLO_TASK_POSE_DEFAULT_EVALUATION_INTERVAL,
    YoloTaskPoseTrainingControlCommand,
    YoloTaskPoseTrainingEpochProgress,
    YoloTaskPoseTrainingPausedError,
    YoloTaskPoseTrainingSavePoint,
    YoloTaskPoseTrainingTerminatedError,
    YoloTaskPoseTrainingExecutionRequest,
...
YOLO_TASK_POSE_TRAINING_TASK_KIND = "yolo-task-pose-training"
YOLO_TASK_POSE_TRAINING_QUEUE_NAME = "yolo-task-pose-trainings"
YOLO_TASK_POSE_TRAINING_CONTROL_METADATA_KEY = "pose_training_control"
YOLO_TASK_POSE_TRAINING_SUPPORTED_MODEL_TYPES = (
    *tuple(YOLO_TASK_POSE_MODEL_SERVICE_MAP.keys()),
)


@dataclass(frozen=True)
class YoloTaskPoseTrainingRequest:
    """描述一次 pose 训练任务创建请求。"""
...
class SqlAlchemyYoloTaskPoseTrainingService:
    """管理 YOLO 主线 pose 训练任务的完整生命周期。"""

    task_type = POSE_TASK_TYPE
    model_label = "YOLO task pose"
    training_task_kind = YOLO_TASK_POSE_TRAINING_TASK_KIND
    training_queue_name = YOLO_TASK_POSE_TRAINING_QUEUE_NAME
    ...
    def submit_training_task(
        self,
        request: YoloTaskPoseTrainingRequest,
        *,
        ...
        *,
        ...
        if manifest_object_key is None or not manifest_object_key.strip():
            raise InvalidRequestError(
                "pose 训练任务缺少 manifest_object_key",
        ...
        if not isinstance(manifest_payload, dict):
            raise InvalidRequestError("pose 训练 manifest 无效")
        ...
        def on_epoch(
            progress: YoloTaskPoseTrainingEpochProgress,
        ) -> YoloTaskPoseTrainingControlCommand | None:
            control_state = self._read_control_state(task_record.task_id)
            if control_state.terminate_requested:
                return YoloTaskPoseTrainingControlCommand(
            ...
            if control_state.save_requested:
                ...
                return YoloTaskPoseTrainingControlCommand(save_checkpoint=True)
            return None

        def on_savepoint(savepoint: YoloTaskPoseTrainingSavePoint) -> None:
        ...
        request = YoloTaskPoseTrainingExecutionRequest(
            ...
            evaluation_interval=int(
                ...
                or YOLO_TASK_POSE_DEFAULT_EVALUATION_INTERVAL
            ),
        ...
        try:
            ...
            self.task_service.append_task_event(
                build_yolo_task_pose_training_cancelled_event(
                    ...
                    control_metadata_key=YOLO_TASK_POSE_TRAINING_CONTROL_METADATA_KEY,
                )
            )
    ...
    def _normalize_model_type(self, model_type: object) -> str:
        ...
        if normalized not in YOLO_TASK_POSE_TRAINING_SUPPORTED_MODEL_TYPES:
            raise InvalidRequestError(
                "当前 pose 训练不支持指定模型分类",
                details={
                    "model_type": normalized,
                    "supported": YOLO_TASK_POSE_TRAINING_SUPPORTED_MODEL_TYPES,
                },
            )
        return normalized
    ...
    def _build_task_spec(
        ...
        *,
        ...
        return build_yolo_task_pose_training_task_spec(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
        )
    ...
    def _build_create_task_metadata(
        ...
        *,
        ...
        return build_yolo_task_pose_training_create_task_metadata(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
            task_spec=task_spec,
    ...
    def _build_queue_payload(
        ...
        *,
        ...
        return build_yolo_task_pose_training_queue_payload(
            task_id=task_id,
            task_kind=task_kind,
            task_spec=task_spec,
        )
    ...
    def _read_task_payload(self, task_record: TaskRecord) -> dict[str, object]:
        ...
        return read_yolo_task_pose_training_payload(task_record)
    ...
    def _resolve_dataset_export(
        ...
        *,
        ...
        return resolve_yolo_task_pose_training_dataset_export(
    ...
    def _build_training_summary(
        ...
        *,
        task_record: TaskRecord,
        dataset_export: DatasetExport,
        execution_result: YoloTaskPoseTrainingExecutionResult,
    ...
    def _register_training_output_model_version(
        ...
        *,
        ...
        return register_yolo_task_pose_training_output_model_version(
            session_factory=self.session_factory,
            task_record=task_record,
            dataset_export=dataset_export,
            payload=payload,
    ...
    def _resolve_implementation_mode(model_type: str) -> str:
        ...
        return resolve_yolo_task_pose_implementation_mode(model_type)
    ...
    def _read_control_state(self, task_id: str) -> YoloTaskPoseTrainingControlState:
        """从任务 metadata 中读取最新控制状态。"""

        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        return read_yolo_task_pose_training_control_state(
            metadata=metadata,
            control_metadata_key=YOLO_TASK_POSE_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _clear_manual_save_request(self, task_id: str) -> None:
        ...
        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        updated_metadata = clear_yolo_task_pose_manual_save_request(
            metadata=metadata,
            control_metadata_key=YOLO_TASK_POSE_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _set_control_flag(
        ...
        updated_metadata = build_yolo_task_pose_training_control_metadata(
            metadata=metadata,
            control_metadata_key=YOLO_TASK_POSE_TRAINING_CONTROL_METADATA_KEY,
            flag=flag,
            value=value,
    ...
    @staticmethod
    def _read_pose_keypoint_shape(
        execution_result: YoloTaskPoseTrainingExecutionResult,
    ) -> list[int]:
        """从训练结果中读取 pose keypoint shape。"""

        metrics_payload = dict(execution_result.metrics_payload)
        raw_shape = metrics_payload.get("kpt_shape")
