...
class Yolo11PoseTaskExecutionRequest(Protocol):
    """描述平台传入 YOLO11 pose 训练执行层的字段集合。"""
...
def run_yolo11_pose_training_from_task_request(
    request: Yolo11PoseTaskExecutionRequest,
) -> Yolo11PoseTrainingExecutionResult:
    """把平台 pose task request 转成 YOLO11 专属训练请求。"""

    return run_yolo11_pose_training(
        Yolo11PoseTrainingExecutionRequest(
...
def _build_yolo11_epoch_callback(request: Yolo11PoseTaskExecutionRequest):
...
def _build_yolo11_savepoint_callback(request: Yolo11PoseTaskExecutionRequest):
    """把平台 savepoint callback 适配成 YOLO11 callback。"""

    if request.savepoint_callback is None:
        return None

    def on_yolo11_savepoint(savepoint: Yolo11PoseTrainingSavePoint) -> None:
