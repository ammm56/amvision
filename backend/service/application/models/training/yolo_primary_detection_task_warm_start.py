"""YOLO 主线 detection warm start 解析工具。"""
...
class YoloPrimaryDetectionWarmStartReference:
...
def resolve_yolo_primary_detection_warm_start_reference(
    *,
...
) -> YoloPrimaryDetectionWarmStartReference | None:
    """解析可用于 warm start 的本地 checkpoint。

    - model_version_id：请求使用的 ModelVersion id。
    - model_service_cls：当前模型类型对应的模型服务类。
    - file_types：当前模型类型对应的文件类型集合。
    ...
    checkpoint_file = next(
        (
            model_file
            for model_file in model_service.list_model_files(model_version_id=model_version_id)
    ...
    return YoloPrimaryDetectionWarmStartReference(
...
def build_yolo_primary_detection_warm_start_source_summary(
    warm_start_reference: YoloPrimaryDetectionWarmStartReference,
) -> dict[str, object]:
    """把 warm start 来源记录成训练执行可消费的摘要。

    - warm_start_reference：已解析的 warm start 引用。
    """
