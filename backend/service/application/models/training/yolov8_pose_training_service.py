...
from backend.service.application.models.training.yolov8_pose_training_control import (
    ...
    clear_yolov8_pose_manual_save_request,
    read_yolov8_pose_training_control_state,
)
from backend.service.application.models.training.yolov8_pose_training_dataset import (
    resolve_yolov8_pose_training_dataset_export,
)
from backend.service.application.models.training.yolov8_pose_training_events import (
    build_yolov8_pose_training_cancelled_event,
    build_yolov8_pose_training_failed_event,
    build_yolov8_pose_training_paused_event,
    build_yolov8_pose_training_started_event,
    build_yolov8_pose_training_succeeded_event,
)
from backend.service.application.models.training.yolov8_pose_training_payload import (
    build_yolov8_pose_training_create_task_metadata,
    build_yolov8_pose_training_queue_payload,
    build_yolov8_pose_training_task_spec,
    read_yolov8_pose_training_payload,
)
from backend.service.application.models.training.yolov8_pose_training_registration import (
    YOLOV8_POSE_MODEL_SERVICE_MAP,
...
YOLOV8_POSE_TRAINING_SUPPORTED_MODEL_TYPES = (
    *tuple(YOLOV8_POSE_MODEL_SERVICE_MAP.keys()),
)


@dataclass(frozen=True)
class YoloV8PoseTrainingRequest:
    """描述一次 pose 训练任务创建请求。"""
...
class SqlAlchemyYoloV8PoseTrainingService:
    """管理 YOLOv8 pose 训练任务的完整生命周期。"""

    task_type = POSE_TASK_TYPE
    model_label = "YOLOv8 pose"
    training_task_kind = YOLOV8_POSE_TRAINING_TASK_KIND
    training_queue_name = YOLOV8_POSE_TRAINING_QUEUE_NAME
    ...
    def submit_training_task(
        self,
        request: YoloV8PoseTrainingRequest,
        *,
    ...
    def process_training_task(
        ...
        *,
        ...
        def resolve_control_command(
            *,
            ...
            if decision.terminate_requested:
                return YoloV8PoseTrainingControlCommand(
            ...
            if decision.save_requested:
                ...
                return YoloV8PoseTrainingControlCommand(save_checkpoint=True)
            return None

        def on_epoch(
            progress: YoloV8PoseTrainingEpochProgress,
        ) -> YoloV8PoseTrainingControlCommand | None:
            append_yolo_task_epoch_progress(
        ...
        def on_savepoint(savepoint: YoloV8PoseTrainingSavePoint) -> None:
        ...
        def poll_control() -> YoloV8PoseTrainingControlCommand | None:
    ...
    def _normalize_model_type(self, model_type: object) -> str:
        ...
        if normalized not in YOLOV8_POSE_TRAINING_SUPPORTED_MODEL_TYPES:
            raise InvalidRequestError(
                "当前 pose 训练不支持指定模型分类",
                details={
                    "model_type": normalized,
                    "supported": YOLOV8_POSE_TRAINING_SUPPORTED_MODEL_TYPES,
                },
            )
        return normalized
    ...
    def _build_task_spec(
        ...
        *,
        ...
        return build_yolov8_pose_training_task_spec(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
        )
    ...
    def _build_create_task_metadata(
        ...
        *,
        ...
        return build_yolov8_pose_training_create_task_metadata(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
            task_spec=task_spec,
    ...
    def _build_queue_payload(
        ...
        *,
        ...
        return build_yolov8_pose_training_queue_payload(
            task_id=task_id,
            task_kind=task_kind,
            task_spec=task_spec,
        )
    ...
    def _read_task_payload(self, task_record: TaskRecord) -> dict[str, object]:
        ...
        return read_yolov8_pose_training_payload(task_record)
    ...
    def _resolve_dataset_export(
        ...
        *,
        ...
        return resolve_yolov8_pose_training_dataset_export(
            session_factory=self.session_factory,
    ...
    def _build_training_summary(
        ...
        *,
        task_record: TaskRecord,
        dataset_export: DatasetExport,
        execution_result: YoloV8PoseTrainingExecutionResult,
    ...
    def _register_training_output_model_version(
        ...
        *,
        ...
        return register_yolov8_pose_training_output_model_version(
            session_factory=self.session_factory,
            task_record=task_record,
            dataset_export=dataset_export,
            payload=payload,
    ...
    def _resolve_implementation_mode(model_type: str) -> str:
        ...
        return resolve_yolov8_pose_implementation_mode(model_type)
    ...
    def _read_control_state(self, task_id: str) -> YoloV8PoseTrainingControlState:
        """从任务 metadata 中读取最新控制状态。"""

        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        return read_yolov8_pose_training_control_state(
            metadata=metadata,
            control_metadata_key=YOLOV8_POSE_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _clear_manual_save_request(self, task_id: str) -> None:
        ...
        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        updated_metadata = clear_yolov8_pose_manual_save_request(
            metadata=metadata,
            control_metadata_key=YOLOV8_POSE_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _set_control_flag(
        ...
        updated_metadata = build_yolov8_pose_training_control_metadata(
            metadata=metadata,
            control_metadata_key=YOLOV8_POSE_TRAINING_CONTROL_METADATA_KEY,
            flag=flag,
            value=value,
