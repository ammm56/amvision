...
YOLO_TASK_SEGMENTATION_IMPLEMENTATION_MODE = "yolo-task-segmentation"
...
class YoloTaskSegmentationTrainingBatchProgress:
    epoch: int
    max_epochs: int
    iteration: int
    max_iterations: int
...
@dataclass(frozen=True)
class YoloTaskSegmentationTrainingEpochProgress:
    epoch: int
    max_epochs: int
    input_size: tuple[int, int]
    learning_rate: float
...
@dataclass(frozen=True)
class YoloTaskSegmentationTrainingSavePoint:
...
@dataclass(frozen=True)
class YoloTaskSegmentationTrainingControlCommand:
    save_checkpoint: bool = False
    pause_training: bool = False
    terminate_training: bool = False


class YoloTaskSegmentationTrainingPausedError(Exception):
    """训练被显式暂停。"""


class YoloTaskSegmentationTrainingTerminatedError(Exception):
    """训练被显式终止。"""
...
@dataclass(frozen=True)
class YoloTaskSegmentationTrainingExecutionRequest:
    ...
    savepoint_callback: (
        Callable[[YoloTaskSegmentationTrainingSavePoint], None] | None
    ) = None


@dataclass(frozen=True)
class YoloTaskSegmentationTrainingExecutionResult:
    best_metric_value: float
    best_metric_name: str
    latest_checkpoint_bytes: bytes
...
def run_yolo_task_segmentation_training(
    ...
    for epoch in range(start_epoch, me):
        ...
        ep_progress = YoloTaskSegmentationTrainingEpochProgress(
            epoch=epoch,
            max_epochs=me,
        ...
        if cmd is not None and cmd.terminate_training:
            raise YoloTaskSegmentationTrainingTerminatedError()
        ...
        if cmd is not None and cmd.pause_training:
            raise YoloTaskSegmentationTrainingPausedError()
...
def _seg_evaluate(
    ...
    if eval_count > 0:
        return {
            "map50": round(map50_sum / max(1, eval_count), 6),
            "map50_95": round(map_sum / max(1, eval_count), 6),
        }
