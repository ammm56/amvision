...
def build_yolo_detection_training_output_files(
...
def require_complete_yolo_detection_training_output_files(
...
def write_yolo_detection_training_execution_outputs(
    *,
    ...
    dataset_storage.write_json(
        output_files.metrics_object_key, execution_result.metrics_payload
    )
    dataset_storage.write_json(
...
def write_yolo_detection_epoch_metric_snapshots(
    *,
    dataset_storage: LocalDatasetStorage,
    output_files: DetectionTrainingOutputFiles,
    ...
    if output_files.metrics_object_key is not None:
        dataset_storage.write_json(
            output_files.metrics_object_key, progress.train_metrics_snapshot
        )
    if (
        progress.validation_snapshot is not None
        and output_files.validation_metrics_object_key is not None
    ):
        dataset_storage.write_json(
            output_files.validation_metrics_object_key, progress.validation_snapshot
        )


def write_yolo_detection_training_savepoint_outputs(
    *,
    dataset_storage: LocalDatasetStorage,
    output_files: DetectionTrainingOutputFiles,
    ...
    dataset_storage.write_bytes(
        latest_checkpoint_object_key, savepoint.latest_checkpoint_bytes
    )
    if savepoint.best_checkpoint_bytes is not None:
        dataset_storage.write_bytes(
            output_files.checkpoint_object_key, savepoint.best_checkpoint_bytes
        )
...
def write_yolo_detection_training_summary_payload(
    *,
    dataset_storage: LocalDatasetStorage,
    output_files: DetectionTrainingOutputFiles,
    ...
    if output_files.summary_object_key is None:
        raise ServiceConfigurationError(
            "当前 YOLO detection 训练缺少 summary_object_key"
        )
    dataset_storage.write_json(output_files.summary_object_key, summary)


def write_yolo_detection_training_labels_file(
    *,
    dataset_storage: LocalDatasetStorage,
