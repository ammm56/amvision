...
YOLO_TASK_OBB_IMPLEMENTATION_MODE = "yolo-task-obb"
_OBB_DEF_INPUT = (640, 640)
_OBB_DEF_BS = 4
_OBB_DEF_EP = 50
YOLO_TASK_OBB_DEFAULT_EVALUATION_INTERVAL = 5


@dataclass(frozen=True)
class YoloTaskObbTrainingBatchProgress:
    epoch: int
    max_epochs: int
    iteration: int
    max_iterations: int
...
@dataclass(frozen=True)
class YoloTaskObbTrainingEpochProgress:
    epoch: int
    max_epochs: int
    input_size: tuple[int, int]
    learning_rate: float
...
@dataclass(frozen=True)
class YoloTaskObbTrainingSavePoint:
...
@dataclass(frozen=True)
class YoloTaskObbTrainingControlCommand:
    save_checkpoint: bool = False
    pause_training: bool = False
    terminate_training: bool = False


class YoloTaskObbTrainingPausedError(Exception):
    pass


class YoloTaskObbTrainingTerminatedError(Exception):
    pass
...
@dataclass(frozen=True)
class YoloTaskObbTrainingExecutionRequest:
    ...
    evaluation_interval: int = YOLO_TASK_OBB_DEFAULT_EVALUATION_INTERVAL
...
@dataclass(frozen=True)
class YoloTaskObbTrainingExecutionResult:
    best_metric_value: float
    best_metric_name: str
    latest_checkpoint_bytes: bytes
...
def run_yolo_task_obb_training(
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        for batch_start in range(0, len(shuffled), bs):
            ...
            with autocast_ctx:
                ...
                if request.model_type == "yolov8":
                    ...
                    from backend.service.application.models.yolo_core_common.losses.obb_loss import (
                        compute_obb_loss,
                    )
            ...
            if (
                ...
                bp = YoloTaskObbTrainingBatchProgress(
                    epoch=epoch,
                    max_epochs=max_epochs,
                ...
                if cmd and cmd.terminate_training:
                    raise YoloTaskObbTrainingTerminatedError()
        ...
        ep_progress = YoloTaskObbTrainingEpochProgress(
            epoch=epoch,
            max_epochs=max_epochs,
            input_size=input_size,
            learning_rate=float(scheduler.get_last_lr()[0]),
        ...
        if cmd and cmd.pause_training:
            raise YoloTaskObbTrainingPausedError()

    return YoloTaskObbTrainingExecutionResult(
        best_metric_value=best_metric_value,
        best_metric_name=best_metric_name,
        latest_checkpoint_bytes=ckpt_bytes,
        metrics_payload={
