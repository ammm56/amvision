...
from backend.service.application.models.training.yolo11_pose_task_control import (
    Yolo11PoseTrainingControlState,
    build_yolo11_pose_training_control_metadata,
    clear_yolo11_pose_manual_save_request,
    read_yolo11_pose_training_control_state,
)
from backend.service.application.models.training.yolo11_pose_task_dataset import (
    resolve_yolo11_pose_training_dataset_export,
)
from backend.service.application.models.training.yolo11_pose_task_events import (
...
from backend.service.application.models.training.yolo11_pose_task_service import (
    build_yolo11_pose_service_create_task_metadata,
    build_yolo11_pose_service_queue_payload,
    build_yolo11_pose_service_task_spec,
    get_yolo11_pose_paused_error_types,
    get_yolo11_pose_terminated_error_types,
    read_yolo11_pose_service_task_payload,
    register_yolo11_pose_service_training_output,
    resolve_yolo11_pose_service_implementation_mode,
...
from backend.service.application.models.training.yolo11_pose_training import (
    YOLO11_POSE_DEFAULT_EVAL_INTERVAL,
...
class Yolo11PoseTrainingTaskRequest:
    """描述一次 YOLO11 pose 训练任务创建请求。"""
...
class SqlAlchemyYolo11PoseTrainingTaskService:
    """管理 YOLO11 pose 训练任务的 service 层入口。"""

    task_type = POSE_TASK_TYPE
    model_label = "YOLO11 pose"
    training_task_kind = YOLO11_POSE_TRAINING_TASK_KIND
    training_queue_name = YOLO11_POSE_TRAINING_QUEUE_NAME
    ...
    def submit_training_task(
        self,
        request: Yolo11PoseTrainingTaskRequest,
        *,
        created_by: str | None = None,
    ) -> dict[str, object]:
        """创建 YOLO11 pose 训练任务并入队。"""
    ...
    def process_training_task(
        ...
        *,
        ...
        on_control_state_change: Callable[[Yolo11PoseTrainingControlState], None]
        | None = None,
    ) -> dict[str, object]:
        """执行 YOLO11 pose 训练工作负载。"""
        ...
        if manifest_object_key is None or not manifest_object_key.strip():
            raise InvalidRequestError(
                "YOLO11 pose 训练任务缺少 manifest_object_key",
        ...
        if not isinstance(manifest_payload, dict):
            raise InvalidRequestError("YOLO11 pose 训练 manifest 无效")
        ...
        def resolve_control_command(
            *,
            ...
            if decision.save_requested:
                ...
                return Yolo11PoseTrainingControlCommand(save_checkpoint=True)
            return None

        def on_epoch(
            progress: Yolo11PoseTrainingEpochProgress,
        ) -> Yolo11PoseTrainingControlCommand | None:
            nonlocal control_state
            append_yolo_task_epoch_progress(
        ...
        def on_savepoint(savepoint: Yolo11PoseTrainingSavePoint) -> None:
        ...
        def poll_control() -> Yolo11PoseTrainingControlCommand | None:
        ...
        request = Yolo11PoseTrainingExecutionRequest(
            ...
            evaluation_interval=int(
                payload.get("evaluation_interval") or YOLO11_POSE_DEFAULT_EVAL_INTERVAL
            ),
    ...
    def request_training_save(self, task_record: TaskRecord) -> None:
        """请求 YOLO11 pose 训练保存 checkpoint。"""
    ...
    def _normalize_model_type(self, model_type: object) -> str:
        ...
        return require_yolo11_model_type(model_type, task_type="pose")
    ...
    def _build_task_spec(
        ...
        *,
        ...
        return build_yolo11_pose_service_task_spec(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
        )
    ...
    def _build_create_task_metadata(
        ...
        *,
        ...
        return build_yolo11_pose_service_create_task_metadata(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
            task_spec=task_spec,
    ...
    def _build_queue_payload(
        ...
        *,
        ...
        return build_yolo11_pose_service_queue_payload(
            task_id=task_id,
            task_kind=task_kind,
            task_spec=task_spec,
        )
    ...
    def _read_task_payload(self, task_record: TaskRecord) -> dict[str, object]:
        """从任务记录中解析 YOLO11 pose 训练负载。"""

        return read_yolo11_pose_service_task_payload(task_record)
    ...
    def _run_pose_training_execution(
        ...
        return run_yolo11_pose_service_training_execution(request)

    @staticmethod
    def _terminated_error_types() -> tuple[type[BaseException], ...]:
        """返回 YOLO11 pose 应按取消处理的异常类型。"""

        return get_yolo11_pose_terminated_error_types()

    @staticmethod
    def _paused_error_types() -> tuple[type[BaseException], ...]:
        """返回 YOLO11 pose 应按暂停处理的异常类型。"""

        return get_yolo11_pose_paused_error_types()
    ...
    def _resolve_dataset_export(
        ...
        *,
        ...
        """根据 id 或 manifest key 解析 YOLO11 pose 训练输入。"""

        return resolve_yolo11_pose_training_dataset_export(
            session_factory=self.session_factory,
    ...
    def _resolve_resume_checkpoint_path(self, task_record: TaskRecord) -> Path | None:
        """为 paused 的 YOLO11 pose 训练任务解析 resume checkpoint 路径。"""
    ...
    def _build_training_summary(
        ...
        *,
        task_record: TaskRecord,
        dataset_export: DatasetExport,
        execution_result: Yolo11PoseTrainingExecutionResult,
        ...
        input_size = self._read_input_size(payload.get("input_size"))
        keypoint_shape = self._read_pose_keypoint_shape(execution_result)
    ...
    def _register_training_output_model_version(
        ...
        *,
        ...
        """把 YOLO11 pose 训练输出登记为 ModelVersion。"""

        return register_yolo11_pose_service_training_output(
            session_factory=self.session_factory,
            task_record=task_record,
            dataset_export=dataset_export,
            payload=payload,
    ...
    def _resolve_implementation_mode(model_type: str) -> str:
        """返回 YOLO11 pose 训练实现标记。"""

        return resolve_yolo11_pose_service_implementation_mode(model_type)
    ...
    def _read_control_state(self, task_id: str) -> Yolo11PoseTrainingControlState:
        """从任务 metadata 中读取最新 YOLO11 pose 控制状态。"""

        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        return read_yolo11_pose_training_control_state(metadata=metadata)
    ...
    def _clear_manual_save_request(self, task_id: str) -> None:
        ...
        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        updated_metadata = clear_yolo11_pose_manual_save_request(metadata=metadata)
    ...
    def _set_control_flag(
        ...
        """设置 YOLO11 pose 训练控制标记。"""

        metadata = dict(task_record.metadata) if task_record.metadata else {}
        updated_metadata = build_yolo11_pose_training_control_metadata(
            metadata=metadata,
            flag=flag,
            value=value,
        )
