...
YOLO_TASK_POSE_IMPLEMENTATION_MODE = "yolo-task-pose"
_POSE_DEF_INPUT_SIZE = (640, 640)
_POSE_DEF_BS = 4
_POSE_DEF_EPOCHS = 50
YOLO_TASK_POSE_DEFAULT_EVALUATION_INTERVAL = 5
...
class YoloTaskPoseTrainingBatchProgress:
    epoch: int
    max_epochs: int
    iteration: int
    max_iterations: int
...
@dataclass(frozen=True)
class YoloTaskPoseTrainingEpochProgress:
    epoch: int
    max_epochs: int
    input_size: tuple[int, int]
    learning_rate: float
...
@dataclass(frozen=True)
class YoloTaskPoseTrainingSavePoint:
...
@dataclass(frozen=True)
class YoloTaskPoseTrainingControlCommand:
    save_checkpoint: bool = False
    pause_training: bool = False
    terminate_training: bool = False


class YoloTaskPoseTrainingPausedError(Exception):
    pass


class YoloTaskPoseTrainingTerminatedError(Exception):
    pass
...
@dataclass(frozen=True)
class YoloTaskPoseTrainingExecutionRequest:
    ...
    evaluation_interval: int = YOLO_TASK_POSE_DEFAULT_EVALUATION_INTERVAL
...
@dataclass(frozen=True)
class YoloTaskPoseTrainingExecutionResult:
    best_metric_value: float
    best_metric_name: str
    latest_checkpoint_bytes: bytes
...
def run_yolo_task_pose_training(
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        for batch_start in range(0, len(shuffled), bs):
            ...
            with autocast_ctx:
                ...
                if request.model_type == "yolov8":
                    ...
                    from backend.service.application.models.yolo_core_common.losses.pose_loss import (
                        compute_pose_loss,
                    )
        ...
        ep_progress = YoloTaskPoseTrainingEpochProgress(
            epoch=epoch,
            max_epochs=max_epochs,
        ...
        if cmd and cmd.terminate_training:
            raise YoloTaskPoseTrainingTerminatedError()
        ...
        if cmd and cmd.pause_training:
            raise YoloTaskPoseTrainingPausedError()
