...
from backend.service.application.models.training.yolo_task_segmentation_training_control import (
    YoloTaskSegmentationTrainingControlState,
    build_yolo_task_segmentation_training_control_metadata,
    clear_yolo_task_segmentation_manual_save_request,
    read_yolo_task_segmentation_training_control_state,
)
from backend.service.application.models.training.yolo_task_segmentation_training_dataset import (
    resolve_yolo_task_segmentation_training_dataset_export,
)
from backend.service.application.models.training.yolo_task_segmentation_training_events import (
    build_yolo_task_segmentation_training_cancelled_event,
    build_yolo_task_segmentation_training_failed_event,
    build_yolo_task_segmentation_training_paused_event,
    build_yolo_task_segmentation_training_queue_failed_event,
    build_yolo_task_segmentation_training_queued_event,
    build_yolo_task_segmentation_training_started_event,
    build_yolo_task_segmentation_training_succeeded_event,
)
from backend.service.application.models.training.yolo_task_segmentation_training_payload import (
    build_yolo_task_segmentation_training_create_task_metadata,
    build_yolo_task_segmentation_training_queue_payload,
    build_yolo_task_segmentation_training_task_spec,
    read_yolo_task_segmentation_training_payload,
)
from backend.service.application.models.training.yolo_task_segmentation_training_registration import (
    YOLO_TASK_SEGMENTATION_MODEL_SERVICE_MAP,
    register_yolo_task_segmentation_training_output_model_version,
    resolve_yolo_task_segmentation_implementation_mode,
)
from backend.service.application.models.training.yolo_task_segmentation_training import (
    YoloTaskSegmentationTrainingControlCommand,
    YoloTaskSegmentationTrainingEpochProgress,
    YoloTaskSegmentationTrainingPausedError,
    YoloTaskSegmentationTrainingSavePoint,
    YoloTaskSegmentationTrainingTerminatedError,
    YoloTaskSegmentationTrainingExecutionRequest,
...
YOLO_TASK_SEGMENTATION_TRAINING_TASK_KIND = "yolo-task-segmentation-training"
YOLO_TASK_SEGMENTATION_TRAINING_QUEUE_NAME = "yolo-task-segmentation-trainings"
YOLO_TASK_SEGMENTATION_TRAINING_CONTROL_METADATA_KEY = (
    "segmentation_training_control"
)
YOLO_TASK_SEGMENTATION_DEFAULT_EVALUATION_INTERVAL = 5


@dataclass(frozen=True)
class YoloTaskSegmentationTrainingRequest:
...
class SqlAlchemyYoloTaskSegmentationTrainingService:
    """管理 YOLO 主线 segmentation 训练任务的完整生命周期。"""

    task_type = SEGMENTATION_TASK_TYPE
    model_label = "YOLO task segmentation"
    training_task_kind = YOLO_TASK_SEGMENTATION_TRAINING_TASK_KIND
    training_queue_name = YOLO_TASK_SEGMENTATION_TRAINING_QUEUE_NAME
    ...
    def submit_training_task(
        self,
        request: YoloTaskSegmentationTrainingRequest,
        *,
    ...
    def process_training_task(
        ...
        *,
        ...
        def on_epoch(
            progress: YoloTaskSegmentationTrainingEpochProgress,
        ) -> YoloTaskSegmentationTrainingControlCommand | None:
            control_state = self._read_control_state(task_record.task_id)
            if control_state.terminate_requested:
                return YoloTaskSegmentationTrainingControlCommand(
        ...
        def on_savepoint(savepoint: YoloTaskSegmentationTrainingSavePoint) -> None:
        ...
        try:
            if resolved_model_type == "rfdetr":
                ...
                execution_request = YoloTaskSegmentationTrainingExecutionRequest(
                    dataset_storage=self.dataset_storage,
                    manifest_payload=manifest_payload,
                    model_type=resolved_model_type,
                    model_scale=str(payload.get("model_scale") or "nano"),
                    ...
                    evaluation_interval=int(
                        ...
                        or YOLO_TASK_SEGMENTATION_DEFAULT_EVALUATION_INTERVAL
                    ),
                    input_size=input_size,
                    precision=str(payload.get("precision") or "fp32"),
                    resume_checkpoint_path=resume_checkpoint_path,
            ...
            self.task_service.append_task_event(
                build_yolo_task_segmentation_training_cancelled_event(
                    ...
                    control_metadata_key=YOLO_TASK_SEGMENTATION_TRAINING_CONTROL_METADATA_KEY,
                )
            )
    ...
    def _register_training_output_model_version(
        ...
        *,
        ...
        model_type: str,
        execution_result: YoloTaskSegmentationTrainingExecutionResult,
        ...
        return register_yolo_task_segmentation_training_output_model_version(
    ...
    def _normalize_model_type(self, model_type: object) -> str:
        ...
        if normalized not in YOLO_TASK_SEGMENTATION_MODEL_SERVICE_MAP:
            raise InvalidRequestError(
                "当前 segmentation 训练不支持指定模型分类",
                details={
                    "model_type": normalized,
                    "supported": tuple(
                        YOLO_TASK_SEGMENTATION_MODEL_SERVICE_MAP.keys()
                    ),
                },
            )
        return normalized
    ...
    def _build_task_spec(
        ...
        *,
        ...
        return build_yolo_task_segmentation_training_task_spec(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
        )
    ...
    def _build_create_task_metadata(
        ...
        *,
        ...
        return build_yolo_task_segmentation_training_create_task_metadata(
            request=request,
            dataset_export=dataset_export,
            model_type=model_type,
            task_spec=task_spec,
    ...
    def _build_queue_payload(
        ...
        *,
        ...
        return build_yolo_task_segmentation_training_queue_payload(
            task_id=task_id,
            task_kind=task_kind,
            task_spec=task_spec,
        )
    ...
    def _read_task_payload(self, task_record: TaskRecord) -> dict[str, object]:
        ...
        return read_yolo_task_segmentation_training_payload(task_record)

    def _run_yolo_segmentation_training_execution(
        self,
        request: YoloTaskSegmentationTrainingExecutionRequest,
    ) -> YoloTaskSegmentationTrainingExecutionResult:
        """执行 primary segmentation 训练。"""

        return run_yolo_task_segmentation_training(request)
    ...
    def _resolve_dataset_export(
        ...
        *,
        ...
        return resolve_yolo_task_segmentation_training_dataset_export(
            session_factory=self.session_factory,
    ...
    def _resolve_implementation_mode(model_type: str) -> str:
        ...
        return resolve_yolo_task_segmentation_implementation_mode(model_type)
    ...
    def _read_control_state(
        ...
        return read_yolo_task_segmentation_training_control_state(
            metadata=metadata,
            control_metadata_key=YOLO_TASK_SEGMENTATION_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _clear_manual_save_request(self, task_id: str) -> None:
        ...
        task = self.task_service.get_task(task_id).task
        metadata = dict(task.metadata) if task.metadata else {}
        updated_metadata = clear_yolo_task_segmentation_manual_save_request(
            metadata=metadata,
            control_metadata_key=YOLO_TASK_SEGMENTATION_TRAINING_CONTROL_METADATA_KEY,
        )
    ...
    def _set_control_flag(
        ...
        updated_metadata = build_yolo_task_segmentation_training_control_metadata(
            metadata=metadata,
            control_metadata_key=YOLO_TASK_SEGMENTATION_TRAINING_CONTROL_METADATA_KEY,
            flag=flag,
            value=value,
