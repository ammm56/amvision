"""YOLO 主线 classification 训练输出登记工具。"""
...
YOLOV8_CLASSIFICATION_MODEL_SERVICE_MAP: dict[str, tuple[type, type]] = {
    "yolov8": (SqlAlchemyYoloV8ModelService, YoloV8TrainingOutputRegistration),
}


def register_yolov8_classification_training_output_model_version(
    *,
    ...
    - task_record：训练任务记录。
    - dataset_export：训练使用的 DatasetExport。
    - payload：任务负载快照。
    - model_type：当前模型分类。
    - execution_result：训练执行结果。
    - checkpoint_object_key：best checkpoint 对象路径。
    - labels_object_key：labels.txt 对象路径。
    - train_metrics_object_key：训练指标对象路径。
    - summary：训练摘要。
    """

    service_cls, registration_cls = YOLOV8_CLASSIFICATION_MODEL_SERVICE_MAP[
        model_type
    ]
    ...
    return model_service.register_training_output(
        registration_cls(
            ...
            metadata={
                ...
                "implementation_mode": YOLOV8_CLASSIFICATION_IMPLEMENTATION_MODE,
            },
        )
    )
