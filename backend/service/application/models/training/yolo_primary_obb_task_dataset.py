...
def resolve_yolo_primary_obb_training_dataset_export(
    *,
    ...
    """根据训练请求解析可用于 OBB 训练的 DatasetExport。

    - session_factory：数据库 session factory。
    - project_id：训练任务所属项目 id。
    - dataset_export_id：可选 DatasetExport id。
    - dataset_export_manifest_key：可选 DatasetExport manifest object key。
    - model_type：当前模型分类，例如 yolov8、yolo11、yolo26。
    """

    export_by_id = None
    if dataset_export_id is not None:
        export_by_id = _get_yolo_primary_obb_dataset_export(
    ...
    if dataset_export_manifest_key is not None:
        export_by_manifest = _get_yolo_primary_obb_dataset_export_by_manifest(
            session_factory=session_factory,
...
def _get_yolo_primary_obb_dataset_export(
    *,
    session_factory: SessionFactory,
    dataset_export_id: str,
) -> DatasetExport:
    """按 id 读取一个 OBB DatasetExport。"""
...
def _get_yolo_primary_obb_dataset_export_by_manifest(
    *,
    session_factory: SessionFactory,
    manifest_object_key: str,
) -> DatasetExport:
    """按 manifest object key 读取一个 OBB DatasetExport。"""
