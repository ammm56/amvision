    ...
    if test_split is not None and test_samples:
        ...
        if not isinstance(checkpoint_ema_state, dict):
            raise InvalidRequestError(
                "YOLO26 detection best checkpoint 缺少 ema_state_dict"
            )
        ema.load_state_dict(checkpoint_ema_state, strict=False)
        test_metrics = _evaluate_yolo26_detection_model(
            imports=imports,
            model=ema.model,
...
def _evaluate_yolo26_detection_model(
    *,
    ...
    validation_losses, validation_map = _evaluate_yolo26_detection_model_once(
        imports=imports,
        model=model,
        samples=samples,
        category_ids=category_ids,
        ...
        with imports.torch.no_grad():
            for batch in validation_dataloader:
                ...
                with autocast_context():
                    ...
                    loss_components = _compute_yolo26_detection_loss(
                        imports=imports,
                        model=model,
