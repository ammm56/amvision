...
from backend.service.application.models.yolo26_core.training.detection_support import (
    ...
    resolve_yolo26_detection_runtime as resolve_yolo26_detection_single_process_runtime,
...
Yolo26DetectionTrainingExecutionRequest = YoloDetectionTrainingExecutionRequest
...
class _Yolo26LoadedResumeState:
    """描述 YOLO26 detection resume checkpoint 解析后的训练状态。"""
...
def run_yolo26_detection_training(
    ...
    """执行 YOLO26 detection 训练。"""

    _require_yolo26_detection_request(request)
    imports = require_yolo26_detection_training_imports()
    manifest_payload = dict(request.manifest_payload)
    data_context = prepare_yolo26_detection_training_data_context(
    ...
    evaluation_interval = max(
        1,
        int(
            request.evaluation_interval or YOLO26_DETECTION_DEFAULT_EVALUATION_INTERVAL
        ),
    )
    ...
    rank_batch_size = _resolve_yolo26_per_rank_batch_size(
        global_batch_size=batch_size,
        ddp_context=ddp_context,
    )
    training_options = _resolve_yolo26_detection_training_options(extra_options)
    augmentation_options = build_yolo26_task_augmentation_options(extra_options)
    ...
    warm_start_summary = _resolve_yolo26_warm_start_summary(
        imports=imports,
        model=model,
        request=request,
    )
    ...
    if request.resume_checkpoint_path is not None:
        resume_state = _load_yolo26_resume_checkpoint(
            imports=imports,
            model=model,
            optimizer=optimizer,
            scheduler=scheduler,
    ...
    training_model = _wrap_yolo26_detection_training_model(
        imports=imports,
        model=model,
    ...
    execution_plan = plan_yolo26_detection_training_execution(
        train_sample_count=len(train_samples),
    ...
    best_checkpoint_bytes = (
        _build_yolo26_checkpoint_bytes_from_state(
            imports=imports,
    ...
    try:
        loop_result = run_yolo26_detection_training_loop(
            ...
            augmentation_options=serialize_yolo26_detection_augmentation_options(
            ...
            epoch_callback=(
                _build_yolo26_epoch_progress_adapter(request.epoch_callback)
                if request.epoch_callback is not None and is_rank_zero
                else None
            ),
            savepoint_callback=(
                _build_yolo26_savepoint_adapter(request.savepoint_callback)
    ...
    best_metric_value = _normalize_yolo26_best_metric_value(
    ...
    validation_metrics_payload = _build_yolo26_validation_metrics_payload(
        validation_split_name=validation_split_name,
        validation_sample_count=len(validation_samples),
        evaluation_interval=evaluation_interval,
    ...
    metrics_payload = _build_yolo26_metrics_payload(
        request=request,
    ...
    return Yolo26DetectionTrainingExecutionResult(
...
def _require_yolo26_detection_request(
    request: Yolo26DetectionTrainingExecutionRequest,
) -> None:
    """确认当前执行入口只接收 YOLO26 detection 请求。"""

    if request.model_type != "yolo26":
        raise InvalidRequestError(
            "YOLO26 detection 训练入口只支持 yolo26 model_type",
...
def resolve_yolo26_detection_runtime(
    *,
    ...
    """解析 YOLO26 detection 单进程或 DDP rank 使用的运行时资源。"""

    if ddp_context is None:
        return resolve_yolo26_detection_single_process_runtime(
            imports=imports,
    ...
    if not ddp_context.is_distributed:
        raise InvalidRequestError("YOLO26 detection DDP 上下文的 world_size 必须大于 1")
    if requested_gpu_count is not None and requested_gpu_count != ddp_context.world_size:
        raise InvalidRequestError(
            "YOLO26 detection DDP 上下文与训练请求 gpu_count 不一致",
            details={
                "requested_gpu_count": requested_gpu_count,
    ...
    if ddp_context.device.startswith("cuda") and not imports.torch.cuda.is_available():
        raise InvalidRequestError("YOLO26 detection DDP 请求使用 CUDA，但当前环境没有可用 GPU")
    if ddp_context.device.startswith("cuda"):
        imports.torch.cuda.set_device(ddp_context.local_rank)
    precision = _resolve_yolo26_ddp_runtime_precision(
        imports=imports,
        requested_precision=requested_precision,
        extra_options=extra_options,
...
def _resolve_yolo26_ddp_runtime_precision(
    *,
    imports: Any,
    requested_precision: str | None,
    extra_options: dict[str, object] | None,
    ...
    """解析 YOLO26 detection DDP rank 内使用的 precision。"""
    ...
    if precision == "fp8":
        raise InvalidRequestError("YOLO26 detection DDP 训练暂不支持 fp8")
...
def _resolve_yolo26_per_rank_batch_size(
    *,
    global_batch_size: int,
    ddp_context: DdpTrainingContext | None,
) -> int:
    """把前端总 batch size 拆成单个 YOLO26 detection DDP rank 的 batch size。"""

    if ddp_context is None or not ddp_context.is_distributed:
        return global_batch_size
    if global_batch_size < ddp_context.world_size:
        raise InvalidRequestError(
            "YOLO26 detection DDP 总 batch size 不能小于 GPU 数量",
            details={
    ...
    if global_batch_size % ddp_context.world_size != 0:
        raise InvalidRequestError(
            "YOLO26 detection DDP 总 batch size 必须能被 GPU 数量整除",
            details={
                "batch_size": global_batch_size,
                "world_size": ddp_context.world_size,
            },
...
def _wrap_yolo26_detection_training_model(
    *,
    imports: Any,
    model: Any,
    ...
    """按单进程或 DDP rank 包装 YOLO26 detection 训练模型。"""
    ...
    if not distributed.is_available() or not distributed.is_initialized():
        raise InvalidRequestError("YOLO26 detection DDP 训练必须先初始化 torch.distributed")
    if ddp_context.device.startswith("cuda"):
...
def _resolve_yolo26_detection_training_options(
    extra_options: dict[str, object],
) -> dict[str, Any]:
    """解析 YOLO26 detection 训练参数。"""

    return {
        "learning_rate": read_yolo26_float_option(
            extra_options, "learning_rate", default=0.01
        ),
        "weight_decay": read_yolo26_float_option(
            extra_options, "weight_decay", default=5e-4
        ),
        "class_loss_weight": read_yolo26_float_option(
            extra_options,
            "class_loss_weight",
            default=YOLO26_DETECTION_DEFAULT_CLASS_LOSS_WEIGHT,
        ),
        "box_loss_weight": read_yolo26_float_option(
            extra_options,
            "box_loss_weight",
        ...
        "assign_topk": max(
            ...
            read_yolo26_int_option(
                extra_options,
                "assign_topk",
                default=YOLO26_DETECTION_DEFAULT_ASSIGN_TOPK,
            ),
        ),
        "assign_alpha": read_yolo26_float_option(
            extra_options,
            "assign_alpha",
            default=YOLO26_DETECTION_DEFAULT_ASSIGN_ALPHA,
        ),
        "assign_beta": read_yolo26_float_option(
            extra_options,
            "assign_beta",
            default=YOLO26_DETECTION_DEFAULT_ASSIGN_BETA,
        ),
        "min_lr_ratio": read_yolo26_float_option(
            extra_options,
            "min_lr_ratio",
            default=YOLO26_DETECTION_DEFAULT_MIN_LR_RATIO,
        ),
        "grad_clip_norm": read_yolo26_float_option(
            extra_options,
            "grad_clip_norm",
            default=YOLO26_DETECTION_DEFAULT_GRAD_CLIP_NORM,
        ),
    }


def _resolve_yolo26_warm_start_summary(
    *,
    imports: Any,
    model: Any,
    request: Yolo26DetectionTrainingExecutionRequest,
...
def _load_yolo26_resume_checkpoint(
    *,
...
) -> _Yolo26LoadedResumeState:
    """加载并校验 YOLO26 detection resume checkpoint。"""

    checkpoint_payload = imports.torch.load(checkpoint_path, map_location="cpu")
    if not isinstance(checkpoint_payload, dict):
        raise InvalidRequestError("YOLO26 resume checkpoint 内容不合法")
    validate_yolo26_detection_resume_checkpoint(
        checkpoint_payload=checkpoint_payload,
        request=Yolo26DetectionResumeValidationRequest(
    ...
    if not isinstance(scheduler_state_dict, dict):
        raise InvalidRequestError("YOLO26 resume checkpoint 缺少 scheduler_state_dict")
    scheduler.load_state_dict(scheduler_state_dict)
    scaler_state_dict = checkpoint_payload.get("scaler_state_dict")
    if not isinstance(scaler_state_dict, dict):
        raise InvalidRequestError("YOLO26 resume checkpoint 缺少 scaler_state_dict")
    ...
    return _Yolo26LoadedResumeState(
        ...
        validation_history=_normalize_yolo26_history_items(
            checkpoint_payload.get("validation_history")
        ),
        evaluated_epochs=_normalize_yolo26_evaluated_epochs(
...
def _build_yolo26_checkpoint_bytes_from_state(
    *,
    imports: Any,
    checkpoint_state: dict[str, object] | None,
) -> bytes:
    """把 YOLO26 detection checkpoint state 重新编码为 bytes。"""

    return encode_yolo26_detection_checkpoint_state(
        torch_module=imports.torch,
        checkpoint_state=checkpoint_state,
    )


def _compute_yolo26_detection_loss(
    *,
    imports: Any,
    model: Any,
    ...
    del num_classes
    return compute_yolo26_detection_training_loss(
        torch_module=imports.torch,
        model=model,
...
def _evaluate_yolo26_detection_model(
    *,
    imports: Any,
    model: Any,
    samples: tuple[Any, ...],
    ...
    validation_losses = evaluate_yolo26_detection_validation_losses(
        torch_module=imports.torch,
        model=model,
        samples=samples,
        batch_size=batch_size,
        build_batch=lambda batch_samples: build_yolo26_detection_training_batch(
            imports=imports,
        ...
        autocast_context=lambda: build_yolo26_autocast_context(
            torch_module=imports.torch,
            device=device,
            runtime_precision=runtime_precision,
        )(),
        freeze_batch_norm=lambda: _freeze_yolo26_batch_norm_modules(
            imports=imports,
            model=model,
        ),
        restore_batch_norm=_restore_yolo26_batch_norm_modules,
        class_loss_weight=class_loss_weight,
        box_loss_weight=box_loss_weight,
        dfl_loss_weight=dfl_loss_weight,
        assign_topk=assign_topk,
    ...
    validation_map = _evaluate_yolo26_validation_map(
        imports=imports,
        model=model,
        samples=samples,
        input_size=input_size,
...
def _evaluate_yolo26_validation_map(
    *,
    imports: Any,
    model: Any,
    samples: tuple[Any, ...],
    ...
    """执行 YOLO26 detection COCO bbox mAP。"""
    ...
    try:
        with imports.torch.no_grad():
            for batch_samples in _iter_yolo26_batches(samples, batch_size):
                images, batch_targets = build_yolo26_detection_training_batch(
                    imports=imports,
                    samples=batch_samples,
                    input_size=input_size,
                    device=device,
    ...
    ground_truth = _load_yolo26_coco_ground_truth_silently(
        imports=imports,
        annotation_file=annotation_file,
        annotation_payload=annotation_payload,
    )
...
def _load_yolo26_coco_ground_truth_silently(
    *,
    imports: Any,
    annotation_file: Any,
    annotation_payload: dict[str, object] | None,
) -> Any:
    """静默加载 YOLO26 validation 使用的 COCO ground truth。"""
    ...
    with redirect_stdout(io.StringIO()):
        ...
        if annotation_payload is None:
            raise InvalidRequestError("缺少可用的 YOLO26 COCO ground truth 数据")
...
def _iter_yolo26_batches(samples: tuple[Any, ...], batch_size: int):
    """按 batch size 迭代 YOLO26 样本。"""
...
def _freeze_yolo26_batch_norm_modules(
    *,
    imports: Any,
    model: Any,
) -> tuple[tuple[Any, bool], ...]:
    """在 YOLO26 validation loss 统计阶段冻结 BatchNorm。"""
...
def _restore_yolo26_batch_norm_modules(
    batch_norm_states: tuple[tuple[Any, bool], ...],
) -> None:
    """恢复 YOLO26 validation 前的 BatchNorm 状态。"""

    for module, was_training in batch_norm_states:
        module.train(was_training)


def _normalize_yolo26_history_items(value: object) -> list[dict[str, object]]:
    """把 YOLO26 checkpoint 指标历史归一成列表。"""
...
def _normalize_yolo26_evaluated_epochs(value: object) -> tuple[int, ...]:
    """把 YOLO26 checkpoint 中的验证 epoch 归一成整数元组。"""
...
def _build_yolo26_batch_progress_adapter(callback):
    """把 YOLO26 core batch 进度转成平台 batch 进度。"""

    def on_yolo26_batch_progress(
        progress: Yolo26DetectionTrainingBatchProgress,
    ) -> None:
        callback(
            YoloDetectionTrainingBatchProgress(
    ...
    return on_yolo26_batch_progress


def _build_yolo26_epoch_progress_adapter(callback):
    """把 YOLO26 core epoch 进度转成平台 epoch 控制命令。"""

    def on_yolo26_epoch_progress(progress: Yolo26DetectionTrainerEpochProgress):
...
def _build_yolo26_savepoint_adapter(callback):
    """把 YOLO26 core savepoint 转成平台 savepoint。"""

    def on_yolo26_savepoint(savepoint_payload) -> None:
        callback(
            YoloDetectionTrainingSavePoint(
...
def _normalize_yolo26_best_metric_value(*, value: float, has_validation: bool) -> float:
    """把 YOLO26 best metric 哨兵值转成平台可展示数值。"""
...
def _build_yolo26_validation_metrics_payload(
    *,
    ...
    """组装 YOLO26 validation metrics payload。"""
...
def _build_yolo26_metrics_payload(
    *,
    request: Yolo26DetectionTrainingExecutionRequest,
    ...
    return {
        ...
        "augmentation": serialize_yolo26_detection_augmentation_options(
            augmentation_options
        ),
