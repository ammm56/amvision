...
@dataclass(frozen=True)
class YoloV8TrainingTaskResult:
    """描述一次 YOLOv8 detection 训练任务处理结果。"""
...
class SqlAlchemyYoloV8TrainingTaskService:
    ...
    def process_training_task(self, task_id: str) -> YoloV8TrainingTaskResult:
        ...
        self.task_service.append_task_event(
            AppendTaskEventRequest(
                task_id=task_id,
                event_type="status",
                message="yolov8 training started",
                payload={
                    "state": "running",
        ...
        try:
            execution_result = run_yolov8_detection_training(
                YoloV8DetectionTrainingExecutionRequest(
            ...
            task_result = YoloV8TrainingTaskResult(
                task_id=task_id,
                status="succeeded",
                dataset_export_id=dataset_export.dataset_export_id,
                dataset_export_manifest_key=dataset_export.manifest_object_key,
            ...
            self.task_service.append_task_event(
                AppendTaskEventRequest(
                    task_id=task_id,
                    event_type="result",
                    message="yolov8 training completed",
                    payload={
                        "state": "succeeded",
            ...
            self.task_service.append_task_event(
                AppendTaskEventRequest(
                    task_id=task_id,
                    event_type="result",
                    message="yolov8 training failed",
                    payload={
                        "state": "failed",
                        "finished_at": self._now_iso(),
    ...
    def _build_request_from_task_record(self, task_record: TaskRecord) -> YoloV8TrainingTaskRequest:
        ...
        return YoloV8TrainingTaskRequest(
    ...
    def _append_batch_progress(self, task_id: str, progress: YoloV8TrainingBatchProgress) -> None:
        """回写单个 batch 的进度事件。"""
        ...
        self.task_service.append_task_event(
            AppendTaskEventRequest(
                task_id=task_id,
                event_type="progress",
                message="yolov8 training batch progressed",
                payload={
                    "progress": {
                        "stage": "training",
                        "granularity": "batch",
    ...
    def _append_epoch_progress(self, task_id: str, progress: YoloV8TrainingEpochProgress) -> None:
        """回写单轮训练结束后的进度事件。"""
        ...
        self.task_service.append_task_event(
            AppendTaskEventRequest(
                task_id=task_id,
                event_type="progress",
                message="yolov8 training epoch progressed",
                payload={
                    "progress": {
                        "stage": "training",
                        "granularity": "epoch",
    ...
    def _build_training_summary(
        ...
        *,
        ...
        request: YoloV8TrainingTaskRequest,
        dataset_export: DatasetExport,
        execution_result: YoloV8DetectionTrainingExecutionResult,
        output_files: DetectionTrainingOutputFiles,
    ) -> dict[str, object]:
    ...
    def _resolve_warm_start_reference(
        ...
        checkpoint_file = next(
            (
                ...
                if model_file.file_type == YOLOV8_DETECTION_FILE_TYPES.checkpoint_file_type
            ),
            None,
        )
    ...
    def _build_existing_result(self, task_record: TaskRecord) -> YoloV8TrainingTaskResult | None:
        ...
        if not all(isinstance(result.get(field_name), str) for field_name in required_fields):
            return None
        return YoloV8TrainingTaskResult(
    ...
    def _serialize_task_result(self, task_result: YoloV8TrainingTaskResult) -> dict[str, object]:
