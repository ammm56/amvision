...
from backend.service.infrastructure.integrations.directory._directory_trigger_support import (
    ...
    build_directory_watch_checkpoint_path,
...
CHECKPOINT_FORMAT_ID = "amvision.directory-watch-checkpoint.v1"


@dataclass
class _DirectoryWatchAdapterState:
    """描述一条 directory-watch TriggerSource 的运行状态。"""
    ...
    last_change_batch_size: int = 0
    sequence_id: int = 0
    pending_paths: dict[str, str] = field(default_factory=dict)
    known_identity_keys: set[str] = field(default_factory=set)


class DirectoryWatchTriggerAdapter:
    """监听目录事件并把稳定文件批次提交给 WorkflowRun。"""
        ...
        *,
        ...
        if trigger_source.submit_mode != "async":
            raise InvalidRequestError(
                "directory-watch 当前只支持 async submit_mode",
                details={
        ...
        checkpoint_state = _load_checkpoint_state(
            checkpoint_path=config.scan_config.checkpoint_path,
            trigger_source=trigger_source,
        )
        ...
        if state.startup_error is not None:
            ...
            raise ServiceConfigurationError(
                ...
                details={
                    "trigger_source_id": trigger_source.trigger_source_id,
                    "directory_path": str(config.scan_config.directory_path),
                    "error": state.startup_error,
                },
            )

    def stop(self, *, trigger_source_id: str) -> None:
        """停止一条目录监听 TriggerSource。"""
    ...
    def get_health(self, *, trigger_source_id: str) -> dict[str, object]:
        ...
        scan_config = state.config.scan_config
        return {
            "adapter_kind": self.adapter_kind,
            "running": state.running,
            "trigger_source_id": normalized_trigger_source_id,
            "directory_path": str(scan_config.directory_path),
            "recursive": scan_config.recursive,
            "include_hidden": scan_config.include_hidden,
            "glob_pattern": scan_config.glob_pattern,
            "extensions": list(scan_config.extensions),
            "sort_by": scan_config.sort_by,
            "descending": scan_config.descending,
            "dedupe_by": scan_config.dedupe_by,
            "batch_size": scan_config.batch_size,
            "min_stable_age_seconds": scan_config.min_stable_age_seconds,
            "persist_checkpoint": scan_config.persist_checkpoint,
            "checkpoint_path": str(scan_config.checkpoint_path),
            "watch_debounce_ms": state.config.watch_debounce_ms,
            "watch_step_ms": state.config.watch_step_ms,
            "watch_timeout_ms": state.config.watch_timeout_ms,
            ...
            "last_ready_count": state.last_ready_count,
            "last_batch_file_count": state.last_batch_file_count,
            "last_change_batch_size": state.last_change_batch_size,
            "sequence_id": state.sequence_id,
            "pending_path_count": len(state.pending_paths),
    ...
    def _watch_trigger_source(
        ...
        try:
            ...
            for changes in watch(
                scan_config.directory_path,
                debounce=state.config.watch_debounce_ms,
                step=state.config.watch_step_ms,
                stop_event=state.stop_event,
                rust_timeout=state.config.watch_timeout_ms,
                ...
                if state.stop_event.is_set():
                    break
                if changes:
                    self._apply_changes(changes=changes, state=state)
    ...
    def _apply_changes(
        ...
        *,
        ...
        state.last_event_at = _now_isoformat()
        state.last_change_batch_size = len(changes)
        scan_config = state.config.scan_config
        for change, raw_path in changes:
            path_key = _normalize_path_key(raw_path)
            if change == Change.deleted:
                state.pending_paths.pop(path_key, None)
                continue
            candidate_path = Path(raw_path).resolve()
            if not matches_directory_candidate_path(
                candidate_path,
                directory_path=scan_config.directory_path,
                recursive=scan_config.recursive,
                include_hidden=scan_config.include_hidden,
                glob_pattern=scan_config.glob_pattern,
                ...
                state.pending_paths.pop(path_key, None)
                continue
            state.pending_paths[path_key] = str(candidate_path)
    ...
    def _flush_pending_records(
        ...
        *,
        ...
        duplicate_path_keys: set[str] = set()
        pending_path_keys = set(state.pending_paths.keys())
        for record in records:
            record_path = str(record.get("path") or "")
            path_key = _normalize_path_key(record_path)
            if path_key not in pending_path_keys:
                continue
            identity_key = build_record_identity_key(
                record,
                dedupe_by=scan_config.dedupe_by,
            )
            if identity_key in state.known_identity_keys:
                duplicate_path_keys.add(path_key)
                continue
            ready_records.append(record)
        state.last_ready_count = len(ready_records)

        ready_path_keys = {
            _normalize_path_key(str(record.get("path") or ""))
            for record in ready_records
        }
        retained_pending_paths: dict[str, str] = {}
        for pending_path_key, pending_path in state.pending_paths.items():
            if pending_path_key in duplicate_path_keys:
                continue
            if pending_path_key in ready_path_keys:
                retained_pending_paths[pending_path_key] = pending_path
                continue
            candidate_path = Path(pending_path)
            ...
            if not matches_directory_candidate_path(
                ...
                continue
            retained_pending_paths[pending_path_key] = pending_path
        state.pending_paths = retained_pending_paths

        if not ready_records:
            state.last_batch_file_count = 0
            return

        pending_ready_records = list(ready_records)
        while pending_ready_records and not state.stop_event.is_set():
            batch_records = pending_ready_records[: scan_config.batch_size]
            ...
            for record in batch_records:
                ...
                state.pending_paths.pop(
                    _normalize_path_key(str(record.get("path") or "")),
                    None,
                )
            self._save_checkpoint_safely(state)
            pending_ready_records = pending_ready_records[scan_config.batch_size :]

    def _save_checkpoint_safely(self, state: _DirectoryWatchAdapterState) -> None:
        """尽量保存 checkpoint，但不让写盘失败导致当前线程退出。"""

        if not state.config.scan_config.persist_checkpoint:
            return
        try:
            _save_checkpoint_state(
                checkpoint_path=state.config.scan_config.checkpoint_path,
...
def _build_raw_event(
    *,
    ...
    payload: dict[str, object] = {
        "directory_path": str(state.config.scan_config.directory_path),
        "directory_path_value": {"value": str(state.config.scan_config.directory_path)},
        "files": batch_records,
        ...
        "scan_summary": {
            **scan_summary,
            "ready_count": state.last_ready_count,
            "pending_path_count": len(state.pending_paths),
    ...
    return RawTriggerEvent(
        payload=payload,
        event_id=f"directory-watch-{trigger_source.trigger_source_id}-{sequence_id}",
        trace_id=f"directory-watch-{trigger_source.trigger_source_id}-{sequence_id}",
...
def _load_checkpoint_state(
    *,
    ...
    try:
        ...
        raise ServiceConfigurationError(
            "Directory Watch checkpoint 读取失败",
            details={
                "trigger_source_id": trigger_source.trigger_source_id,
    ...
    if not isinstance(payload, dict):
        raise ServiceConfigurationError(
            "Directory Watch checkpoint 格式不正确",
            details={
                "trigger_source_id": trigger_source.trigger_source_id,
                "checkpoint_path": str(checkpoint_path),
            },
    ...
    if payload.get("format_id") != CHECKPOINT_FORMAT_ID:
        raise ServiceConfigurationError(
            "Directory Watch checkpoint format_id 不受支持",
            details={
                "trigger_source_id": trigger_source.trigger_source_id,
                "checkpoint_path": str(checkpoint_path),
    ...
    if stored_trigger_source_id != trigger_source.trigger_source_id:
        raise ServiceConfigurationError(
            "Directory Watch checkpoint 与当前 trigger_source_id 不匹配",
            details={
                "trigger_source_id": trigger_source.trigger_source_id,
                "checkpoint_path": str(checkpoint_path),
    ...
    if isinstance(sequence_id, bool) or not isinstance(sequence_id, int) or sequence_id < 0:
        raise ServiceConfigurationError(
            "Directory Watch checkpoint.sequence_id 非法",
            details={
                "trigger_source_id": trigger_source.trigger_source_id,
                "checkpoint_path": str(checkpoint_path),
            },
    ...
    if not isinstance(seen_identity_keys, list) or any(
        ...
        raise ServiceConfigurationError(
            "Directory Watch checkpoint.seen_identity_keys 非法",
            details={
                "trigger_source_id": trigger_source.trigger_source_id,
                "checkpoint_path": str(checkpoint_path),
            },
...
def _record_adapter_error(state: _DirectoryWatchAdapterState, error: Exception) -> None:
    """记录目录监听线程中的异常。"""
