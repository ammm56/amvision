...
def create_classification_conversion_task(body: ClassificationConversionTaskCreateRequestBody, principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("models:read", "tasks:write"))], sf: Annotated[SessionFactory, Depends(get_session_factory)], qb: Annotated[LocalFileQueueBackend, Depends(get_queue_backend)], ds: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)]) -> ClassificationConversionTaskSubmissionResponse:
    ...
    r = svc.submit_conversion_task(YoloPrimaryConversionTaskRequest(project_id=body.project_id, model_type=mt, task_type="classification", model_version_id=body.model_version_id, model_build_id=body.model_build_id, target_format=body.target_format, extra_options=dict(body.extra_options)), created_by=principal.principal_id)
    return ClassificationConversionTaskSubmissionResponse(task_id=r["task_id"], queue_name=YOLO_PRIMARY_CONVERSION_QUEUE_NAME, queue_task_id=r["queue_task_id"])
