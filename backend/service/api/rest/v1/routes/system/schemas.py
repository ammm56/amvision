...
class SystemCurrentPrincipalContract(BaseModel):
    ...
    auth_provider_kind: str | None = Field(
        default=None, description="账号 provider 类型"
    )
...
class DatasetExportCapabilityContract(BaseModel):
    ...
    implemented_formats: list[str] = Field(
        default_factory=list, description="当前已实现并可用的格式"
    )
...
class DatasetImportCapabilityContract(BaseModel):
    ...
    implemented_task_types: list[str] = Field(
        default_factory=list, description="当前已实现并可用的导入任务类型"
    )
    format_types_by_task_type: dict[str, list[str]] = Field(
        default_factory=dict,
...
class SystemBootstrapCapabilitiesContract(BaseModel):
    ...
    dataset_import: DatasetImportCapabilityContract = Field(
        description="数据集导入能力"
    )
    dataset_export: DatasetExportCapabilityContract = Field(
        description="数据集导出格式能力"
    )
    project_summary_topics: list[str] = Field(
        default_factory=list, description="projects.events 支持的 topic 列表"
    )
    platform_model_types_by_task_type: dict[str, list[str]] = Field(
        default_factory=dict,
        description="按 task_type 汇总的平台模型分类列表",
    )
    training_export_formats_by_task_and_model_type: dict[
        str,
        dict[str, list[str]],
    ] = Field(
        default_factory=dict,
        description="按 task_type 和 model_type 汇总的训练数据集导出格式列表",
    )
...
class SystemBootstrapResponse(BaseModel):
    ...
    websocket_query_token_enabled: bool = Field(
        description="WebSocket 是否允许 access_token 查询参数"
    )
    current_user: SystemCurrentPrincipalContract | None = Field(
        default=None, description="当前已登录主体；未登录时为空"
    )
    providers: list[SystemAuthProviderContract] = Field(
        default_factory=list, description="公开可发现的账号 provider 列表"
    )
    ...
    capabilities: SystemBootstrapCapabilitiesContract = Field(
        description="前端需要读取的关键能力摘要"
    )
    devices: dict[str, object] = Field(
        default_factory=dict, description="设备与推理运行时摘要"
    )
...
class SystemDiagnosticsResponse(BaseModel):
    ...
    about: dict[str, object] = Field(
        default_factory=dict, description="应用版本、构建和许可证摘要"
    )
    system: dict[str, object] = Field(
        default_factory=dict, description="操作系统、路径、磁盘、CPU 和内存摘要"
    )
    python_runtime: dict[str, object] = Field(
        default_factory=dict, description="Python 解释器、环境和关键依赖摘要"
    )
    devices: dict[str, object] = Field(
        default_factory=dict, description="设备与推理运行时摘要"
    )
    services: dict[str, object] = Field(
        default_factory=dict, description="服务组件运行摘要"
    )
...
class SystemConfigResponse(BaseModel):
    ...
    config: dict[str, object] = Field(
        default_factory=dict, description="当前进程已解析并合并后的配置"
    )
    metadata: dict[str, object] = Field(
        default_factory=dict, description="配置快照附加信息"
    )
