...
def create_segmentation_deployment_instance(
    ...
    service = SqlAlchemySegmentationDeploymentService(session_factory=session_factory, dataset_storage=dataset_storage)
...
@segmentation_deployments_router.post("/segmentation/deployment-instances/{deployment_instance_id}/sync/start")
def sync_start_segmentation_deployment(
    deployment_instance_id: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("models:read", "models:write"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
    dataset_storage: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)],
    supervisor: Annotated[YoloXDeploymentProcessSupervisor, Depends(get_segmentation_sync_deployment_process_supervisor)],
) -> dict[str, str]:
...
@segmentation_deployments_router.post("/segmentation/deployment-instances/{deployment_instance_id}/sync/stop")
def sync_stop_segmentation_deployment(
    deployment_instance_id: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("models:read", "models:write"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
    dataset_storage: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)],
...
@segmentation_deployments_router.post("/segmentation/deployment-instances/{deployment_instance_id}/sync/health")
def sync_health_segmentation_deployment(
    deployment_instance_id: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("models:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
    dataset_storage: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)],
...
def _require_sup(s: YoloXDeploymentProcessSupervisor | None) -> YoloXDeploymentProcessSupervisor:
    if s is None:
        raise ServiceConfigurationError("segmentation deployment supervisor 未启动", details={"cause": "no-supervisor"})
    return s
