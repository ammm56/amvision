"""detection validation session 响应模型和构建函数。"""
...
class DetectionValidationPredictionSummaryResponse(BaseModel):
    ...
    input_file_id: str | None = Field(
        default=None, description="最近一次预测输入 file id；输入不是 file_id 时为空"
    )
...
class DetectionValidationRuntimeSessionInfoResponse(BaseModel):
    ...
    input_spec: DetectionValidationRuntimeTensorSpecResponse = Field(
        description="输入张量规格"
    )
    output_spec: DetectionValidationRuntimeTensorSpecResponse = Field(
        description="输出张量规格"
    )
    metadata: dict[str, object] = Field(
        default_factory=dict, description="附加运行时元数据"
    )
...
class DetectionValidationSessionDetailResponse(BaseModel):
    ...
    runtime_profile_id: str | None = Field(
        default=None, description="runtime profile id；当前仅回传"
    )
    ...
    runtime_artifact_storage_uri: str = Field(
        description="当前运行实际加载的模型文件存储 URI"
    )
    runtime_artifact_file_type: str = Field(
        description="当前运行实际加载的模型文件类型"
    )
    ...
    labels_storage_uri: str | None = Field(
        default=None, description="labels 文件存储 URI"
    )
    ...
    last_prediction: DetectionValidationPredictionSummaryResponse | None = Field(
...
class DetectionValidationPredictionResponse(BaseModel):
    ...
    labels: list[str] = Field(default_factory=list, description="类别列表")
    runtime_session_info: DetectionValidationRuntimeSessionInfoResponse = Field(
        description="runtime 会话信息"
    )


def build_detection_validation_session_response(
    ...
    return DetectionValidationSessionDetailResponse(
        ...
        last_prediction=build_detection_validation_prediction_summary_response(
            session_view.last_prediction
        ),
    )


def build_detection_validation_prediction_response(
    ...
    return DetectionValidationPredictionResponse(
        ...
        detections=[
            build_detection_validation_detection_response(detection)
        ...
        runtime_session_info=DetectionValidationRuntimeSessionInfoResponse(
            ...
            input_spec=DetectionValidationRuntimeTensorSpecResponse(
                **build_tensor_spec_payload(
                    prediction_view.runtime_session_info.input_spec
                )
            ),
            output_spec=DetectionValidationRuntimeTensorSpecResponse(
                **build_tensor_spec_payload(
...
def build_detection_validation_prediction_summary_response(
...
def build_detection_validation_detection_response(
