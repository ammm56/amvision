...
async def create_segmentation_inference_task(
    ...
    try:
        ...
        raise InvalidRequestError("segmentation 推理任务请求格式无效", details={"error": str(error)}) from error
    ...
    normalized = normalize_segmentation_inference_input(
        dataset_storage=dataset_storage,
        request_id=f"segmentation-inference-task-{body.deployment_instance_id}",
        source=SegmentationInferenceInputSource(
    ...
    submission = SqlAlchemySegmentationInferenceTaskService(
        ...
        SegmentationInferenceTaskRequest(
            ...
            async_inference_owner_id=read_async_inference_service_id(request, task_type="segmentation"),
