...
class WorkflowTriggerSourceHealthSummaryResponse(BaseModel):
    ...
    recent_error: dict[str, object] | str | None = Field(default=None, description="最近一次错误摘要")
    supervisor: dict[str, object] = Field(default_factory=dict, description="底层 supervisor 原始健康摘要")
...
class WorkflowTriggerSourceHealthResponse(BaseModel):
    ...
    last_triggered_at: str | None = Field(default=None, description="最近一次收到触发的时间")
    last_error: dict[str, object] | str | None = Field(default=None, description="最近一次资源级错误摘要")
    health_summary: WorkflowTriggerSourceHealthSummaryResponse = Field(description="适配器与 supervisor 健康摘要")
...
def get_workflow_trigger_source_health(
    ...
    _ensure_project_visible(principal=principal, project_id=trigger_source.project_id)
    return _build_trigger_source_health_response(
        _build_trigger_source_service(request).get_trigger_source_health(trigger_source_id)
    )
...
def _build_trigger_source_health_response(
    ...
    return WorkflowTriggerSourceHealthResponse(
        ...
        last_triggered_at=payload.get("last_triggered_at") if isinstance(payload.get("last_triggered_at"), str) else None,
        last_error=_normalize_health_error(payload.get("last_error")),
        health_summary=WorkflowTriggerSourceHealthSummaryResponse(
            ...
            request_count=_read_health_counter(health_summary, "request_count"),
            request_count_rollover_count=_read_health_counter(health_summary, "request_count_rollover_count"),
            success_count=_read_health_counter(health_summary, "success_count"),
            success_count_rollover_count=_read_health_counter(health_summary, "success_count_rollover_count"),
            error_count=_read_health_counter(health_summary, "error_count"),
            error_count_rollover_count=_read_health_counter(health_summary, "error_count_rollover_count"),
            timeout_count=_read_health_counter(health_summary, "timeout_count"),
            timeout_count_rollover_count=_read_health_counter(health_summary, "timeout_count_rollover_count"),
            recent_error=_normalize_health_error(health_summary.get("recent_error")),
            supervisor=dict(health_summary.get("supervisor")) if isinstance(health_summary.get("supervisor"), dict) else {},
        ),
    )


def _normalize_health_error(value: object) -> dict[str, object] | str | None:
...
def _read_health_counter(payload: dict[str, object], key: str) -> int:
    """从健康摘要中读取整数计数值。"""
