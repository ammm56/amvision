...
obb_evaluation_tasks_router = APIRouter(prefix="/models/obb", tags=["obb"])


class ObbEvaluationTaskCreateBody(BaseModel):
    """OBB 评估任务创建请求。"""
...
class ObbEvaluationTaskSubmitResponse(BaseModel):
    """OBB 评估任务提交响应。"""
    task_id: str
    status: str
    queue_name: str
...
@obb_evaluation_tasks_router.post(
    "/evaluation-tasks",
    response_model=ObbEvaluationTaskSubmitResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
def create_obb_evaluation_task(
    body: ObbEvaluationTaskCreateBody,
    ...
    task_service = SqlAlchemyObbEvaluationTaskService(
    ...
    task_id = task_svc.create_task(CreateTaskRequest(
        ...
        task_spec={
            ...
            "output_prefix": body.output_prefix or f"task-runs/obb-evaluation",
        },
    ))
    ...
    # 记录事件
    task_svc.append_task_event(task_id, "status", "queued", "OBB 评估任务已入队")

    return ObbEvaluationTaskSubmitResponse(
        task_id=task_id,
        status="queued",
        queue_name=OBB_EVALUATION_QUEUE_NAME,
