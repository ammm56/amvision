...
from backend.service.api.rest.v1.routes.obb_training_tasks.controls import (
...
from backend.service.api.rest.v1.routes.obb_training_tasks.responses import (
...
from backend.service.api.rest.v1.routes.obb_training_tasks.services import (
    submit_obb_training_task,
)
...
def create_obb_training_task(
    ...
    return submit_obb_training_task(
        body=body,
...
@obb_training_tasks_router.get(
    "/obb/training-tasks/{task_id}/output-files",
    response_model=list[TrainingOutputFileSummaryResponse],
)
def list_obb_training_output_files(
    task_id: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("tasks:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
    dataset_storage: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)],
...
@obb_training_tasks_router.get(
    "/obb/training-tasks/{task_id}/output-files/{file_name}",
    response_model=TrainingOutputFileDetailResponse,
)
def get_obb_training_output_file(
    task_id: str,
    file_name: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("tasks:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
...
@obb_training_tasks_router.post(
    "/obb/training-tasks/{task_id}/save",
    response_model=TrainingTaskDetailResponse,
)
...
@obb_training_tasks_router.post(
    "/obb/training-tasks/{task_id}/pause",
    response_model=TrainingTaskDetailResponse,
)
...
@obb_training_tasks_router.post(
    "/obb/training-tasks/{task_id}/terminate",
    response_model=TrainingTaskDetailResponse,
)
...
def resume_obb_training_task(
    ...
    """从完整 latest checkpoint 继续 OBB 训练任务。"""
