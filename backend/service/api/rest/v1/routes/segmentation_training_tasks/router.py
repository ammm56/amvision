...
@segmentation_training_tasks_router.get(
    "/segmentation/training-tasks/{task_id}/output-files",
    response_model=list[TrainingOutputFileSummaryResponse],
)
def list_segmentation_training_output_files(
    task_id: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("tasks:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
    dataset_storage: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)],
...
@segmentation_training_tasks_router.get(
    "/segmentation/training-tasks/{task_id}/output-files/{file_name}",
    response_model=TrainingOutputFileDetailResponse,
)
def get_segmentation_training_output_file(
    task_id: str,
    file_name: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("tasks:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
...
def resume_segmentation_training_task(
    ...
    """从完整 latest checkpoint 继续 segmentation 训练任务。"""
