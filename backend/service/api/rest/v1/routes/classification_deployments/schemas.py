"""classification deployment API schema。"""
...
class ClassificationDeploymentInstanceCreateRequestBody(BaseModel):
    ...
    runtime_configuration: DeploymentRuntimeConfigurationBody | None = Field(
        default=None,
        description="完整运行时配置；省略时按 backend/device 使用明确默认值",
    )
...
class ClassificationDeploymentInstanceResponse(BaseModel):
    """描述 classification DeploymentInstance 响应。"""
