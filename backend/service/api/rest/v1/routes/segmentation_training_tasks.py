...
def create_segmentation_training_task(
    ...
    if principal.project_ids and body.project_id not in principal.project_ids:
        raise PermissionDeniedError(
            "无权访问该 Project", details={"project_id": body.project_id}
        )
...
@segmentation_training_tasks_router.get(
    "/segmentation/training-tasks", response_model=list[TrainingTaskSummaryResponse]
)
...
@segmentation_training_tasks_router.get(
    "/segmentation/training-tasks/{task_id}", response_model=TrainingTaskDetailResponse
)
...
@segmentation_training_tasks_router.delete(
    "/segmentation/training-tasks/{task_id}", status_code=status.HTTP_204_NO_CONTENT
)
