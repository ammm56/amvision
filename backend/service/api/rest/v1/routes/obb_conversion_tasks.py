...
def create_obb_conversion_task(body: ObbConversionTaskCreateRequestBody, principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("models:read", "tasks:write"))], sf: Annotated[SessionFactory, Depends(get_session_factory)], qb: Annotated[LocalFileQueueBackend, Depends(get_queue_backend)], ds: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)]) -> ObbConversionTaskSubmissionResponse:
    ...
    r = svc.submit_conversion_task(req_cls(project_id=body.project_id, model_type=mt, task_type="obb", model_version_id=body.model_version_id, model_build_id=body.model_build_id, target_format=body.target_format, extra_options=dict(body.extra_options)), created_by=principal.principal_id)
    return ObbConversionTaskSubmissionResponse(task_id=r["task_id"], queue_name=queue_name, queue_task_id=r["queue_task_id"])
