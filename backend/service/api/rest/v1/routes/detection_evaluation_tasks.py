...
detection_evaluation_tasks_router = APIRouter(prefix="/models/detection", tags=["detection"])


class DetectionEvaluationTaskCreateBody(BaseModel):
    """Detection 评估任务创建请求。"""
...
class DetectionEvaluationTaskSubmitResponse(BaseModel):
    """Detection 评估任务提交响应。"""
    task_id: str
    status: str
    queue_name: str
...
@detection_evaluation_tasks_router.post(
    "/evaluation-tasks",
    response_model=DetectionEvaluationTaskSubmitResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
def create_detection_evaluation_task(
    body: DetectionEvaluationTaskCreateBody,
    ...
    task_service = SqlAlchemyDetectionEvaluationTaskService(
    ...
    task_id = task_svc.create_task(CreateTaskRequest(
        ...
        task_spec={
            ...
            "output_prefix": body.output_prefix or f"task-runs/detection-evaluation",
        },
    ))
    ...
    # 记录事件
    task_svc.append_task_event(task_id, "status", "queued", "Detection 评估任务已入队")

    return DetectionEvaluationTaskSubmitResponse(
        task_id=task_id,
        status="queued",
        queue_name=DETECTION_EVALUATION_QUEUE_NAME,
