...
class DeploymentRuntimeCapabilitiesResponse(BaseModel):
    """描述当前机器上的 deployment runtime 能力。"""

    runtime_backend: str
    device_name: str
    available: bool
    hardware: dict[str, object] = Field(default_factory=dict)
    supported_backend_fields: list[str] = Field(default_factory=list)
    read_only_properties: dict[str, object] = Field(default_factory=dict)
    default_runtime_configuration: dict[str, object] = Field(default_factory=dict)
...
class PlatformBaseModelVersionSummaryResponse(BaseModel):
    ...
    dataset_version_id: str | None = Field(
        default=None, description="关联 DatasetVersion id"
    )
    training_task_id: str | None = Field(default=None, description="关联训练任务 id")
    parent_version_id: str | None = Field(
        default=None, description="父 ModelVersion id"
    )
    file_ids: tuple[str, ...] = Field(
        default_factory=tuple, description="关联文件 id 列表"
    )
    metadata: dict[str, object] = Field(default_factory=dict, description="附加元数据")
    checkpoint_file_id: str | None = Field(
        default=None, description="checkpoint 文件 id"
    )
    checkpoint_storage_uri: str | None = Field(
        default=None, description="checkpoint 存储 URI"
    )
    catalog_manifest_object_key: str | None = Field(
        default=None, description="预训练目录 manifest object key"
    )
...
class PlatformBaseModelVersionDetailResponse(PlatformBaseModelVersionSummaryResponse):
    ...
    files: list[PlatformBaseModelFileResponse] = Field(
        default_factory=list, description="版本文件列表"
    )
...
class PlatformBaseModelBuildResponse(BaseModel):
    ...
    runtime_profile_id: str | None = Field(
        default=None, description="目标 RuntimeProfile id"
    )
    ...
    metadata: dict[str, object] = Field(default_factory=dict, description="附加元数据")
    files: list[PlatformBaseModelFileResponse] = Field(
        default_factory=list, description="构建文件列表"
    )
...
class PlatformBaseModelSummaryResponse(BaseModel):
    ...
    project_id: str | None = Field(
        default=None, description="所属 Project id；平台基础模型时为空"
    )
...
class PlatformBaseModelDetailResponse(PlatformBaseModelSummaryResponse):
    ...
    versions: list[PlatformBaseModelVersionDetailResponse] = Field(
        default_factory=list, description="完整版本列表"
    )
    builds: list[PlatformBaseModelBuildResponse] = Field(
        default_factory=list, description="完整构建列表"
    )
