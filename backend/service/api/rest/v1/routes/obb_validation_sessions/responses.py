"""OBB validation session 响应模型和构建函数。"""
...
def build_obb_validation_session_response(
    session_view: ObbValidationSessionView,
    ...
    return ObbValidationSessionDetailResponse(
        ...
        last_prediction=build_obb_validation_prediction_summary_response(
            session_view.last_prediction
        ),
    )


def build_obb_validation_prediction_response(
    prediction_view: ObbValidationPredictionView,
    ...
    return ObbValidationPredictionResponse(
        ...
        runtime_session_info=ObbValidationRuntimeSessionInfoResponse(
            ...
            output_specs=[
                ObbValidationTensorSpecResponse(**build_tensor_spec_payload(spec))
...
def build_obb_validation_prediction_summary_response(
    summary: object | None,
) -> ObbValidationPredictionSummaryResponse | None:
    """把预测摘要转换为 REST 响应。"""
