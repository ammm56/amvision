...
from backend.service.api.rest.v1.routes.pose_inference_tasks.services import (
    get_pose_inference_task_detail_response,
    get_pose_inference_task_result_response,
    infer_pose_deployment_instance_from_request,
    list_pose_inference_task_summaries,
    submit_pose_inference_task_from_request,
)
...
async def create_pose_inference_task(
    ...
    return await submit_pose_inference_task_from_request(
        request=request,
        principal=principal,
...
@pose_inference_tasks_router.post(
    "/pose/deployment-instances/{deployment_instance_id}/infer",
    response_model=dict[str, object],
)
async def infer_pose_deployment_instance(
    deployment_instance_id: str,
    request: Request,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("models:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
    dataset_storage: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)],
    deployment_process_supervisor: Annotated[DeploymentProcessSupervisor, Depends(get_pose_sync_deployment_process_supervisor)],
) -> dict[str, object]:
    """直接执行一次同步 pose 推理并返回结果。"""

    return await infer_pose_deployment_instance_from_request(
...
@pose_inference_tasks_router.get(
    "/pose/inference-tasks",
    response_model=list[InferenceTaskSummaryResponse],
)
def list_pose_inference_tasks(
    ...
    return list_pose_inference_task_summaries(
        principal=principal,
        session_factory=session_factory,
...
@pose_inference_tasks_router.get(
    "/pose/inference-tasks/{task_id}",
    response_model=InferenceTaskDetailResponse,
)
def get_pose_inference_task_detail(
    task_id: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("tasks:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
    ...
    return get_pose_inference_task_detail_response(
        principal=principal,
        task_id=task_id,
        session_factory=session_factory,
...
@pose_inference_tasks_router.get(
    "/pose/inference-tasks/{task_id}/result",
    response_model=InferenceTaskResultResponse,
)
def get_pose_inference_task_result(
    task_id: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("tasks:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
    dataset_storage: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)],
    ...
    return get_pose_inference_task_result_response(
        principal=principal,
        task_id=task_id,
        session_factory=session_factory,
        dataset_storage=dataset_storage,
