...
class ObbTrainingTaskCreateRequestBody(BaseModel):
    ...
    model_type: str = Field(
        description=build_platform_model_type_field_description(OBB_TASK_TYPE)
    )
    ...
    evaluation_interval: int | None = Field(
        default=None, ge=1, description="每隔多少轮执行一次验证"
    )
...
def create_obb_training_task(
    ...
    result = svc.submit_training_task(
        request_cls(
...
@obb_training_tasks_router.get(
    "/obb/training-tasks", response_model=list[TrainingTaskSummaryResponse]
)
def list_obb_training_tasks(
...
@obb_training_tasks_router.get(
    "/obb/training-tasks/{task_id}", response_model=TrainingTaskDetailResponse
)
def get_obb_training_task_detail(
    task_id: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("tasks:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
) -> TrainingTaskDetailResponse:
...
@obb_training_tasks_router.post(
    "/obb/training-tasks/{task_id}/save", response_model=TrainingTaskDetailResponse
)
def request_obb_training_save(
    task_id: str,
    principal: Annotated[
        AuthenticatedPrincipal, Depends(require_scopes("tasks:write"))
    ],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
    dataset_storage: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)],
    queue_backend: Annotated[LocalFileQueueBackend, Depends(get_queue_backend)],
...
@obb_training_tasks_router.post(
    "/obb/training-tasks/{task_id}/pause", response_model=TrainingTaskDetailResponse
)
def request_obb_training_pause(
    task_id: str,
    principal: Annotated[
        AuthenticatedPrincipal, Depends(require_scopes("tasks:write"))
    ],
...
@obb_training_tasks_router.post(
    "/obb/training-tasks/{task_id}/terminate", response_model=TrainingTaskDetailResponse
)
...
@obb_training_tasks_router.post(
    "/obb/training-tasks/{task_id}/resume",
    response_model=TrainingTaskSubmissionResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
def resume_obb_training_task(
    task_id: str,
    principal: Annotated[
        AuthenticatedPrincipal, Depends(require_scopes("tasks:write"))
    ],
    ...
    return resume_training_task(
        session_factory=session_factory, queue_backend=queue_backend, task_id=task_id
    )


@obb_training_tasks_router.delete(
    "/obb/training-tasks/{task_id}", status_code=status.HTTP_204_NO_CONTENT
)
def delete_obb_training_task(
    task_id: str,
    principal: Annotated[
        AuthenticatedPrincipal, Depends(require_scopes("tasks:write"))
    ],
