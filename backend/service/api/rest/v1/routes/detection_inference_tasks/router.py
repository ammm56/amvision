...
from backend.service.api.rest.v1.routes.detection_inference_tasks.services import (
    get_detection_inference_task_detail_response,
    infer_detection_deployment_instance_from_request,
    list_detection_inference_task_summaries,
    require_visible_detection_inference_task,
    submit_detection_inference_task_from_request,
)
...
async def create_detection_inference_task(
    ...
    deployment_process_supervisor: Annotated[
        DeploymentProcessSupervisor,
        Depends(get_detection_async_deployment_process_supervisor),
    ],
    ...
    return await submit_detection_inference_task_from_request(
        request=request,
        principal=principal,
...
async def infer_detection_deployment_instance(
    ...
    deployment_process_supervisor: Annotated[
        DeploymentProcessSupervisor,
        Depends(get_detection_sync_deployment_process_supervisor),
    ],
) -> DetectionInferencePayloadResponse:
    """直接执行一次同步 detection 推理并返回结果。"""

    return await infer_detection_deployment_instance_from_request(
...
def list_detection_inference_tasks(
    ...
    return list_detection_inference_task_summaries(
        principal=principal,
        session_factory=session_factory,
...
def get_detection_inference_task_detail(
    ...
    return get_detection_inference_task_detail_response(
        task_id=task_id,
        principal=principal,
        session_factory=session_factory,
...
def get_detection_inference_task_result(
    ...
    return build_detection_inference_task_result_response(
