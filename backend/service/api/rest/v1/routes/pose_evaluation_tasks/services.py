...
from backend.service.api.rest.v1.routes.pose_evaluation_tasks.responses import (
    ...
    build_pose_evaluation_detail_response,
    build_pose_evaluation_summary_response,
)
...
def list_pose_evaluation_task_responses(
    *,
    principal: AuthenticatedPrincipal,
    session_factory: SessionFactory,
    project_id: str,
    ...
    return [build_pose_evaluation_summary_response(task) for task in tasks]


def get_pose_evaluation_task_response(
    *,
    session_factory: SessionFactory,
    task_id: str,
) -> PoseEvaluationDetailResponse:
    """读取 pose evaluation 任务详情响应。"""
    ...
    return build_pose_evaluation_detail_response(task)


def delete_pose_evaluation_task_response(
    *,
    session_factory: SessionFactory,
    task_id: str,
):
