...
from backend.service.api.rest.v1.routes.classification_training_tasks.controls import (
...
from backend.service.api.rest.v1.routes.classification_training_tasks.responses import (
...
from backend.service.api.rest.v1.routes.classification_training_tasks.services import (
    submit_classification_training_task,
)
...
def create_classification_training_task(
    ...
    """创建 classification 训练任务。"""

    require_project_access(
        principal_project_ids=principal.project_ids,
        project_id=body.project_id,
    )
    return submit_classification_training_task(
        body=body,
...
@classification_training_tasks_router.get(
    "/classification/training-tasks/{task_id}/output-files",
    response_model=list[TrainingOutputFileSummaryResponse],
)
def list_classification_training_output_files(
    task_id: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("tasks:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
    dataset_storage: Annotated[LocalDatasetStorage, Depends(get_dataset_storage)],
) -> list[TrainingOutputFileSummaryResponse]:
    """列出 classification 训练输出文件。"""
...
@classification_training_tasks_router.get(
    "/classification/training-tasks/{task_id}/output-files/{file_name}",
    response_model=TrainingOutputFileDetailResponse,
)
def get_classification_training_output_file(
    task_id: str,
    file_name: str,
    principal: Annotated[AuthenticatedPrincipal, Depends(require_scopes("tasks:read"))],
    session_factory: Annotated[SessionFactory, Depends(get_session_factory)],
