...
def build_visual_prompt_tensor(
    *,
    ...
    input_height = int(letterbox_transform.target_height)
    input_width = int(letterbox_transform.target_width)
    prompt_image_width = int(letterbox_transform.source_width)
    prompt_image_height = int(letterbox_transform.source_height)
    ...
    for index, item in enumerate(prompts):
        ...
        resized_width = int(letterbox_transform.resized_width)
        resized_height = int(letterbox_transform.resized_height)
        prompt_tensor = F.interpolate(
            prompt_tensor.unsqueeze(0).unsqueeze(0),
            size=(resized_height, resized_width),
            mode="nearest",
        )[0, 0]
        pad_left = int(letterbox_transform.pad_left)
        pad_top = int(letterbox_transform.pad_top)
        full_resolution_tensor[
            0,
            index,
    ...
    if (
        int(visual_tensor.shape[-2]) != visual_height
        or int(visual_tensor.shape[-1]) != visual_width
    ):
...
def _build_visual_prompt_mask(
    *,
    ...
    prompt_mask = np_module.zeros(
        (int(prompt_image_height), int(prompt_image_width)), dtype=np_module.uint8
    )
    if getattr(item, "prompt_mask", None) is not None:
        normalized_prompt_mask = np_module.asarray(
            item.prompt_mask, dtype=np_module.uint8
        )
        if normalized_prompt_mask.ndim != 2:
            return prompt_mask
        if int(normalized_prompt_mask.shape[0]) != int(prompt_image_height) or int(
            normalized_prompt_mask.shape[1]
        ) != int(prompt_image_width):
            raise InvalidRequestError(
                ...
                details={
                    ...
                    "prompt_image_size": [
                        int(prompt_image_width),
                        int(prompt_image_height),
                    ],
                },
            )
        return (normalized_prompt_mask > 0).astype(np_module.uint8)
    if (
        getattr(item, "prompt_kind", "") == "box"
        and getattr(item, "bbox_xyxy", None) is not None
    ):
        ...
        x2_index = max(
            x1_index + 1, min(int(prompt_image_width), int(np.ceil(float(x2_value))))
        )
        y2_index = max(
            y1_index + 1, min(int(prompt_image_height), int(np.ceil(float(y2_value))))
        )
        ...
        return prompt_mask
    if (
        getattr(item, "prompt_kind", "") == "point"
        and getattr(item, "point_xy", None) is not None
    ):
        point_x_value, point_y_value = item.point_xy
        point_x_index = max(
            0, min(int(prompt_image_width) - 1, int(round(float(point_x_value))))
        )
        point_y_index = max(
            0, min(int(prompt_image_height) - 1, int(round(float(point_y_value))))
        )
