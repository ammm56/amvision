...
def build_sam3_interactive_prompt_tensors(
    ...
    *,
    ...
    box_items: list[list[list[float]]] = []
    ...
    for item in prompt_items:
        ...
        if item.prompt_kind == "point":
            if not item.point_xy_items or len(item.point_xy_items) != len(
                ...
                raise ValueError(
                    "point prompt 缺少成对的 point_xy_items 与 point_labels"
                )
        ...
        if item.prompt_kind in {"polygon", "mask"}:
            if mask_prompt_width is None or mask_prompt_height is None:
                raise ValueError(
                    "mask 类 prompt 编码要求显式提供 mask_prompt_width 与 mask_prompt_height"
                )
    ...
    if max_prompt_length > 0:
        ...
        point_coords = torch.tensor(
            padded_coord_items, dtype=torch.float32, device=device
        )
        point_labels = torch.tensor(
            padded_label_items, dtype=torch.int32, device=device
        )

    boxes: torch.Tensor | None = None
    if box_items and all(len(box_item) == 2 for box_item in box_items):
        boxes = torch.tensor(box_items, dtype=torch.float32, device=device)

    prompt_masks: torch.Tensor | None = None
    if contains_dense_mask_prompt:
        if len(prompt_items) != 1:
            raise ValueError(
                "当前阶段的 polygon/mask prompt 编码要求逐条处理，不支持在同一次编码中混合多条 prompt"
            )
...
def _build_resized_prompt_mask_tensor(
    ...
    *,
    ...
    """把二值草稿转换为 PromptEncoder 训练路径使用的低分辨率 logits。"""

    mask_tensor = torch.as_tensor(prompt_mask, dtype=torch.float32, device=device)
    if mask_tensor.ndim != 2:
        raise ValueError(
            f"prompt_mask 期望二维数组，实际得到 {tuple(mask_tensor.shape)}"
        )
