...
from custom_nodes.http_nodes.categories.request.backend.runtime.http_client import (
    _build_response_payload,
)
from custom_nodes.http_nodes.categories.request.backend.runtime.parameters import (
    _read_auth_kind,
    _read_body_mode,
    _read_method,
...
from custom_nodes.http_nodes.categories.request.backend.runtime.payloads import (
    _build_body_payload,
...
from custom_nodes.http_nodes.categories.request.backend.runtime.request import (
    _read_headers,
    _sanitize_headers,
)
    ...
    *,
    ...
    try:
        response = execute_bounded_http_request(
            ...
            json=body_payload if method not in {"GET", "DELETE"} else None,
            headers=headers,
    ...
    if require_success and not response.is_success:
        raise ServiceError(
            "HTTP 请求返回非成功状态码",
            code="http_request_unsuccessful_status",
            status_code=502,
            details={
                "node_id": request.node_id,
                "url": url,
