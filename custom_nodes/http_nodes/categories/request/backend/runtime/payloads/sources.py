...
from custom_nodes.http_nodes.categories.request.backend.runtime.types import SourceKind
...
def _read_source_roots(
    *,
    ...
    alarm_payload = _read_optional_inline_object_input(
        request=request,
        input_name="alarm",
        node_name=node_name,
    )
    ...
    value_payload = _read_optional_value_object_input(
        request=request,
        input_name="value",
        node_name=node_name,
    )
    ...
    if not primary_sources:
        raise InvalidRequestError(
            f"{node_name} 缺少主业务输入，必须提供 "
            "result / alarm / workflow_result / summary / value 之一"
        )
    if len(primary_sources) > 1:
        raise InvalidRequestError(
            f"{node_name} 的 result / alarm / workflow_result / summary / value "
            "只能同时提供一个",
            details={
