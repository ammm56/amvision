"""HTTP query 和 body payload 构造。"""

from __future__ import annotations

from backend.service.application.errors import InvalidRequestError
from custom_nodes.http_nodes.categories.request.backend.runtime.payloads.values import (
    _clone_json_object,
    _deep_merge_into,
    _resolve_mapping_value,
    _write_object_path_value,
)
from custom_nodes.http_nodes.categories.request.backend.runtime.types import (
    FieldMappingConfig,
    OnMissingPolicy,
    QueryMappingConfig,
    SourceKind,
...
def _build_body_payload(
    *,
    primary_source_kind: SourceKind,
    ...
    if not body_template and not static_fields and not mappings:
        primary_source = source_roots.get(primary_source_kind)
        if primary_source is not None:
            return _clone_json_object(primary_source)
