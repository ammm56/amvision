"""OpenCV shared circles payload 工具。"""
...
def require_circles_payload(payload: object) -> dict[str, object]:
    ...
    for index, item in enumerate(raw_items, start=1):
        ...
        normalized_item["center_xy"] = list(
            normalize_point_xy(item.get("center_xy"), field_name="center_xy")
        )
        normalized_item["radius"] = require_number(
            item.get("radius"), field_name="radius"
        )
        normalized_item["diameter"] = require_number(
            item.get("diameter"), field_name="diameter"
        )
