...
def build_local_features_payload(
    *,
    ...
    payload: dict[str, object] = {
        ...
        "descriptors": [
            [
                float(cell_value) if descriptor_dtype == "float32" else int(cell_value)
                for cell_value in descriptor
            ]
...
def require_local_features_payload(payload: object) -> dict[str, object]:
    ...
    normalized_descriptors: list[list[int | float]] = []
    descriptor_dtype = str(payload.get("descriptor_dtype", "uint8")).strip().lower()
    if descriptor_dtype not in {"uint8", "float32"}:
        raise InvalidRequestError("descriptor_dtype 仅支持 uint8 或 float32")
    for feature_index, raw_item in enumerate(raw_items):
        ...
        if (
            not isinstance(raw_descriptor, list)
            or len(raw_descriptor) != descriptor_length
        ):
            raise InvalidRequestError(
                "当前节点要求每个 descriptor 都必须与 descriptor_length 一致"
            )
        ...
        normalized_items.append(
            {
                ...
                "point_xy": list(
                    normalize_point_xy(
                        raw_item.get("point_xy"), field_name="feature.point_xy"
                    )
                ),
                "size": require_number(raw_item.get("size"), field_name="feature.size"),
                "angle_deg": require_number(
                    raw_item.get("angle_deg"), field_name="feature.angle_deg"
                ),
                "response": require_number(
                    raw_item.get("response"), field_name="feature.response"
                ),
        ...
        normalized_descriptors.append(
            [
                require_number(cell_value, field_name="descriptor")
                if descriptor_dtype == "float32"
                else require_uint8_int(cell_value, field_name="descriptor")
                for cell_value in raw_descriptor
            ]
        )
    ...
    normalized_payload["descriptor_dtype"] = descriptor_dtype
    ...
    if (
        not isinstance(resolved_source_object_key, str)
        or not resolved_source_object_key
    ):
...
def require_feature_matches_payload(payload: object) -> dict[str, object]:
        ...
        normalized_items.append(
            {
                ...
                "distance": require_non_negative_float(
                    raw_item.get("distance"), field_name="distance"
                ),
                "query_xy": list(
                    normalize_point_xy(raw_item.get("query_xy"), field_name="query_xy")
                ),
                "train_xy": list(
                    normalize_point_xy(raw_item.get("train_xy"), field_name="train_xy")
                ),
            }
        )
    ...
    if (
        not isinstance(resolved_source_a_object_key, str)
        or not resolved_source_a_object_key
    ):
    ...
    if (
        not isinstance(resolved_source_b_object_key, str)
        or not resolved_source_b_object_key
    ):
