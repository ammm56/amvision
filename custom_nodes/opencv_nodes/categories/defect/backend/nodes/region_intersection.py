...
NODE_TYPE_ID = "custom.opencv.region-intersection"


def handle_node(request: WorkflowNodeExecutionRequest) -> dict[str, object]:
    """对 region_a 和 region_b 执行二值交集。"""
    ...
    result = cv2.bitwise_and(mask_a, mask_b)
    return build_mask_result(
        request,
        source_payload=payload_a,
        mask=result,
        variant_name="region-intersection",
        summary={"operation": "intersection"},
    )
