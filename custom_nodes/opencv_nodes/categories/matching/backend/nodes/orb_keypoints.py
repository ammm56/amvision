...
from custom_nodes.opencv_nodes.shared.backend.runtime.features import build_local_features_payload
...
def _build_orb_keypoints_interaction(
    *,
    ...
    return build_debug_panel_interaction(
        tools=[
            build_interaction_tool("rect", "参考区域", ["search_bbox_xyxy"]),
        ],
        controls=[
            build_numeric_control(
                "max_features",
            ...
            build_numeric_control("level_count", "Level Count", level_count, min_value=1.0, max_value=16.0, step=1.0),
            build_numeric_control(
                "edge_threshold",
                "Edge Threshold",
                edge_threshold,
            ...
            build_numeric_control("patch_size", "Patch Size", patch_size, min_value=2.0, max_value=128.0, step=1.0),
            build_numeric_control(
                "fast_threshold",
                "FAST Threshold",
                fast_threshold,
    ...
    *,
    ...
    if roi_payload is not None:
        ...
        if isinstance(polygon_xy, list) and len(polygon_xy) >= 3:
            overlays.append(
                build_polygon_overlay(
                    overlay_id=str(roi_payload.get("roi_id") or "roi-mask"),
    ...
    for feature_item in feature_items[:500]:
        ...
        overlays.append(
            build_circle_overlay(
                overlay_id=str(feature_item.get("feature_id") or f"feature-{feature_index}"),
