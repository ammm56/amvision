...
amvar app 的选择式打包下载、上传预检、资源映射、依赖检查、停止态创建及 SDK 配置生成，统一见[应用实施基线](../development/workflow-views-and-app-packages-implementation.md)。这是待实现的业务应用交付，不是本目录已有的 `assemble-release` 平台发行包，也不包含数据库或进程状态恢复；尚不能作为现有部署命令执行。
