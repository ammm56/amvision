...
- DeploymentInstance 通过 `runtime_configuration.lifecycle` 独立配置 `warmup_dummy_inference_count`、`warmup_dummy_image_size`、`keep_warm_enabled` 和 `keep_warm_interval_seconds`；新建部署默认显式关闭 keep-warm，适合笔记本低功耗 GPU 等会快速降频的设备时再按实例启用。TensorRT pinned output 配置位于 `runtime_configuration.backend_options`。
