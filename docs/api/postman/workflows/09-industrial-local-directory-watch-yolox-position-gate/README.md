# 09 Directory Watch YOLOX Position Gate 调试说明

## 文档目的

本目录用于说明 `09-industrial-local-directory-watch-yolox-position-gate.postman_collection.json` 的实际导入、改值和联调顺序。
...
默认回传地址写的是 `http://127.0.0.1:18080/directory-watch/yolox-position-batch-result`。当前 template 里 `http-post` 节点使用 `require_success=false`，因此本地没有回调服务时不会把整条 workflow 直接打失败，但现场联调时仍建议改成真实回传地址。
...
- 这条链依赖已存在的 YOLOX deployment；如果 deployment 不可用，目录监听虽然能收到事件，但 workflow 仍会在推理节点失败。
