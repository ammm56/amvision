...
- WorkflowAppRuntime 通过 active revision 固定不可变 application/template 发布快照，是现场长期运行的宿主。
- WorkflowTriggerSource 绑定 WorkflowAppRuntime，并把外部事件映射到 application 的 `input_bindings`，不修改 workflow 图。
- Trigger 在创建 WorkflowRun 时固定 Runtime 当时的 active revision、version、generation 和 snapshot fingerprint；完整规则见 [docs/architecture/workflow-app-versioning.md](../architecture/workflow-app-versioning.md)。
...
- 同一张 workflow 图可以被多个入口复用，例如 HTTP 调试入口绑定 `request_image_base64`，ZeroMQ 高速入口绑定 `request_image_ref`，然后在图里通过显式转换节点汇合。ZeroMQ TriggerSource 默认只映射 `request_image_ref` 图片入口，避免把大图通过 base64 JSON 传输。
...
当前 `core.output.http-response` 可以继续作为 HTTP invoke 的默认输出，`core.output.response-envelope` 可以作为协议中立的业务包体。当前也已经补了更通用的 `workflow-result.v1` payload 和 `core.output.workflow-result` 节点，用于表达以下内容：
...
当前公开实现只接受以下明确组合，不对未实现的配置做“保存但忽略”：

- `submit_mode=sync`：`ack_policy=ack-after-run-finished`、`result_mode=sync-reply`。
- `submit_mode=async`：`ack_policy=ack-after-run-created`、`result_mode=accepted-then-query` 或 `event-only`。

未显式填写 `ack_policy` 和 `result_mode` 时由 `submit_mode` 推导上述默认值。`async-report` 需要额外的 PUB/DEALER、callback 或 PLC write adapter，仍属于后续阶段，当前 API 不接受该值。

`debounce_window_ms` 已在提交 WorkflowRun 前生效；`idempotency_key_path` 提取出的键按 `trigger_source_id + idempotency_key` 建立进程内 TTL single-flight 记录，并发或短期重试复用首个结果。该缓存用于运行期重复抑制，不替代跨 backend-service 重启的持久化幂等存储。
...
- 单次请求：适规则步高速推理，外部进程发送 metadata 和一张图片，adapter 写入 LocalBufferBroker 后调用 WorkflowAppRuntime，完成后返回 ZeroMQ reply。
...
adapter 收到二进制帧后先写入 LocalBufferBroker，随后把 BufferRef 或 FrameRef 映射到 application input_bindings。这个映射停留在协议入口层，不继续替图做跨 payload type 转换、本地磁盘读图或相机取帧。同步返回时，ResultDispatcher 读取 workflow 输出绑定，转换成 JSON reply 或 multipart reply；需要返回图片时，优先返回 ObjectStore 引用或受控 BufferRef 摘要，不把本机 mmap path 当成长期外部结果。
...
LocalBufferBroker 由 `config/backend-service.json` 的 `local_buffer_broker.default_pool_name` 和 `local_buffer_broker.pools` 显式配置，不再使用 `default_pool` 简化配置；仍出现旧字段时服务启动会直接失败，避免旧配置被静默忽略。仓库默认创建 `image-4k`、`image-1080p` 和 `image-640x640` 三个 pool，默认 pool 为 `image-4k`。`image-4k` 单槽 128MB、16 个槽位，用于覆盖 5000x4000 级 20MP 工业相机 raw RGB/RGBA 图片输入和短时间并发占用；`image-1080p` 单槽 16MB、16 个槽位，用于 1080p 图片和中间结果；`image-640x640` 单槽 4MB、16 个槽位，用于 640x640 低分辨率图片和小图输入。`image-8k` 单槽 256MB、16 个槽位，仅作为现场可选大图 pool，不在仓库默认配置中创建，避免未使用时初始化 4GB 级 mmap 文件。低内存设备可以把 `slot_count` 调整为 8 或 4；现场应按工业相机分辨率、图像编码方式、单帧最大 bytes 和并发占用量调整 `pool_name`、`slot_size_bytes`、`slot_count` 和 `flush_on_write`，不依赖运行时自动猜测图片大小；mmap 文件名按 `pool_name` 自动生成，总大小按 `slot_size_bytes * slot_count` 自动计算。前端集成页面通过 `/api/v1/system/config` 读取当前后端实际配置，再从 `local_buffer_broker.pools` 生成 pool 下拉选项，不维护独立默认 pool 列表。
...
1. 保存同时支持 HTTP base64 和 ZeroMQ image-ref 的 template 和 application：
  - 06：`docs/api/examples/workflows/06-detection-deployment-infer-opencv-health-zeromq-image-ref/save-template.request.json` 与 `save-application.request.json`
...
2. 创建并启动对应 WorkflowAppRuntime：
  - 06 runtime 创建请求体：`docs/api/examples/workflows/06-detection-deployment-infer-opencv-health-zeromq-image-ref/app-runtime.create.request.json`
...
3. 使用 TriggerSource 管理 API 创建 ZeroMQ TriggerSource：
  - 06 请求体：`docs/api/examples/workflows/06-detection-deployment-infer-opencv-health-zeromq-image-ref/trigger-source.create.request.json`
  - 07 请求体：`docs/api/examples/workflows/07-opencv-process-save-image-zeromq-image-ref/trigger-source.create.request.json`
  - 08 请求体：`docs/api/examples/workflows/08-plc-register-modbus-tcp-async-result-record/trigger-source.create.request.json`
  - 09 请求体：`docs/api/examples/workflows/09-industrial-local-directory-watch-detection-position-gate/trigger-source.create.request.json`
  - 11 请求体：`docs/api/examples/workflows/11-industrial-local-directory-poll-detection-position-gate/trigger-source.create.request.json`
...
- 如果同一个 workflow app 既要接 HTTP base64，又要接 ZeroMQ image-ref，应在图里发布两个 binding。默认推荐 `request_image_ref -> Image Ref Coalesce -> 下游节点`，HTTP base64 入口只通过 `Image Base64 Decode` 接入 fallback；高频链路不把 image-ref 转回 base64。
...
当前 .NET SDK 包名为 `Amvision.Workflows`，调用方通过 `AmvisionTriggerClient`、`ImageTriggerRequest` 和 `TriggerResult` 直接接入 ZeroMQ TriggerSource。仓库不再保留独立 `ZeroMqImageInvoke` 示例项目；真实 06/07 backend-service 调试应使用 SDK smoke 测试或在现场调用方程序中嵌入 SDK 调用。

SDK 的 `AmvisionWorkflowClient.GetSystemConfigResponseAsync()` 可读取 `/api/v1/system/config`，并从 `LocalBufferBroker.DefaultPoolName` 与 `LocalBufferBroker.Pools` 获取后端实际 pool 配置。创建 ZeroMQ TriggerSource 时应把选中的 pool 写入 `WorkflowTriggerSourceCreateRequest.TransportConfig["pool_name"]`，避免 SDK 或前端维护独立默认列表。
...
- TriggerSource 正式调用默认使用 `workflow_run_record_mode=minimal`：同步触发只在完成后写一条轻量 WorkflowRun 记录，普通入口保留公开 outputs 和必要状态，不保留 input_payload、template_outputs 和 node_records。HTTP、ZeroMQ 和后续协议入口统一默认关闭 trace；其中 ZeroMQ 图片入口继续使用 LocalBuffer 数据面，并为最高吞吐额外关闭输入和输出持久化，结果仍通过当前同步 reply 返回。
...
- directory-poll：按固定周期扫描目录中新到达且已稳定落地的文件后触发
- directory-watch：监听目录创建、改名进入等文件事件后触发
...
- project_id
- display_name
- trigger_kind，当前规则取值为 directory-poll、directory-watch、plc-register、mqtt-topic、zeromq-topic、grpc-method、io-change、sensor-read、schedule、webhook、http-api
- workflow_runtime_id
...
| --- | --- | --- |
| directory-poll | async | 目录轮询属于常驻后台入口，没有同步调用方连接可用于等待完整执行结果 |
| directory-watch | async | 目录事件监听同样更接近事件到达后创建正式执行记录，不适合阻塞事件线程等待 workflow 执行结束 |
...
- 建议先以 `enabled = false` 创建，再显式调用 `POST /enable` 启动；当前 create 主要负责落配置，不直接承担 adapter 启动动作
...
- backend-service 启动时先完成所有可恢复 `desired_state=running` Runtime 的一次有界并行恢复，再恢复 enabled TriggerSource；`WorkflowRuntimeWorkerManager.start()` 返回就是这条 readiness 屏障。成功恢复的 TriggerSource 因而不会在 worker 尚未就绪时提前接收首批外部请求。Runtime 恢复失败会先落明确失败状态，对应 TriggerSource 不会被伪装成可用。该顺序不引入请求队列或同步调用重试。
- TriggerSource 恢复服务仍能识别 `starting` 等过渡记录，用于独立测试和受控恢复调用；正式 backend bootstrap 不依赖这一宽松条件跨过 worker readiness 屏障。
- trigger_kind 尚未注册 adapter 时，enable 会显式失败；当前资源会写回 `observed_state = failed`、`health_summary.adapter_configured = false` 和最近错误摘要，避免停留在“已启用但未运行”的模糊状态
...
## `plc-register` 最小请求 JSON 示例

当前 `plc-register` 已接入正式 adapter。第一阶段只支持：

- `transport_config.driver = modbus-tcp`
- 后台 polling
- `submit_mode = async`
- 地址语义统一使用 `0xxxx / 1xxxx / 3xxxx / 4xxxx`
- `match_rule.trigger_mode = enter-match`
...
下面这份 JSON 是当前已实现的 Modbus TCP trigger-source 最小示例；checked-in 文件见 `docs/api/examples/workflows/08-plc-register-modbus-tcp-async-result-record/trigger-source.create.request.json`。完整 workflow app 源文件位于 `docs/examples/workflows/plc_register_modbus_tcp_async_result_record.*`。
...
```json
{
  ...
  "match_rule": {
    "operator": "bitmask_all_set",
    "expected_value": 5,
    "stable_match_count": 2,
    "trigger_mode": "enter-match",
    "cooldown_ms": 200,
    "emit_initial_match": false
  },
  "input_binding_mapping": {
  ...
  "result_mapping": {
    "result_binding": "inspection_result",
    "result_mode": "accepted-then-query"
  },
  "default_execution_metadata": {
    "scenario": "plc-register-modbus-tcp-async-result-record",
  ...
  "metadata": {
    "line_id": "line-a",
    "example_kind": "plc-register-modbus-tcp-async-result-record"
  }
}
```
...
当前 `directory-poll` 已接入正式 adapter。更贴现场的推荐用法是：

- `submit_mode = async`
- 按固定 `scan_interval_seconds` 周期扫描目录
- 通过 `min_stable_age_seconds` 过滤仍在写入中的文件
- 通过 `persist_checkpoint = true` 持续保留已处理 identity
- 通过静态 `deployment_request` 绑定固定 deployment，而不是每轮从外层再传一遍

checked-in 的配置请求体见 `docs/api/examples/workflows/11-industrial-local-directory-poll-detection-position-gate/trigger-source.create.request.json`。它对应的 workflow app 源文件位于：

- `docs/examples/workflows/industrial_local_directory_poll_detection_position_gate.template.json`
- `docs/examples/workflows/industrial_local_directory_poll_detection_position_gate.application.json`

如果只需要本地 synthetic event 联调，同目录还提供：
...
- 当前 `directory-poll` 和 `directory-watch` 一样，`input_binding_mapping` 只负责从标准化事件上下文里读取原始 `payload / event` 对象，不会按 `payload_type_id` 自动再包装成 `value.v1`
- 因此这条样例继续把 `request_trigger_payload` 与 `request_trigger_event` 定义成 `response-body.v1`，再在 workflow 图内通过 `payload-to-value` 和 `value-field-extract` 做显式桥接
- `deployment_request` 在这条样例里同样通过 `input_binding_mapping.deployment_request.value` 静态注入；适合“固定目录轮询 + 固定 published deployment”的现场
- `directory-poll` 的扫描频率、稳定期和 checkpoint 恢复都由 adapter 负责；workflow 图只承接“目录批次 -> 检测 -> 规则 -> batch-record / JSON / HTTP”这条业务链
- 对应 Postman collection 的导入变量、推荐执行顺序和现场排查重点见 [docs/api/postman/workflows/11-industrial-local-directory-poll-detection-position-gate/README.md](postman/workflows/11-industrial-local-directory-poll-detection-position-gate/README.md)

## `directory-watch` 工业目录事件配置示例

当前 `directory-watch` 已接入正式 adapter。更贴现场的推荐用法是：

- `submit_mode = async`
- 目录新增或改名进入后触发
- 通过 `min_stable_age_seconds` 过滤仍在写入中的文件
- 通过 `persist_checkpoint = true` 持续保留已处理 identity
- 通过静态 `deployment_request` 绑定固定 deployment，而不是每次从外层再传一遍

checked-in 的配置请求体见 `docs/api/examples/workflows/09-industrial-local-directory-watch-detection-position-gate/trigger-source.create.request.json`。它对应的 workflow app 源文件位于：

- `docs/examples/workflows/industrial_local_directory_watch_detection_position_gate.template.json`
- `docs/examples/workflows/industrial_local_directory_watch_detection_position_gate.application.json`
...
下面这份 JSON 是当前更贴工业目录守护接入的一份正式示例：

```json
{
  "trigger_source_id": "directory-watch-trigger-source-09",
  "project_id": "project-1",
  "display_name": "Directory Watch TriggerSource 09 Industrial Detection Position Gate",
  ...
  "result_mapping": {
    "result_binding": "batch_record",
    "result_mode": "accepted-then-query"
  },
  "default_execution_metadata": {
    "scenario": "industrial-local-directory-watch-detection-position-gate",
    "trigger_source": "directory-watch",
    "trace_level": "none",
    "retain_trace_enabled": false,
    "retain_node_records_enabled": false
  },
  ...
  "result_mode": "accepted-then-query",
  "debounce_window_ms": 0,
  "idempotency_key_path": "payload.batch_id",
  "metadata": {
    "example_kind": "industrial-local-directory-watch-detection-position-gate",
    "station_id": "line-c-01",
    "line_id": "line-c"
  }
}
```
...
- 当前 `directory-watch` 和 `plc-register` 一样，`input_binding_mapping` 只负责从标准化事件上下文里读取原始 `payload / event` 对象，不会按 `payload_type_id` 自动再包装成 `value.v1`
- 因此这条样例继续把 `request_trigger_payload` 与 `request_trigger_event` 定义成 `response-body.v1`，再在 workflow 图内通过 `payload-to-value` 和 `value-field-extract` 做显式桥接
- `deployment_request` 在这条样例里通过 `input_binding_mapping.deployment_request.value` 静态注入；适合“目录监听固定接一条 published deployment”的现场。如果现场需要频繁换 deployment，建议由上层应用保存多条 TriggerSource，而不是把 deployment 选择逻辑埋进 adapter
- `request_roi` 在对应 workflow app 里仍然保持可选运行时输入；当前这条 TriggerSource 示例故意不在 adapter 层静态写死 ROI，目的是保留模板里的默认 ROI，同时避免把换型参数和目录监听配置混在一起
- `force_polling = true` 更适合 Windows 本地磁盘、网络共享目录或“先写临时文件再改名落地”的守护式接入；如果现场已经验证原生目录事件稳定，可改回 `null` 或 `false`
- checkpoint 文件路径当前由 adapter 按 `trigger_source_id` 自动落到 TriggerSource 运行目录，不需要在 create 请求里手工指定；是否真正落盘和恢复，可通过 `GET /api/v1/workflows/trigger-sources/{trigger_source_id}/health` 查看 `checkpoint_path`、`known_identity_count` 和 `sequence_id`
- 对应 Postman collection 的导入变量、推荐执行顺序和现场排查重点见 [docs/api/postman/workflows/09-industrial-local-directory-watch-detection-position-gate/README.md](postman/workflows/09-industrial-local-directory-watch-detection-position-gate/README.md)
...
2. 定义 WorkflowTriggerSource、TriggerEvent、TriggerResult、InputBindingMapping 和 ResultMapping 规则。
...
- 规则先允许 `transport_config`、`match_rule`、`metadata` 使用对象字段，避免第一阶段为每个协议拆过细 schema。
...
- 规则能序列化和反序列化 REST、ZeroMQ、PLC、MQTT 等触发源配置。
...
- 返回规则使用 `WorkflowTriggerSourceContract`，不直接返回内部 adapter 对象。
...
- `workflow-result.v1` payload 规则 已实现。
- `core.output.workflow-result` 节点已实现，输出 status、code、message、data、files、metrics、trace_id 和 event_id。
- ResultDispatcher 支持优先读取 `result_mapping.result_binding`，例如 `http_response`。历史精确值 `workflow_result` 是“返回全部 outputs”的 fallback 标记，不要求公开契约存在同名 output binding；`accepted-then-query` 和 `event-only` 不读取 result binding。其他 `sync-reply` binding 必须存在于版本公开输出契约中。
...
1. gRPC method：适规则步请求响应，和 ZeroMQ sync-reply 共享 ResultDispatcher。
