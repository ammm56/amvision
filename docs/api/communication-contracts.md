...
- node pack events：节点包发现、跳过、兼容性校验、回调触发
...
- 订阅主题必须与任务、部署、节点包、项目或全局事件范围明确对应
...
- ZeroMQ 等触发入口不直接改写 workflow 图；应通过 TriggerSource 映射到 FlowApplication 输入绑定
- 同步触发需要即时返回时，应由结果回执层读取 workflow 输出绑定并转换为对应协议响应
...
- LocalBufferBroker 的当前设计见 [LocalBufferBroker](../architecture/platform/local-buffer-broker.md)
...
- 自定义节点触发与回调应声明可访问的事件范围、资源范围和外部端点范围
