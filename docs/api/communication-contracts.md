...
本文档用于定义平台公开通信边界与内部通信边界，明确 REST API、WebSocket 和 ZeroMQ 各自的职责范围、适用场景和非职责边界。
...
- 任务状态、日志、事件、结果引用和回调消息的契约边界
- 版本化、兼容性、鉴权和错误语义

## 总体原则

- 浏览器前端与外部系统原则上共用同一套公开通信契约
...
- ZeroMQ 传输的消息不能替代公开接口的版本契约与审计要求

## 三类通信边界的职责拆分

### REST API

REST API 负责请求响应式交互、资源查询与治理动作。
...
- 跨公网或跨组织边界的对外公开接口
- 资源查询和治理动作的主接口
- 替代 REST API 与 WebSocket 的版本化契约

## 通信职责矩阵

- REST API：资源读写、治理动作、同步请求响应
