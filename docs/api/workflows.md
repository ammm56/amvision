...
- 任务节点：例如 core.service.yolox-training.submit、core.service.yolox-conversion.submit、core.service.yolox-evaluation.submit、core.service.dataset-export.submit、core.service.detection-inference.submit。这组节点直接调用现有应用服务并提交后台任务，训练、转换、评估、导出和异步推理仍由独立 worker 与 queue backend 执行。
