...
| gpu_count | integer \| null | 否 | 请求参与训练的 GPU 数量；需要是正整数。未指定时按运行环境自动解析；显式指定 GPU 但当前环境无可用 CUDA 时会直接报错，避免静默回退到 CPU。 |
...
- YOLOX / YOLOv8 / YOLO11 / YOLO26 当前 GPU 数量控制采用单机单进程模式；`gpu_count` 大于 1 时使用 `DataParallel`，不引入 exp 文件体系或分布式脚本。RF-DETR detection 会把 `gpu_count` 转入 RF-DETR core 的 Lightning `devices` 配置。
