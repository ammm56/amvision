# 插件 Trigger 与 Hook 规范

## 文档目的

本文档用于定义插件触发器、事件 hook、回调点和数据上报点的统一模型，明确插件在哪些时机被调用、接收什么载荷、允许做什么以及如何治理。
...
- backend-service 负责注册、鉴权、启停和事件范围治理
- 插件的触发或 hook 结果若影响公开状态，必须先回写 backend-service
- 触发与 hook 不能绕开任务状态模型或插件超时治理
...
- 任务完成后可触发回调插件或结果上报插件
- 回调发送前应以 backend-service 中的权威状态和结果引用为准
...
- 后处理不应覆盖原始结果引用，而应产生新的结果视图、衍生 artifact 或结构化摘要
