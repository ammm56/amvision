...
| `YOLOv8 detection 真实短链路` | 2026-06-18 使用真实 `config/backend-worker.json` 启动 backend-service + backend-worker，并基于 `data/files/datasets/detection/medical-pills` 跑通真实 zip 导入、`yolo-detection-v1` DatasetExport、1 epoch 短训练、ONNX / OpenVINO / TensorRT 转换、三类 build 的 deployment sync / async 推理和 stop。该轮验证不进入默认测试；回归点已沉淀到 `tests/test_dataset_import_api.py`、`tests/test_worker_profile_manifests.py`、`tests/test_yolov8_conversion_worker.py`、`tests/test_rfdetr_conversion_worker.py` 和 `tests/test_yolox_async_inference_gateway.py`。 |
