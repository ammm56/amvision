...
- detection 公开接口里的 `gpu_count` 当前已经有前端输入，并在 detection 执行层生效：`yolox` 已删除同步 `DataParallel` 路径，`gpu_count > 1` 会要求走 DDP TrainingBackend；`yolov8 / yolo11 / yolo26` 仍是待替换的单机单进程 `DataParallel`；`rfdetr` 转入 RF-DETR core 的 Lightning `devices` 配置。完整 DDP TrainingBackend 的目标边界、实现顺序和验收标准见 [DDP TrainingBackend 实现计划](ddp-training-backend-plan.md)。classification / segmentation / pose / obb 当前没有公开顶层 `gpu_count`，默认仍按各自单设备训练路径执行。
...
| `gpu_count` | 公开 + 执行 | 未公开 | 未公开 | 未公开 | 未公开 | detection 已暴露 | detection 当前生效但仍是过渡多 GPU；非 detection 当前不公开顶层多 GPU 参数 |
...
| 当前缺口 | 多 GPU 当前是单进程 `DataParallel`，不是多进程 DDP；DDP 已建立通用 support，但 YOLOv8 / YOLO11 / YOLO26 detection 尚未切换实际训练执行路径 |
