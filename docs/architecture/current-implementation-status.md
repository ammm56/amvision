...
- `opencv_basic_nodes` 当前也已补完前三批更贴工业现场的传统视觉节点，并把预处理、形状与形态基础层都补进来了：前置预处理/对齐层 `crop / normalize / clahe / median-blur / bilateral-filter / invert / rotation-correct / perspective-transform / undistort / remap` 已接通，形状/形态补强层 `contour-approx / convex-hull / fit-ellipse / fill-holes / distance-transform` 已接通，第一批 `grayscale / resize / adaptive-threshold / otsu-threshold / contour-filter / min-area-rect / contours-to-regions` 已接通，第二批 `image-diff / absdiff-threshold / connected-components` 已接通，第三批 `hough-lines / hough-circles / fit-line / min-enclosing-circle` 也已接通；其中 `crop` 当前支持 `roi.v1` 或显式矩形参数两种裁剪入口，`rotation-correct` 当前支持直接读取 `value.v1` 角度输入并输出姿态矫正摘要，`perspective-transform` 当前支持 `roi.v1` 或固定 `source_points` 四点输入，并按估算或显式目标尺寸输出规则化后的矫正图，`undistort` 当前支持 `value.v1` 标定配置对象或固定参数输入，`remap` 当前支持 `value.v1` 映射对象里的 `map_xy` 或 `map_x/map_y` 两类映射方式，`fit-ellipse` 当前新增正式 `ellipses.v1` 结构化 payload 并接入 `payload-to-value`，`fill-holes` 当前可直接把白色前景内部孔洞收平，`distance-transform` 当前输出灰度预览图与最大/平均距离摘要，`contours-to-regions` 当前会把 contour 结果直接规整成标准 `regions.v1` 接入既有工业规则链，`min-area-rect` 当前新增 `rotated-rects.v1`，`hough-lines` 与 `fit-line` 当前统一输出 `lines.v1`，`hough-circles` 与 `min-enclosing-circle` 当前统一输出 `circles.v1`，并已由 `custom.opencv.payload-to-value` 统一桥接回 `value.v1` 以便继续做响应拼装和调试预览；而 `image-diff -> absdiff-threshold -> connected-components` 当前已经能把参考图差异或缺陷前景直接规整成带 `mask_image` 的 `regions.v1`，继续接入既有面积、覆盖率、连续性和 OK/NG 工业规则链。第二阶段当前已经收口前两项：`custom.opencv.template-match` 已接通，支持 `template_image + 可选 roi.v1 -> regions.v1 + summary`；`custom.opencv.caliper-edge` 也已接通，支持 `image + 可选 roi.v1 -> lines.v1 + summary`，可直接用于台阶边、工件边和治具定位边的单边基准量测。
