...
- USB / UVC 相机这条线当前也已继续前移到工业单帧主线：`industrial_single_frame_usb_uvc_yolox_position_gate.*` 与 `industrial_single_frame_usb_uvc_sam3_semantic_continuity_gate.*` 已接通文档规则校验，分别覆盖“相机直采 -> YOLOX 检测 -> 位置门判定 -> result-record”和“相机直采 -> SAM3 语义分割 -> 覆盖率/面积/连续性判定 -> result-record”两条现场常用闭环。
