...
- USB / UVC 相机这条线当前也已补出第一批 checked-in workflow 样例：`camera_usb_uvc_enumerate_capture_preview.*`、`camera_usb_uvc_session_single_frame_review.*` 与 `camera_usb_uvc_stream_window_preview.*` 已接通文档合同校验，分别覆盖“枚举 -> 直采预览”“open -> set/get -> 单帧采图 -> close”和“open -> start-stream -> read-window -> frame-window-preview -> close”三类最常见的现场联调入口。
- USB / UVC 相机这条线当前也已继续前移到工业单帧主线：`industrial_single_frame_usb_uvc_yolox_position_gate.*` 与 `industrial_single_frame_usb_uvc_sam3_semantic_continuity_gate.*` 已接通文档合同校验，分别覆盖“相机直采 -> YOLOX 检测 -> 位置门判定 -> result-record”和“相机直采 -> SAM3 语义分割 -> 覆盖率/面积/连续性判定 -> result-record”两条现场常用闭环。
- OpenCV pack 拆分这条线当前已完成前四轮试点：共享底座已从 `custom_nodes/opencv_basic_nodes/backend/support.py` 抽到 `custom_nodes/_opencv_shared/backend/support.py`，OpenCV custom payload contract 也已统一收进 `custom_nodes/_opencv_shared/workflow/payload_contracts.json`；`rotation-correct / perspective-transform / affine-transform / undistort / remap` 已正式迁入新 pack `opencv.geometry-nodes`，`measure / caliper-edge / point-distance / point-to-line-distance / line-angle / circle-diameter / slot-width / parallelism-metrics / concentricity-metrics` 已正式迁入新 pack `opencv.measurement-nodes`，`contour / contour-filter / contour-approx / convex-hull / min-area-rect / fit-ellipse / contours-to-regions / hough-lines / hough-circles / fit-line / min-enclosing-circle` 已正式迁入新 pack `opencv.shape-nodes`，`image-diff / absdiff-threshold / connected-components / fill-holes / distance-transform` 也已正式迁入新 pack `opencv.defect-nodes`。当前 `opencv_basic_nodes` 则保留预处理、匹配、渲染与桥接主线，不再继续承载几何矫正、工业量测、轮廓抽取与差异缺陷后处理实现。`opencv_basic_nodes` 当前已覆盖 `crop / normalize / clahe / median-blur / bilateral-filter / invert / grayscale / resize / adaptive-threshold / otsu-threshold / sobel / laplacian` 这组前置预处理与边缘增强，以及 `template-match / draw-* / mask-overlay / crop-export / gallery-preview / payload-to-value` 这组匹配、调试渲染和桥接节点；`opencv.defect-nodes` 当前专门承载 `image-diff / absdiff-threshold / connected-components / fill-holes / distance-transform` 这条差异缺陷与形态学后处理链，`opencv.shape-nodes` 当前专门承载 contour、线圆抽取、形状拟合和 `contours-to-regions` 结构化桥接，`opencv.geometry-nodes` 当前专门承载 `rotation-correct / perspective-transform / affine-transform / undistort / remap`，其中 `rotation-correct` 支持直接读取 `value.v1` 角度输入并输出姿态矫正摘要，`perspective-transform` 支持 `roi.v1` 或固定 `source_points` 四点输入，`affine-transform` 支持固定参数或 `value.v1` 动态输入两种仿射来源，`undistort` 支持 `value.v1` 标定配置对象或固定参数输入，`remap` 支持 `value.v1` 映射对象里的 `map_xy` 或 `map_x/map_y` 两类映射方式；`opencv.measurement-nodes` 当前则专门承载 `measure / caliper-edge / point-distance / point-to-line-distance / line-angle / circle-diameter / slot-width / parallelism-metrics / concentricity-metrics`。使用面当前分别由 checked-in 样例 `industrial_single_frame_calibrated_template_edge_gate.*`、`industrial_single_frame_line_pair_measure_gate.*` 与 `industrial_single_frame_circle_concentricity_gate.*` 收口，继续验证“标定定位链”“双边线量测链”和“双圆量测链”在拆包后不被破坏。
