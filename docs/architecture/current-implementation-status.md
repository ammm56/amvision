...
- `opencv_basic_nodes` 当前也已补完前三批更贴工业现场的传统视觉节点，并把预处理、形状与形态基础层都补进来了：前置预处理/对齐层 `crop / normalize / clahe / median-blur / bilateral-filter / invert / rotation-correct / perspective-transform / affine-transform / undistort / remap` 已接通，形状/形态补强层 `contour-approx / convex-hull / fit-ellipse / fill-holes / distance-transform` 已接通，第一批 `grayscale / resize / adaptive-threshold / otsu-threshold / contour-filter / min-area-rect / contours-to-regions` 已接通，第二批 `image-diff / absdiff-threshold / connected-components` 已接通，第三批 `hough-lines / hough-circles / fit-line / min-enclosing-circle` 也已接通，补充边缘预增强层 `sobel / laplacian` 也已接通；其中 `crop` 当前支持 `roi.v1` 或显式矩形参数两种裁剪入口，`rotation-correct` 当前支持直接读取 `value.v1` 角度输入并输出姿态矫正摘要，`perspective-transform` 当前支持 `roi.v1` 或固定 `source_points` 四点输入，并按估算或显式目标尺寸输出规则化后的矫正图，`affine-transform` 当前支持固定参数或 `value.v1` 动态输入两种仿射来源，并可按 `matrix_2x3` 或三对点关系输出几何矫正图与变换摘要，`undistort` 当前支持 `value.v1` 标定配置对象或固定参数输入，`remap` 当前支持 `value.v1` 映射对象里的 `map_xy` 或 `map_x/map_y` 两类映射方式，`sobel` 当前支持 `x / y / xy` 三种方向增强并输出边缘强度摘要，`laplacian` 当前支持二阶边缘增强并输出边缘强度摘要，`fit-ellipse` 当前新增正式 `ellipses.v1` 结构化 payload 并接入 `payload-to-value`，`fill-holes` 当前可直接把白色前景内部孔洞收平，`distance-transform` 当前输出灰度预览图与最大/平均距离摘要，`contours-to-regions` 当前会把 contour 结果直接规整成标准 `regions.v1` 接入既有工业规则链，`min-area-rect` 当前新增 `rotated-rects.v1`，`hough-lines` 与 `fit-line` 当前统一输出 `lines.v1`，`hough-circles` 与 `min-enclosing-circle` 当前统一输出 `circles.v1`，并已由 `custom.opencv.payload-to-value` 统一桥接回 `value.v1` 以便继续做响应拼装和调试预览；而 `image-diff -> absdiff-threshold -> connected-components` 当前已经能把参考图差异或缺陷前景直接规整成带 `mask_image` 的 `regions.v1`，继续接入既有面积、覆盖率、连续性和 OK/NG 工业规则链。第二阶段当前已经收口前两项：`custom.opencv.template-match` 已接通，支持 `template_image + 可选 roi.v1 -> regions.v1 + summary`；`custom.opencv.caliper-edge` 也已接通，支持 `image + 可选 roi.v1 -> lines.v1 + summary`，可直接用于台阶边、工件边和治具定位边的单边基准量测。使用面当前也已补出 checked-in 样例 `industrial_single_frame_calibrated_template_edge_gate.*`，把 `json-load-local -> undistort -> remap -> template-match / caliper-edge -> process-decision` 这条本地标定定位主线正式收成文档模板与 app。
- 传统几何结果往工业量测和规则层也继续收了一层：`custom.opencv.caliper-edge / point-distance / point-to-line-distance / line-angle / circle-diameter / parallelism-metrics / concentricity-metrics / slot-width` 当前已接通，分别把 `lines.v1 / value.v1 / circles.v1` 直接收成可继续进入 `threshold-check / range-check / process-decision` 的结构化几何量测结果；`core.vision.edge-break-check / linearity-check / circularity-check / multi-part-presence-check / pair-offset-check / reference-mark-align-check / spacing-check / sequence-order-check` 当前也已接通，并保持只依赖标准 `regions.v1`，用于把分割、轮廓、连通域、边缘型区域或多部件检测结果统一收成“是否存在明显断边 / 是否足够直 / 是否足够圆 / 是否满足多零件装配存在性 / 是否满足双零件相对装配偏移 / 是否满足参考标记对位 / 是否满足孔距、销距或边间距 / 是否满足左右或上下方向排列顺序”的工业语义判定。当前这一组装配节点的 selector 也已支持 `leftmost / rightmost / topmost / bottommost` 这类空间排序策略，`sequence-order-check` 还能按顺序逐项排除已选目标，便于直接处理同类多目标的顺序检查。
