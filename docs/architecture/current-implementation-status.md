...
- 2026-06-18 已用真实 `config/backend-worker.json` 启动 backend-service + backend-worker，基于 `data/files/datasets/detection/medical-pills` 跑通 `YOLOv8 detection` 的真实 zip 导入、DatasetExport、1 epoch 短训练、ONNX / OpenVINO / TensorRT 转换、deployment sync / async 推理和 stop。该轮同时修复了 YOLO `data.yaml path` 同名根目录解析、async inference gateway 参数名和转换结果主 `model_build_id` 指向中间 ONNX 的问题。
