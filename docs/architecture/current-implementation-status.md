...
- 传统几何结果往工业量测和规则层也继续收了一层：`custom.opencv.point-to-line-distance / line-angle / circle-diameter` 当前已接通，分别把 `lines.v1 / circles.v1` 直接收成可继续进入 `threshold-check / range-check / process-decision` 的数值 `value.v1`；`core.vision.linearity-check` 当前也已接通，并保持只依赖标准 `regions.v1`，用于把分割、轮廓或连通域结果统一收成“是否足够直”的工业语义判定。
