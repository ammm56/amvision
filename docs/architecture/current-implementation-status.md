...
- 相机接入这条线当前也已经从“只有规划”进入第一批正式实现：`custom_nodes/camera_usb_uvc_nodes/` 已默认启用，首批 `enumerate-devices / capture-frame` 已接通，当前已经可以把“本机 USB / UVC 相机枚举 -> 单帧采图 -> 标准 `image-ref.v1` -> 检测/分割/工业规则链”这条最短现场采图主线先跑起来。
...
- `custom_nodes/camera_usb_uvc_nodes/` 当前已经作为第一层相机 custom node pack 落地，默认启用；对外节点面第一批已收口为 `custom.camera.usb.enumerate-devices / capture-frame`，用于覆盖“本机相机探测 + 单帧采图”这条最低门槛现场主线。当前实现基于项目内 `OpenCV VideoCapture` 适配层，节点已支持运行时 request 覆盖、标准 `image-ref.v1` 输出以及结构化采集摘要；更长期的 `open-device / start-stream / read-latest-frame / set-parameter` 会继续在同一 pack 内扩展。
