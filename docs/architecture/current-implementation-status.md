...
- 几何量测的现场调试面当前也已补到第一轮可用：`custom.opencv.draw-contours / draw-lines / draw-circles / draw-measurements` 已接通，当前已经可以把 `contours.v1 / lines.v1 / circles.v1` 以及量测 summary 直接画回原图，便于现场复核槽宽、平行度、孔径、同心度和偏移依据，而不需要只看数值结果猜测哪里出了问题。
- workflow 测试基础依赖这层当前也已经收稳：`requirements.txt` 已显式纳入 `httpx2`，`fastapi.testclient / starlette.testclient` 的已知弃用 warning 已按当前环境确认消失，不再依赖隐式安装状态。
