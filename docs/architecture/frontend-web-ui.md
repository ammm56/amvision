...
`/tasks` 是全局任务索引页，不是所有任务的默认详情页。任务列表主链接应优先进入对应业务详情页，只有无法识别业务归属或用户明确选择“任务状态”时，才进入 `/tasks/:taskId`。

推荐前端路由关系：

- `dataset-import` 主链接进入 `/datasets/imports/:datasetImportId`
- `dataset-export` 主链接进入 `/datasets/exports/:datasetExportId`
- `*-training` 主链接进入 `/models/:taskType/training-tasks/:taskId`
- `*-conversion` 主链接进入 `/models/:taskType/conversion-tasks/:taskId`
- 未识别任务进入 `/tasks/:taskId`

`/tasks/:taskId` 作为通用任务状态页，只读展示事件、进度、WebSocket 状态、错误详情和底层 TaskRecord 状态。业务详情页负责展示业务结果、文件、下载、登记、删除和清理入口，不重复渲染完整任务事件；需要查看任务事件时，通过页面中的“任务状态”入口跳转到 `/tasks/:taskId`。

删除入口不放在 `/tasks/:taskId`。数据集导入、数据集导出、模型训练和模型转换的删除按钮应分别放在对应业务详情页，并使用业务对象文案，例如“删除导入记录”“删除导出记录”“删除训练任务”“删除转换任务”。
