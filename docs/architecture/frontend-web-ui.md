...
`/tasks` 是全局任务索引页，不是所有任务的默认详情页。任务列表主链接应优先进入对应业务详情页，只有无法识别业务归属或用户明确选择“事件/诊断”时，才进入 `/tasks/:taskId`。
...
`/tasks/:taskId` 作为通用任务诊断页，只读展示事件、WebSocket 状态、错误详情和底层 TaskRecord 状态。业务详情页负责展示业务结果、文件、下载、登记、删除和清理入口，并在底部内嵌任务事件摘要或提供“查看任务诊断”链接。
