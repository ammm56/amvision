...
- 与插件节点编辑器、集成端点和任务状态流的关系

## 前端定位

前端是面向浏览器的 Web 应用，不是终端控制台，不直接承载业务执行，也不直接访问数据库、对象存储或执行器内部能力。

前端的职责是把数据集、任务、模型、部署、流程、插件和外部系统集成等复杂能力组织成清晰、稳定、可操作的浏览器界面。
...
- 展示项目、数据集、任务、模型、部署、流程和插件相关信息
- 提供创建任务、查看状态、管理配置和执行治理动作的操作界面
- 提供节点编辑器、结果查看器、图像叠加和图表可视化界面
- 通过 WebSocket 订阅状态流、日志流和告警事件
- 通过 REST API 执行资源读写、查询和治理动作

## 前端非职责边界

- 不直接执行训练、推理、转换或流程任务
- 不直接连接 executors、数据库或对象存储
- 不持有任务权威状态，仅消费 backend-service 暴露的状态视图
- 不在前端重新发明插件生命周期或任务状态模型
...
- plugins：插件浏览、启停、版本与能力界面
- settings：系统配置、运行时配置和治理设置界面
...
- 负责组件、状态封装、API client、WebSocket client、组合式能力和前端契约封装
...
- 服务端权威状态优先，前端状态以查询结果、订阅视图和交互草稿为主
...
- 前端应读取 backend-service 暴露的插件目录、节点定义和 UI schema
- 插件扩展的节点、参数面板和结果展示元数据应通过受控 schema 接入
- 前端不直接加载未经治理的任意脚本来扩展核心界面
