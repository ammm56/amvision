...
2. 再收 `yolox_core`：YOLOX detection full core 主链已经进入 `models/yolox_core/`，包括 COCO / VOC 数据入口、训练、评估、导出、postprocess 和预览渲染；后续只继续拆小应用服务外壳、runtime session 外壳和发布验收。
...
- `models/training/yolox_detection.py` 是 YOLOX detection 训练应用入口，数据增强、loss、EMA、scheduler、checkpoint 和训练循环核心已迁入 `yolox_core/training`。
...
- `runtime/contracts/*.py` 和 `runtime/serialization/*.py` 不迁入 core。它们描述部署推理时的输入输出和传输格式，属于 runtime 边界。
...
- YOLOX predictor 外壳已按职责拆到 `runtime/predictors/yolox*.py`。如果其中仍有模型结构、网络层、loss、训练逻辑或权重映射，应迁入 `models/yolox_core`；predictor 只保留 runtime session、加载、预处理、后处理桥接和结果序列化。
- detection runtime 共享支持工具位于 `runtime/support/detection.py`。共享 helper 不再继续放到 runtime 根目录，避免后续把模型 predictor、backend adapter 和通用工具混在一起。

runtime 的目录整理同样按模型纵向推进。RF-DETR 已先完成第一批目录整理：`rfdetr_predictor.py`、`rfdetr_segmentation_predictor.py` 已迁到 `runtime/predictors/`，`rfdetr_runtime_target.py` 已迁到 `runtime/targets/`。YOLOX predictor 已迁到 `runtime/predictors/` 并按 PyTorch / ONNXRuntime / OpenVINO / TensorRT、buffer、serialization、preview render 等职责拆小文件。YOLOv8 和 YOLO11 的五类 predictor 也已迁到 `runtime/predictors/`，其中 YOLO11 detection / classification / segmentation / pose / OBB 均已按 backend session 拆分。YOLO26 的五类 predictor 当前也已迁到 `runtime/predictors/`，classification / segmentation / pose / OBB 与 detection 都已按 PyTorch / ONNXRuntime / OpenVINO / TensorRT backend session 拆分。YOLOv8 / YOLO11 / YOLO26 runtime target 已迁到 `runtime/targets/`，五类 runtime contract 已迁到 `runtime/contracts/`，五类 runtime payload 序列化文件已迁到 `runtime/serialization/`。后续重点是继续清少量根目录过渡外壳；只有确认模型内部逻辑已经回到对应 core 后，才把剩余 runtime 外壳迁到 `predictors/`、`targets/`、`contracts/`、`serialization/` 等子目录。
...
当前 YOLO26 已开始按最终结构补齐 `yolo26_core/cfg`、`yolo26_core/decode`、`yolo26_core/nn/modules` 和 `yolo26_core/nn/tasks`，并且 detection / classification / segmentation / pose / OBB 的 data、training、loss、assigner、target、evaluation、inference 与 postprocess 第一段已经进入 `yolo26_core`。YOLO26 五类 task 的 PyTorch / ONNXRuntime / OpenVINO / TensorRT deployment session 已按 backend 拆到 `runtime/predictors/yolo26_*`；`yolo26_core/export/` 也已拆成 `source / plan / onnx / openvino / tensorrt` 与各 task 的导出 forward 文件。runtime 文件只保留 session、backend、buffer、preview 和序列化边界，模型语义继续放在 `yolo26_core`。
...
- `conversion`：消费转换任务，做任务状态、目标格式、产物登记和子进程隔离；ONNX/OpenVINO/TensorRT 的稳定导出入口调用对应 `*_core/export/`。
...
SAM3 的模型支撑不放在 `backend/nodes` 平台 shared support 中；它属于 SAM3 custom node 私有实现，放在 `custom_nodes/sam3_segment_nodes/backend/core/`。
...
- 模型 core / runtime 之外的代码结构收口详见 [docs/architecture/code-structure-cleanup-plan.md](code-structure-cleanup-plan.md)
