...
当前 YOLO26 已开始按最终结构补齐 `yolo26_core/cfg`、`yolo26_core/decode`、`yolo26_core/nn/modules` 和 `yolo26_core/nn/tasks`，并且 detection / classification / segmentation / pose / OBB 的 data、training、loss、assigner、target、evaluation、inference 与 postprocess 第一段已经进入 `yolo26_core`。YOLO26 classification / segmentation / pose / OBB 对应 PyTorch / ONNXRuntime / OpenVINO / TensorRT deployment session 已按 backend 拆到 `runtime/predictors/yolo26_*`，detection 入口已迁到 `runtime/predictors/yolo26_detection.py`。这些 runtime 文件只保留 session、backend、buffer、preview 和序列化边界，模型语义继续放在 `yolo26_core`。
