...
- 每个 pool 使用固定大小 mmap 文件，例如 `runtime/buffers/image-1080p/image-1080p-001.dat`。
...
建议按尺寸预分配不同池。默认配置使用 `local_buffer_broker.default_pool_name + pools`，仓库默认创建 `image-1080p` 和 `image-640x640` 两个 pool，默认值为 `image-1080p`。`image-1080p` 单槽 16MB、默认 32 个槽位，用于覆盖常见 1080p raw BGR/RGBA 图片输入和短时间并发占用；`image-640x640` 单槽 4MB、默认 32 个槽位，用于 640x640 低分辨率图片和小图输入；4K 或 20MB 级原始图输入场景应按现场配置增加更大的 pool。
...
```json
{
  ...
  "buffer_id": "buffer-1",
  "lease_id": "lease-1",
  "path": "runtime/buffers/image-1080p/image-1080p-001.dat",
  "offset": 1048576,
  "size": 6220800,
  "shape": [1080, 1920, 3],
  "dtype": "uint8",
