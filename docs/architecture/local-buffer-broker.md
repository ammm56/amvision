...
建议按尺寸预分配不同池。当前内置 `image-small`、`image-1080p` 和 `image-4k` 三档 preset，启动时通过 `config/backend-service.json` 的 `local_buffer_broker.default_pool_name` 选择实际使用的默认池。默认值为 `image-1080p`，单槽 16MB、默认 32 个槽位，用于覆盖常见 1080p raw BGR/RGBA 图片输入和短时间并发占用；4K 原始图输入场景应切换为 `image-4k`。
