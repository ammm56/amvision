...
LocalBufferBroker process 必须服从 backend-service 的进程生命周期：backend-service 正常停止或 reload 时，broker 先关闭 mmap 句柄再退出；broker 会定期检查 supervisor 的 parent process sentinel，父进程被强制结束后也会自行退出，避免作为孤立进程长期持有默认 pool 文件。supervisor 启动阶段会持续检查子进程状态，子进程提前退出时立即返回 process id 和 exit code，不再等待完整启动超时；真正的启动超时会同时记录 root directory 和 pool file 路径，便于区分文件占用、磁盘权限与子进程初始化错误。控制台中断在 broker 内只触发清理，不重复输出无意义的 `KeyboardInterrupt` traceback。
...
- 每个 pool 使用固定大小 mmap 文件，例如 `runtime/buffers/image-4k/image-4k-001.dat`。
...
建议按尺寸预分配不同池。默认配置使用 `local_buffer_broker.default_pool_name + pools`，仓库默认创建 `image-4k`、`image-1080p` 和 `image-640x640` 三个 pool，默认值为 `image-4k`。`image-4k` 单槽 128MB、默认 16 个槽位，用于覆盖 5000x4000 级 20MP 工业相机 raw RGB/RGBA 图片输入和短时间并发占用；`image-1080p` 单槽 16MB、默认 16 个槽位，用于 1080p 图片和中间结果；`image-640x640` 单槽 4MB、默认 16 个槽位，用于 640x640 低分辨率图片和小图输入。`image-8k` 单槽 256MB、默认 16 个槽位，仅作为现场可选大图 pool，不在仓库默认配置中创建，避免未使用时初始化 4GB 级 mmap 文件。低内存设备可以显式配置为 8 或 4 个槽位；运行时始终按实际 `slot_count` 计算容量、偏移和可用槽位，不依赖固定数量。

- image-640x640：640x640 低分辨率图片、小图和小中间结果
- image-1080p：1080p 图片和中间结果
- image-4k：默认大图输入，覆盖 20MP 工业相机常见 raw 图
- image-8k：可选大图输入，更高分辨率、更多通道或相机专用大图输入
- output-preview：预览图和可选渲染结果

池大小应由配置控制，不在运行中无限增长。

需要使用 `image-8k` 时，在 `local_buffer_broker.pools` 中手动增加以下配置：

```json
{
  "pool_name": "image-8k",
  "slot_size_bytes": 268435456,
  "slot_count": 16,
...
```json
{
  ...
  "buffer_id": "buffer-1",
  "lease_id": "lease-1",
  "path": "runtime/buffers/image-4k/image-4k-001.dat",
  "offset": 1048576,
  "size": 6220800,
  "shape": [1080, 1920, 3],
  "dtype": "uint8",
