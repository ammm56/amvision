...
- 训练任务服务、转换任务服务、推理任务服务当前都以 `yolox_*` 为主。
- `ModelRuntime` 虽然已经出现通用命名，但当前请求和返回结构仍引用 YOLOX 预测对象。
- deployment runtime pool、deployment supervisor、runtime target resolver 当前都主要服务 YOLOX。
- workflow service node runtime 当前主要组装 YOLOX 相关 service。
- worker consumer 注册表当前也主要按 YOLOX 训练、转换、评估、推理来声明消费者。
...
1. 先把 YOLOX 抽成 detection 参考实现
2. 再接入 YOLOv8/11/26
3. 再接入 RT-DETR
4. 再打开 segmentation 任务分类，并接入 SAM2/3
5. 最后打开 multimodal-vl 任务分类，并接入 QwenVL

这个顺序的理由很简单：

- YOLOv8/11/26 与 YOLOX 最接近，最容易验证 detection 通用层
- RT-DETR 能继续验证 detection 层是否真的独立于单个模型
- SAM2/3 会打开第二种任务分类，复杂度明显上升
- QwenVL 与当前 YOLOX 主线差异最大，应放到 detection 和 segmentation 两层稳定之后再进入
...
- [docs/architecture/yolo-model-plan.md](yolo-model-plan.md)
