...
- 当前保持“一条连接只订阅一个资源流”的简单模型，不引入单连接多订阅命令协议。
...
- 当前不提供单连接上的 subscribe、unsubscribe、ack 等命令协议。
...
- 客户端不向连接内发送业务命令；建立连接后只接收服务端推送，取消、重试、启停等动作走 REST。
...
| `workflows.preview-runs.events` | `GET /api/v1/workflows/preview-runs/{preview_run_id}`、`GET /api/v1/workflows/preview-runs/{preview_run_id}/events` | `service_event_bus` + `events.jsonl` + `WorkflowPreviewRunManager` | 已接入统一 WebSocket；实时分发走 service_event_bus，历史回放读取追加式 events.jsonl |
| `workflows.runs.events` | `GET /api/v1/workflows/runs/{workflow_run_id}?response_mode=run`、`GET /api/v1/workflows/runs/{workflow_run_id}/events` | `service_event_bus` + `events.jsonl` + `WorkflowRuntimeService` | 已接入统一 WebSocket；run 生命周期事件逐行追加并分发；不带 response_mode 的 run 查询默认返回公开 App Result |
...
- `POST /api/v1/workflows/preview-runs` 同步执行并返回 preview run 终态；REST 事件接口和对应 WebSocket 资源流使用同一份 JSONL 事件回放。
...
2. `workflows.preview-runs.events` 已经按同一骨架落地：实时事件由 preview manager 写入 `service_event_bus`，`events.jsonl` 负责回放。
