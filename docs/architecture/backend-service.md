...
7. `start_runtime` 启动 LocalBufferBroker、workflow 相关 manager、sync / async deployment supervisor，以及 worker 调用 async deployment owner 所需的 async inference gateway dispatcher
8. 关闭应用时执行：
...
当前 backend-service 不再托管任何队列消费者。dataset-import、dataset-export、training、conversion、evaluation 和 inference 六类任务全部由独立 backend-worker 消费；service 只保留控制面、deployment supervisor、workflow runtime manager、PublishedInferenceGateway 和 async inference gateway dispatcher。
