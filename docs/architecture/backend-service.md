...
本文档用于定义后端服务在整个平台中的职责、边界、任务状态模型、执行调度方式，以及与 QueueBackend、workers 和公开接口之间的关系。

本文档解决的问题不是具体接口字段或类设计，而是“哪些责任属于 backend-service，哪些不属于”，以及“任务如何被提交、调度、执行和回写状态”。
...
- 后端服务与浏览器前端、外部系统、workers、对象存储和插件体系的接口边界

## 后端服务的定位

backend-service 是平台的统一后端服务入口，负责元数据、任务编排、公开接口、状态流和治理规则。

backend-service 不是训练、推理、转换或流程执行本体，也不直接承载硬件驱动与模型运行细节。它的核心价值是把“任务定义、任务状态、资源引用、公开契约和治理约束”统一收敛在一个可审计、可回滚、可扩展的边界中。
...
- 负责 REST API、WebSocket 和外部系统接入边界
- 处理鉴权、输入校验、响应包装、订阅会话和错误映射
- 不负责业务编排和持久化细节

### application 层

- 负责任务创建、状态迁移、事务边界和用例编排
...
- 负责后端服务内部对数据库、对象存储、队列、缓存、ZeroMQ 和协议适配器的接入
- 负责把外部技术实现适配到 application 层所依赖的稳定接口
...
- cancel_requested 和 retry_waiting 属于治理状态，不代表执行已经结束
...
- REST API：创建任务、查询详情、读取元数据、触发治理动作
- WebSocket：订阅任务状态、日志流、进度和系统事件
- 协议适配插件：在受控契约下把外部触发和回传接入后端服务

### 面向 workers 的内部边界

- QueueBackend：任务分发与消费边界
- 状态回写通道：任务进度、日志、结果和错误的标准回传边界
- 可选 ZeroMQ：仅用于同机本地部署中的内部进程通信，不作为公开契约
