...
- 后端服务与浏览器前端、外部系统、workers、对象存储和节点扩展体系之间的接口规则
...
`backend/` 不是按模型拆顶层目录，而是按平台职责拆层。

- `backend/service` 是控制面和应用服务层，负责 API、状态、元数据、任务创建、workflow runtime manager 和 deployment supervisor。
- `backend/workers` 是任务消费层，负责领取数据集、训练、验证/评估、转换和异步推理任务，并把进度、日志、文件和结果回写给 service。
- `backend/contracts` 是跨层共享的数据规则层，负责 payload、数据集格式、文件引用、节点和 workflow 规则，不放业务实现。
- `backend/nodes` 是平台内建节点层，和 `custom_nodes` 的外部扩展层分开。
- `backend/queue` 是 QueueBackend 抽象和本地队列实现，不放任务业务和模型逻辑。
- `backend/bootstrap`、`backend/maintenance`、`backend/alembic` 分别处理启动装配、离线维护和数据库迁移。

模型相关代码要再分两层：`backend/service/application/models/*_core` 放模型结构、loss、assigner、target、postprocess、weights、train/eval/export 核心；`backend/service/application/runtime` 放部署后的 session、predictor、runtime target、输入输出序列化、长期驻留和 sync/async 推理。`runtime/yolox_core` 属于历史遗留位置，目标是迁到 `models/yolox_core` 后删除旧目录。

## 后端服务做什么

- 对浏览器前端、上位机和其他外部系统暴露统一的公开接口边界
- 装配当前已经公开的 auth、system、projects、datasets、dataset-exports、models、五类训练/验证/转换/部署/推理/评估资源、workflow runtime、trigger-sources 和 tasks REST API，以及对应的 WebSocket 资源流
- 管理 DatasetImport、DatasetVersion、TaskRecord、TaskEvent、Model、ModelVersion、ModelBuild、DeploymentInstance、WorkflowAppRuntime 和 WorkflowTriggerSource 等当前已落地的元数据
- 通过 SQLAlchemy Unit of Work 和 Repository 管理数据库事务边界
- 管理本地 DatasetStorage、LocalFileQueueBackend、NodeCatalogRegistry、WorkflowRuntime 相关 manager、TriggerSourceSupervisor、LocalBufferBroker supervisor、PublishedInferenceGateway 和 async inference gateway dispatcher
- 把 DatasetImport、DatasetExport、training、conversion、evaluation、inference 和 workflow runtime 这类长于请求生命周期的处理从 REST 请求链路里隔离出去
- 为导入、导出、训练、转换、评估、推理和 workflow 执行创建正式 TaskRecord / WorkflowRun，并通过 REST 与 WebSocket 暴露状态变化
- 在应用生命周期内启动、停止 LocalBufferBroker、workflow runtime manager、TriggerSourceSupervisor、deployment supervisor 和 async inference gateway dispatcher
...
- api/bootstrap.py：装配 settings、SessionFactory、DatasetStorage、QueueBackend、sync / async deployment supervisor、PublishedInferenceGateway 和 async inference gateway dispatcher
...
7. `start_runtime` 启动 LocalBufferBroker、WorkflowPreviewRunManager、WorkflowRuntimeWorkerManager、TriggerSourceSupervisor、sync / async deployment supervisor，以及 worker 调用 async deployment owner 所需的 async inference gateway dispatcher
8. 关闭应用时执行：
	- `bootstrap.stop_runtime(runtime)`
	- 停止 async inference gateway dispatcher
	- 停止 TriggerSourceSupervisor、workflow runtime 相关 manager 与 LocalBufferBroker
...
- 当前已经公开的 REST 资源组包括：
  - auth、本地用户、token 与审计
  - system、projects 与 Project 文件读取
...
- 具体路径与 scope 规则以 [docs/api/current-api.md](../api/current-api.md) 为准；本页只说明后端服务分层与职责，不重复铺完整接口表
...
- 各资源流继续复用同一套主体请求头、scope 规则、实时事件总线和历史回放语义
- 后续新增资源流时，也应进入同一套版本化 WebSocket 分层，而不是为单个功能继续扩出专用 socket
...
- 自定义节点管理面、集成端点和 service account 必须独立 scope，不复用普通用户的宽权限
...
- file compatibility
- queue lane
- node pack capability requirements
- local resource tags
...
- lease recovery：worker 或 service 异常退出后，把超过 lease 超时的任务恢复到待执行状态
- ack：确认完成
...
- 外部系统结果回传、自定义节点回调和后处理结果，也应先汇聚到后端服务再对外发布
...
- timeout 规则由后端服务根据任务类型、运行时能力和节点包约束统一设定
...
- backend-service 负责读取 `custom_nodes` 目录、node pack manifest 和节点定义目录
- 自定义节点触发器、回调器和后处理器必须通过后端服务注册和管理
- 自定义节点可以扩展任务编排和状态分发，但不能绕开后端服务的状态模型
