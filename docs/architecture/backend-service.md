...
模型相关代码要再分两层：`backend/service/application/models/*_core` 放模型结构、loss、assigner、target、postprocess、weights、train/eval/export 核心；`backend/service/application/runtime` 放部署后的 session、predictor、runtime target、输入输出序列化、长期驻留和 sync/async 推理。`YOLOX` core 已迁到 `models/yolox_core`，runtime 下不再保留历史 core 目录。
