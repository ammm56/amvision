...
- infrastructure：放 SQLAlchemy、Repository、ObjectStore、QueueBackend、CacheBackend、ZeroMQ 和外部协议适配
...
- 所有公开资源按版本组织，例如 /api/v1/datasets、/api/v1/models、/api/v1/tasks
...
- WebSocket 不承担权威写入，不承担资源查询，不替代 REST 分页和详情查询
- WebSocket 主题建议至少按 project、task、deployment 三个范围组织
...
- 数据集 zip 上传后的原始包、解压 staging 和 canonical 版本文件走本地 ObjectStore，不直接把二进制内容塞进数据库
...
4. 再落 datasets zip 导入、导入记录和 DatasetVersion 冻结链路
