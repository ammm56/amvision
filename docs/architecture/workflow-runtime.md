...
用途：查询单条 WorkflowAppRuntime 的生命周期和运行态事件。

该接口作为 app runtime 历史事件读取和断线恢复的正式读取面；对应的实时资源流为 `/ws/v1/workflows/app-runtimes/events`。
...
该接口当前已经作为 WorkflowRun 历史事件读取和断线恢复的正式读取面；对应的实时资源流为 `/ws/v1/workflows/runs/events`。
