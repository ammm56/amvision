...
版本发布、选择、归档、恢复和回滚语义见 [Workflow App 版本管理](workflow-app-versioning.md)。
...
生产 Runtime、Trigger 和 Preview 都优先通过 LocalBuffer 传递大图片。ObjectStore 用于需要持久化、跨执行复用或对外返回的对象；Base64 只适合小型兼容输入，不是高性能主链路。详见 [高性能图片数据面](high-performance-image-data-plane.md) 和 [数据与文件](data-and-files.md)。
