...
- `release/full/`：由 assemble-release 生成的发布结果，不作为源码手工维护。
...
相关设计：[LocalBufferBroker](local-buffer-broker.md)、[高性能图片数据面](high-performance-image-data-plane.md)、[运行时打包](runtime-packaging.md)。
