...
本文档用于固化平台中的关键对象、对象关系、版本链路和可追溯原则，明确哪些内容属于权威元数据，哪些内容属于大对象产物引用。

本文档解决的问题是“平台最重要的对象有哪些，它们如何连接起来，以及训练、转换、部署、推理和流程执行的结果如何被追踪”。

## 适用范围

- 项目、数据集、模型、任务、部署、插件和集成对象的关系
- 后端服务中与数据集、模型注册、转换产物和结果暂存相关的领域层级
- artifact 的元数据与内容分离原则
- 版本链路、引用规则和回滚追踪关系
- 数据对象与运行时对象的权责边界

## 总体原则

- 元数据以数据库中的权威对象为主，产物内容以 ObjectStore 中的大对象为主
- artifact 一旦对外可见，应优先视为不可变对象，通过新版本替代原地覆盖
...
- Model：逻辑模型条目，用于承载同一模型线的统一身份、业务标签和治理边界
- ModelVersion：模型的源版本，可来自预训练导入、训练产出或人工上传
- ModelVariant：面向特定运行时、硬件平台、精度或部署形态的派生变体，通常由转换任务产生
...
- ModelArtifact：模型文件、转换产物、benchmark 结果、配置包等统一 artifact 抽象
- ArtifactRef：数据库中对 ObjectStore 内容的引用视图
- ResultArtifact：推理、流程执行、评估或后处理生成的结果 artifact
...
- TrainingTask 产出新的 ModelVersion，并挂接一个或多个 ModelArtifact

### 预训练导入到模型注册

- 外部预训练模型导入后，应先落为 Project 下的一个 Model
- 首次导入形成一个 ModelVersion，source kind 标记为 pretrained-import
- 预训练模型原始权重、配置和来源说明通过 ModelArtifact 与 ArtifactRef 挂接
- 后续微调训练可选择引用该 ModelVersion 作为 warm start 来源

### 模型到转换

- ConversionTask 默认读取某个 ModelVersion 或已有 ModelVariant
- ConversionTask 产出新的 ModelVariant，并挂接面向特定运行时或平台的新 ModelArtifact
- 转换前后对象之间应保留来源链路、兼容性信息、目标平台信息和 benchmark 记录

### 模型到部署

- DeploymentInstance 绑定某个可部署 ModelVariant；如未转换，也可绑定满足运行条件的 ModelVersion
- DeploymentInstance 同时绑定 RuntimeProfile、部署配置和回滚候选版本
- InferenceTask 引用 DeploymentInstance，而不是直接引用裸模型文件路径

### 推理与结果

- InferenceTask 读取 DeploymentInstance，并通过其绑定的 ModelVersion 或 ModelVariant 执行
- InferenceTask 产生临时运行结果、结构化摘要和可选持久化结果引用
- 需要保留的结果应提升为 ResultArtifact；仅用于短期查看或回传的结果应保留在 task-scoped staging 区

### 流程与插件关系

- PipelineTemplate 引用 NodeDefinition、PluginVersion、参数 schema 和节点连接关系
- PipelineTemplate 可引用逻辑输入类型、模型版本占位或具体集成端点
- 流程执行任务产出的结果也应形成可追踪 artifact 或结果引用
...
- backend/service/domain/model-registry：定义 Model、ModelVersion、ModelVariant、版本 lineage 和发布候选规则
- backend/service/domain/artifacts：定义 ArtifactRef、ModelArtifact、ResultArtifact、checksum、存储引用和保留策略
...
- dataset-management：导入、切分、冻结、归档和清理 DatasetVersion
- model-registry：导入预训练模型、登记训练产物、创建版本、建立 lineage、打标签和归档
- model-conversion：提交转换任务、登记 ModelVariant、写入兼容性与 benchmark
- inference-results：写入 task staging、生成 ResultArtifact、执行保留或清理策略
- deployment-binding：把 ModelVersion 或 ModelVariant 绑定到 DeploymentInstance，并维护回滚候选
...
- 因此需要把“外部格式兼容”与“内部统一表示”分开：前者解决导入，后者解决平台治理与多模型复用
...
- 数据治理层最终记录的应是 DatasetImport.format_type 与 DatasetImport.task_type，而不是某次猜测结果
...
- 不要求所有模型共享同一种原始文件结构
- 只要求同一任务家族在平台内部共享同一组逻辑字段和版本治理方式
...
### Model、ModelVersion 与 ModelVariant
...
- ModelVariant 是源模型的派生部署变体，通常对应 ONNX、OpenVINO、TensorRT、CoreML 或特定量化版本
- ModelVersion 与 ModelVariant 都通过 ModelArtifact 挂接具体文件，但治理层级不同，不应混成一个对象

### ModelArtifact

- 是统一产物抽象，不只代表训练得到的权重文件
- 也可以代表转换产物、benchmark 报告、推理结果包和模型配置包
- 应包含 artifact type、source lineage、compatibility、checksum 和 storage reference

### ResultArtifact 与 task staging

- ResultArtifact 用于保存需要复查、下载、复现、上报或再次进入流程的结果对象
- task staging 只用于短期暂存推理中间结果、上传回包、预览图和临时日志，不作为长期权威存储
- task staging 必须绑定 task id、retention policy 和清理时间，避免与正式 artifact 混用
- 从 staging 提升为 ResultArtifact 时，应生成新的 ArtifactRef，而不是重写既有路径
...
- 每个 ModelVariant 都应能追到来源 ModelVersion、目标运行时、目标硬件、精度策略和转换任务
- 每个流程模板都应能追到节点定义来源和插件版本引用
- 每个外部回调或上报结果都应能追到原始任务或部署实例

## 回滚与替换原则

- 回滚通过切换版本引用实现，而不是覆盖 artifact 内容
- 插件回滚、模型回滚和部署回滚都应保留历史链路
- 数据版本、模型版本和流程模板版本之间应能形成组合快照，支撑问题复现

## ObjectStore 推荐布局

以下布局用于说明“对象如何组织”，不是强制要求把业务逻辑写死到路径字符串中。业务层始终通过 ArtifactRef 和 ObjectStore 接口访问内容。
...
- 管理动作由后端服务的 dataset-management 用例负责，包括导入、冻结、标记、归档和清理

### 预训练模型放哪里

- 预训练模型不作为“系统公共裸文件夹”直接使用，而应先导入到某个 Project 的 model registry
- 导入后形成 Model + ModelVersion，source kind 标记为 pretrained-import
- 原始权重、配置和许可说明挂到 ModelArtifact 与 ArtifactRef，物理内容放在 models/{model_id}/versions/{model_version_id}/source 下

### 训练后模型放哪里

- 训练完成后先登记为新的 ModelVersion，不直接覆盖预训练版本
- checkpoint、最佳权重、训练配置、指标报告等以 ModelArtifact 形式挂接
- 物理内容放在 models/{model_id}/versions/{model_version_id}/checkpoints、configs、reports 下

### 转换后的模型放哪里

- 转换结果登记为 ModelVariant，而不是与源模型版本混成同一对象
- 每个 ModelVariant 绑定目标运行时、硬件平台、精度策略和来源转换任务
- 物理内容放在 models/{model_id}/variants/{model_variant_id}/packages 下，benchmark 与转换 manifest 分开存放

### 推理结果暂存放哪里

- 暂存结果放在 task-runs/inference/{task_id}/staging 下，只用于短期预览、回包或二次处理
- staging 结果必须带 TTL、大小限制和清理策略，不参与长期模型版本管理
- 需要长期保留的结果由后端服务提升为 ResultArtifact，并移动或复制到 promoted-results 或统一结果 artifact 路径

## 管理组织建议

- 数据集、模型、部署、任务和结果都先按 Project 归属，再按对象 id 管理，避免直接按用户名或日期裸分目录
- 逻辑对象用数据库做权威索引，路径只做存储组织，不做业务主键
- 预训练导入、训练产出、转换产出和推理结果分别对应不同对象层级，避免都塞进 ModelArtifact 一个层里
- 清理策略分三类：dataset archive、task staging cleanup、artifact retention，不混用同一规则
- UI 和 API 默认展示 Model -> ModelVersion -> ModelVariant 的三级结构，而不是直接暴露底层文件树
