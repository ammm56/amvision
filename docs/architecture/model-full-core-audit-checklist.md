...
| 下一步 | 继续核对 v8 loss / assigner / data augmentation / export forward / runtime output layout；长时间训练、更长 release/full 常驻 soak、资源占用和异常恢复基线单独跑 |
...
| YOLO26 | detection | 已记录 | 已记录 | 已记录 | 已记录 | workflow / 前端已记录 | 已记录 | 2026-06-20 已跑五任务三格式 full chain，结果在 `.tmp/yolo-model-full-chain-smoke/20260620211256/result.json`；2026-06-21 又用真实转换产物复跑 `--run-id yolo26-runtime-artifact-smoke-20260621-r2`。2026-06-23 修复后补跑 detection / classification workflow，结果目录：`.tmp/yolo-model-full-chain-smoke/yolo26-detection-classification-workflow-record-20260623-fix1`。前端页面操作记录：`.tmp/frontend-operation-record-20260623`。 |
| YOLO26 | classification | 已记录 | 已记录 | 已记录 | 已记录 | workflow / 前端已记录 | 已记录 | 同 YOLO26 detection 记录；另有 non-detection backend matrix 覆盖 conversion -> runtime predict。前端页面操作记录：`.tmp/frontend-operation-record-20260623`。 |
| YOLO26 | segmentation | 已记录 | 已记录 | 已记录 | 已记录 | workflow / 前端已记录 | 已记录 | 2026-06-23 修复 YOLO26 segmentation bbox 反序和零面积 mask polygon 后，复跑真实 DatasetImport / DatasetExport、短训练、评估、ONNX conversion、deployment sync / async、workflow 成功。结果目录：`.tmp/yolo-model-full-chain-smoke/yolo26-segmentation-workflow-record-20260623-fix2`。前端页面操作记录：`.tmp/frontend-operation-record-20260623`。 |
| YOLO26 | pose | 已记录 | 已记录 | 已记录 | 已记录 | workflow / 前端已记录 | 已记录 | 2026-06-23 补跑 pose / OBB 真实 DatasetImport / DatasetExport、短训练、评估、ONNX conversion、deployment sync / async、workflow 成功。结果目录：`.tmp/yolo-model-full-chain-smoke/yolo26-pose-obb-workflow-record-20260623-fix1`。前端页面操作记录：`.tmp/frontend-operation-record-20260623`。 |
| YOLO26 | obb | 已记录 | 已记录 | 已记录 | 已记录 | workflow / 前端已记录 | 已记录 | 同 YOLO26 pose 记录；前端页面操作记录：`.tmp/frontend-operation-record-20260623`。 |
