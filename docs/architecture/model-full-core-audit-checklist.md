...
- 已完成：`plc_nodes/protocols/modbus_tcp`、`database_nodes/providers/sql`、`http_nodes/categories/request` 的旧 `_runtime.py` 已拆到正式 `backend/runtime/`。
- 已完成：`camera_nodes/providers/usb_uvc` 和 `barcode_nodes` 已删除旧 `backend/support.py`，并拆到正式 `backend/runtime/`。
...
4. 第五批 custom nodes 已进入结构收口：`plc_nodes/protocols/modbus_tcp`、`database_nodes/providers/sql` 和 `http_nodes/categories/request` 的旧 `backend/nodes/_runtime.py` 均已删除，协议连接、输出 payload、参数读取、client / database / HTTP 调用和执行入口已拆到各自 `backend/runtime/`。
