...
| 风险点 | LoRA / PEFT 当前未启用；早期 checkpoint 转换保留 `legacy_checkpoint_format / legacy_ema_state_dict` 是读取 RF-DETR 旧训练产物所需，不属于旧平台入口；checkpoint 覆盖率、真实 mAP、长时训练和 deployment soak 仍需要显式记录 |
...
| 风险点 | segmentation mask AP、pose OKS、OBB rotated AP 当前是项目内 COCO-style AP 实现，不等同于 Ultralytics 完整 `SegmentationValidator / PoseValidator / OBBValidator` 及 pycocotools / 专用 rotated evaluator 的全部输出口径；全 scale checkpoint 和真实短链路需要统一记录 |
| 下一步 | 继续核对 v8 loss / assigner / data augmentation / export forward / runtime output layout；之后补五任务真实全链路记录 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RF-DETR | detection | 待跑 | 部分已跑 | 部分已跑 | 部分已跑 | 待跑 | 部分已跑 | `tests/integration/test_rfdetr_full_core_soak_benchmark.py` 已提供 checkpoint、tiny backward、ONNX smoke 入口，默认跳过；还需用真实 DatasetImport / DatasetExport 跑完整链路，并补 OpenVINO / TensorRT、deployment、workflow 记录。 |
| RF-DETR | segmentation | 待跑 | 部分已跑 | 部分已跑 | 部分已跑 | 待跑 | 部分已跑 | 同 detection；本地 checkpoint 清单已覆盖 segmentation `nano / s / m / l / x`，但 full chain 结果仍需补全。 |
| YOLOX | detection，COCO DatasetExport | 待补记录 | 待补记录 | 待补记录 | 待补记录 | 待跑 | 待补记录 | 代码已支持 YOLOX detection core 和 COCO 训练 / 评估 / 转换主链；需要重新跑并记录 COCO full chain。 |
| YOLOX | detection，VOC DatasetExport | 待补记录 | 待补记录 | 待补记录 | 待补记录 | 待跑 | 待补记录 | VOC parser 和 DatasetExport 支持已经接入 YOLOX core，但仍需要一条可追踪 VOC full chain 记录。 |
| YOLOv8 | detection | 待补记录 | 待补记录 | 部分已跑 | 部分已跑 | 待跑 | 部分已跑 | detection runtime backend ONNXRuntime / OpenVINO / TensorRT smoke 已跑通；还需补真实 DatasetImport -> DatasetExport -> 短训练 -> evaluation -> conversion -> deployment -> workflow 的完整记录。 |
| YOLOv8 | classification | 待补记录 | 待补记录 | 部分已跑 | 部分已跑 | 待跑 | 部分已跑 | non-detection runtime backend matrix 已覆盖 conversion -> runtime predict；还需补真实数据短训练、评估和 workflow / 前端操作记录。 |
| YOLOv8 | segmentation | 待补记录 | 待补记录 | 部分已跑 | 部分已跑 | 待跑 | 部分已跑 | non-detection runtime backend matrix 已覆盖 conversion -> runtime predict；还需补真实数据短训练、评估和 workflow / 前端操作记录。 |
| YOLOv8 | pose | 待补记录 | 待补记录 | 部分已跑 | 部分已跑 | 待跑 | 部分已跑 | non-detection runtime backend matrix 已覆盖 conversion -> runtime predict；还需补真实数据短训练、评估和 workflow / 前端操作记录。 |
| YOLOv8 | obb | 待补记录 | 待补记录 | 部分已跑 | 部分已跑 | 待跑 | 部分已跑 | non-detection runtime backend matrix 已覆盖 conversion -> runtime predict；还需补真实数据短训练、评估和 workflow / 前端操作记录。 |
| YOLO11 | detection | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | 已记录 | 2026-06-20 已跑 `tests.integration.yolo_model_full_chain_smoke --model-type yolo11 --tasks detection classification segmentation pose obb --target-formats onnx openvino-ir tensorrt-engine --max-epochs 1 --batch-size 1 --max-images-per-split 4 --start-processes`，结果在 `.tmp/yolo-model-full-chain-smoke/20260620110947/result.json`；还需补 workflow / 前端逐任务操作记录。 |
| YOLO11 | classification | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | 已记录 | 同 YOLO11 detection 记录。 |
| YOLO11 | segmentation | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | 已记录 | 同 YOLO11 detection 记录。 |
| YOLO11 | pose | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | 已记录 | 2026-06-19 另有 pose / OBB ONNX 子集记录 `.tmp/yolo11-pose-obb-20260619161813`；完整三格式链路见 2026-06-20 记录。 |
| YOLO11 | obb | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | 已记录 | 同 YOLO11 pose 记录。 |
| YOLO26 | detection | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | 已记录 | 2026-06-20 已跑五任务三格式 full chain，结果在 `.tmp/yolo-model-full-chain-smoke/20260620211256/result.json`；2026-06-21 又用真实转换产物复跑 `--run-id yolo26-runtime-artifact-smoke-20260621-r2`。 |
| YOLO26 | classification | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | 已记录 | 同 YOLO26 detection 记录；另有 non-detection backend matrix 覆盖 conversion -> runtime predict。 |
| YOLO26 | segmentation | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | 已记录 | 同 YOLO26 detection 记录；2026-06-23 又复跑 segmentation ONNXRuntime / OpenVINO / TensorRT runtime backend smoke，确认 end2end TopK 小输入和 processed mask coeff 布局修复可用。 |
| YOLO26 | pose | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | 已记录 | 同 YOLO26 detection 记录；2026-06-23 又复跑 pose ONNXRuntime / OpenVINO / TensorRT runtime backend smoke。 |
| YOLO26 | obb | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | 已记录 | 同 YOLO26 detection 记录；2026-06-23 又复跑 OBB ONNXRuntime / OpenVINO / TensorRT runtime backend smoke。 |

当前最小补缺顺序：

1. RF-DETR detection / segmentation：补真实 DatasetImport / DatasetExport、短训练、评估、ONNX / OpenVINO / TensorRT、deployment sync / async、workflow 记录。
2. YOLOX detection：分别补 COCO 与 VOC DatasetExport 的完整记录。
3. YOLOv8 五任务：补真实数据短训练和三格式转换 / deployment / workflow 记录，避免只依赖 runtime backend matrix。
4. YOLO11 / YOLO26：补 workflow 调用和前端逐任务操作记录；如果模型 core 后续继续修复，需要重新跑对应 task 行。
