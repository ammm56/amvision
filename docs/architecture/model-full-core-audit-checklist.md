...
- `backend/service/application/models/training/yolo_task_detection_training.py` 已删除。YOLOv8 detection 训练执行已下沉到 `backend/service/application/models/yolov8_core/training/detection_execution.py`，应用层只保留 `yolov8_detection_training.py` 入口。
...
| 明确缺口 | `yolo_core_common/primary` 已删除；`yolo_primary` 命名的 worker、training、evaluation、catalog 和测试入口已收口；`yolo_task_detection_training.py` 已删除，YOLOv8 detection 执行编排已进入 `yolov8_core/training/detection_execution.py` |
...
| 明确缺口 | non-detection service / worker / evaluation 命名已收成 `yolo_task_*`，`yolo_core_common/primary` 已删除；YOLOv8 / YOLO11 / YOLO26 detection 均已切到各自专属训练入口 |
...
1. 复跑 YOLOv8 / YOLO11 / YOLO26 五任务真实短链路，把每条链的失败原因和修复记录写入本清单。
2. 逐模型做 full core 对照复核：RF-DETR -> YOLOX -> YOLOv8 -> YOLO11 -> YOLO26，重点确认没有轻量近似、旧入口或错误共享。
...
- 旧 `yolo_primary_*` 入口已经删除或改为中性命名；`yolo_task_detection_training.py` 共享 detection 训练入口也已删除或完全下沉到 `yolov8_core`。
