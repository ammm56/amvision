...
| 状态 | `需验收`，API full chain 已补记录，workflow / 前端待补 |
...
| 风险点 | LoRA / PEFT 当前未启用；早期 checkpoint 转换保留 `legacy_checkpoint_format / legacy_ema_state_dict` 是读取 RF-DETR 旧训练产物所需，不属于旧平台入口；长时训练、release/full 常驻 soak、workflow 和前端操作仍需要显式记录 |
| 下一步 | 补 workflow / 前端调用记录；长时间训练、release/full 常驻 soak、资源占用和异常恢复基线单独跑，不放默认测试 |
...
| 下一步 | 跑 YOLOX COCO / VOC 全链路，记录 DatasetImport、DatasetExport、短训练、评估、conversion、deployment 和 workflow 结果 |
...
| 状态 | `需验收`，API full chain 已补记录，workflow / 前端待补；局部 evaluator 仍需靠近官方口径 |
...
| 风险点 | segmentation mask AP、pose OKS、OBB rotated AP 当前是项目内 COCO-style AP 实现，不等同于 Ultralytics 完整 `SegmentationValidator / PoseValidator / OBBValidator` 及 pycocotools / 专用 rotated evaluator 的全部输出口径；全 scale checkpoint、workflow / 前端操作和 release/full soak 仍需单独记录 |
| 下一步 | 补 workflow / 前端调用记录；继续核对 v8 loss / assigner / data augmentation / export forward / runtime output layout；长时间训练、release/full 常驻 soak、资源占用和异常恢复基线单独跑 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| RF-DETR | detection | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | API 链路已记录 | 2026-06-23 已跑真实 DatasetImport / DatasetExport、短训练、评估、ONNX / OpenVINO / TensorRT、deployment sync / async / stop-reset。结果目录：`.tmp/model-full-core-validation/rfdetr-api-onnx-20260623-fix3`、`.tmp/model-full-core-validation/rfdetr-api-openvino-20260623-fix1`、`.tmp/model-full-core-validation/rfdetr-api-tensorrt-20260623`。还需补 workflow / 前端记录。 |
| RF-DETR | segmentation | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | API 链路已记录 | 2026-06-23 已跑真实 DatasetImport / DatasetExport、短训练、评估、ONNX / OpenVINO / TensorRT、deployment sync / async / stop-reset。结果目录：`.tmp/model-full-core-validation/rfdetr-api-seg-onnx-20260623-fix4`、`.tmp/model-full-core-validation/rfdetr-api-openvino-20260623-fix1`、`.tmp/model-full-core-validation/rfdetr-api-tensorrt-20260623`。还需补 workflow / 前端记录。 |
| YOLOX | detection，COCO DatasetExport | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | API 链路已记录 | 2026-06-23 已跑真实 DatasetImport / DatasetExport、短训练、评估、ONNX / OpenVINO / TensorRT、deployment sync / async / stop-reset。结果目录：`.tmp/model-full-core-validation/yolox-api-onnx-20260623`、`.tmp/model-full-core-validation/yolox-api-openvino-20260623`、`.tmp/model-full-core-validation/yolox-api-tensorrt-20260623`。还需补 workflow / 前端记录。 |
| YOLOX | detection，VOC DatasetExport | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | API 链路已记录 | 2026-06-23 已跑真实 DatasetImport / DatasetExport、短训练、VOC DatasetExport 原生评估入口、ONNX / OpenVINO / TensorRT、deployment sync / async / stop-reset。结果目录同 COCO 行；还需补 workflow / 前端记录。 |
| YOLOv8 | detection | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | API 链路已记录 | 2026-06-23 已跑真实 DatasetImport / DatasetExport、短训练、评估、ONNX / OpenVINO / TensorRT、deployment sync / async / stop-reset。结果目录：`.tmp/yolo-model-full-chain-smoke/yolov8-api-full-chain-20260623`。该 run 后续因 segmentation ONNX 校验失败整体标记 failed，但 detection 子链路已完整成功；还需补 workflow / 前端记录。 |
| YOLOv8 | classification | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | API 链路已记录 | 2026-06-23 已跑真实 DatasetImport / DatasetExport、短训练、评估、ONNX / OpenVINO / TensorRT、deployment sync / async / stop-reset。结果目录：`.tmp/yolo-model-full-chain-smoke/yolov8-api-full-chain-20260623`。该 run 后续因 segmentation ONNX 校验失败整体标记 failed，但 classification 子链路已完整成功；还需补 workflow / 前端记录。 |
| YOLOv8 | segmentation | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | API 链路已记录 | 2026-06-23 修复 YOLOv8 ONNX 数值校验输入和 mean-ratio accepted 摘要后，已跑真实 DatasetImport / DatasetExport、短训练、评估、ONNX / OpenVINO / TensorRT、deployment sync / async / stop-reset。结果目录：`.tmp/yolo-model-full-chain-smoke/yolov8-segmentation-full-chain-20260623-fix1`；还需补 workflow / 前端记录。 |
| YOLOv8 | pose | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | API 链路已记录 | 2026-06-23 已跑真实 DatasetImport / DatasetExport、短训练、评估、ONNX / OpenVINO / TensorRT、deployment sync / async / stop-reset。结果目录：`.tmp/yolo-model-full-chain-smoke/yolov8-pose-obb-full-chain-20260623-fix1`；还需补 workflow / 前端记录。 |
| YOLOv8 | obb | 已记录 | 已记录 | 已记录 | 已记录 | 待跑 | API 链路已记录 | 2026-06-23 已跑真实 DatasetImport / DatasetExport、短训练、评估、ONNX / OpenVINO / TensorRT、deployment sync / async / stop-reset。结果目录：`.tmp/yolo-model-full-chain-smoke/yolov8-pose-obb-full-chain-20260623-fix1`；还需补 workflow / 前端记录。 |
| YOLO11 | detection | 已记录 | 已记录 | 已记录 | 已记录 | workflow 已记录；前端待补 | 已记录 | 2026-06-20 已跑 `tests.integration.yolo_model_full_chain_smoke --model-type yolo11 --tasks detection classification segmentation pose obb --target-formats onnx openvino-ir tensorrt-engine --max-epochs 1 --batch-size 1 --max-images-per-split 4 --start-processes`，结果在 `.tmp/yolo-model-full-chain-smoke/20260620110947/result.json`。2026-06-23 补跑五任务 workflow，结果目录：`.tmp/yolo-model-full-chain-smoke/yolo11-workflow-record-20260623`。前端逐任务页面操作仍需单独记录。 |
| YOLO11 | classification | 已记录 | 已记录 | 已记录 | 已记录 | workflow 已记录；前端待补 | 已记录 | 同 YOLO11 detection 记录；2026-06-23 另有 classification workflow probe 修复记录，结果使用 `yolo11-classification-workflow-probe-20260623-fix2`。 |
| YOLO11 | segmentation | 已记录 | 已记录 | 已记录 | 已记录 | workflow 已记录；前端待补 | 已记录 | 同 YOLO11 detection 记录。 |
| YOLO11 | pose | 已记录 | 已记录 | 已记录 | 已记录 | workflow 已记录；前端待补 | 已记录 | 2026-06-19 另有 pose / OBB ONNX 子集记录 `.tmp/yolo11-pose-obb-20260619161813`；完整三格式链路见 2026-06-20 记录，workflow 见 2026-06-23 记录。 |
| YOLO11 | obb | 已记录 | 已记录 | 已记录 | 已记录 | workflow 已记录；前端待补 | 已记录 | 同 YOLO11 pose 记录。 |
| YOLO26 | detection | 已记录 | 已记录 | 已记录 | 已记录 | workflow 已记录；前端待补 | 已记录 | 2026-06-20 已跑五任务三格式 full chain，结果在 `.tmp/yolo-model-full-chain-smoke/20260620211256/result.json`；2026-06-21 又用真实转换产物复跑 `--run-id yolo26-runtime-artifact-smoke-20260621-r2`。2026-06-23 修复后补跑 detection / classification workflow，结果目录：`.tmp/yolo-model-full-chain-smoke/yolo26-detection-classification-workflow-record-20260623-fix1`。 |
| YOLO26 | classification | 已记录 | 已记录 | 已记录 | 已记录 | workflow 已记录；前端待补 | 已记录 | 同 YOLO26 detection 记录；另有 non-detection backend matrix 覆盖 conversion -> runtime predict。 |
| YOLO26 | segmentation | 已记录 | 已记录 | 已记录 | 已记录 | workflow 已记录；前端待补 | 已记录 | 2026-06-23 修复 YOLO26 segmentation bbox 反序和零面积 mask polygon 后，复跑真实 DatasetImport / DatasetExport、短训练、评估、ONNX conversion、deployment sync / async、workflow 成功。结果目录：`.tmp/yolo-model-full-chain-smoke/yolo26-segmentation-workflow-record-20260623-fix2`。 |
| YOLO26 | pose | 已记录 | 已记录 | 已记录 | 已记录 | workflow 已记录；前端待补 | 已记录 | 2026-06-23 补跑 pose / OBB 真实 DatasetImport / DatasetExport、短训练、评估、ONNX conversion、deployment sync / async、workflow 成功。结果目录：`.tmp/yolo-model-full-chain-smoke/yolo26-pose-obb-workflow-record-20260623-fix1`。 |
| YOLO26 | obb | 已记录 | 已记录 | 已记录 | 已记录 | workflow 已记录；前端待补 | 已记录 | 同 YOLO26 pose 记录。 |

当前最小补缺顺序：

1. RF-DETR detection / segmentation：API 链路已补；后续补 workflow / 前端记录和长时间 soak。
2. YOLOX detection COCO / VOC：API 链路已补；后续补 workflow / 前端记录和长时间 soak。
3. YOLOv8 五任务：API 链路已补；后续补 workflow / 前端记录、全 scale checkpoint 和长时间 soak。
4. YOLO11 / YOLO26：workflow 调用已补记录；后续补前端逐任务页面操作记录。如果模型 core 后续继续修复，需要重新跑对应 task 行。
5. 所有模型：release/full 长时间 soak、资源占用和异常恢复基线单独记录，不放进默认 pytest。
...
1. 按验收记录表补缺口，顺序固定为 RF-DETR -> YOLOX -> YOLOv8 -> YOLO11 -> YOLO26。
2. 每补一条链，都同步记录 DatasetImport、DatasetExport、training、evaluation、conversion、deployment、workflow 的结果位置和失败修复。
3. 逐模型做 full core 对照复核，重点确认没有轻量近似、旧入口或错误共享；如果复核发现代码修复，修复后重新跑对应 task 行。
