...
- 哪些 payload 规则 先进入 core catalog
- 第一批 core 视频节点先做哪些
- `SAM3` 视频/多帧节点应该放在哪一层
- 通用视频结果与跟踪结果应该如何抽象
...
### 第 1 步：保留当前 core 节点与 payload 规则
...
- 后续切到 `ffmpeg/ffprobe` 时，应保持 payload 规则、core 节点 id 和 workflow 连接方式不变。
- 也就是说，替换的是“节点内部工具实现”，不是“视频 workflow 抽象层”。

## 第一批通用 payload 规则

第一批先固定 3 个通用 payload 规则：
...
1. 把 `video-ref.v1 / frame-window.v1 / tracks.v1` 接进 core payload 规则
