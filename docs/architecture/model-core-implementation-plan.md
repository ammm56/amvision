...
当前 full core 完成度、过渡残留和真实全链路验收结果不在本文档重复维护，统一记录到 [model-full-core-audit-checklist.md](model-full-core-audit-checklist.md)。进入第五批 custom nodes 前，必须先按该清单完成 `RF-DETR / YOLOX / YOLOv8 / YOLO11 / YOLO26` 的逐模型逐任务验收。
...
- YOLO 共用层：`yolo_core_common` 中真正跨 `YOLOv8 / YOLO11 / YOLO26` 共用的基础层、几何工具、权重读取辅助和不含任务语义的底层数学工具。包含 task head、loss、assigner、target、postprocess、export 输出规则的逻辑应放入各自 `*_core`。
...
- YOLO 主线 ONNX 校验分两层：所有 task 都必须通过 ONNX checker、输出数量、shape 和 finite 检查；`classification` 和 raw `detection` 输出继续要求逐元素 allclose。YOLO26 end2end export 的 detection 输出已经是 processed TopK `[B,K,6]`，PyTorch 与 ONNXRuntime 在大量同分候选框下可能产生等价但不同顺序的候选行，因此 YOLO26 detection 在 `yolo26_core/export/validation.py` 中使用 processed TopK 语义校验，保留 `raw_row_allclose` 摘要但不把 raw row allclose 作为失败条件。`segmentation / pose / obb` 的 raw 输出包含 mask prototype、keypoint、angle 等多头几何张量，PyTorch 2.8 新 exporter 与 ONNXRuntime 在长链 FP32 上会出现放大的 raw 差异，因此转换报告保留 `allclose / max_abs_diff / mean_abs_diff` 摘要，但不把 raw allclose 作为失败条件。后续如果新增 task-specific 语义校验，应放到对应 `*_core/export` 或 `*_core/postprocess` 边界中，不在 worker 中临时判断。
...
1. 先做 `YOLOv8 full core`。YOLOv8 是普通 YOLO 主线最稳定的基准，先把 cfg、nn、task head、weights、validation、loss、target、postprocess、training、evaluation、export 和 runtime 调用全部收进 `yolov8_core`。
2. 再做 `YOLO11 full core`。目录规则复用 YOLOv8，但 `yolo11_core` 不直接依赖 `yolov8_core`，只能依赖 `yolo_core_common` 中真正共用的基础能力。
3. 最后做 `YOLO26 full core`。YOLO26 的 `Segment26 / Pose26 / OBB26`、end2end detection、raw angle decode、RLE pose 等差异只能在 `yolo26_core` 内部处理，不能回写到 YOLOv8 / YOLO11 路径。
4. 每个模型完成 import graph 替换后，删除对应旧实现。当前共享 detection 训练入口已删除，YOLOv8 detection 训练执行已进入 `yolov8_core/training/detection_execution.py`；YOLOv8 classification / segmentation / pose / OBB 训练执行已分别进入 `yolov8_core/training/classification_execution.py`、`segmentation_execution.py`、`pose_execution.py` 和 `obb_execution.py`，旧共享训练入口已删除；classification 训练 / evaluation 平台 service 已收成 `yolov8_classification_*`，共享 segmentation 平台 service / evaluation service 已中性化为 `segmentation_training_*` 和 `segmentation_evaluation*`，只保留 YOLOv8 与 RF-DETR segmentation 的任务状态、队列、对象存储和登记分发。根目录旧 `runtime/yolo_primary_*_predictor.py` 已删除，deployment runtime 统一走 `runtime/predictors/`。
5. runtime 拆分只处理 deployment session、backend adapter、buffer、warmup/reset、序列化和请求参数。模型 decode、postprocess、preview render、inference 输出解释归各自 core；推理是否画框由调用参数控制，独立部署推理默认可画框，workflow 模型节点可以关闭。

`yolo_core_common` 只放三类普通 YOLO 共同使用、且不需要知道 `model_type` 的底层能力：基础层、几何函数、anchor / DFL / bbox decode、通用 NMS 前过滤、通用 export plan。凡是包含模型代际差异、task 组合策略、训练增强、loss 编排、输出名、preview render、state_dict 映射或 inference 输出解释的逻辑，都必须放回 `yolov8_core / yolo11_core / yolo26_core`。common 中不得出现 `model_type` 分支，也不得为了 YOLO26 特例影响 YOLOv8 / YOLO11 的行为。
...
- 已收窄 `rfdetr_core/models/__init__.py`，默认不导入在线下载等非本项目核心入口；`deprecate` 外部依赖已用本项目内装饰器替代。
...
当前实现已按普通 YOLO 主线拆出模型分类 core：

- `backend/service/application/models/yolov8_core/`
- `backend/service/application/models/yolo11_core/`
- `backend/service/application/models/yolo26_core/`

第一阶段已经把每个模型分类的任务配置和 builder 入口放入各自 core 包。`model_builders.py` 只保留平台统一分发职责，不再承载三类模型的任务配置。

YOLOv8 full core 边界已按最终目录继续收口：

- `backend/service/application/models/yolov8_core/cfg/`：已拆出 detection、segmentation、classification、pose、obb 五类任务配置，`yolov8_core/config.py` 只保留兼容入口，不再从 `detection_builder.py` 读取 YOLOv8 detection 配置。
- `backend/service/application/models/yolov8_core/nn/tasks/`：已承接 YOLOv8 自己的 `Detect / Segment / Classify / Pose / OBB` task head 实现，不再继承或别名到 `yolo_core_common/tasks`。`heads.py` 只负责登记 YOLOv8 head module map。
- `backend/service/application/models/yolov8_core/nn/modules/`：已拆出 YOLOv8 当前配置实际使用的 `Concat / Bottleneck / C2f / SPPF`，不把 YOLO11 / YOLO26 的 `C3k2 / C2PSA / Segment26 / Pose26 / OBB26` 混入 YOLOv8。
- `backend/service/application/models/yolov8_core/nn/model.py`：已承接 YOLOv8 配置 parser、scale profile、顺序图构建和跨层引用执行；`build_yolov8_model()` 不再依赖共享 `yolo_detection_model.py` 的模型构建入口。
- `backend/service/application/models/yolov8_core/assigners/`、`targets/`、`losses/`：已承接 YOLOv8 detection 的 TAL assigner、bbox distance target、DFL loss 和 detection loss 编排；detection assigner 已按 Ultralytics `TaskAlignedAssigner` 的核心规则使用 align metric、gt 最大 overlap、target score 归一，detection box / DFL loss 已按 `BboxLoss` 核心规则使用 target score 加权并按 `target_scores_sum` 归一，bbox overlap 使用 CIoU。该层也已承接 YOLOv8 segmentation 的正样本分配、polygon mask target 栅格化、box/class/DFL loss 和 mask BCE loss；segmentation assigner 已复用 YOLOv8 detection TAL 边界，mask loss 使用 `BCEWithLogits`、proto resize、bbox crop 和归一 bbox area，避免回退成整图 mask BCE。pose / OBB 的 OKS、visibility、probiou、angle loss、keypoint target、rotated target 已进入 YOLOv8 core 本地文件，不再从 `yolo_core_common/losses` 或 `yolo_core_common/targets` 透传。YOLOv8 detection 训练执行已进入 `yolov8_core/training/detection_execution.py`；YOLO11 / YOLO26 detection 已使用各自专属训练入口。
- `backend/service/application/models/yolov8_core/training/detection.py`：已承接 YOLOv8 detection 训练 loss 调度入口，负责识别 YOLOv8 core 模型、定位 detection head 并调用 YOLOv8 loss。
- `backend/service/application/models/yolov8_core/training/runtime.py`：已承接 YOLOv8 detection 的 optimizer、scheduler、GradScaler 和 optimizer state device 迁移。
- `backend/service/application/models/yolov8_core/training/samples.py`：已承接 YOLOv8 detection train / validation 样本计划和类别映射一致性校验。DatasetExport split 解析已拆到 `yolov8_core/data/detection_splits.py`，样本对象读取已拆到 `yolov8_core/data/detection_samples.py`，core training 层只判断训练样本数量、validation 是否启用，以及 validation categories / category_id 是否与 train split 一致。
- `backend/service/application/models/yolov8_core/training/dataloader.py`：已承接 YOLOv8 detection 训练 dataloader 执行计划，包括 batch size 归一、每个 epoch 的 batch 数、总 iteration 和 resume 后初始 global iteration。batch tensor 构建已拆到 `yolov8_core/data/detection_batch.py`，Mosaic / MixUp / HSV / flip / affine 增强已拆到 `yolov8_core/data/detection_augmentation.py`，应用层只负责调用 core data 入口。
- `backend/service/application/models/yolov8_core/training/execution.py`：已承接 YOLOv8 detection 训练数据上下文和执行计划，包括 DatasetExport split 解析、train / validation 样本读取、类别映射一致性、样本计划、dataloader 计划、resume best metric 读取和初始 global iteration。应用层继续保留任务状态、事件、暂停/保存控制、对象存储写入和数据库登记。
- `backend/service/application/models/yolov8_core/training/validation.py`：已承接 YOLOv8 detection validation loss 循环，应用层只提供 batch 构建、输出解包和 BatchNorm freeze/restore callback。COCO mAP、评估任务登记和状态回写仍留在应用层。
- `backend/service/application/models/yolov8_core/training/checkpoint.py`：已承接 YOLOv8 detection checkpoint state 构建、bytes 编码、bytes 解码，以及 epoch 后 latest / best checkpoint bytes 更新。`training/savepoint.py` 已承接 YOLOv8 detection epoch savepoint payload 组装，应用层只把 core payload 转成平台 savepoint 并触发回调。
- `backend/service/application/models/training/yolo_detection_task_outputs.py`：已承接 YOLO 主线 detection 训练产物布局和 object store 写入，包括 best / latest checkpoint、labels、training metrics、validation metrics、summary 和 savepoint 输出。
- `backend/service/application/models/training/yolo_detection_task_registration.py`：已承接 YOLO 主线 detection 训练输出登记到 ModelVersion / ModelFile 的规则，包括 runtime summary、metrics summary 和 metadata 组装。`yolo_detection_training_service.py` 继续负责任务状态事件写入时机、pause/resume 控制、调用训练 runner 和结果串联。
- `backend/service/application/models/training/yolo_detection_task_events.py`：已承接 YOLO 主线 detection 训练任务事件构造，包括 queue、save/pause/terminate、resume、started、batch / epoch progress、checkpoint saved、completed、paused、cancelled 和 failed。应用层仍负责决定事件写入时机、事务边界和 pause/resume 控制链路。
- `backend/service/application/models/training/yolo_detection_task_control.py`：已承接 YOLO 主线 detection 训练控制状态读取和构造，包括 save/pause/terminate 请求、resume pending、一次性请求清理、savepoint 保存状态和 latest checkpoint 恢复入口解析。应用层仍负责校验任务状态、读取 task record、写任务事件和重新入队。
- `backend/service/application/models/training/yolo_detection_task_payload.py`：已承接 YOLO 主线 detection 训练 task metadata、queue metadata、task spec 字典、TaskRecord 到 request kwargs、TaskRecord.result 到 result kwargs、局部训练结果 kwargs 和 TaskRecord.result 序列化。应用层仍保留 request / result class 选择和训练执行编排。
- `backend/service/application/models/training/yolo_detection_task_dataset.py`：已承接 YOLO 主线 detection 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、模型支持的 detection 导出格式和 manifest object key 完整性校验。应用层只传入请求、模型名称和模型展示名，不再直接维护 DatasetExport 查询细节。
- `backend/service/application/models/training/yolo_detection_task_warm_start.py`：已承接 YOLO 主线 detection warm start ModelVersion、checkpoint file、storage uri 和本地 checkpoint path 解析。应用层只传入模型服务类、文件类型、session factory 和 dataset storage。
- `backend/service/application/models/training/yolo_detection_task_summary.py`：已承接 YOLO 主线 detection 训练 summary、training config、validation summary 和 resolved extra options 组装。应用层只负责在训练完成和 ModelVersion 登记后写入 summary 文件。
- `backend/service/application/models/training/yolov8_classification_training_dataset.py`：已承接 YOLOv8 classification 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、classification task_type、模型支持的 classification 导出格式和 manifest object key 完整性校验。
- `backend/service/application/models/training/yolov8_classification_training_payload.py`：已承接 YOLOv8 classification 训练 task_spec、TaskRecord metadata、queue payload 和 TaskRecord 到训练负载的反解析。应用层只保留 request / result class 和训练执行串联。
- `backend/service/application/models/training/yolov8_classification_training_control.py`：已承接 YOLOv8 classification 训练控制状态读取、手动 save 清理以及 save / pause / terminate 控制标记写入所需 metadata 构造。应用层仍负责 task_service 读写和训练循环回调。
- `backend/service/application/models/training/yolov8_classification_training_events.py`：已承接 YOLOv8 classification 训练状态事件构造，包括 queue failed、queued、started、cancelled、paused、failed 和 succeeded。应用层仍负责决定事件写入时机和异常控制流。
- `backend/service/application/models/training/yolov8_classification_training_registration.py`：已承接 YOLOv8 classification 训练输出登记到 ModelVersion / ModelFile 的规则，包括 metadata 组装和 file type 登记。YOLO11 / YOLO26 classification 已由各自专属 service 与 registration helper 承接，不再通过该 helper 分发。
- `backend/service/application/models/training/segmentation_training_dataset.py`：已承接共享 segmentation 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、segmentation task_type、模型支持的 segmentation 导出格式和 manifest object key 完整性校验。该 helper 保留 YOLOv8 与 RF-DETR segmentation 支持边界，不和 classification / pose / obb 混用。
- `backend/service/application/models/training/segmentation_training_payload.py`、`segmentation_training_control.py`、`segmentation_training_events.py`、`segmentation_training_registration.py`：已承接 segmentation 自己的 task_spec、queue payload、控制状态、任务事件和训练输出登记规则。该组 helper 独立服务 segmentation，不复用 classification / pose / obb helper；应用层继续负责训练执行串联、对象存储写入时机和异常控制流。
- `backend/service/application/models/training/yolov8_pose_training_dataset.py`：已承接 YOLOv8 pose 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、pose task_type、模型支持的 pose 导出格式和 manifest object key 完整性校验。
- `backend/service/application/models/training/yolov8_pose_training_payload.py`、`yolov8_pose_training_control.py`、`yolov8_pose_training_events.py`、`yolov8_pose_training_registration.py`：已承接 YOLOv8 pose 自己的 task_spec、queue payload、控制状态、任务事件和训练输出登记规则。该组 helper 独立服务 YOLOv8 pose，不复用 classification / segmentation / obb helper；应用层继续负责训练执行串联、对象存储写入时机和异常控制流。
- `backend/service/application/models/training/yolov8_obb_training_dataset.py`：已承接 YOLOv8 OBB 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、obb task_type、模型支持的 obb 导出格式和 manifest object key 完整性校验。
- `backend/service/application/models/training/yolov8_obb_training_payload.py`、`yolov8_obb_training_control.py`、`yolov8_obb_training_events.py`、`yolov8_obb_training_registration.py`：已承接 YOLOv8 OBB 自己的 task_spec、queue payload、控制状态、任务事件和训练输出登记规则。该组 helper 独立服务 YOLOv8 OBB，不复用 classification / segmentation / pose helper；应用层继续负责训练执行串联、对象存储写入时机和异常控制流。
- `backend/service/application/models/yolov8_core/training/resume.py`：已承接 YOLOv8 detection resume checkpoint 的模型分类、scale、类别数、input size、batch size、epoch、precision、validation 配置、optimizer / loss / assigner 参数校验。YOLOv8 detection 训练执行通过 `yolov8_core/training/detection_execution.py` 调用该 core 校验入口，其他模型必须走各自专属训练入口。
- `backend/service/application/models/yolov8_core/training/epoch.py`：已承接 YOLOv8 detection epoch 边界规则，包括 validation 触发条件、默认 best metric 名称、初始 best metric 哨兵值、best checkpoint 更新判断和对外 best metric 序列化。YOLOv8 detection 训练执行通过 `yolov8_core/training/detection_execution.py` 调用这些规则，YOLO11 / YOLO26 detection 不走共享 runner。
- `backend/service/application/models/yolov8_core/training/control.py`：已承接 YOLOv8 detection epoch 控制动作规则，只判断 save checkpoint、pause training 和 terminate training 这类训练循环动作。读取控制状态、写任务事件、保存 object key 和 resume 入队仍留在应用服务层。
- `backend/service/application/models/yolov8_core/training/runner.py`：已承接 YOLOv8 detection 单 epoch batch 循环，包含样本 shuffle、batch 迭代、forward、loss backward、grad clip、optimizer step、GradScaler 更新和 batch progress 汇总。应用层仍负责写 task progress、把 savepoint 写入对象存储并登记输出文件。
- `backend/service/application/models/yolov8_core/losses/classification.py`：已承接 YOLOv8 classification 训练态 logits 归一和交叉熵 loss，单 tensor 输出明确按 logits 处理。YOLOv8 classification 训练执行已进入 `yolov8_core/training/classification_execution.py`，该入口只接受 `model_type=yolov8`；YOLO11 / YOLO26 使用各自专属 classification 训练入口。
...
- `obb`：当前 `YOLOV8_OBB_MODEL_SERVICE_MAP` 只包含 `yolov8`，训练执行已经进入 `yolov8_core/training/obb_execution.py`；平台 service/helper 已收成 `yolov8_obb_training_*`，task_kind / queue 已显式为 `yolov8-obb-training` / `yolov8-obb-trainings`。YOLO11 / YOLO26 OBB 走各自专属 service。
- `segmentation`：当前 `SEGMENTATION_TRAINING_MODEL_SERVICE_MAP` 包含 `yolov8` 和 `rfdetr`，因此不能改成 `yolov8_segmentation_*`。这组 helper 后续应中性化为 `segmentation_training_*`，并明确把 YOLOv8 与 RF-DETR 的执行分支留在各自 core / model service，平台 service 只负责任务状态、对象存储、队列、DatasetExport 校验和登记分发。

后续改名只剩 `segmentation`。该步骤必须同步 `routes/task_training/`、各 task route 的 `services.py`、`backend/workers/training/`、`workflow service_runtime/builders.py`、core node platform training request 映射、测试和文档；旧文件删除，不保留兼容壳。
- `backend/service/application/models/yolov8_core/postprocess/detection.py`、`postprocess/segmentation.py`：已承接 YOLOv8 detection / segmentation runtime 后处理。detection 的 detection record 组装、score / NMS 结果排序和 YOLOv8 默认调试预览图渲染已从 runtime 切到 core 入口；segmentation 的 prediction/proto 输出归一、NMS 候选、mask coeff × proto 解码、segments 提取和实例记录组装已进入 YOLOv8 core，不再调用 `yolo_core_common/postprocess`。
- `backend/service/application/models/yolov8_core/postprocess/classification.py`、`postprocess/pose.py`、`postprocess/obb.py`、`postprocess/preview.py`：已承接 YOLOv8 classification top-k、pose / OBB 的 score 过滤、NMS、bbox / keypoint / angle 实例组装，以及 YOLOv8 调试预览图渲染。runtime 中 YOLOv8 detection / segmentation / classification / pose / OBB 分支调用 core builder 后再映射到公开 API 契约，`save_result_image` 仍由推理请求控制；YOLO11 / YOLO26 暂时仍走原 runtime 组装逻辑。
- `backend/service/application/models/yolov8_core/export/`：已按目录承接 YOLOv8 export 边界。`source.py` 负责导出源 PyTorch 模型加载，`plan.py` 负责 YOLOv8 自己的导出目标、输出名和 export mode 规则，`execution.py` 负责 YOLOv8 ONNX 导出、ONNXRuntime 数值校验、OpenVINO IR 构建和 TensorRT engine 构建，`classification.py / segmentation.py / pose.py / obb.py` 负责各 task 输出规范。`onnx.py / openvino.py / tensorrt.py` 只保留格式入口；`yolov8_conversion_runner.py` 只做任务编排和 runner 方法覆盖，不再直接依赖 runtime predictor session 构建导出源模型，也不再通过 `yolo_core_common/export` 执行 YOLOv8 task 输出校验。
- `backend/service/application/models/yolov8_core/inference/`：已承接 YOLOv8 classification / pose / OBB / segmentation 的 inference 输出归一、top-k 或实例后处理 adapter。这里不放 deployment session，不依赖 ONNXRuntime / OpenVINO / TensorRT 会话对象，只解释模型输出张量。
- `backend/service/application/runtime/predictors/yolov8_detection_* / yolov8_segmentation_* / yolov8_classification_* / yolov8_pose_* / yolov8_obb_*`：YOLOv8 五类 task 的 `pytorch / onnxruntime / openvino / tensorrt` backend session 已从薄壳拆成真实 runtime 文件。`backend / io / timing / preview / serialization / buffer` 支撑层也已按 task 命名拆开；deployment session、backend adapter、CUDA buffer 和序列化仍留在 runtime，模型输出解释和预览渲染调用 `yolov8_core/postprocess`。
- `backend/service/application/models/yolov8_core/validation.py`：已提供 YOLOv8 结构快照入口，用于后续参数量、输出形状和 state_dict 覆盖率验收。
...
当前 YOLOv8 的配置、task head、parser、backbone / neck blocks、detection loss/assigner/target、detection 训练 loss 调度入口、optimizer / scheduler / GradScaler 构建、训练数据上下文、dataloader 执行计划、validation loss 循环、resume checkpoint 校验、epoch 边界规则、epoch 控制动作规则、单 epoch batch runner、checkpoint state / bytes 编码、训练产物 object store 写入、ModelVersion / ModelFile 登记 helper、任务事件构造 helper、训练控制状态 helper、任务 payload 组装 helper、DatasetExport 解析 helper、warm start 解析 helper、summary 组装 helper、segmentation assigner / mask target / mask loss、classification loss、pose / OBB assigner / decode / target / loss、classification / segmentation / pose / OBB 训练 batch 编码、训练期 eval、detection / segmentation / pose / OBB 数据集级 evaluation 外壳、detection / segmentation postprocess、classification / pose / OBB 后处理实例组装、YOLOv8 preview render、各 task export 输出规范、YOLOv8 export plan / execution、classification / segmentation / pose / OBB inference adapter、detection / segmentation / classification / pose / OBB runtime backend session、结构快照已经进入清晰目录。YOLOv8 不再引用 `yolo_core_common/tasks`、`yolo_core_common/losses`、`yolo_core_common/targets`、`yolo_core_common/decode`、`yolo_core_common/postprocess` 或 `yolo_core_common/export`；当前只复用 `layers / geometry / weights` 这类不含模型代际判断的基础能力。YOLOv8 detection loss 已补 `TaskAlignedAssigner / BboxLoss` 的关键行为：target score 使用 align metric 和最大 overlap 归一，box / DFL loss 使用 target score 加权并按 `target_scores_sum` 归一，bbox overlap 使用 CIoU。YOLOv8 segmentation 已补 detection loss 复用边界、DFL logits 输入、target score 加权、proto resize、bbox crop 和归一 bbox area mask loss；pose / OBB 已补 box / DFL 的 target score 加权和 `target_scores_sum` 归一，pose assigner 复用 YOLOv8 detection TAL / CIoU，pose keypoint / visibility loss 已修正累计尺度，OBB assigner 已补 rotated IoU target score 归一和 tiny rbox 候选扩张，OBB angle loss 已修正聚合尺度。segmentation / pose / OBB evaluation 已从旧 bbox 近似收成项目内 COCO-style AP 插值：segmentation 输出 bbox AP 与 mask AP，pose 输出 OKS AP，OBB 输出 rotated AP；OBB runtime postprocess 也已按 YOLOv8 OBB 输出的 `xywh + angle` 边界组装 `bbox_xywhr` 和旋转框外接矩形，并按同类别 rotated IoU 做 NMS，避免把前四维误当 `xyxy` 或按普通外接框压制旋转框。segmentation / pose / OBB 数据入口已接入受控 HSV、水平翻转、random affine / perspective matrix、mosaic、mixup、multi-scale 和 close-mosaic/no-aug schedule，并覆盖 mask、keypoint 左右交换、keypoint 仿射变换、mosaic/mixup target 合并和 rotated box angle / corner 几何同步；task data 配置入口已支持 `no_aug / no_augmentation / disable_augmentation` 关闭增强；segmentation parser 已按 scale 缩放 `Segment` 的 `npr` proto 宽度。2026-06-17 已用 `data/files/models/pretrained/yolov8` 下 default 本地预训练 checkpoint 跑通 detection / classification / segmentation / pose / OBB 五类任务、nano / s / m / l / x 五个 scale 的短时加载覆盖率，全部 `loadable_ratio = 1.0`，无 missing / shape mismatch / unexpected keys。2026-06-18 已用真实本地数据跑通 YOLOv8 detection / classification / segmentation / pose / OBB 的 DatasetImport、DatasetExport、1 epoch 短训练、ONNX / OpenVINO / TensorRT 转换、deployment sync / async 推理和 stop/reset；同日 `release/full` 重新装配后完成 30 秒短时常驻、资源采样和 stop 回收验收。detection training service 当前主要保留任务编排、训练 runner 调用和异常处理；classification / segmentation / pose / OBB training service 已分别拆出各自的 DatasetExport、task payload、控制状态、事件构造和输出登记 helper，没有复用 detection helper，也没有把四类 non-detection task 混成同一个大 helper。后续差距主要是更长时间真实训练、release/full 负载 soak、资源/异常恢复基线，以及按需要继续对齐 `pycocotools` COCO mask / keypoint evaluator 或专门 rotated box evaluator 的数值细节。后续先沿 YOLOv8 单模型纵向闭环迁移，不再同时强切 YOLO11 / YOLO26。

2026-06-17 再次对照 `projectsrc/ultralytics/ultralytics` 后的边界结论：

- `projectsrc/ultralytics/ultralytics/cfg/models/v8`、`11`、`26` 分别是三代模型配置来源。本项目可以对齐配置职责和数学行为，但未做许可证决策前不直接复制 AGPL 运行时代码。
- `projectsrc/ultralytics/ultralytics/nn/tasks.py` 的 `parse_model`、`DetectionModel / SegmentationModel / PoseModel / OBBModel / ClassificationModel` 是模型构建职责参考；本项目当前 `yolov8_core / yolo11_core / yolo26_core` 都已建立 `cfg / nn / data / losses / targets / assigners / training / evaluation / export / inference / postprocess` 纵向结构，后续按审计清单继续核对数学细节和真实链路记录。
- `projectsrc/ultralytics/ultralytics/nn/modules/head.py` 中的 `Detect / Segment / Segment26 / Pose / Pose26 / OBB / OBB26 / Classify` 是 head 行为参考。YOLO26 专属 head 差异必须留在 `yolo26_core`，不得回写到 `yolov8_core` 或在 `yolo_core_common` 中用 `model_type` 分支处理。
- `projectsrc/ultralytics/ultralytics/utils/loss.py` 与 `utils/tal.py` 是 loss、assigner 和 target 规则参考。当前 YOLOv8 detection 已补齐 `TaskAlignedAssigner / BboxLoss` 的 target score 加权、最大 overlap 归一、CIoU 和 `target_scores_sum` 归一等关键行为；YOLOv8 segmentation 已补 `v8SegmentationLoss` 中最关键的 detection loss 复用、proto resize、bbox crop 和 mask area 归一边界；YOLOv8 pose / OBB 已补 box / DFL target score 加权、pose keypoint / visibility loss 累计尺度、OBB rotated target score 归一、tiny rbox 候选扩张和 angle loss 聚合尺度。该实现是项目内重写的 full core 路径，不依赖 Ultralytics runtime 包；后续如果要做官方逐项一致性，只补数值细节和回归，不再回退到旧 primary/common 近似实现。
- `yolo_core_common` 当前未出现 `model_type / yolov8 / yolo11 / yolo26 / Segment26 / Pose26 / OBB26` 分支命中，这个方向是正确的。common 只能放不关心模型代际的基础数学工具。
- `detection_builder.py` 和 `yolo_core_common/weights.py` 中存在 `ultralytics.nn.*` 临时 module path shim，只用于官方风格 `.pt` checkpoint 反序列化，不是运行时依赖 `ultralytics` 包。后续 YOLO11 / YOLO26 full core 完成后，应继续把这类 checkpoint 兼容逻辑下沉到各自 core weights 边界，减少旧共享文件存在感。

当前 `YOLOv8 / YOLO11 / YOLO26` 的结构、head、decode、loss、target、assigner、training、evaluation、inference、postprocess、export 和 runtime backend session 已进入各自 `*_core` 与 `runtime/predictors/` 路径；根目录旧 `runtime/yolo_primary_*_predictor.py` 已删除，旧 `yolo_primary` / `yolo_task` 训练过渡命名也已收口。真实链路验收状态不在本文档重复判断，统一以 [model-full-core-audit-checklist.md](model-full-core-audit-checklist.md) 的记录表为准：YOLOv8、YOLO11、YOLO26 都已有五任务真实短链路记录；其中 YOLOv8 的首次五任务 run 最终因 segmentation ONNX 校验失败整体标记 failed，但各成功子链已有记录，之后也补了 detection 代表性链路复验，具体状态以审计清单逐项记录为准。YOLO26 detection / segmentation / pose / OBB 已在 2026-06-21 补齐 end2end top-k、export forward、输出 box 格式和 processed 张量布局：export=True 且 end2end 时输出官方 top-k processed layout，runtime 后处理同时接受 raw PyTorch 输出和 processed ONNX/OpenVINO/TensorRT 输出；同日已用真实转换产物复跑 YOLO26 ONNX / OpenVINO / TensorRT conversion 与 deployment sync / async smoke，结果记录在 `.tmp/yolo-model-full-chain-smoke/yolo26-runtime-artifact-smoke-20260621-r2/result.json`。后续工作不再是把旧 primary 行为搬进新目录，而是按审计清单补 full core 复核、真实验收记录、release/full 常驻 soak、资源占用和异常恢复基线。
...
这些能力不包含模型代际判断。后续 loss、assigner、target 编码、postprocess 继续下沉时，只能复用这里的底层数学和张量工具，不能把任务级逻辑重新放回 common。

`Detect / Classify / Segment / Pose / OBB` 已从 `yolo_detection_model.py` 迁到 task 文件。当前这些 common task 文件只作为 YOLO26 full core 完成前的过渡来源；YOLOv8 与 YOLO11 已分别改为使用各自 `*_core/nn/tasks/` 内的本代实现：
...
`yolo11_core/heads.py` 当前已切到 `yolo11_core/nn/tasks/`，不再从 `yolo_detection_model.py` 或 `yolo_core_common/tasks` 中转；`yolov8_core/heads.py` 已切到本代 task head，避免 YOLOv8 / YOLO11 继续和 YOLO26 的过渡实现混用。
...
- `backend/service/application/models/yolo_core_common/decode/pose.py`：负责不关心 `model_type` 的 pose keypoint 几何 decode；具体 offset 规则由各模型 core 传入，不在 common 内判断模型代际。
- `backend/service/application/models/yolo_core_common/decode/obb.py`：负责不关心 `model_type` 的 obb angle decode 和 rotated box prediction 组装；具体 angle decode mode 由各模型 core 传入，不在 common 内判断模型代际。
...
- YOLOv8 detection 训练执行、`pose_loss.py`、`obb_loss.py` 已改为调用这些 common 入口，不再互相引用 detection training service 的私有函数。

Pose 专属 loss / target 编码已开始下沉到共用边界：

- `backend/service/application/models/yolo_core_common/losses/pose.py`：负责 OKS keypoint loss、visibility loss、pose keypoint decode 辅助和 OKS sigma。YOLO26 专属 RLE loss 和 RLE target weight 放在 `backend/service/application/models/yolo26_core/losses/pose.py`。
...
- `backend/service/application/models/yolov8_core/export/`、`backend/service/application/models/yolo11_core/export/`、`backend/service/application/models/yolo26_core/export/`：已分别承接各自模型的 export plan、ONNX 导出、ONNXRuntime 数值校验、OpenVINO IR 构建和 TensorRT engine 构建；各 task 的 export forward、输出名和 shape 规则放在对应 core 内，不再集中堆到 conversion runner 或单个 `export.py`。
...
- YOLOv8 segmentation `Segment` proto 宽度按 scale 缩放，避免 nano / s / m / x 权重加载时出现 proto shape mismatch。
...
- YOLOv8 已补训练数据上下文、执行计划、训练产物 object store 写入 helper、ModelVersion / ModelFile 登记 helper、任务事件构造 helper、训练控制状态 helper、任务 payload 组装 helper、DatasetExport 解析 helper、warm start 解析 helper、summary 组装 helper、conversion worker 短回归和 detection deployment sync / async 控制面 smoke；classification / segmentation / pose / OBB training service 已各自拆出 DatasetExport、task payload、控制状态、事件构造和输出登记 helper，四类 non-detection task 之间不互相复用 helper；segmentation / pose / OBB 已接入受控 HSV、水平翻转、random affine / perspective matrix、mosaic、mixup、multi-scale 和 close-mosaic/no-aug schedule；segmentation / pose / OBB 数据集级 evaluation wrapper 已接入 YOLOv8 core，task service 会按 `model_type == yolov8` 分发到对应 core 入口；2026-06-17 已复跑 `release/full` 5 秒短时常驻验收并通过，也已补项目内 COCO-style AP 插值指标，覆盖 segmentation mask AP、pose OKS AP 和 OBB rotated AP。2026-06-18 已补 `tests/integration/test_yolov8_detection_runtime_backend_smoke.py`，用于显式验证 YOLOv8 detection 的 ONNXRuntime / OpenVINO / TensorRT conversion -> runtime predict；同日复核 `data/files/datasets` 派生的真实短链路结果，detection / classification / segmentation / pose / OBB 均已有 DatasetImport、DatasetExport、1 epoch 短训练、ONNX / OpenVINO / TensorRT 转换和 deployment sync / async 结果记录。后续继续补 workflow 显式短验收、官方 COCO mask / keypoint evaluator 或专门 rotated box evaluator 的等价差距，以及目标机更长时间真实训练和 release/full soak；完成后再按同一目录规则推进 YOLO11，最后推进 YOLO26。
- YOLO11 / YOLO26 当前不得直接依赖 `yolov8_core`；只能依赖 `yolo_core_common` 中没有模型代际分支的基础能力。
...
- 已完成本批：detection bbox decode、segmentation mask decode、pose keypoint decode、obb angle / rotated box decode、NMS 前候选过滤、detection 训练态 prediction decode、TAL assigner、bbox target 编码、DFL loss、pose 共用 OKS / visibility loss、YOLO26 专属 RLE loss 下沉到 `yolo26_core`、keypoint target 归一、OBB probiou、rotated target 编码、angle loss、segmentation polygon mask target 编码、segmentation assigner、segmentation box/class loss 编排、segmentation 完整 postprocess 第一层、segmentation export 双输出边界、YOLOv8 / YOLO11 / YOLO26 的 weights 正式入口、checkpoint 文件读取入口、YOLO26 pickle 兼容绑定下沉到 `yolo26_core/weights.py`、export plan 正式入口、export execution 正式入口、mask BCE loss 迁入 core 边界、worker 侧共享转换 helper 中性化、YOLO11 detection validation loss / epoch runner / epoch control / savepoint / checkpoint / resume / runtime / execution plan / trainer 主循环入口、DatasetExport split/sample/data context 和 detection data batch / Mosaic / MixUp / HSV / flip / random affine 入口下沉，以及 YOLO11 segmentation / pose / OBB 的 inference、postprocess、runtime backend 文件拆分和去 primary 基类。
- 已完成本批：YOLO11 detection 训练服务入口已切到 `yolo11_detection_training.py`，`SqlAlchemyYolo11TrainingTaskService.training_runner` 不再指向共享 detection 训练入口。新入口只接受 `model_type=yolo11`，并调用 `yolo11_core` 的 data context、batch、augmentation、runtime、trainer、checkpoint/resume 和 validation 边界。
- 已完成本批：共享 detection 训练入口已删除。YOLO11 detection 训练使用 `yolo11_detection_training.py`，YOLO26 detection 训练使用 `yolo26_detection_training.py`，YOLOv8 detection 训练执行已进入 `yolov8_core/training/detection_execution.py`。
- 已完成本批：YOLO11 classification 已切出 `yolo11_classification_training.py` 训练入口。该入口只接受 `model_type=yolo11`，并直接调用 `yolo11_core` 的 classification model、DatasetExport manifest 解析、batch、loss 和 evaluation；YOLOv8 classification 执行也已进入 `yolov8_core/training/classification_execution.py`，旧 `yolov8_classification_training.py` 已删除。YOLO11 classification runtime 入口已迁到 `runtime/predictors/yolo11_classification.py`，根目录旧 predictor 文件已删除；PyTorch / ONNXRuntime / OpenVINO / TensorRT backend session 已拆到 `runtime/predictors/yolo11_classification_pytorch.py`、`onnxruntime.py`、`openvino.py` 和 `tensorrt.py`，不再继承旧 primary classification backend。训练侧也已补 `yolo11_core/data/classification_manifest.py`、`yolo11_core/training/classification_defaults.py`、`classification_runtime.py`、`classification_checkpoint.py` 和 `classification_trainer.py`，分别承接 DatasetExport / manifest 解析、默认训练参数、optimizer/scheduler/scaler、resume/checkpoint bytes 和主 epoch loop；`models/training/yolo11_classification_task_execution.py` 只负责平台 task DTO 与 YOLO11 执行 DTO 的转换，避免 service 直接理解 YOLO11 训练内部字段。
- 已完成本批：YOLO11 segmentation 已切出 `yolo11_segmentation_training.py` 训练入口。该入口只接受 `model_type=yolo11`，并直接调用 `yolo11_core` 的 segmentation model、batch、assigner、mask target、detection/mask loss 和 evaluation；`models/training/yolo11_segmentation_task_execution.py` 只负责平台 task DTO 与 YOLO11 执行 DTO 的转换。YOLOv8 segmentation 训练执行也已进入 `yolov8_core/training/segmentation_execution.py`，旧共享训练入口已删除；共享 segmentation 平台 service 已中性化为 `segmentation_training_*`，只服务 YOLOv8 与 RF-DETR，YOLO11 / YOLO26 segmentation 使用各自专属 service、task_kind 和 queue。YOLO11 segmentation 的 manifest、imports/device/autocast、anchor、resume 和 checkpoint helper 已迁到 `yolo11_core/training/segmentation_*.py`，不再从旧共享训练文件借私有函数。
- 已完成本批：YOLO11 pose / OBB 已补模型专属 service DTO、执行入口、输出登记和 core training 主循环。`yolo11_pose_training.py`、`yolo11_obb_training.py` 负责定义 YOLO11 pose / OBB 的执行请求和 result DTO，并直接调用 `yolo11_core/training/pose_*` 与 `yolo11_core/training/obb_*` 入口；`models/training/yolo11_pose_task_execution.py`、`models/training/yolo11_obb_task_execution.py` 负责平台 task DTO 与 YOLO11 执行 DTO 的转换；`models/training/yolo11_pose_task_registration.py`、`models/training/yolo11_obb_task_registration.py` 负责 YOLO11 pose / OBB 的 ModelVersion 与 ModelFile 登记。pose / OBB service 对 `model_type=yolo11` 已分发到对应 adapter，ModelVersion metadata 和 task summary 也会返回 `yolo11-pose-core` / `yolo11-obb-core`。`pose_manifest.py` / `obb_manifest.py` 承接 DatasetExport manifest 解析，`pose_checkpoint.py` / `obb_checkpoint.py` 承接 resume 和 checkpoint bytes，`pose_trainer.py` / `obb_trainer.py` 承接 epoch loop、validation、savepoint 与 pause/terminate 控制。YOLO11 pose / OBB 不再通过 YOLOv8 训练执行入口或 YOLOv8 registration 直接执行。2026-06-19 已用真实 `hand-keypoints` 和 `dota128` 数据集跑通 YOLO11 pose / OBB 的 DatasetImport、DatasetExport、1 epoch 短训练、ONNX 转换、deployment sync / async 推理和 stop/reset。
- 已完成本批：YOLOv8 pose / OBB 训练执行已从旧共享执行入口迁到 `yolov8_core/training/pose_execution.py` 和 `yolov8_core/training/obb_execution.py`，并新增 `models/training/yolov8_pose_training.py` 与 `models/training/yolov8_obb_training.py` 应用层入口。YOLOv8 pose / OBB service/helper 已收成 `yolov8_pose_training_*`、`yolov8_obb_training_*`，task_kind / queue 分别为 `yolov8-pose-training` / `yolov8-pose-trainings` 和 `yolov8-obb-training` / `yolov8-obb-trainings`；YOLO11 / YOLO26 pose / OBB 继续走各自专属 service。
...
- 后续批次：YOLOv8、YOLO11 和 YOLO26 已完成五任务短链路验收，后续只继续清 service / worker / runtime 外壳与更长时间运行基线；YOLO26 已按 full core 模板完成第一轮纵向切断，`cfg`、`decode`、`nn/modules`、`nn/tasks`、data、loss、target、assigner、training、evaluation、inference、postprocess、export 和 runtime backend session 都已进入 YOLO26 专属路径，并切断模型结构入口对 `detection_builder.py` 与根目录 `yolo_detection_model.py` 的依赖。classification、segmentation、pose、OBB 的 task kind / queue 已分别收为 `yolo26-*`，旧根目录 `runtime/yolo26_classification_predictor.py`、`runtime/yolo26_segmentation_predictor.py`、`runtime/yolo26_pose_predictor.py`、`runtime/yolo26_obb_predictor.py` 已删除。2026-06-20 已用真实数据跑通 YOLO26 detection / classification / segmentation / pose / OBB 的 DatasetImport、DatasetExport、1 epoch 短训练、evaluation、ONNX / OpenVINO / TensorRT 转换、deployment sync / async 推理和 stop/reset，结果记录在 `.tmp/yolo-model-full-chain-smoke/20260620211256/result.json`；随后继续完成 YOLO26 detection runtime backend 拆分、`yolo26_core/export/` 拆包和 conversion service 中性化。
- 2026-06-21 YOLO26 复核结论：短链路跑通不等同于 full core 行为完全对齐。参考 `projectsrc/ultralytics/ultralytics/nn/modules/head.py` 后，YOLO26 detection 已补 `end2end-topk` 后处理：先按 anchor 最大类别分数取 top-k anchor，再在这些 anchor 的所有类别分数中取最终 top-k；该逻辑固定在 `yolo26_core/postprocess/detection.py`，不回退到旧 primary 或通用 NMS。segmentation / pose / OBB 也已改为复用 YOLO26 core 内的 end2end top-k 选择逻辑，不再通过普通 NMS 压制候选。本轮进一步把 `Detect26 / Segment26 / Pose26 / OBB26` 的 export forward 形态、输出 box 格式和 gather 后张量布局收进 core：raw decode 按 end2end 的 `xyxy` box 边界处理；detection processed 输出为 `[x1, y1, x2, y2, score, class]`；segmentation / pose 在相同 6 列后追加 mask coeff 或 keypoints；OBB 输出为 `[x, y, w, h, score, class, angle]`。随后继续按 `utils/tal.py` 复核 assigner：detection / segmentation / pose / OBB 已删除最近 anchor 强制 fallback，tiny box / tiny rbox 候选筛选改为按 stride[0] 与下一层 stride 扩张后再进入 top-k，避免把链路可跑误写成参考实现一致；YOLO26 pose / OBB runtime 的 `yolov8_*` 支撑引用也已迁到中性 `yolo_runtime_*` helper。同日复跑真实 ONNX / OpenVINO / TensorRT 产物和 deployment sync / async smoke 已通过，结果记录在 `.tmp/yolo-model-full-chain-smoke/yolo26-runtime-artifact-smoke-20260621-r2/result.json`；因此这一轮不再把旧 NMS、`xywh` 假设或跨代 runtime 命名当成 YOLO26 full core 行为。
- 2026-06-23 YOLO26 二次对照修复：继续参考 `Detect26 / Segment26 / Pose26 / OBB26` 的 forward / export 行为后，`yolo26_core/nn/tasks/*` 已明确区分 raw head 输出和 official processed 输出。训练态保留 raw head 输出；推理非 export 返回 processed 与 raw，便于调试和内部检查；export 只返回 official processed layout，供 ONNX / OpenVINO / TensorRT runtime 使用。`Segment26` 已修正 `Proto26 npr` 参数传递，避免非默认 proto 数量时结构和 checkpoint 覆盖率偏离参考实现。YOLO26 PyTorch runtime 已新增 detection / segmentation 输出 adapter，避免继续借用旧 detection helper 假设 raw `[B, N, 4 + nc]`。
- 2026-06-24 YOLO26 processed layout 复验：真实 DatasetImport / DatasetExport、1 epoch 短训练、评估、ONNX / OpenVINO / TensorRT conversion、deployment sync / async smoke 已覆盖 detection、classification、segmentation、pose 和 OBB。detection / classification 结果目录：`.tmp/yolo-model-full-chain-smoke/yolo26-processed-layout-smoke-20260624-r2/result.json`；segmentation / pose / OBB 结果目录：`.tmp/yolo-model-full-chain-smoke/yolo26-processed-layout-smoke-20260624-r3/result.json`。本轮还修复了 YOLO26 segmentation 训练期 evaluation 对 `((processed, proto), raw)` nested output 的解包，三种 runtime backend 均按 processed layout 正常解析。
- 2026-06-24 普通 YOLO export layout 复核：对照 `projectsrc/ultralytics/ultralytics/nn/modules/head.py` 后，YOLOv8 / YOLO11 的普通非 end2end task export forward 已统一为 Ultralytics `[B, C, N]` channel-first 布局；平台 runtime/postprocess 入口负责转为内部 `[B, N, C]` 消费。YOLO26 保持 `end2end=True`，export 只返回 official processed layout，`((processed, proto), raw)` 只属于非 export 调试与内部检查路径，不进入 ONNX / OpenVINO / TensorRT backend 输出。
- 2026-06-24 严格真实转换产物复验：使用 `tests.integration.yolo_model_full_chain_smoke --model-type yolo26 --model-scale nano --tasks detection classification segmentation pose obb --target-formats onnx openvino-ir tensorrt-engine --max-epochs 1 --batch-size 1 --max-images-per-split 4 --start-processes --run-workflow --run-id yolo26-strict-real-artifact-smoke-20260624-r1` 复跑。五个 task 均完成真实 DatasetImport / DatasetExport、短训练、评估、ONNX / OpenVINO / TensorRT conversion、deployment sync / async、workflow 调用和脚本托管进程回收。结果目录：`.tmp/yolo-model-full-chain-smoke/yolo26-strict-real-artifact-smoke-20260624-r1/result.json`。
- runtime 目录整理当前已完成 RF-DETR 第一层、YOLOX predictor 第一层、YOLOv8 / YOLO11 五类 predictor 拆分，以及 YOLO26 五类 predictor拆分：`runtime/predictors/rfdetr*.py`、`runtime/predictors/yolox*.py`、`runtime/predictors/yolov8*.py`、`runtime/predictors/yolo11*.py`、`runtime/predictors/yolo26_detection*.py`、`runtime/predictors/yolo26_classification*.py`、`runtime/predictors/yolo26_segmentation*.py`、`runtime/predictors/yolo26_pose*.py`、`runtime/predictors/yolo26_obb*.py`、`runtime/targets/rfdetr.py`、`runtime/targets/yolov8.py`、`runtime/targets/yolo11.py`、`runtime/targets/yolo26.py`、`runtime/contracts/*.py`、`runtime/serialization/*.py`、`runtime/support/tensorrt_runtime.py` 和 `runtime/support/detection.py` 已从平铺目录拆出；后续继续按模型纵向闭环清少量根目录过渡外壳。
