...
4. 每个模型完成 import graph 替换后，删除对应旧实现。当前 `yolo_task_detection_training.py` 共享 detection 训练入口已删除，YOLOv8 detection 训练执行已进入 `yolov8_core/training/detection_execution.py`；non-detection 共享训练执行层已经改为 `yolo_task_classification_training.py`、`yolo_task_segmentation_training.py`、`yolo_task_pose_training.py`、`yolo_task_obb_training.py` 与 `yolo_task_*_training_service.py`，classification / segmentation 共享评估执行层也已改为 `yolo_task_*_evaluation.py`。根目录旧 `runtime/yolo_primary_*_predictor.py` 已删除，deployment runtime 统一走 `runtime/predictors/`。
