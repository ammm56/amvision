...
- YOLO 主线 ONNX 校验分两层：所有 task 都必须通过 ONNX checker、输出数量、shape 和 finite 检查；`classification / detection` 继续要求 raw 输出逐元素 allclose。`segmentation / pose / obb` 的 raw 输出包含 mask prototype、keypoint、angle 等多头几何张量，PyTorch 2.8 新 exporter 与 ONNXRuntime 在长链 FP32 上会出现放大的 raw 差异，因此转换报告保留 `allclose / max_abs_diff / mean_abs_diff` 摘要，但不把 raw allclose 作为失败条件。后续如果新增 task-specific 语义校验，应放到对应 `*_core/export` 或 `*_core/postprocess` 边界中，不在 worker 中临时判断。
...
因此当前不能说 `YOLOv8 / YOLO11 / YOLO26` 都已经 full core 完成。准确状态是：YOLOv8 已进入 full core 纵向收口和真实短链路验收阶段；YOLO11 已完成本代 cfg、head、parser、weights、detection/classification/segmentation/pose/obb 的 export 输出规范，并在 2026-06-18 跑通五类 task 的真实 DatasetImport、DatasetExport、1 epoch 短训练、ONNX 转换、deployment sync / async 推理和 stop/reset；YOLO11 segmentation / pose / OBB runtime 入口已从根目录旧外壳迁到 `runtime/predictors/`；YOLO11 segmentation 已补 `yolo11_core/inference/segmentation.py`，PyTorch / ONNXRuntime / OpenVINO / TensorRT runtime backend 已拆成独立 session 文件，不再继承旧 `yolo_primary_segmentation_predictor.py`；YOLO11 detection 验证 loss 统计已进入 `yolo11_core/training/validation.py`，单 epoch batch loop 已进入 `yolo11_core/training/runner.py`，epoch 规则、epoch control、savepoint payload、checkpoint state/bytes、resume checkpoint 校验、训练 runtime 构建、resume 后执行计划和主 epoch loop 已进入 `yolo11_core/training/epoch.py`、`control.py`、`savepoint.py`、`checkpoint.py`、`resume.py`、`runtime.py`、`plan.py` 和 `trainer.py`；`yolo11_core/training/execution.py` 已去掉对 `yolo_primary_detection_training.py` 的反向 import，只保留 YOLO11 core 执行边界和实现模式常量。YOLO11 pose / OBB 训练侧已补 `data / losses / assigners / targets / evaluation / inference / postprocess` 的模型专属 core 入口，其中 pose 关键点 decode 使用 Ultralytics 风格 `pred * 2 + anchor - 0.5`，OBB assigner 使用 rotated inside 判断、tiny rbox 候选保护和 pixel/grid 坐标分界，训练期 validation 也已经走 YOLO11 自己的 OKS AP 与 rotated AP sample evaluator。YOLO11 pose / OBB runtime backend 已按 PyTorch / ONNXRuntime / OpenVINO / TensorRT 拆成独立 session 文件，并补齐 backend、io、buffer、preview、timing、runtime postprocess 支撑模块，不再继承旧 `yolo_primary_pose_predictor.py` / `yolo_primary_obb_predictor.py`。YOLO11 后续差距主要是继续把 detection batch 构造、DatasetExport data context、数据增强和更完整评估指标从旧 primary/shared 边界收进 core，以及 segmentation / pose / OBB 官方指标和训练增强细节的继续对齐；YOLO26 仍处在初始 core 和差异入口阶段，不能把旧 `yolo_primary` 行为简单搬进新目录。
