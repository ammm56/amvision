...
4. 每个模型完成 import graph 替换后，删除对应旧实现。旧文件包括 `yolo_detection_model.py`、`yolo_primary_detection_training.py`、`yolo_primary_classification_training.py`、`yolo_primary_segmentation_training.py`、`yolo_primary_pose_training.py`、`yolo_primary_obb_training.py` 和 `yolo_primary_*_service.py` 中已经被 core 覆盖的部分；根目录旧 `runtime/yolo_primary_*_predictor.py` 已删除，deployment runtime 统一走 `runtime/predictors/`。
...
- `backend/service/application/models/yolov8_core/cfg/`：已拆出 detection、segmentation、classification、pose、obb 五类任务配置，`yolov8_core/config.py` 只保留兼容入口，不再从 `yolo_primary_detection_model.py` 读取 YOLOv8 detection 配置。
...
- `backend/service/application/models/training/yolo_primary_detection_task_outputs.py`：已承接 YOLO 主线 detection 训练产物布局和 object store 写入，包括 best / latest checkpoint、labels、training metrics、validation metrics、summary 和 savepoint 输出。
- `backend/service/application/models/training/yolo_primary_detection_task_registration.py`：已承接 YOLO 主线 detection 训练输出登记到 ModelVersion / ModelFile 的规则，包括 runtime summary、metrics summary 和 metadata 组装。`yolo_primary_training_service.py` 继续负责任务状态事件写入时机、pause/resume 控制、调用训练 runner 和结果串联。
- `backend/service/application/models/training/yolo_primary_detection_task_events.py`：已承接 YOLO 主线 detection 训练任务事件构造，包括 queue、save/pause/terminate、resume、started、batch / epoch progress、checkpoint saved、completed、paused、cancelled 和 failed。应用层仍负责决定事件写入时机、事务边界和 pause/resume 控制链路。
- `backend/service/application/models/training/yolo_primary_detection_task_control.py`：已承接 YOLO 主线 detection 训练控制状态读取和构造，包括 save/pause/terminate 请求、resume pending、一次性请求清理、savepoint 保存状态和 latest checkpoint 恢复入口解析。应用层仍负责校验任务状态、读取 task record、写任务事件和重新入队。
- `backend/service/application/models/training/yolo_primary_detection_task_payload.py`：已承接 YOLO 主线 detection 训练 task metadata、queue metadata、task spec 字典、TaskRecord 到 request kwargs、TaskRecord.result 到 result kwargs、局部训练结果 kwargs 和 TaskRecord.result 序列化。应用层仍保留 request / result class 选择和训练执行编排。
- `backend/service/application/models/training/yolo_primary_detection_task_dataset.py`：已承接 YOLO 主线 detection 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、模型支持的 detection 导出格式和 manifest object key 完整性校验。应用层只传入请求、模型名称和模型展示名，不再直接维护 DatasetExport 查询细节。
- `backend/service/application/models/training/yolo_primary_detection_task_warm_start.py`：已承接 YOLO 主线 detection warm start ModelVersion、checkpoint file、storage uri 和本地 checkpoint path 解析。应用层只传入模型服务类、文件类型、session factory 和 dataset storage。
- `backend/service/application/models/training/yolo_primary_detection_task_summary.py`：已承接 YOLO 主线 detection 训练 summary、training config、validation summary 和 resolved extra options 组装。应用层只负责在训练完成和 ModelVersion 登记后写入 summary 文件。
- `backend/service/application/models/training/yolo_primary_classification_task_dataset.py`：已承接 YOLO 主线 classification 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、classification task_type、模型支持的 classification 导出格式和 manifest object key 完整性校验。
- `backend/service/application/models/training/yolo_primary_classification_task_payload.py`：已承接 YOLO 主线 classification 训练 task_spec、TaskRecord metadata、queue payload 和 TaskRecord 到训练负载的反解析。应用层只保留 request / result class 和训练执行串联。
- `backend/service/application/models/training/yolo_primary_classification_task_control.py`：已承接 YOLO 主线 classification 训练控制状态读取、手动 save 清理以及 save / pause / terminate 控制标记写入所需 metadata 构造。应用层仍负责 task_service 读写和训练循环回调。
- `backend/service/application/models/training/yolo_primary_classification_task_events.py`：已承接 YOLO 主线 classification 训练状态事件构造，包括 queue failed、queued、started、cancelled、paused、failed 和 succeeded。应用层仍负责决定事件写入时机和异常控制流。
- `backend/service/application/models/training/yolo_primary_classification_task_registration.py`：已承接 YOLO 主线 classification 训练输出登记到 ModelVersion / ModelFile 的规则，包括按 model_type 分发到 yolov8 / yolo11 / yolo26 对应模型服务、metadata 组装和 file type 登记。
- `backend/service/application/models/training/yolo_primary_segmentation_task_dataset.py`：已承接 YOLO 主线 segmentation 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、segmentation task_type、模型支持的 segmentation 导出格式和 manifest object key 完整性校验。该 helper 保留 RF-DETR segmentation 支持边界，不和 classification / pose / obb 混用。
- `backend/service/application/models/training/yolo_primary_segmentation_task_payload.py`、`yolo_primary_segmentation_task_control.py`、`yolo_primary_segmentation_task_events.py`、`yolo_primary_segmentation_task_registration.py`：已承接 segmentation 自己的 task_spec、queue payload、控制状态、任务事件和训练输出登记规则。该组 helper 独立服务 segmentation，不复用 classification / pose / obb helper；应用层继续负责训练执行串联、对象存储写入时机和异常控制流。
- `backend/service/application/models/training/yolo_primary_pose_task_dataset.py`：已承接 YOLO 主线 pose 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、pose task_type、模型支持的 pose 导出格式和 manifest object key 完整性校验。
- `backend/service/application/models/training/yolo_primary_pose_task_payload.py`、`yolo_primary_pose_task_control.py`、`yolo_primary_pose_task_events.py`、`yolo_primary_pose_task_registration.py`：已承接 pose 自己的 task_spec、queue payload、控制状态、任务事件和训练输出登记规则。该组 helper 独立服务 pose，不复用 classification / segmentation / obb helper；应用层继续负责训练执行串联、对象存储写入时机和异常控制流。
- `backend/service/application/models/training/yolo_primary_obb_task_dataset.py`：已承接 YOLO 主线 OBB 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、obb task_type、模型支持的 obb 导出格式和 manifest object key 完整性校验。
- `backend/service/application/models/training/yolo_primary_obb_task_payload.py`、`yolo_primary_obb_task_control.py`、`yolo_primary_obb_task_events.py`、`yolo_primary_obb_task_registration.py`：已承接 OBB 自己的 task_spec、queue payload、控制状态、任务事件和训练输出登记规则。该组 helper 独立服务 OBB，不复用 classification / segmentation / pose helper；应用层继续负责训练执行串联、对象存储写入时机和异常控制流。
- `backend/service/application/models/yolov8_core/training/resume.py`：已承接 YOLOv8 detection resume checkpoint 的模型分类、scale、类别数、input size、batch size、epoch、precision、validation 配置、optimizer / loss / assigner 参数校验。`yolo_primary_detection_training.py` 在 `model_type == "yolov8"` 时调用该 core 校验入口，其他模型暂时仍走原路径。
- `backend/service/application/models/yolov8_core/training/epoch.py`：已承接 YOLOv8 detection epoch 边界规则，包括 validation 触发条件、默认 best metric 名称、初始 best metric 哨兵值、best checkpoint 更新判断和对外 best metric 序列化。`yolo_primary_detection_training.py` 在 `model_type == "yolov8"` 时调用这些规则，YOLO11 / YOLO26 暂时仍走原路径。
...
- `backend/service/application/models/yolov8_core/losses/classification.py`：已承接 YOLOv8 classification 训练态 logits 归一和交叉熵 loss，单 tensor 输出明确按 logits 处理。`yolo_primary_classification_training.py` 在 `model_type == "yolov8"` 时调用该入口，YOLO11 / YOLO26 暂时不变。
- `backend/service/application/models/yolov8_core/losses/pose.py`、`losses/obb.py`：已承接 YOLOv8 pose / OBB loss 编排，内部调用 YOLOv8 自己的 assigner、decode、target 编码和几何入口；pose / OBB 的 box / DFL loss 已按 target score 加权并按 `target_scores_sum` 归一，不再用 foreground count 简单平均；pose keypoint / visibility loss 已避免按 foreground 二次缩小，OBB angle loss 已按 target score 聚合后再统一归一。`yolo_primary_pose_training.py`、`yolo_primary_obb_training.py` 在 `model_type == "yolov8"` 时只调用 YOLOv8 core loss；在 `model_type == "yolo11"` 时已切到 `yolo11_core/losses/pose.py` 与 `yolo11_core/losses/obb.py`，YOLO26 暂时仍走自己的后续重构路径。
...
- `backend/service/application/models/yolov8_core/data/detection.py`：已收成 YOLOv8 detection data 层统一入口，内部按职责拆到 `detection_types.py`、`detection_splits.py`、`detection_samples.py`、`detection_batch.py` 和 `detection_augmentation.py`。`yolo_primary_detection_training.py` 在 `model_type == "yolov8"` 时调用该 data 层，YOLO11 / YOLO26 暂时仍走旧共享路径。
- `backend/service/application/models/yolo_primary_detection_training.py` 中剩余的 detection data / augmentation 主入口已改成 `non_yolov8` 前缀，只作为 YOLO11 / YOLO26 full core 完成前的暂存路径；YOLOv8 detection 不再调用这些旧主入口。
