...
因此当前不能说 `YOLOv8 / YOLO11 / YOLO26` 都已经 full core 完成。准确状态是：YOLOv8 已进入 full core 纵向收口和真实短链路验收阶段；YOLO11 / YOLO26 仍处在初始 core 和差异入口阶段，下一批实现必须继续参考 Ultralytics 的配置、head、loss、assigner、target、trainer、validator 和 exporter 职责做项目内重写，而不是把旧 `yolo_primary` 行为简单搬进新目录。
...
`Detect / Classify / Segment / Pose / OBB` 已从 `yolo_detection_model.py` 迁到 task 文件。当前这些 common task 文件只作为 YOLO11 / YOLO26 full core 完成前的过渡来源；YOLOv8 已改为使用 `yolov8_core/nn/tasks/` 内的本代实现：
...
`yolo11_core/heads.py` 仍直接引用这些 common task head，不再从 `yolo_detection_model.py` 中转；`yolov8_core/heads.py` 已切到本代 task head，避免 YOLOv8 继续和 YOLO11 / YOLO26 共用 head 实现。
