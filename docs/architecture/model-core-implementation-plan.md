...
YOLOv8 第一批 full core 边界已经开始按最终目录落地：
...
- `backend/service/application/models/yolov8_core/nn/tasks/`：已建立 YOLOv8 自己的任务 head 入口，`heads.py` 只负责登记 YOLOv8 head module map。
...
- `backend/service/application/models/yolov8_core/evaluation/classification.py`、`evaluation/segmentation.py`、`evaluation/pose.py`、`evaluation/obb.py`：已承接 YOLOv8 四类任务的训练期轻量验证入口。classification 输出 top1/top5，segmentation / pose / OBB 输出基于 class + bbox IoU 匹配的短验证 `map50 / map50_95` 近似指标；segmentation / pose / OBB 也已补入 YOLOv8 数据集级 evaluation wrapper，并由对应 task service 在 `model_type == yolov8` 时调用 core 入口。当前数据集级 segmentation 已使用 COCO-style AP 插值计算 bbox AP 与 mask IoU AP，pose 已使用 OKS AP，OBB 已使用 polygon / rotated box IoU AP；这已经替代原来的 `precision * recall` 过渡统计，但仍不是完整 `pycocotools` COCO mask / keypoint 或专门 rotated box evaluator 的等价实现。
...
- `backend/service/application/models/yolov8_core/postprocess/detection.py`、`postprocess/segmentation.py`：已承接 YOLOv8 detection / segmentation runtime 后处理 wrapper。detection 的 detection record 组装、score / NMS 结果排序和 YOLOv8 默认调试预览图渲染已从 runtime 切到 core 入口；segmentation runtime predictor 在 `model_type == "yolov8"` 时调用 core 后处理入口，YOLO11 / YOLO26 仍走 common。
...
- `backend/service/application/models/yolov8_core/export/`：已按目录承接 YOLOv8 export 边界。`source.py` 负责导出源 PyTorch 模型加载，`classification.py / segmentation.py / pose.py / obb.py` 负责各 task 输出名和输出校验；`onnx.py / openvino.py / tensorrt.py` 负责 ONNX 导出与数值校验、OpenVINO IR 构建、TensorRT engine 构建。`yolov8_conversion_runner.py` 只做任务编排和 runner 方法覆盖，不再直接依赖 runtime predictor session 构建导出源模型。
...
当前 YOLOv8 的配置、head 入口、parser、backbone / neck blocks、detection loss/assigner/target、detection 训练 loss 调度入口、optimizer / scheduler / GradScaler 构建、训练数据上下文、dataloader 执行计划、validation loss 循环、resume checkpoint 校验、epoch 边界规则、epoch 控制动作规则、单 epoch batch runner、checkpoint state / bytes 编码、训练产物 object store 写入、ModelVersion / ModelFile 登记 helper、任务事件构造 helper、训练控制状态 helper、任务 payload 组装 helper、DatasetExport 解析 helper、warm start 解析 helper、summary 组装 helper、segmentation assigner / mask target / mask loss、classification loss、pose / OBB assigner / decode / target / loss、classification / segmentation / pose / OBB 训练 batch 编码、训练期轻量 eval、detection / segmentation / pose / OBB 数据集级 evaluation 外壳、detection / segmentation postprocess wrapper、classification / pose / OBB 后处理实例组装、YOLOv8 preview render、各 task export 输出规范、YOLOv8 export 目录、classification / segmentation / pose / OBB inference adapter、detection / segmentation / classification / pose / OBB runtime backend session、结构快照已经进入清晰目录。YOLOv8 detection loss 已补 `TaskAlignedAssigner / BboxLoss` 的关键行为：target score 使用 align metric 和最大 overlap 归一，box / DFL loss 使用 target score 加权并按 `target_scores_sum` 归一，bbox overlap 使用 CIoU。YOLOv8 segmentation 已补 detection loss 复用边界、DFL logits 输入、target score 加权、proto resize、bbox crop 和归一 bbox area mask loss；pose / OBB 已补 box / DFL 的 target score 加权和 `target_scores_sum` 归一，pose assigner 复用 YOLOv8 detection TAL / CIoU，pose keypoint / visibility loss 已修正累计尺度，OBB assigner 已补 rotated IoU target score 归一和 tiny rbox 候选扩张，OBB angle loss 已修正聚合尺度。segmentation / pose / OBB 训练期轻量 eval 已改为 class + bbox IoU 匹配的 `map50 / map50_95` 近似指标；segmentation / pose / OBB 数据入口已接入受控 HSV、水平翻转、random affine / perspective matrix、mosaic、mixup、multi-scale 和 close-mosaic/no-aug schedule，并覆盖 mask、keypoint 左右交换、keypoint 仿射变换、mosaic/mixup target 合并和 rotated box angle / corner 几何同步；task data 配置入口已支持 `no_aug / no_augmentation / disable_augmentation` 关闭增强；segmentation parser 已按 scale 缩放 `Segment` 的 `npr` proto 宽度。2026-06-17 已用 `data/files/models/pretrained/yolov8` 下 default 本地预训练 checkpoint 跑通 detection / classification / segmentation / pose / OBB 五类任务、nano / s / m / l / x 五个 scale 的短时加载覆盖率，全部 `loadable_ratio = 1.0`，无 missing / shape mismatch / unexpected keys。detection training service 当前主要保留任务编排、训练 runner 调用和异常处理；classification / segmentation / pose / OBB training service 已分别拆出各自的 DatasetExport、task payload、控制状态、事件构造和输出登记 helper，没有复用 detection helper，也没有把四类 non-detection task 混成同一个大 helper。segmentation / pose / OBB 数据集级 evaluation 现在已有 YOLOv8 core wrapper 和 task service 分发，并已接入项目内 COCO-style AP 插值：segmentation 同时输出 bbox AP 与 mask AP，pose 输出 OKS AP，OBB 输出 rotated AP。后续差距主要是替换或对齐完整官方 COCO mask / keypoint evaluator、专门 rotated box evaluator，以及十分钟内真实训练 / 转换 / deployment smoke。后续先沿 YOLOv8 单模型纵向闭环迁移，不再同时强切 YOLO11 / YOLO26。
...
因此当前不能说 `YOLOv8 / YOLO11 / YOLO26` 都已经 full core 完成。准确状态是：YOLOv8 正在按 full core 纵向收口，YOLO11 / YOLO26 仍处在初始 core 和差异入口阶段，下一批实现必须继续参考 Ultralytics 的配置、head、loss、assigner、target、trainer、validator 和 exporter 职责做项目内重写，而不是把旧 `yolo_primary` 行为简单搬进新目录。
...
- `backend/service/application/models/yolov8_core/export/`、`yolo11_core/export.py`、`yolo26_core/export.py`：负责各模型代际的 export 输出规范和导出计划正式入口。YOLOv8 已先按目录拆分，YOLO11 / YOLO26 后续按同一规则迁移为 `export/` 包。
