...
- `yolo_primary_*` 表示普通 YOLO 旧过渡 helper，当前只允许服务尚未完全拆出的 `yolov8` 和少量历史外层路径继续使用，不能再作为新 full core 的实现模板。
...
`yolo_primary_*` 中关于 YOLO11 / YOLO26 的代码只能保留“明确拒绝旧入口”的防护或历史外层兼容，不得再放模型专属训练、评估、导出、postprocess 或 runtime 实现。
...
4. 每个模型完成 import graph 替换后，删除对应旧实现。当前旧入口重点剩余在 `yolo_detection_model.py`、`yolo_task_detection_training.py` 和 detection 训练 helper；non-detection 共享训练执行层已经改为 `yolo_task_classification_training.py`、`yolo_task_segmentation_training.py`、`yolo_task_pose_training.py`、`yolo_task_obb_training.py` 与 `yolo_task_*_training_service.py`，classification / segmentation 共享评估执行层也已改为 `yolo_task_*_evaluation.py`。根目录旧 `runtime/yolo_primary_*_predictor.py` 已删除，deployment runtime 统一走 `runtime/predictors/`。
...
- `backend/service/application/models/training/yolo_task_classification_training_dataset.py`：已承接 YOLO 主线 classification 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、classification task_type、模型支持的 classification 导出格式和 manifest object key 完整性校验。
- `backend/service/application/models/training/yolo_task_classification_training_payload.py`：已承接 YOLO 主线 classification 训练 task_spec、TaskRecord metadata、queue payload 和 TaskRecord 到训练负载的反解析。应用层只保留 request / result class 和训练执行串联。
- `backend/service/application/models/training/yolo_task_classification_training_control.py`：已承接 YOLO 主线 classification 训练控制状态读取、手动 save 清理以及 save / pause / terminate 控制标记写入所需 metadata 构造。应用层仍负责 task_service 读写和训练循环回调。
- `backend/service/application/models/training/yolo_task_classification_training_events.py`：已承接 YOLO 主线 classification 训练状态事件构造，包括 queue failed、queued、started、cancelled、paused、failed 和 succeeded。应用层仍负责决定事件写入时机和异常控制流。
- `backend/service/application/models/training/yolo_task_classification_training_registration.py`：已承接 YOLO 主线 classification 训练输出登记到 ModelVersion / ModelFile 的规则，包括按 model_type 分发到 yolov8 / yolo11 / yolo26 对应模型服务、metadata 组装和 file type 登记。
- `backend/service/application/models/training/yolo_task_segmentation_training_dataset.py`：已承接 YOLO 主线 segmentation 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、segmentation task_type、模型支持的 segmentation 导出格式和 manifest object key 完整性校验。该 helper 保留 RF-DETR segmentation 支持边界，不和 classification / pose / obb 混用。
- `backend/service/application/models/training/yolo_task_segmentation_training_payload.py`、`yolo_task_segmentation_training_control.py`、`yolo_task_segmentation_training_events.py`、`yolo_task_segmentation_training_registration.py`：已承接 segmentation 自己的 task_spec、queue payload、控制状态、任务事件和训练输出登记规则。该组 helper 独立服务 segmentation，不复用 classification / pose / obb helper；应用层继续负责训练执行串联、对象存储写入时机和异常控制流。
- `backend/service/application/models/training/yolo_task_pose_training_dataset.py`：已承接 YOLO 主线 pose 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、pose task_type、模型支持的 pose 导出格式和 manifest object key 完整性校验。
- `backend/service/application/models/training/yolo_task_pose_training_payload.py`、`yolo_task_pose_training_control.py`、`yolo_task_pose_training_events.py`、`yolo_task_pose_training_registration.py`：已承接 pose 自己的 task_spec、queue payload、控制状态、任务事件和训练输出登记规则。该组 helper 独立服务 pose，不复用 classification / segmentation / obb helper；应用层继续负责训练执行串联、对象存储写入时机和异常控制流。
- `backend/service/application/models/training/yolo_task_obb_training_dataset.py`：已承接 YOLO 主线 OBB 训练 DatasetExport 解析，包括按 id / manifest object key 查找、project_id 一致性、导出状态、obb task_type、模型支持的 obb 导出格式和 manifest object key 完整性校验。
- `backend/service/application/models/training/yolo_task_obb_training_payload.py`、`yolo_task_obb_training_control.py`、`yolo_task_obb_training_events.py`、`yolo_task_obb_training_registration.py`：已承接 OBB 自己的 task_spec、queue payload、控制状态、任务事件和训练输出登记规则。该组 helper 独立服务 OBB，不复用 classification / segmentation / pose helper；应用层继续负责训练执行串联、对象存储写入时机和异常控制流。
- `backend/service/application/models/yolov8_core/training/resume.py`：已承接 YOLOv8 detection resume checkpoint 的模型分类、scale、类别数、input size、batch size、epoch、precision、validation 配置、optimizer / loss / assigner 参数校验。`yolo_task_detection_training.py` 当前只允许 `model_type == "yolov8"` 调用该 core 校验入口，其他模型必须走各自专属训练入口。
- `backend/service/application/models/yolov8_core/training/epoch.py`：已承接 YOLOv8 detection epoch 边界规则，包括 validation 触发条件、默认 best metric 名称、初始 best metric 哨兵值、best checkpoint 更新判断和对外 best metric 序列化。`yolo_task_detection_training.py` 当前只允许 `model_type == "yolov8"` 调用这些规则，YOLO11 / YOLO26 detection 不再走该过渡 runner。
...
- `backend/service/application/models/yolov8_core/losses/classification.py`：已承接 YOLOv8 classification 训练态 logits 归一和交叉熵 loss，单 tensor 输出明确按 logits 处理。`yolo_task_classification_training.py` 在 `model_type == "yolov8"` 时调用该入口，YOLO11 / YOLO26 暂时不变。
- `backend/service/application/models/yolov8_core/losses/pose.py`、`losses/obb.py`：已承接 YOLOv8 pose / OBB loss 编排，内部调用 YOLOv8 自己的 assigner、decode、target 编码和几何入口；pose / OBB 的 box / DFL loss 已按 target score 加权并按 `target_scores_sum` 归一，不再用 foreground count 简单平均；pose keypoint / visibility loss 已避免按 foreground 二次缩小，OBB angle loss 已按 target score 聚合后再统一归一。`yolo_task_pose_training.py`、`yolo_task_obb_training.py` 在 `model_type == "yolov8"` 时只调用 YOLOv8 core loss；在 `model_type == "yolo11"` 时已切到 `yolo11_core/losses/pose.py` 与 `yolo11_core/losses/obb.py`，YOLO26 暂时仍走自己的后续重构路径。
...
- `backend/service/application/models/yolov8_core/data/detection.py`：已收成 YOLOv8 detection data 层统一入口，内部按职责拆到 `detection_types.py`、`detection_splits.py`、`detection_samples.py`、`detection_batch.py` 和 `detection_augmentation.py`。`yolo_task_detection_training.py` 当前只允许 `model_type == "yolov8"` 走该 data 层，YOLO11 / YOLO26 会明确拒绝共享入口并使用各自 detection 专属训练入口。
- `backend/service/application/models/yolo_task_detection_training.py` 中剩余的 detection data / augmentation 主入口仍带有历史 `non_yolov8` 前缀，但已经不会承接 YOLO11 / YOLO26 主训练入口；后续应继续把该文件瘦成 YOLOv8 detection 过渡壳，最终将剩余执行编排下沉到 `yolov8_core`。
...
- `yolo_task_detection_training.py`、`pose_loss.py`、`obb_loss.py` 已改为调用这些 common 入口，不再互相引用 detection training service 的私有函数。
