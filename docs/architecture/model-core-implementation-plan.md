...
- `projectsrc/ultralytics/ultralytics/nn/tasks.py` 的 `parse_model`、`DetectionModel / SegmentationModel / PoseModel / OBBModel / ClassificationModel` 是模型构建职责参考；本项目当前 `yolov8_core/nn/model.py` 已按 YOLOv8 纵向重写，`yolo11_core / yolo26_core` 还没有达到同等目录深度。
...
当前 `YOLOv8 / YOLO11 / YOLO26` 都已经完成 detection / classification / segmentation / pose / OBB 五任务真实短链路验收：DatasetImport、DatasetExport、短训练、evaluation、ONNX / OpenVINO / TensorRT 转换、deployment sync / async 推理和 stop/reset 均已跑通。三代模型结构、head、decode、loss、target、assigner、training、evaluation、inference、postprocess、export 和 runtime backend session 已进入各自 `*_core` 与 `runtime/predictors/` 路径；根目录旧 `runtime/yolo_primary_*_predictor.py` 已删除，YOLOv8 / YOLO11 / YOLO26 conversion runner 和 conversion service 也已切到中性 YOLO 系列共享入口。这里的“短链路验收通过”只表示平台链路可执行，不等同于所有 task 的官方后处理数学细节已经完全一致；YOLO26 detection / segmentation / pose / OBB 已在 2026-06-21 补齐 end2end top-k、export forward、输出 box 格式和 processed 张量布局：export=True 且 end2end 时输出官方 top-k processed layout，runtime 后处理同时接受 raw PyTorch 输出和 processed ONNX/OpenVINO/TensorRT 输出。同日已用真实转换产物复跑 YOLO26 ONNX / OpenVINO / TensorRT conversion 与 deployment sync / async smoke，结果记录在 `.tmp/yolo-model-full-chain-smoke/yolo26-runtime-artifact-smoke-20260621-r2/result.json`。后续工作不再是把旧 primary 行为搬进新目录，而是继续清理 service / worker 外壳、减少 `yolo_primary_*` 训练过渡 helper 的存在感，并补长时间真实训练、release/full 常驻 soak、资源占用和异常恢复基线。
