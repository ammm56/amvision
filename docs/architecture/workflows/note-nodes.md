...
> 当前状态：代码、自动化测试和真实 Workflow App 验收已完成。本文定义说明节点的契约与执行边界；不得把说明节点改成可执行 NodeDefinition，或加入 Runtime 特殊执行分支。
...
- 当前只提供一个支持 Markdown 的说明节点。普通文本天然是合法 Markdown，不重复实现 Plain Text Note。
...
- 后端：`backend/contracts/workflows/workflow_graph.py`。
- 前端：Workflow 编辑器的说明节点、Markdown 渲染、节点组和画布文档适配。
- 已有契约、保存往返、安全渲染和执行范围测试；长期现场 soak 属于发行验收。
