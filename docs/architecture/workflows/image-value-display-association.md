...
现场再次检查发现：Value Display 行未勾选时，标题、尺寸和排序均灰显，且该行没有随图显示的说明，容易被理解为节点或数据显示已禁用。顶部“独立显示”说明不能替代每行的可见语义。当前画布有未保存修改，本次检查没有保存、刷新或修改连线。
...
1. 对话框使用“应用模式布局”，输出区使用“独立面板”。每行复选框旁显示完整操作名称，不能只放在 aria-label 或顶部说明中。
2. Image Preview 使用“显示图片面板”；Value Preview 使用“显示数据面板”；Value Display 已有随图用途时使用“另加独立面板”，无随图用途时使用“显示独立面板”。无论文案如何，持久化含义均为独立面板选择。
3. Value Display 行以配置的 Title 为主、节点类型为辅，避免两个相同的 Value Display 标题和 Node 占位文本。显示由当前连线和当前面板选择实时推导的用途，例如“随图显示：治具原图、治具结果图”。
...
本轮审阅截图：`display-config-boundaries.png`、`display-config-value-rows.png`，保存在上述本机验证目录。该节为后续实现设计，尚未应用到代码。
