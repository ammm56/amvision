...
- Ctrl+C 复制选中节点及其内部连线，Ctrl+V 在画布鼠标位置粘贴；鼠标位于侧栏等区域时在可见画布中央粘贴。右键菜单提供相同能力。
- 节点与连线生成新 ID，保持参数、启用状态、相对位置和端口关联；同位置连续粘贴逐次偏移 32 个画布坐标单位。
...
- 外部连线、应用公开接口、App Mode 布局、组框、便签和执行结果不自动复制。复制 Value Display 与 Image Preview 时，只有两端都被选中的显示数据连线才保留。
- 剪贴板是当前编辑文档内的配置快照；切换文档清理，不使用系统剪贴板。通用撤销栈、跨文档粘贴和整组/便签复制不在当前能力中。
