...
执行参数默认值仅来自 `parameter_schema.properties.<name>.default`，按 `default` 字段是否存在判断。UI 元数据中的 `default_value: null` 不能代表存在默认值，也不能自动写入 Template。

- 参数缺省且没有 Schema default：保持缺省。
- 新建节点时，参数缺省且 Schema 明确声明 default：深拷贝默认值，包括显式 `default: null`。已保存、导入和历史载入的节点保留原始参数，不补入当前定义的新默认值。
- 已有参数值：保留 `null`、`false`、`0`、空字符串、空数组和空对象，不用默认值替换。
- 取消设置使用编辑器内部 `undefined` 删除参数键，不向 JSON 快照写入 undefined；枚举的“未设置”与实际 null 选项不同。
- 参数显示不反向写入数据。保存、复制、Preview 和发布使用相同参数规则；后端 Schema 仍负责校验实际值，不自动修正非法 null。
- 布尔输入的勾号只在实际选中时渲染，不能依赖透明文字颜色隐藏，避免通用文字样式覆盖后误显为 true。

历史草稿若曾保存非法 null，应精确核对节点、参数定义和连线后显式修正；不能批量删除 null，也不能原位改写已发布版本。

### 工具栏与浮层布局

属性面板和错误提示通过 `--workflow-toolbar-bottom` 共用工具栏实际底部边界。页面生命周期内的 ResizeObserver 观察画布与工具栏尺寸，响应窗口变化、文本换行和语言切换；面板使用剩余空间并内部滚动。卸载时断开观察和 resize 监听，不使用定时轮询或固定行数推算。画布坐标、节点位置、连线和缩放语义不变。

## Workflow App 编辑

多节点编排边界见 [ADR-0013](../../decisions/ADR-0013-workflow-multi-node-copy.md)，实现与验证范围见[多节点复制验收](../../development/workflow-multi-node-copy.md)。
...
- Ctrl＋左键从画布或节点组空白区域拖动，选中完整落入矩形的节点；Ctrl＋点击节点增减选择。普通左键拖动画布继续平移。
- Ctrl+C 复制选中节点、完整框选的节点组及内部连线，Ctrl+V 在画布鼠标位置粘贴；鼠标位于侧栏等区域时在可见画布中央粘贴。右键菜单提供相同能力。
- 节点、连线与组分别生成新 ID，保持参数、启用状态、相对位置和端口关联；同位置连续粘贴逐次偏移 32 个画布坐标单位。
- 拖动已选节点时整体移动，Delete/Backspace 按选择集合删除并清理关联连线、公开绑定和组成员；多选属性面板只显示选中数量。
- 完整框住组框且组内节点全部选中时，保留组名、颜色、尺寸、锁定及启用配置；只选部分节点不复制组，也不补选其余成员。空组和仅含便签的组不进入节点片段。
- 新组成员只引用新节点，组框与节点统一偏移；从已选节点标题或已选组背景可整体移动。锁定组可以随完整片段移动，锁定配置保持不变；未选锁定组仍用于平移画布。明确选中的完整组片段批量删除时同时移除组框，单独删除组按钮仍仅删除组框。
- Esc 取消框选并恢复原选择；没有框选时清除选择。输入框、JSON 编辑器、下拉控件、弹窗和菜单保留原有文本及控件快捷键。
- 外部连线、应用公开接口、App Mode 布局、便签和执行结果不自动复制。复制 Value Display 与 Image Preview 时，只有两端都被选中的显示数据连线才保留。
- 剪贴板是当前编辑文档内的配置快照；切换文档清理，不使用系统剪贴板。通用撤销栈、跨文档粘贴和便签批量复制不在当前能力中。
- 粘贴先完整构造和校验后一次性加入图；缺少外部输入的片段可以继续编排，执行前仍需通过既有校验。文件节点保留路径参数，不自动为副本创建独立文件路径。
- 框选和多选拖动按帧更新坐标，组成员在动作结束后统一计算，不在鼠标移动时保存或请求后端。此功能不改变 Preview、Runtime、Trigger、模型推理或 LocalBuffer 数据面。

## 文档导入与导出

空白画布右键菜单底部提供导出与导入。导出当前编辑内容，包含未保存修改；导入 `amvision.workflow-app-document.v1` 后直接替换当前图。文件含完整 Application/Template，保留节点、连线、参数、公开绑定、分组、说明和 App Mode，不包含 Runtime/Trigger、服务端摘要及图片或模型二进制文件。顶部不再提供文件和说明按钮，添加说明沿用空白画布右键操作；主要操作顺序为预览、保存、发布，版本按钮位于属性面板按钮之后的最右侧，保存、预览、发布和版本入口采用一致的次级按钮样式。空白画布菜单前三项为添加节点、添加说明、预览；小地图显隐只在底部控制。

本地文件与历史快照经同一载入器处理，保留当前 Project、Application/Template 保存目标；失败保留原图，成功标记未保存，不备份、不确认、不自动保存或发布。文件固定限制为 16 MiB，并在构造画布前检查图元素数量、自由 JSON 嵌套和总值数量。未改动时不把显示兜底位置写入文档。详细字段、限制与验收见 [文档导入/导出](../../design/frontend/workflow-document-import-export.md)。
...
编辑器发布先保存草稿，再按保存指纹创建不可变 Workflow App Version；重复内容沿用已有去重规则。保存成功而发布失败时明确显示“草稿已保存，发布失败”。发布旁“历史版本”图标打开右侧分页列表，published/archived 均可直接载入当前编辑内容，不自动切换 Runtime。完整边界见 [历史版本管理](../../design/frontend/workflow-version-history.md)。

应用详情与 Runtime 页面继续支持：

- 草稿与已发布版本比较
- 归档/取消归档（archive/restore，与载入编辑内容不同）
- 新建 Runtime 时选择版本
- 已停止 Runtime 选择目标版本
...
应用模式配置沿用 760px 宽度，输出项在桌面窗口横向排列，窄窗口自动换行。应用模式、版本命名和发布使用统一的对话框动效和键盘焦点管理；版本命名保持默认 520px 宽度。应用模式的本地编辑仅在“应用”后生效，取消不修改配置；缺失输出引用、空输出选择和超长标题不能提交。

保存过程包含服务端写入与页面反馈，期间阻止重复保存及导入、历史载入等内容替换操作。页面已切换文档时，迟到的保存反馈不会重新载入旧内容。

### 发布历史面板

侧栏默认从工具栏下方 104px 处开始，标题下的状态行在加载和刷新时保留高度。工具栏按钮因窗口变窄而换行时，侧栏继续按实际工具栏底部下移，底部缩放与小地图位置保持不变。版本编号显示为 V1、V2，自定义名称同时显示；悬停和键盘聚焦与工具栏按钮共用 `--graph-hover` 背景色，亮色复用现有中性灰色，暗色保持原有按钮悬停色，保留现有背景过渡，时间线始终位于背景之上。属性栏、版本栏、工具栏和画布菜单使用普通光标，内部按钮、输入框及小地图保持各自交互光标，避免继承画布的拖动光标。

节点组色板提供 12 色，包含已有灰色 `#667085`；新增浅灰、青绿、橄榄绿、珊瑚橙和玫瑰粉。默认节点组仍使用原绿色，选色只修改当前编辑内容。色板打开时位于节点卡片之上。

历史入口使用独立的 320px 窄面板，与属性面板（360px）分别实现并互斥显示。面板位于画布内右侧，没有抽屉遮罩；版本以垂直时间线显示。行尾菜单支持直接载入、通过对话框修改标题和版本说明、导出指定发布快照的 Workflow JSON，以及删除无 Runtime revision/Run 引用的版本。载入只替换浏览器编辑内容，保存和发布仍由显式操作决定。接口和删除边界见 [Workflow App 版本接口](../../api/workflow-app-versions.md)。
