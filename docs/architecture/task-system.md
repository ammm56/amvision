...
- `/tasks`：全局任务索引页，只负责列出所有后台任务、显示状态摘要，并把用户带到正确的业务详情或任务状态页面。
- `/tasks/{task_id}`：通用任务状态页，只读展示 TaskRecord、TaskEvent、WebSocket 连接状态、错误详情和底层执行状态；不承载业务删除、下载、登记、部署等管理动作。
- 业务详情页：展示对应业务对象的完整信息、业务结果、文件和清理入口，例如 DatasetImport、DatasetExport、TrainingTask、ConversionTask 等详情页。

全局任务列表的主链接应优先进入业务详情页，而不是默认进入 `/tasks/{task_id}`。只有当任务没有可识别业务归属，或用户明确点击“任务状态”入口时，才进入 `/tasks/{task_id}`。
...
业务详情页不重复渲染完整任务事件。任务事件、进度和错误详情统一由 `/tasks/{task_id}` 任务状态页承载，业务详情页应提供“查看任务状态”入口。这样现场用户默认看到业务结果，排查问题时仍能进入统一状态页查看事件、进度和错误详情。

删除和清理入口必须放在业务详情页，而不是通用任务状态页。删除文案应按业务对象命名，例如“删除导入记录”“删除导出记录”“删除训练任务”“删除转换任务”。删除时应按业务占用关系做防呆：
...
```json
{
  ...
  "status_path": "/tasks/task-ee6b340450b9"
}
```
