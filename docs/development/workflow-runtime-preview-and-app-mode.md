...
状态：设计已接受，待实现。本文记录同一 Workflow App 的 Runtime 画布监视和轻量应用模式方案；不是当前已交付的页面或 API 说明。
...
| 部分 | 当前事实 | 待补齐能力 |
| --- | --- | --- |
| 预览节点 | Image、Value、Table、Gallery 等节点已有结构化显示输出 | 将本次实际执行的预览内容交付给 Runtime 监视页面 |
| 编辑画布 | 从同步 Preview 返回的 `node_records` 识别 `*-preview` 内容，按节点显示 | 显示适配与 Preview Run 会话解耦，复用于 Runtime |
| 高性能 Runtime | none 等模式不保留完整节点输入输出载荷 | 不开启 full record，单独交接预览显示内容 |
...
- 显示身份至少包括 Runtime、实际版本/revision、generation、run ID、节点 ID、输出端口和显示类型；具体消息字段在最小链路实施时固定为版本化契约，不将本文示意当成已注册接口。
