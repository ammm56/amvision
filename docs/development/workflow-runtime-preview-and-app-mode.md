...
Runtime 停止/启动和异常退出自动恢复均已按“监视页面保持打开”实际验证。旧 Worker 通道关闭后，页面每 2 秒重新读取权威快照；新 Worker 身份出现时清除旧代显示、重置序号并自动订阅，随后可接收新执行结果。普通同身份网络重连保留最近完成画面；关闭页面立即停止重连。该恢复只属于浏览器观察连接，不启动 Runtime、不补发历史、不调用 Workflow。
