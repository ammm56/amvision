...
- `roi.v1` 是平台核心 payload，创建、转换、规则判断应放在 core 节点体系。
...
这些节点负责产出标准 `roi.v1` 或 ROI 列表，不直接修改图像。
...
| `core.vision.roi-list-create` | 新增 | 把多个 ROI 或 value 列表规整为 ROI 列表 | 可选 |

`roi-create` 应成为矩形、多边形、四点取参的主要入口。用户在图上画框、画四点、多边形，最终写回的是 `roi-create` 的参数。

`roi-grid-create` 应成为槽位网格取参入口。用户在图上拖拽网格区域、调整 rows/columns/step，最终输出 ROI 列表。
...
| `custom.opencv.crop-export` | `custom_nodes/opencv_basic_nodes/backend/nodes/crop_export.py` | 批量导出 detection / region / ROI crop | 扩展支持 ROI 列表，命名从 detection 限定中收回来 |
...
| `custom.opencv.draw-regions` | 可新增 | 批量绘制 regions / ROI list | 否 |
...
| 节点类型 | 工具 | 写回参数 |
| --- | --- | --- |
| `roi-create` | bbox / polygon / four-point | `bbox_xyxy`、`polygon_xy` |
...
| `perspective-transform` | four-point | `source_points`、`output_width`、`output_height` |
...
3. `core.vision.roi-create` 已增加调试图输出和 `interaction.tools[]`：bbox、four-point。
4. `core.vision.roi-grid-create` 保留 grid 交互取参，并作为槽位分割主入口。
5. `core.vision.roi-from-contour` 已实现，旧 custom bridge 已删除。

### 后续调整

1. `crop-export` 增加 ROI list 输入，支持按 ROI 列表批量裁剪。
2. 新增 `core.vision.roi-list-create` 或 `core.vision.rois-create`，统一收敛多个 ROI。
3. 新增 `custom.opencv.draw-rois` 或扩展 `draw-roi` 支持 ROI list，专门批量绘制槽位。
4. 更新 catalog 分类，ROI 创建类统一显示到 `vision.roi`，OpenCV render/filter 不再出现 ROI 创建参数。
...
1. 已完成：`roi-create` 补 debug preview、bbox/four-point interaction、默认 bbox 参数。
2. 已完成：`crop` 删除手填矩形和交互取参，只消费 `roi.v1`。
3. 已完成：`core.vision.roi-from-contour` 作为 contour 到 ROI 的 core 节点。
4. 后续：扩展 `crop-export` 支持 ROI list。
5. 后续：增加 `draw-rois` 或扩展 `draw-roi` 批量绘制。
6. 后续：更新空盘检测 workflow：使用 `ROI Create / ROI Grid Create -> Crop / Draw ROI` 的新链路。
7. 后续：更新 tests 和文档，确保不再出现旧的 Crop 取参路径。
