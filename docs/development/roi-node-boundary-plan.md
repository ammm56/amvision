...
| `hough-circles` | Search ROI + Reference Circle + 参数滑块 | `minimum_center_distance_px`、`minimum_radius_px`、`maximum_radius_px`、`canny_high_threshold`、`center_vote_threshold` 等；Reference Circle 不得写回 Search ROI |
| `circle-measure` | Search ROI + Reference Circle + 参数滑块 | `reference_center_xy`、`reference_radius_px`、中心/半径容差、边缘极性、圆弧覆盖率和 robust fitting 参数 |
