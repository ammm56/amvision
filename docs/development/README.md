...
- [运行界面与 App 应用包实施基线](workflow-views-and-app-packages-implementation.md)：已接受，待实现；设计/试运行/发布/生产页操作表与状态模型、资源与接口草案、显示生命周期、Workflow JSON 导入导出、App 打包恢复和模型生成界面的唯一详细设计与阶段门禁。
