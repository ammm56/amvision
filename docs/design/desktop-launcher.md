...
默认 `manage_service=true`。配置不提供任意命令或自由 URL。本机监听表确认端口空闲时启动发行 full 服务；已有监听但请求超时、错误页面或其他程序占用时显示问题，不重复启动。TCP 超时本身不能决定服务是否存在。终端手工开发和第三方启动的服务不会被接管。
...
主窗口不显示底部服务归属和页面地址栏，内容区直接延伸到窗口内边缘。服务管理状态仍保留在生命周期模型和日志中。

应用图标、托盘图标及启动页标识使用同一份 Inkscape 图稿：背景 `#00D99B`、AM 字形 `#10221D`，Arial Bold 字体保持原始比例并居中。发布用 SVG 由 Inkscape 将文字转换为路径；EXE/窗口/托盘使用 GIMP 导出的多尺寸 `Assets/app.ico`，启动页使用 Inkscape 导出的 `Assets/app.png`。可编辑母版及维护方式见 [视觉与组件系统](frontend/design-system.md)。

F11 在原生界面和已载入网页中切换全屏，隐藏顶部栏；再次按下恢复之前的普通或最大化状态。全屏设置位于管理服务开关下方，名为“启动时全屏”，默认关闭、下次启动生效。临时 F11 切换不修改此开关。
...
外壳配色与前端 `shared/styles/tokens/` 一致：暗色页面 #101010、面板 #171918、输入 #141615、悬停 #242825；亮色页面 #F6F7F9、面板白色、悬停 #ECEFF2。顶部文字与图标采用次级文字色。帮助与外观下拉使用圆角、统一边框和中性悬停背景，背景过渡为 120ms，选中项以勾选标记识别。 关于页链接按钮与帮助菜单项显式使用 `ShellButtonTheme`，不叠加 Fluent 默认按钮状态；悬停只改变中性背景，文字颜色和控件尺寸保持稳定。链接按钮的默认背景使用 `ShellBackground`，帮助菜单项使用 `ShellSurface`，与实际承载表面一致；不从 `Transparent` 做 BrushTransition，避免透明色的 RGB 插值在暗色中产生偏亮的中间帧。回归测试需要采样移入、移出过程中的动画帧，不能只检查最终颜色。

启动等待时间输入框只由外层绘制边框，内部文本框不重复描边。保存按钮在亮暗外观中均使用白字和深绿色背景，悬停、按下只改变背景，不降低文字透明度。
...
启动器版本为 0.1.8，与前后端一致，版本源为 `launcher/Directory.Build.props`。构建时间由 MSBuild 写入程序集，不使用应用启动时间或文件修改时间。发行阶段从具名 BuildInformation 对象生成 JSON，配置不能修改该信息。后端版本仍由视觉页面负责显示。
