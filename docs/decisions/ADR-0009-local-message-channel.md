...
| 链路 | 当前实现 | 所有者与语义 |
| --- | --- | --- |
| Workflow Trigger | 独立 LocalMessage Mailbox + Trigger 业务字段 | backend-service 单 server owner；PREPARE、WRITING、REQUEST、RESPONSE、ACK |
| Inference daemon | 独立 LocalMessage Mailbox | inference daemon 单 server owner；REQUEST、PROCESSING、RESPONSE、ACK |
...
Workflow Trigger、Inference 和训练遥测原有的重复底层实现已经分别在阶段 4、阶段 3、阶段 2 删除，并接入通用 Mailbox/EventRing engine。Trigger 的两阶段握手与 LocalBuffer handoff 保留为窄业务 extension，不进入 common layout 或 EventRing。
...
用户显式执行 `Image Base64 Encode` 后得到的 `image-base64.v1` 属于结构化 JSON，可以进入 Mailbox inline/page-chain，但它是兼容输出而不是本机高性能图片路径，并继续受序列化前 32 MiB 单响应上限约束。默认图片输入输出继续使用 LocalBuffer 引用。
...
Binary contract 分为四个逻辑层，但当前实现把 common、Mailbox 和 EventRing layout 收敛在唯一的 `local_message_channel.v1.json` 中，Workflow Trigger 业务契约 保持独立 schema。同文件用于统一代码生成和 fixture，不能把 Mailbox 与 EventRing 解释为共享完整 header、状态机或容量几何：
...
└─ Workflow Trigger Mailbox extension v1
   └─ PREPARE / WRITING / LocalBuffer receipt / output handoff
```

Overflow page-chain 只属于 Mailbox，不进入 common primitives 或 EventRing。公共原语和两类 layout 由同一 schema 与生成工具组合，但 Mailbox 文件只按 common + Mailbox layout 解析，EventRing 文件只按 common + Event layout 解析。不得因为 schema 物理文件统一，就要求两种 Channel 使用相同状态机、header 全字段或容量几何。

路径 containment、guard 获取顺序、publication 顺序、owner lock 和异常恢复属于 engine 规范及公共实现，不是 on-disk binary schema 字段，也不进入代码生成 DTO。`.NET` 只生成公开 Workflow Trigger 使用的 common、Mailbox 和 Trigger 业务契约 contract；内部 Training EventRing 不生成 SDK 类型。

Health 同样采用“公共 envelope + 分类型指标”：公共部分只报告 channel id/kind、owner epoch、layout fingerprint、服务状态、容量摘要和最近错误；Mailbox 单独报告 descriptor/page、request、ACK、cancel 和 timeout；EventRing 单独报告 slot、sequence、gap/drop 和 producer。不能为形式统一向 EventRing 填充无意义的 ACK/page 指标。
...
Workflow Trigger 在通用 Mailbox engine 上保留 PREPARE、WRITING、External LocalBuffer Writer Lease、Runtime admission 和 output handoff 扩展。Inference 使用普通 Mailbox 路径，不承担 Trigger 的 External Writer 状态。
...
当前协议仍处于发布前开发阶段。schema 物理文件为 `local_message_channel.v1.json` 和 `workflow_trigger_mailbox.v1.json`；前者内含 common/Mailbox/EventRing 三个逻辑 layout，领域 contract 继续使用各自 v1 标识并按 Channel kind 只组合需要的 layout。
...
- `inference_daemon.mmap_mailbox.enabled` 只控制 Inference Mailbox Channel；
...
- LocalMessage 基础设施、Inference Mailbox 和 Training EventRing 不依赖 `local_buffer_broker.enabled`，训练遥测可以在没有 LocalBuffer 的训练 Worker 中工作；
...
迁移前先采集每个 Channel 的 request/response P50、P95、P99、最大值、并发、page 高水位和容量拒绝。随后为 Trigger Mailbox、Inference Mailbox 和 Training Event 分别在代码中冻结有名称的稳定默认 profile：
...
LocalMessage 的 Mailbox/Event adapter 只通过四个窄 port、DTO、payload codec 和领域错误进入 application，不能把 `mmap`、文件路径、offset、guard 或 page layout 暴露给 application：

- `MailboxClientPort`：提交不可变 wire bytes，并返回带受控 ACK/close 生命周期的 response handle；
- `MailboxServerPort`：接收 request context、观察取消并发布一次 response；
...
本机同步 Mailbox deadline 统一使用同一主机、同一启动周期、同一 monotonic clock domain 内的绝对 monotonic nanoseconds；外部 duration 只在权威 server 入口换算一次，公开 SDK 不能把自身 monotonic 计数作为 Python server 的绝对 deadline。换算后的 deadline 不持久化且由 owner epoch 隔离重启。取消使用固定 `cancel_reason`，server 通过 request context 读取取消状态。client 完整取得 response bytes 后由 response handle ACK；普通 Queue adapter 的 ACK 可以是幂等 no-op，Workflow Trigger 业务契约 可以把 ACK 延迟到输出 LocalBuffer result dispose。所有 port 的 `close(deadline_ns)` 必须幂等，owner epoch 变化统一映射为 `ChannelRestarted` 领域错误，不能向 application 暴露 offset、state 数值或 guard 异常。

阶段 5 实际链路审计确认，不能把这四个 port 扩大为所有进程通信的统一接口：Workflow Runtime response Queue 同时承载异步结果、主动 heartbeat 和 runtime state；PublishedInferenceGateway 支持多个调用线程和乱序响应；LocalBuffer Broker 还组合每 client route、同进程直达和跨进程 pipe。把三者强制适配为当前串行 `MailboxClientPort` 会丢失语义或降低并发能力。三条链路继续使用各自的领域 Channel/Client 边界，不进入 LocalMessage common schema，也不创建新的通用大接口。

阶段 5 使用相同 `MailboxPort`、相同 wire envelope/bytes 和同一跨进程 echo 拓扑比较 Queue adapter 与候选 Mailbox。1/6/64 KiB 三档均未满足 P95、P99 同时改善 10% 的门槛；mmap 的 P50/P95、CPU 和 page fault 明显更高，仅改善了 Windows Queue 的偶发 P99 尾峰。因此正式裁决为 `retain-queue`。不创建 `workflow-runtime/`、PublishedInferenceGateway 或 LocalBuffer Broker mmap 文件，也不在保留链路的热路径增加 JSON/bytes 二次适配。将来只有新的真实领域基准同时证明性能收益和语义适配成立时，才单独提出迁移 ADR。
...
- 把所有消息强制塞进一个状态机：Mailbox 与允许覆盖的 telemetry ring 语义不同。
...
- 新增分层 LocalMessageChannel contract、公共原语、Mailbox/Event engine、path、配置和分类型 health；不新增一份要求所有 Channel 共享完整字段的 contract。
...
- Workflow Runtime、PublishedInferenceGateway 和 LocalBuffer Broker 保留现有领域 Channel 与 Queue/pipe 传输；阶段 5 基准不支持把它们切换到 Mailbox，也不支持把不同异步语义压缩成串行 `MailboxPort`。
