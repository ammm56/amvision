...
def test_barcode_node_catalog_builder_matches_checked_in_catalog() -> None:
    """验证 builder 生成结果与仓库内 catalog.json 保持一致。"""

    repository_root = Path(__file__).resolve().parents[1]
    node_pack_dir = repository_root / "custom_nodes" / "barcode_nodes"
...
def test_barcode_node_catalog_builder_rejects_non_object_node_fragment(tmp_path: Path) -> None:
    """验证 builder 会拒绝非对象结构的节点目录碎片。"""

    repository_root = Path(__file__).resolve().parents[1]
    source_node_pack_dir = repository_root / "custom_nodes" / "barcode_nodes"
    node_pack_dir = tmp_path / "barcode_nodes"
    shutil.copytree(source_node_pack_dir, node_pack_dir)
    node_fragment_path = (
        node_pack_dir
        / "categories"
        / "decode"
    ...
    with pytest.raises(ValueError, match="节点目录碎片必须是对象"):
        build_custom_node_catalog_payload(node_pack_dir=node_pack_dir)
