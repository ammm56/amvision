...
from custom_nodes.database_nodes.providers.sql.backend.nodes import sql_upsert


def test_sql_upsert_inserts_and_updates_sqlite_row(tmp_path: Path) -> None:
    """验证 SQL Upsert 会对 SQLite 表执行单行 upsert。"""

    database_path = tmp_path / "inspection.db"
    database_url = f"sqlite:///{database_path.as_posix()}"
    _create_results_table(database_url=database_url)

    first_output = sql_upsert.handle_node(
        WorkflowNodeExecutionRequest(
            node_id="sql-upsert-insert",
            node_definition=SimpleNamespace(node_type_id=sql_upsert.NODE_TYPE_ID),
            parameters={
    ...
    second_output = sql_upsert.handle_node(
        WorkflowNodeExecutionRequest(
            node_id="sql-upsert-update",
            node_definition=SimpleNamespace(node_type_id=sql_upsert.NODE_TYPE_ID),
            parameters={
                "database_url": database_url,
                "table_name": "inspection_results",
...
def test_sql_upsert_rejects_multiple_primary_inputs(tmp_path: Path) -> None:
    """验证 sql-upsert 会拒绝同时提供多个主业务输入。"""

    database_url = f"sqlite:///{(tmp_path / 'inspection.db').as_posix()}"

    with pytest.raises(InvalidRequestError, match="只能同时提供一个"):
        sql_upsert.handle_node(
            WorkflowNodeExecutionRequest(
                node_id="sql-upsert-invalid",
                node_definition=SimpleNamespace(
                    node_type_id=sql_upsert.NODE_TYPE_ID
                ),
                parameters={
                    "database_url": database_url,
                    "table_name": "inspection_results",
...
def test_sql_upsert_requires_key_columns_to_match_unique_constraint(
    tmp_path: Path,
) -> None:
    """验证 sql-upsert 会拒绝非主键/非唯一键的冲突列。"""
    ...
    with pytest.raises(InvalidRequestError, match="主键或唯一约束"):
        sql_upsert.handle_node(
            WorkflowNodeExecutionRequest(
                node_id="sql-upsert-invalid-key",
                node_definition=SimpleNamespace(
                    node_type_id=sql_upsert.NODE_TYPE_ID
                ),
                parameters={
