...
def test_classification_training_runs_one_epoch():
    ...
    result = run_yolov8_classification_training(request)
...
def test_yolov8_classification_runner_rejects_other_model_type():
    """验证非 YOLOv8 classification 不会回落到 YOLOv8 执行器。"""
    import shutil
...
def test_classification_training_imports():
    ...
    assert YoloV8ClassificationTrainingExecutionRequest is not None
    assert run_yolov8_classification_training is not None
    assert SqlAlchemyYoloV8ClassificationTrainingService is not None
    assert YoloV8ClassificationTrainingRequest is not None
