...
from backend.nodes.core_nodes.logic.value.categories_batch_to_items import (
    CORE_NODE_SPEC as CATEGORIES_ITEMS_SPEC,
)
from backend.nodes.core_nodes.logic.value.detections_batch_to_items import (
    CORE_NODE_SPEC as DETECTIONS_ITEMS_SPEC,
)
from backend.nodes.core_nodes.logic.value.obbs_batch_to_items import (
    CORE_NODE_SPEC as OBBS_ITEMS_SPEC,
)
from backend.nodes.core_nodes.logic.value.poses_batch_to_items import (
    CORE_NODE_SPEC as POSES_ITEMS_SPEC,
)
from backend.nodes.core_nodes.logic.value.segments_batch_to_items import (
    CORE_NODE_SPEC as SEGMENTS_ITEMS_SPEC,
)
...
_BATCH_CASES = (
    (
        ...
        DETECTIONS_ITEMS_SPEC,
        "detection",
        "detections_batch",
    ),
    (
        CLASSIFICATION_BATCH_SPEC,
        CATEGORIES_ITEMS_SPEC,
        "classification",
        "categories_batch",
    ),
    (
    ...
    (POSE_BATCH_SPEC, POSES_ITEMS_SPEC, "pose", "poses_batch"),
    (OBB_BATCH_SPEC, OBBS_ITEMS_SPEC, "obb", "obbs_batch"),
)


def test_core_catalog_contains_five_batch_nodes_items_and_payloads() -> None:
    ...
    assert {
        ...
        "core.logic.detections-batch-to-items",
        "core.logic.categories-batch-to-items",
        "core.logic.segments-batch-to-items",
        "core.logic.poses-batch-to-items",
        "core.logic.obbs-batch-to-items",
    } <= node_ids
    assert {
        "detections-batch.v1",
        "categories-batch.v1",
...
@pytest.mark.parametrize(
    ("batch_spec", "items_spec", "task_type", "output_name"),
    _BATCH_CASES,
)
def test_model_batch_node_preserves_complete_ordered_items(
    batch_spec: object,
    items_spec: object,
    task_type: str,
    output_name: str,
) -> None:
    """验证五类 Batch 同序输出、locator 清理和完整关联项转换。"""
    ...
    restored = items_spec.handler(
        WorkflowNodeExecutionRequest(
            node_id=f"{task_type}-items",
            node_definition=items_spec.node_definition,
            parameters={},
    ...
    assert [item["item_index"] for item in restored] == [0, 1, 2]
    assert [item["item_id"] for item in restored] == [
        "crop-1",
        "crop-2",
        "crop-3",
    ]
    assert [item["source"]["crop_index"] for item in restored] == [1, 2, 3]
    assert all(
        item["source"]["source_image_identity"]["content_sha256"]
        == "parent-sha"
        for item in restored
    )
    assert all(set(item) == {"item_index", "item_id", "source", "result"} for item in restored)
...
def test_batch_to_items_rejects_invalid_order_duplicate_ids_and_locator_leaks() -> None:
    """验证转换节点拒绝乱序、重复 item_id 和临时 locator。"""
    ...
    with pytest.raises(InvalidRequestError, match="item_index"):
        _invoke_items(CATEGORIES_ITEMS_SPEC, "categories_batch", payload)
    ...
    with pytest.raises(InvalidRequestError, match="item_id 不能重复"):
        _invoke_items(CATEGORIES_ITEMS_SPEC, "categories_batch", payload)
...
def _invoke_items(
    ...
    """执行一个 Batch To Items 节点。"""

    return items_spec.handler(
        WorkflowNodeExecutionRequest(
            node_id="batch-to-items",
            node_definition=items_spec.node_definition,
            parameters={},
