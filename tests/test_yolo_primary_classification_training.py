...
def _build_classification_smoke_data(root: Path) -> tuple[LocalDatasetStorage, dict[str, object]]:
    ...
    for split_name in ("train", "val"):
        ...
        annotations = {
            "images": [{"id": j + 1, "file_name": f"{split_name}-{j}.jpg", "width": 128, "height": 128} for j in range(4)],
            "annotations": [{"id": j + 1, "image_id": j + 1, "category_id": 1 if j < 2 else 2} for j in range(4)],
            "categories": [{"id": 1, "name": "bright"}, {"id": 2, "name": "dim"}],
        }
        (ann_dir / f"{split_name}.json").write_text(json.dumps(annotations), encoding="utf-8")
    manifest = {
        "splits": [
            {"name": "train", "image_root": "exports/demo/images/train", "annotation_file": "exports/demo/annotations/train.json"},
            {"name": "val", "image_root": "exports/demo/images/val", "annotation_file": "exports/demo/annotations/val.json"},
        ],
    }
