"""YOLO 主线 classification 内部执行链测试。"""
...
def test_yolov8_classification_model_service_planner_runtime_target_and_predictor(
    tmp_path: Path,
) -> None:
    """验证 classification 内部链已经接通到 PyTorch predictor。"""

    session_factory = _create_session_factory()
    dataset_storage = _create_dataset_storage(tmp_path)
    seeded = _seed_classification_model_version(
    ...
    execution_result = PyTorchYoloV8ClassificationRuntimeSession.load(
        dataset_storage=dataset_storage,
...
def test_yolov8_classification_conversion_runner_exports_onnx_and_runtime_can_predict(
    tmp_path: Path,
) -> None:
    ...
    conversion_result = runner.run_conversion(
        YoloV8ConversionRunRequest(
            ...
            output_object_prefix="task-runs/conversion/classification-test-1",
            model_type="yolov8",
            task_type="classification",
        )
    )
    ...
    assert onnx_output.metadata["validation_summary"]["output_count"] == 1
    ...
    execution_result = DefaultClassificationModelRuntime().load_session(
        dataset_storage=dataset_storage,
        runtime_target=runtime_target,
    ).predict(
        ClassificationPredictionRequest(
...
def _seed_classification_model_version(
    *,
    session_factory: SessionFactory,
    dataset_storage: LocalDatasetStorage,
) -> dict[str, str]:
    """写入一条最小 classification ModelVersion。"""

    checkpoint_uri = "projects/project-1/models/yolov8/classification-source-1/artifacts/checkpoints/best.pt"
    ...
    model_version_id = service.register_training_output(
        YoloV8TrainingOutputRegistration(
            ...
            labels_file_id="labels-file-yolov8-classification-source-1",
