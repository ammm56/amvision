...
def test_classification_training_runs_one_epoch():
    ...
    result = run_yolo_task_classification_training(request)
...
def test_primary_classification_runner_rejects_yolo11():
    ...
    with pytest.raises(InvalidRequestError, match="yolo11_classification_training"):
        run_yolo_task_classification_training(request)
    shutil.rmtree(root)
...
def test_classification_training_imports():
    ...
    assert YoloTaskClassificationTrainingExecutionRequest is not None
    assert run_yolo_task_classification_training is not None
    assert SqlAlchemyYoloTaskClassificationTrainingService is not None
    assert YoloTaskClassificationTrainingRequest is not None
