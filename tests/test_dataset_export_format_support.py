...
@pytest.mark.parametrize(
    ("model_type", "task_type", "expected_format"),
    [
...
def test_resolve_supported_dataset_export_format_returns_current_runner_format(
    model_type: str,
    task_type: str,
    expected_format: str,
) -> None:
    """验证每个已实现模型任务组合对应当前真正支持的导出格式。"""
...
def test_yolo_primary_profiles_match_current_supported_dataset_formats() -> None:
    ...
    for profile in (YOLOV8_MODEL_PROFILE, YOLO11_MODEL_PROFILE, YOLO26_MODEL_PROFILE):
        ...
        assert profile.resolve_default_dataset_format(SEGMENTATION_TASK_TYPE) == COCO_INSTANCE_SEGMENTATION_DATASET_FORMAT
        assert profile.resolve_default_dataset_format(POSE_TASK_TYPE) == COCO_KEYPOINTS_DATASET_FORMAT
...
@pytest.mark.parametrize(
    ...
    [
        ("yolo26", SEGMENTATION_TASK_TYPE, YOLO_INSTANCE_SEGMENTATION_DATASET_FORMAT),
        ("yolo11", POSE_TASK_TYPE, YOLO_POSE_DATASET_FORMAT),
