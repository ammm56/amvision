...
from custom_nodes.output_local_db_nodes.workflow.catalog_builder import (
    build_custom_node_catalog_payload,
)


def test_local_db_node_catalog_builder_matches_checked_in_catalog() -> None:
    """验证 catalog_builder 生成结果与 checked-in catalog.json 一致。"""

    workflow_dir = (
        Path(__file__).resolve().parents[1]
        / "custom_nodes"
        / "output_local_db_nodes"
        / "workflow"
    )
...
def test_repository_local_db_node_pack_is_enabled_by_default() -> None:
    """验证仓库内置 output.local-db-nodes 会被默认加载。"""
    ...
    assert "output.local-db-nodes" in node_pack_ids
    assert {"custom.output.local-db-upsert"}.issubset(loaded_node_type_ids)
