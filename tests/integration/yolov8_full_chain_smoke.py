...
该工具用于手动验证 YOLOv8 在平台里的真实使用链路，不属于默认测试。
默认链路为：
...
SMOKE_ROOT = PROJECT_ROOT / ".tmp" / "yolov8-full-chain-smoke"
...
class YoloV8TaskCase:
    """描述一个 YOLOv8 task 的真实短链路输入。"""
...
def build_default_task_cases() -> dict[str, YoloV8TaskCase]:
    """返回本项目当前真实数据资产对应的 YOLOv8 验收任务。"""

    dataset_root = PROJECT_ROOT / "data" / "files" / "datasets"
    return {
        "detection": YoloV8TaskCase(
            task_type="detection",
            dataset_dir=dataset_root / "detection" / "medical-pills",
            dataset_id="smoke-yolov8-detection",
        ...
        "classification": YoloV8TaskCase(
            task_type="classification",
            dataset_dir=dataset_root / "classification" / "computerasurfacedefect",
            dataset_id="smoke-yolov8-classification",
        ...
        "segmentation": YoloV8TaskCase(
            task_type="segmentation",
            dataset_dir=dataset_root / "segmentation" / "package-seg",
            dataset_id="smoke-yolov8-segmentation",
...
class SmokeApiClient:
    ...
    @staticmethod
    def _read_json_response(*, response: httpx.Response, method: str, path: str) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise RuntimeError(f"{method} {path} did not return JSON object: {payload!r}")
        return payload
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.start_processes:
            ...
            processes = start_service_processes(run_dir=run_dir, port=port)
        wait_for_service(base_url=base_url, timeout_seconds=args.service_timeout_seconds)
...
def parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    cases = build_default_task_cases()
    parser = argparse.ArgumentParser(description="运行 YOLOv8 真实短链路验收")
    parser.add_argument("--base-url", default="http://127.0.0.1:8000", help="已有 backend-service 地址")
    parser.add_argument("--token", default=DEFAULT_TOKEN, help="调用 API 使用的 Bearer token")
    ...
    parser.add_argument("--tasks", nargs="+", choices=tuple(cases), default=tuple(cases))
    parser.add_argument(
        "--target-formats",
        nargs="+",
    ...
    parser.add_argument("--start-processes", action="store_true", help="由脚本启动 service 和 worker")
    parser.add_argument("--port", type=int, default=None, help="由脚本启动 service 时使用的端口")
...
def start_service_processes(*, run_dir: Path, port: int) -> list[ManagedProcess]:
    """启动本脚本独占使用的 backend-service 和 worker。"""
...
def wait_for_service(*, base_url: str, timeout_seconds: float) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(1.0)
    raise RuntimeError(f"backend-service 未在 {timeout_seconds:.0f}s 内就绪：{last_error}")
...
def run_task_case(
    *,
    ...
    dataset_import_detail = poll_resource(
        ...
        fetch=lambda: client.get(f"/datasets/imports/{dataset_import['dataset_import_id']}"),
        state_reader=lambda payload: str(payload.get("status")),
        timeout_seconds=timeout_seconds,
    )
    dataset_version_id = read_required_string(dataset_import_detail, "dataset_version_id")
    ...
    dataset_export_detail = poll_resource(
        ...
        fetch=lambda: client.get(f"/datasets/exports/{dataset_export['dataset_export_id']}"),
        state_reader=lambda payload: str(payload.get("status")),
        timeout_seconds=timeout_seconds,
    )
    ...
    output_model_name = build_output_model_name(task_type=case.task_type, model_scale=model_scale)
...
def collect_smoke_dataset_paths(
    *,
    ...
    fallback_images = [
        path
        for path in sorted(case.dataset_dir.rglob("*"), key=lambda item: item.as_posix().lower())
...
def collect_split_image_paths(
    *,
    ...
    for candidate_root in candidate_roots:
        ...
        image_paths.extend(
            path
            for path in sorted(candidate_root.rglob("*"), key=lambda item: item.as_posix().lower())
...
def submit_training_task(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "display_name": f"smoke yolov8 {case.task_type}",
    }
    ...
    if case.task_type == "detection":
        ...
        return client.post(
            ...
            json={
                ...
                "extra_options": {},
                "display_name": f"smoke yolov8 detection {target_format}",
            },
        )
    return client.post(
        ...
        json={
            ...
            "extra_options": {},
            "display_name": f"smoke yolov8 {case.task_type} {target_format}",
        },
    )
...
def run_deployment_smoke(
    *,
    ...
    try:
        ...
        async_status = client.get(f"{case.deployment_route}/{deployment_id}/async/status")
        ...
        async_payload = client.get(f"{case.inference_route}/inference-tasks/{async_task_id}/result")
...
def poll_task(
    *,
    ...
    return poll_until(
        label=label,
        fetch=lambda: client.get(f"/tasks/{task_id}", params={"include_events": "true"}),
...
def poll_conversion_task(
    *,
    ...
    return poll_until(
        label=label,
        fetch=lambda: client.get(f"{case.conversion_route}/{task_id}", params={"include_events": "true"}),
        state_reader=lambda payload: str(payload.get("state")),
        success_states={"succeeded"},
        failure_states=TERMINAL_TASK_STATES - {"succeeded"},
        timeout_seconds=timeout_seconds,
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if state in failure_states:
            raise RuntimeError(f"{label} failed: {json.dumps(payload, ensure_ascii=False)[:4000]}")
        time.sleep(2.0)
    raise RuntimeError(f"{label} timed out: {json.dumps(last_payload, ensure_ascii=False)[:4000]}")


def select_conversion_build(payload: dict[str, Any], *, target_format: str) -> dict[str, Any]:
...
def safe_stop_deployment(*, client: SmokeApiClient, case: YoloV8TaskCase, deployment_id: str) -> None:
    """尽量停止 deployment，避免失败时残留子进程。"""

    for runtime_mode in ("sync", "async"):
        try:
            client.post_no_json(f"{case.deployment_route}/{deployment_id}/{runtime_mode}/stop")
        except Exception:
            pass
...
def build_output_model_name(*, task_type: str, model_scale: str) -> str:
    """构造可追溯的训练输出模型名。"""

    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    return f"yolov8-{task_type}-{model_scale}-{timestamp}"
...
def collect_process_snapshots(processes: Iterable[ManagedProcess]) -> dict[str, Any]:
    ...
    try:
        ...
        return {item.name: {"pid": item.process.pid, "resource_status": "psutil-unavailable"} for item in processes}
