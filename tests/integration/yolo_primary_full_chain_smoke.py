...
SMOKE_ROOT = PROJECT_ROOT / ".tmp" / "yolo-primary-full-chain-smoke"
...
def build_default_task_cases() -> dict[str, YoloPrimaryTaskCase]:
    """返回本项目当前真实数据资产对应的 YOLO 主线验收任务。"""

    dataset_root = PROJECT_ROOT / "data" / "files" / "datasets"
    return {
        "detection": YoloPrimaryTaskCase(
            task_type="detection",
        ...
        "classification": YoloPrimaryTaskCase(
        ...
        "segmentation": YoloPrimaryTaskCase(
...
def safe_stop_deployment(
    *, client: SmokeApiClient, case: YoloPrimaryTaskCase, deployment_id: str
) -> None:
